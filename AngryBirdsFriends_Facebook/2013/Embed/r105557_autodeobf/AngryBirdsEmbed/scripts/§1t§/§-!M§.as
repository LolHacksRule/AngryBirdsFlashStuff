package §1t§
{
   import §&U§.§05§;
   import com.rovio.assets.§&%§;
   import flash.display.MovieClip;
   import flash.display.Shape;
   import flash.geom.Rectangle;
   import flash.text.TextField;
   import flash.text.TextFormat;
   import flash.ui.Mouse;
   import flash.ui.MouseCursor;
   import flashx.textLayout.formats.TextAlign;
   
   public class §-!M§ extends §@B§
   {
      
      public static const COMPONENT_STATE_ACTIVE_DEFAULT:String = "Active";
      
      public static const §"!H§:String = "Deactive";
      
      public static const §,!-§:String = "Disabled";
      
      public static const §5[§:String = "Out";
      
      public static const §-!K§:String = "Over";
      
      public static const §0s§:String = "Up_Default";
      
      public static const §!!>§:String = "Down";
      
      public static var §"'§:Class;
      
      public static var §,!I§:Boolean = true;
      
      public static const §"A§:String = "Tooltip";
      
      public static var §?Z§:Boolean = true;
       
      
      public var §,a§:String;
      
      public var §07§:String;
      
      public var §;!=§:String;
      
      public var §<G§:MovieClip;
      
      public var §6!+§:int = 6;
      
      private var §`D§:Boolean = false;
      
      private var §4!<§:String = "auto";
      
      public function §-!M§(param1:XML, param2:§05§, param3:MovieClip = null)
      {
         super(param1,param2,param3);
         this.§,a§ = COMPONENT_STATE_ACTIVE_DEFAULT;
         this.§07§ = §0s§;
         this.§84§();
         this.§#!'§(param1.@Tooltip);
         if(param1.@Toggle.toString().toUpperCase() == "TRUE")
         {
            this.§`D§ = true;
         }
         var _loc4_:Array = [MouseCursor.ARROW,MouseCursor.AUTO,MouseCursor.BUTTON,MouseCursor.HAND,MouseCursor.IBEAM];
         var _loc5_:String = param1.@cursor.toString().toLowerCase();
         if(_loc4_.indexOf(_loc5_) > -1)
         {
            this.§4!<§ = _loc5_;
         }
      }
      
      public static function §`X§() : void
      {
         §"'§ = §&%§.§<7§(§"A§);
      }
      
      override public function setActiveStatus(param1:Boolean) : void
      {
         super.setActiveStatus(param1);
         if(!param1)
         {
            this.setComponentVisualState(§5[§);
         }
      }
      
      public function §#!'§(param1:String) : void
      {
         this.§;!=§ = param1;
         if(this.§;!=§ && this.§;!=§.length == 0)
         {
            this.§;!=§ = null;
         }
         if(this.§;!=§)
         {
            if(§,!I§)
            {
               §,!I§ = false;
               §`X§();
            }
            this.§<G§ = new MovieClip();
            if(§"'§)
            {
               this.§,!=§();
            }
            else
            {
               this.§,X§();
            }
            this.§<G§.mouseEnabled = false;
            this.§<G§.mouseChildren = false;
            this.§<G§.visible = false;
            getParentView().addChild(this.§<G§);
         }
      }
      
      private function §,X§() : void
      {
         this.§<G§ = new MovieClip();
         var _loc1_:TextField = new TextField();
         _loc1_.name = "Text";
         this.§<G§.addChild(_loc1_);
         var _loc2_:Shape = new Shape();
         _loc2_.name = "Base";
         this.§<G§.addChildAt(_loc2_,0);
         this.§,t§(this.§;!=§);
      }
      
      private function §,!=§() : void
      {
         this.§<G§ = new §"'§();
         this.§&!F§(this.§;!=§);
      }
      
      public function §&!F§(param1:String) : void
      {
         this.§;!=§ = param1;
         if(§"'§)
         {
            this.§%r§(param1);
         }
         else
         {
            this.§,t§(param1);
         }
      }
      
      private function §%r§(param1:String) : void
      {
         var _loc2_:TextField = this.§<G§.getChildByName("Text") as TextField;
         var _loc3_:MovieClip = this.§<G§.getChildByName("Base") as MovieClip;
         _loc2_.text = this.§;!=§;
         _loc2_.multiline = false;
         _loc2_.width = _loc2_.textWidth * 1.25;
         var _loc4_:Number = Math.max(_loc3_.width * 0.75,_loc2_.width * 1.2);
         _loc3_.scaleX = _loc4_ / _loc3_.width;
         _loc3_.x = 0;
         var _loc5_:int = 0;
         while(_loc5_ < _loc3_.numChildren)
         {
            _loc3_.getChildAt(0).x = 0;
            _loc5_++;
         }
         _loc2_.x = (_loc3_.width - _loc2_.width) / 2;
      }
      
      private function §,t§(param1:String) : void
      {
         var _loc2_:TextField = this.§<G§.getChildByName("Text") as TextField;
         var _loc3_:TextFormat = _loc2_.defaultTextFormat;
         _loc3_.align = TextAlign.CENTER;
         _loc3_.bold = true;
         _loc3_.size = 12;
         _loc2_.defaultTextFormat = _loc3_;
         _loc2_.border = false;
         _loc2_.text = this.§;!=§;
         _loc2_.height = _loc2_.textHeight * 1.25;
         _loc2_.width = _loc2_.textWidth * 1.25;
         _loc2_.textColor = 0;
         _loc2_.selectable = false;
         _loc2_.mouseEnabled = false;
         var _loc4_:Shape;
         (_loc4_ = this.§<G§.getChildByName("Base") as Shape).graphics.clear();
         _loc4_.graphics.lineStyle(1);
         _loc4_.graphics.beginFill(0,1);
         _loc4_.graphics.drawRect(-this.§6!+§,-this.§6!+§,_loc2_.width + 2 * this.§6!+§,_loc2_.height + 2 * this.§6!+§);
         _loc4_.graphics.beginFill(16777215,1);
         _loc4_.graphics.drawRect(-this.§6!+§ / 2,-(this.§6!+§ / 2),_loc2_.width + 1 * this.§6!+§,_loc2_.height + 1 * this.§6!+§);
      }
      
      private function §,u§(param1:Boolean) : void
      {
         var _loc2_:Rectangle = null;
         var _loc3_:Rectangle = null;
         if(!this.§<G§ || this.§;!=§ == null || this.§;!=§.length == 0)
         {
            return;
         }
         if(param1 && §?Z§)
         {
            _loc2_ = mClip.getRect(getParentView());
            this.§<G§.y = _loc2_.y - this.§<G§.height - this.§6!+§;
            this.§<G§.x = _loc2_.x + mClip.width / 2 - this.§<G§.width / 2;
            _loc3_ = this.§<G§.getRect(mClip.stage);
            if(_loc3_.right > mClip.stage.stageWidth - this.§6!+§)
            {
               this.§<G§.x -= 1 + (_loc3_.right - (mClip.stage.stageWidth - this.§6!+§));
            }
            if(_loc3_.left < this.§6!+§)
            {
               this.§<G§.x += 1 + (this.§6!+§ - _loc3_.left);
            }
            if(_loc3_.bottom > mClip.stage.stageHeight - this.§6!+§)
            {
               this.§<G§.y -= 1 + (_loc3_.bottom - (mClip.stage.stageHeight - this.§6!+§));
            }
            if(_loc3_.top < this.§6!+§)
            {
               this.§<G§.y += mClip.height + this.§<G§.height + this.§6!+§ * 2;
            }
            this.§<G§.visible = true;
         }
         else
         {
            this.§<G§.visible = false;
         }
      }
      
      override public function clear() : void
      {
         if(this.§<G§)
         {
            this.§;!=§ = null;
            while(this.§<G§.numChildren > 0)
            {
               this.§<G§.removeChildAt(0);
            }
            if(this.§<G§.parent)
            {
               this.§<G§.parent.removeChild(this.§<G§);
            }
            this.§<G§ = null;
         }
         super.clear();
      }
      
      public function setComponentState(param1:String) : void
      {
         this.§,a§ = param1;
         this.§84§();
      }
      
      public function setComponentVisualState(param1:String) : void
      {
         this.§07§ = param1;
         this.§84§();
      }
      
      public function §84§() : void
      {
         if(mClip.totalFrames < 2)
         {
            return;
         }
         var _loc1_:Array = new Array(this.§,a§ + "_" + this.§07§,this.§,a§ + "_" + §0s§,COMPONENT_STATE_ACTIVE_DEFAULT + "_" + this.§07§,COMPONENT_STATE_ACTIVE_DEFAULT + "_" + §0s§);
         var _loc2_:int = 0;
         for(; _loc2_ < _loc1_.length; _loc2_++)
         {
            try
            {
               mClip.gotoAndStop(_loc1_[_loc2_] as String);
               if(mClip.numChildren > 0)
               {
                  return;
               }
            }
            catch(e:Error)
            {
               continue;
            }
         }
      }
      
      override public function listenerEventOccured(param1:int, param2:String) : void
      {
         if(this.§,a§ == §,!-§)
         {
            return;
         }
         super.listenerEventOccured(param1,param2);
         if(this.§`D§ && param1 == LISTENER_EVENT_MOUSE_UP)
         {
            this.§ d§();
         }
         if(param1 == LISTENER_EVENT_MOUSE_DOWN)
         {
            this.setComponentVisualState(§!!>§);
         }
         if(param1 == LISTENER_EVENT_MOUSE_UP)
         {
            this.setComponentVisualState(§-!K§);
         }
         if(param1 == §8!9§)
         {
            Mouse.cursor = MouseCursor.AUTO;
            this.setComponentVisualState(§5[§);
         }
         if(param1 == §0!>§)
         {
            Mouse.cursor = this.§4!<§;
            this.setComponentVisualState(§-!K§);
         }
         this.§,u§(param1 == §0!>§);
      }
      
      public function § d§() : void
      {
         if(this.§,a§ == §-!M§.COMPONENT_STATE_ACTIVE_DEFAULT)
         {
            this.setComponentState(§-!M§.§"!H§);
         }
         else if(this.§,a§ == §-!M§.§"!H§)
         {
            this.setComponentState(§-!M§.COMPONENT_STATE_ACTIVE_DEFAULT);
         }
      }
      
      override public function setEnabled(param1:Boolean) : void
      {
         super.setEnabled(param1);
      }
      
      override public function changeMovieClip(param1:MovieClip) : void
      {
         super.changeMovieClip(param1);
         var _loc2_:Number = mClip.x;
         var _loc3_:Number = mClip.y;
         var _loc4_:Number = mParentContainer.mClip.getChildIndex(mClip);
         if(this.mParentContainer)
         {
            mParentContainer.mClip.removeChild(mClip);
         }
         mClip = null;
         mClip = param1;
         if(this.mParentContainer)
         {
            mParentContainer.mClip.addChildAt(mClip,_loc4_);
         }
         mClip.x = _loc2_;
         mClip.y = _loc3_;
         this.§84§();
      }
      
      override public function setVisibility(param1:Boolean) : void
      {
         super.setVisibility(param1);
         if(!mClip.visible)
         {
            this.§,u§(false);
         }
      }
      
      override public function onParentVisibilityChange(param1:Boolean) : void
      {
         super.onParentVisibilityChange(param1);
         if(!param1)
         {
            this.§,u§(false);
         }
      }
   }
}
