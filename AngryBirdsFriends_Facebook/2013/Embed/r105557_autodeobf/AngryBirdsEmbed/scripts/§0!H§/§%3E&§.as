package §0!H§
{
   import §"`§.§]r§;
   import flash.utils.*;
   
   public class §>&§
   {
      
      public static const §0K§:int = 0;
      
      public static const § case§:int = 1;
      
      public static const COMPRESSION_REDUCED_1:int = 2;
      
      public static const COMPRESSION_REDUCED_2:int = 3;
      
      public static const COMPRESSION_REDUCED_3:int = 4;
      
      public static const COMPRESSION_REDUCED_4:int = 5;
      
      public static const §5!,§:int = 6;
      
      public static const §<@§:int = 7;
      
      public static const §5!4§:int = 8;
      
      public static const §]!&§:int = 9;
      
      public static const §0u§:int = 10;
      
      protected static var §"W§:Boolean = describeType(ByteArray).factory.method.(@name == "uncompress").parameter.length() > 0;
      
      protected static var §7"§:Boolean = describeType(ByteArray).factory.method.(@name == "inflate").length() > 0;
       
      
      protected var §>!F§:int = 0;
      
      protected var §4§:String = "2.0";
      
      protected var §=Y§:int = 8;
      
      protected var §=E§:Boolean = false;
      
      protected var § U§:int = -1;
      
      protected var §1>§:int = -1;
      
      protected var §60§:int = -1;
      
      protected var §4!4§:Boolean = false;
      
      protected var §`j§:Boolean = false;
      
      protected var §%!F§:Date;
      
      protected var §`c§:uint;
      
      protected var § ,§:Boolean = false;
      
      protected var §[l§:uint = 0;
      
      protected var §7V§:uint = 0;
      
      protected var §@3§:String = "";
      
      protected var §3!%§:String;
      
      protected var §[j§:Dictionary;
      
      protected var §?!@§:String = "";
      
      protected var §!r§:ByteArray;
      
      var §>!I§:uint;
      
      var §7z§:uint = 0;
      
      var §3Y§:uint = 0;
      
      protected var §4L§:Boolean = false;
      
      protected var §?§:Function;
      
      public function §>&§(param1:String = "utf-8")
      {
         this.§?§ = this.§'l§;
         super();
         this.§3!%§ = param1;
         this.§[j§ = new Dictionary();
         this.§!r§ = new ByteArray();
         this.§!r§.endian = Endian.BIG_ENDIAN;
      }
      
      public function get §7i§() : Date
      {
         return this.§%!F§;
      }
      
      public function set §7i§(param1:Date) : void
      {
         this.§%!F§ = param1 != null ? param1 : new Date();
      }
      
      public function get §[u§() : String
      {
         return this.§@3§;
      }
      
      public function set §[u§(param1:String) : void
      {
         this.§@3§ = param1;
      }
      
      function get §-!G§() : Boolean
      {
         return this.§4!4§;
      }
      
      public function get content() : ByteArray
      {
         if(this.§4L§)
         {
            this.uncompress();
         }
         return this.§!r§;
      }
      
      public function set content(param1:ByteArray) : void
      {
         this.§%!M§(param1);
      }
      
      public function §%!M§(param1:ByteArray, param2:Boolean = true) : void
      {
         if(param1 != null && param1.length > 0)
         {
            param1.position = 0;
            param1.readBytes(this.§!r§,0,param1.length);
            this.§>!I§ = §]r§.§5L§(this.§!r§);
            this.§ ,§ = false;
         }
         else
         {
            this.§!r§.length = 0;
            this.§!r§.position = 0;
            this.§4L§ = false;
         }
         if(param2)
         {
            this.compress();
         }
         else
         {
            this.§3Y§ = this.§7z§ = this.§!r§.length;
         }
      }
      
      public function get §-!"§() : String
      {
         return this.§4§;
      }
      
      public function get §[I§() : uint
      {
         return this.§7z§;
      }
      
      public function get §1!D§() : uint
      {
         return this.§3Y§;
      }
      
      public function §6!M§(param1:Boolean = true, param2:String = "utf-8") : String
      {
         var _loc3_:String = null;
         if(this.§4L§)
         {
            this.uncompress();
         }
         this.§!r§.position = 0;
         if(param2 == "utf-8")
         {
            _loc3_ = this.§!r§.readUTFBytes(this.§!r§.bytesAvailable);
         }
         else
         {
            _loc3_ = this.§!r§.readMultiByte(this.§!r§.bytesAvailable,param2);
         }
         this.§!r§.position = 0;
         if(param1)
         {
            this.compress();
         }
         return _loc3_;
      }
      
      public function §7v§(param1:String, param2:String = "utf-8", param3:Boolean = true) : void
      {
         this.§!r§.length = 0;
         this.§!r§.position = 0;
         this.§4L§ = false;
         if(param1 != null && param1.length > 0)
         {
            if(param2 == "utf-8")
            {
               this.§!r§.writeUTFBytes(param1);
            }
            else
            {
               this.§!r§.writeMultiByte(param1,param2);
            }
            this.§>!I§ = §]r§.§5L§(this.§!r§);
            this.§ ,§ = false;
         }
         if(param3)
         {
            this.compress();
         }
         else
         {
            this.§3Y§ = this.§7z§ = this.§!r§.length;
         }
      }
      
      public function §5O§(param1:IDataOutput, param2:Boolean, param3:Boolean = false, param4:uint = 0) : uint
      {
         var _loc10_:* = null;
         var _loc15_:ByteArray = null;
         var _loc16_:Boolean = false;
         if(param1 == null)
         {
            return 0;
         }
         if(param3)
         {
            param1.writeUnsignedInt(§-_§.§'h§);
            param1.writeShort(this.§>!F§ << 8 | 20);
         }
         else
         {
            param1.writeUnsignedInt(§-_§.§99§);
         }
         param1.writeShort(this.§>!F§ << 8 | 20);
         param1.writeShort(this.§3!%§ == "utf-8" ? 2048 : 0);
         param1.writeShort(!!this.§4L§ ? int(§5!4§) : int(§0K§));
         var _loc5_:Date = this.§%!F§ != null ? this.§%!F§ : new Date();
         var _loc6_:uint = uint(_loc5_.getSeconds()) | uint(_loc5_.getMinutes()) << 5 | uint(_loc5_.getHours()) << 11;
         var _loc7_:uint = uint(_loc5_.getDate()) | uint(_loc5_.getMonth() + 1) << 5 | uint(_loc5_.getFullYear() - 1980) << 9;
         param1.writeShort(_loc6_);
         param1.writeShort(_loc7_);
         param1.writeUnsignedInt(this.§>!I§);
         param1.writeUnsignedInt(this.§7z§);
         param1.writeUnsignedInt(this.§3Y§);
         var _loc8_:ByteArray;
         (_loc8_ = new ByteArray()).endian = Endian.LITTLE_ENDIAN;
         if(this.§3!%§ == "utf-8")
         {
            _loc8_.writeUTFBytes(this.§@3§);
         }
         else
         {
            _loc8_.writeMultiByte(this.§@3§,this.§3!%§);
         }
         var _loc9_:uint = _loc8_.position;
         for(_loc10_ in this.§[j§)
         {
            if((_loc15_ = this.§[j§[_loc10_] as ByteArray) != null)
            {
               _loc8_.writeShort(uint(_loc10_));
               _loc8_.writeShort(uint(_loc15_.length));
               _loc8_.writeBytes(_loc15_);
            }
         }
         if(param2)
         {
            if(!this.§ ,§)
            {
               if(_loc16_ = this.§4L§)
               {
                  this.uncompress();
               }
               this.§`c§ = §]r§.§1!%§(this.§!r§,0,this.§!r§.length);
               this.§ ,§ = true;
               if(_loc16_)
               {
                  this.compress();
               }
            }
            _loc8_.writeShort(56026);
            _loc8_.writeShort(4);
            _loc8_.writeUnsignedInt(this.§`c§);
         }
         var _loc11_:uint = _loc8_.position - _loc9_;
         if(param3 && this.§?!@§.length > 0)
         {
            if(this.§3!%§ == "utf-8")
            {
               _loc8_.writeUTFBytes(this.§?!@§);
            }
            else
            {
               _loc8_.writeMultiByte(this.§?!@§,this.§3!%§);
            }
         }
         var _loc12_:uint = _loc8_.position - _loc9_ - _loc11_;
         param1.writeShort(_loc9_);
         param1.writeShort(_loc11_);
         if(param3)
         {
            param1.writeShort(_loc12_);
            param1.writeShort(0);
            param1.writeShort(0);
            param1.writeUnsignedInt(0);
            param1.writeUnsignedInt(param4);
         }
         if(_loc9_ + _loc11_ + _loc12_ > 0)
         {
            param1.writeBytes(_loc8_);
         }
         var _loc13_:uint = 0;
         if(!param3 && this.§!r§.length > 0)
         {
            if(this.§4L§)
            {
               if(§"W§ || §7"§)
               {
                  _loc13_ = this.§!r§.length;
                  param1.writeBytes(this.§!r§,0,_loc13_);
               }
               else
               {
                  _loc13_ = this.§!r§.length - 6;
                  param1.writeBytes(this.§!r§,2,_loc13_);
               }
            }
            else
            {
               _loc13_ = this.§!r§.length;
               param1.writeBytes(this.§!r§,0,_loc13_);
            }
         }
         var _loc14_:uint = 30 + _loc9_ + _loc11_ + _loc12_ + _loc13_;
         if(param3)
         {
            _loc14_ += 16;
         }
         return _loc14_;
      }
      
      function parse(param1:IDataInput) : Boolean
      {
         while(param1.bytesAvailable && this.§?§(param1))
         {
         }
         return this.§?§ === this.§?X§;
      }
      
      protected function §?X§(param1:IDataInput) : Boolean
      {
         return false;
      }
      
      protected function §'l§(param1:IDataInput) : Boolean
      {
         if(param1.bytesAvailable >= 30)
         {
            this.§^D§(param1);
            if(this.§[l§ + this.§7V§ > 0)
            {
               this.§?§ = this.§6!?§;
            }
            else
            {
               this.§?§ = this.§4e§;
            }
            return true;
         }
         return false;
      }
      
      protected function §6!?§(param1:IDataInput) : Boolean
      {
         if(param1.bytesAvailable >= this.§[l§ + this.§7V§)
         {
            this.§@2§(param1);
            this.§?§ = this.§4e§;
            return true;
         }
         return false;
      }
      
      protected function §4e§(param1:IDataInput) : Boolean
      {
         var _loc2_:Boolean = true;
         if(this.§4!4§)
         {
            this.§?§ = this.§?X§;
            _loc2_ = false;
         }
         else if(this.§7z§ == 0)
         {
            this.§?§ = this.§?X§;
         }
         else if(param1.bytesAvailable >= this.§7z§)
         {
            this.§!d§(param1);
            this.§?§ = this.§?X§;
         }
         else
         {
            _loc2_ = false;
         }
         return _loc2_;
      }
      
      protected function §^D§(param1:IDataInput) : void
      {
         var _loc2_:uint = param1.readUnsignedShort();
         this.§>!F§ = _loc2_ >> 8;
         this.§4§ = Math.floor((_loc2_ & 255) / 10) + "." + (_loc2_ & 255) % 10;
         var _loc3_:uint = param1.readUnsignedShort();
         this.§=Y§ = param1.readUnsignedShort();
         this.§=E§ = (_loc3_ & 1) !== 0;
         this.§4!4§ = (_loc3_ & 8) !== 0;
         this.§`j§ = (_loc3_ & 32) !== 0;
         if((_loc3_ & 800) !== 0)
         {
            this.§3!%§ = "utf-8";
         }
         if(this.§=Y§ === §5!,§)
         {
            this.§ U§ = (_loc3_ & 2) !== 0 ? 8192 : 4096;
            this.§1>§ = (_loc3_ & 4) !== 0 ? 3 : 2;
         }
         else if(this.§=Y§ === §5!4§)
         {
            this.§60§ = (_loc3_ & 6) >> 1;
         }
         var _loc4_:uint = param1.readUnsignedShort();
         var _loc5_:uint = param1.readUnsignedShort();
         var _loc6_:* = _loc4_ & 31;
         var _loc7_:* = (_loc4_ & 2016) >> 5;
         var _loc8_:* = (_loc4_ & 63488) >> 11;
         var _loc9_:* = _loc5_ & 31;
         var _loc10_:* = (_loc5_ & 480) >> 5;
         var _loc11_:int = ((_loc5_ & 65024) >> 9) + 1980;
         this.§%!F§ = new Date(_loc11_,_loc10_ - 1,_loc9_,_loc8_,_loc7_,_loc6_,0);
         this.§>!I§ = param1.readUnsignedInt();
         this.§7z§ = param1.readUnsignedInt();
         this.§3Y§ = param1.readUnsignedInt();
         this.§[l§ = param1.readUnsignedShort();
         this.§7V§ = param1.readUnsignedShort();
      }
      
      protected function §@2§(param1:IDataInput) : void
      {
         var _loc3_:uint = 0;
         var _loc4_:uint = 0;
         var _loc5_:ByteArray = null;
         if(this.§3!%§ == "utf-8")
         {
            this.§@3§ = param1.readUTFBytes(this.§[l§);
         }
         else
         {
            this.§@3§ = param1.readMultiByte(this.§[l§,this.§3!%§);
         }
         var _loc2_:uint = this.§7V§;
         while(_loc2_ > 4)
         {
            _loc3_ = param1.readUnsignedShort();
            if((_loc4_ = param1.readUnsignedShort()) > _loc2_)
            {
               throw new Error("Parse error in file " + this.§@3§ + ": Extra field data size too big.");
            }
            if(_loc3_ === 56026 && _loc4_ === 4)
            {
               this.§`c§ = param1.readUnsignedInt();
               this.§ ,§ = true;
            }
            else if(_loc4_ > 0)
            {
               _loc5_ = new ByteArray();
               param1.readBytes(_loc5_,0,_loc4_);
               this.§[j§[_loc3_] = _loc5_;
            }
            _loc2_ -= _loc4_ + 4;
         }
         if(_loc2_ > 0)
         {
            param1.readBytes(new ByteArray(),0,_loc2_);
         }
      }
      
      function §!d§(param1:IDataInput) : void
      {
         var _loc2_:uint = 0;
         if(this.§=Y§ === §5!4§ && !this.§=E§)
         {
            if(§"W§ || §7"§)
            {
               param1.readBytes(this.§!r§,0,this.§7z§);
            }
            else
            {
               if(!this.§ ,§)
               {
                  throw new Error("Adler32 checksum not found.");
               }
               this.§!r§.writeByte(120);
               _loc2_ = ~this.§60§ << 6 & 192;
               _loc2_ += 31 - (120 << 8 | _loc2_) % 31;
               this.§!r§.writeByte(_loc2_);
               param1.readBytes(this.§!r§,2,this.§7z§);
               this.§!r§.position = this.§!r§.length;
               this.§!r§.writeUnsignedInt(this.§`c§);
            }
            this.§4L§ = true;
         }
         else
         {
            if(this.§=Y§ != §0K§)
            {
               throw new Error("Compression method " + this.§=Y§ + " is not supported.");
            }
            param1.readBytes(this.§!r§,0,this.§7z§);
            this.§4L§ = false;
         }
         this.§!r§.position = 0;
      }
      
      protected function compress() : void
      {
         if(!this.§4L§)
         {
            if(this.§!r§.length > 0)
            {
               this.§!r§.position = 0;
               this.§3Y§ = this.§!r§.length;
               if(§7"§)
               {
                  this.§!r§.deflate();
                  this.§7z§ = this.§!r§.length;
               }
               else if(§"W§)
               {
                  this.§!r§.compress.apply(this.§!r§,["deflate"]);
                  this.§7z§ = this.§!r§.length;
               }
               else
               {
                  this.§!r§.compress();
                  this.§7z§ = this.§!r§.length - 6;
               }
               this.§!r§.position = 0;
               this.§4L§ = true;
            }
            else
            {
               this.§7z§ = 0;
               this.§3Y§ = 0;
            }
         }
      }
      
      protected function uncompress() : void
      {
         if(this.§4L§ && this.§!r§.length > 0)
         {
            this.§!r§.position = 0;
            if(§7"§)
            {
               this.§!r§.inflate();
            }
            else if(§"W§)
            {
               this.§!r§.uncompress.apply(this.§!r§,["deflate"]);
            }
            else
            {
               this.§!r§.uncompress();
            }
            this.§!r§.position = 0;
            this.§4L§ = false;
         }
      }
      
      public function toString() : String
      {
         return "[FZipFile]" + "\n  name:" + this.§@3§ + "\n  date:" + this.§%!F§ + "\n  sizeCompressed:" + this.§7z§ + "\n  sizeUncompressed:" + this.§3Y§ + "\n  versionHost:" + this.§>!F§ + "\n  versionNumber:" + this.§4§ + "\n  compressionMethod:" + this.§=Y§ + "\n  encrypted:" + this.§=E§ + "\n  hasDataDescriptor:" + this.§4!4§ + "\n  hasCompressedPatchedData:" + this.§`j§ + "\n  filenameEncoding:" + this.§3!%§ + "\n  crc32:" + this.§>!I§.toString(16) + "\n  adler32:" + this.§`c§.toString(16);
      }
   }
}
