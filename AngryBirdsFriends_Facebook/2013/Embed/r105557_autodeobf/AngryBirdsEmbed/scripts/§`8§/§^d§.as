package §`8§
{
   import flash.geom.Matrix3D;
   import flash.geom.Point;
   import flash.geom.Vector3D;
   import flash.utils.ByteArray;
   import flash.utils.Endian;
   
   public class §^d§
   {
      
      public static const §5M§:int = 4;
      
      public static const §6!@§:int = 2;
      
      public static const § null§:int = 3;
      
      public static const §3L§:int = 4;
      
      public static const §6s§:int = 0;
      
      public static const §]R§:int = 0;
      
      public static const §25§:int = 0;
       
      
      private var §"F§:ByteArray;
      
      private var §!'§:Vector.<Number>;
      
      private var §&s§:Vector.<Number>;
      
      private var §+i§:Boolean;
      
      private var §7U§:int;
      
      private var §&!4§:Vector.<Number>;
      
      public function §^d§(param1:int, param2:Boolean = false)
      {
         this.§&!4§ = new Vector.<Number>(12,true);
         super();
         this.§"F§ = new ByteArray();
         this.§"F§.endian = Endian.LITTLE_ENDIAN;
         this.§"F§.length = param1 * §6!@§ * §3L§;
         var _loc3_:int = 0;
         while(_loc3_ < param1 * §6!@§)
         {
            this.§"F§.writeFloat(0);
            _loc3_++;
         }
         this.§!'§ = new Vector.<Number>(param1 * §5M§,true);
         this.§&s§ = new Vector.<Number>(param1 * § null§,true);
         this.§+i§ = param2;
         this.§7U§ = param1;
      }
      
      public function clone() : §^d§
      {
         var _loc1_:§^d§ = new §^d§(0,this.§+i§);
         _loc1_.§"F§.length = this.§"F§.length;
         _loc1_.§"F§.writeBytes(this.§"F§);
         _loc1_.§!'§ = this.§!'§.concat();
         _loc1_.§!'§.fixed = true;
         _loc1_.§&s§ = this.§&s§.concat();
         _loc1_.§&s§.fixed = true;
         _loc1_.§7U§ = this.§7U§;
         return _loc1_;
      }
      
      public function copyTo(param1:§^d§, param2:int, param3:Number, param4:Boolean, param5:Matrix3D) : void
      {
         var _loc6_:int = 0;
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         var _loc11_:Vector.<Number> = null;
         var _loc12_:int = 0;
         var _loc13_:Number = NaN;
         var _loc9_:Vector.<Number> = param1.§&s§;
         _loc6_ = this.§7U§ * § null§;
         _loc7_ = param2 * § null§;
         if(param5)
         {
            param5.transformVectors(this.§&s§,this.§&!4§);
            _loc8_ = 0;
            while(_loc8_ < _loc6_)
            {
               var _loc14_:*;
               _loc9_[_loc14_ = _loc7_++] = this.§&!4§[_loc8_];
               _loc8_++;
            }
         }
         else
         {
            _loc8_ = 0;
            while(_loc8_ < _loc6_)
            {
               _loc9_[_loc14_ = _loc7_++] = this.§&s§[_loc8_];
               _loc8_++;
            }
         }
         var _loc10_:ByteArray = param1.§"F§;
         _loc6_ = this.§7U§ * §6!@§ * §3L§;
         _loc7_ = param2 * §6!@§ * §3L§;
         if(_loc10_.length < _loc7_ + _loc6_)
         {
            _loc10_.length = _loc7_ + _loc6_;
         }
         _loc10_.position = _loc7_;
         _loc10_.writeBytes(this.§"F§);
         if(param4 || param3 != 1)
         {
            _loc11_ = param1.§!'§;
            _loc6_ = this.§7U§ * §5M§;
            _loc7_ = param2 * §5M§;
            if(param3 == 1)
            {
               _loc8_ = 0;
               while(_loc8_ < _loc6_)
               {
                  _loc11_[_loc14_ = _loc7_++] = this.§!'§[_loc8_];
                  _loc8_++;
               }
            }
            else
            {
               _loc12_ = 0;
               _loc8_ = 0;
               while(_loc8_ < this.§7U§)
               {
                  if(this.§+i§)
                  {
                     _loc13_ = this.§!'§[_loc12_ + 3];
                     _loc13_ = param3 / _loc13_;
                     _loc11_[_loc14_ = _loc7_++] = this.§!'§[_loc12_++] * _loc13_;
                     var _loc15_:*;
                     _loc11_[_loc15_ = _loc7_++] = this.§!'§[_loc12_++] * _loc13_;
                     var _loc16_:*;
                     _loc11_[_loc16_ = _loc7_++] = this.§!'§[_loc12_++] * _loc13_;
                     var _loc17_:*;
                     _loc11_[_loc17_ = _loc7_++] = param3;
                  }
                  else
                  {
                     _loc11_[_loc14_ = _loc7_++] = this.§!'§[_loc12_++];
                     _loc11_[_loc15_ = _loc7_++] = this.§!'§[_loc12_++];
                     _loc11_[_loc16_ = _loc7_++] = this.§!'§[_loc12_++];
                     _loc11_[_loc17_ = _loc7_++] = param3;
                  }
                  _loc12_++;
                  _loc8_++;
               }
            }
         }
      }
      
      public function append(param1:§^d§) : void
      {
         var _loc2_:int = this.§"F§.length;
         this.§"F§.length += param1.§"F§.length;
         this.§"F§.position = _loc2_;
         this.§"F§.writeBytes(param1.§"F§);
         this.§!'§.fixed = false;
         var _loc3_:Vector.<Number> = param1.§!'§;
         var _loc4_:int = _loc3_.length;
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            this.§!'§.push(_loc3_[_loc5_]);
            _loc5_++;
         }
         this.§!'§.fixed = true;
         this.§&s§.fixed = false;
         var _loc6_:Vector.<Number>;
         var _loc7_:int = (_loc6_ = param1.§&s§).length;
         var _loc8_:int = 0;
         while(_loc8_ < _loc7_)
         {
            this.§&s§.push(_loc6_[_loc8_]);
            _loc8_++;
         }
         this.§&s§.fixed = true;
         this.§7U§ += param1.§"!"§;
      }
      
      public function setPosition(param1:int, param2:Number, param3:Number, param4:Number = 0.0) : void
      {
         var _loc5_:int = param1 * § null§;
         var _loc6_:*;
         this.§&s§[_loc6_ = _loc5_++] = param2;
         var _loc7_:*;
         this.§&s§[_loc7_ = _loc5_++] = param3;
         this.§&s§[_loc5_] = param4;
      }
      
      public function §0!§(param1:int, param2:Number, param3:Number) : void
      {
         var _loc4_:int = param1 * § null§;
         var _loc5_:*;
         this.§&s§[_loc5_ = _loc4_++] = param2;
         this.§&s§[_loc4_] = param3;
      }
      
      public function §6v§(param1:int, param2:Vector3D) : void
      {
         var _loc3_:int = param1 * § null§;
         param2.x = this.§&s§[_loc3_++];
         param2.y = this.§&s§[_loc3_++];
         param2.z = this.§&s§[_loc3_];
      }
      
      public function §2;§(param1:int, param2:uint, param3:Number = 1.0) : void
      {
         var _loc4_:Number = (_loc4_ = !!this.§+i§ ? Number(param3) : Number(1)) / 255;
         var _loc5_:Number = §,&§.§^B§(param2) * _loc4_;
         var _loc6_:Number = §,&§.§^!;§(param2) * _loc4_;
         var _loc7_:Number = §,&§.§`!H§(param2) * _loc4_;
         this.§%&§(param1,1,_loc5_,_loc6_,_loc7_,param3);
      }
      
      private function §%&§(param1:int, param2:int, param3:Number, param4:Number, param5:Number, param6:Number) : void
      {
         var _loc7_:int = param1 * §5M§;
         var _loc8_:int = param2 - 1;
         while(_loc8_ >= 0)
         {
            var _loc9_:*;
            this.§!'§[_loc9_ = _loc7_++] = param3;
            var _loc10_:*;
            this.§!'§[_loc10_ = _loc7_++] = param4;
            var _loc11_:*;
            this.§!'§[_loc11_ = _loc7_++] = param5;
            var _loc12_:*;
            this.§!'§[_loc12_ = _loc7_++] = param6;
            _loc8_--;
         }
      }
      
      private function §@D§(param1:int, param2:Number) : void
      {
         var _loc3_:int = param1 * §5M§;
         var _loc4_:*;
         this.§!'§[_loc4_ = _loc3_++] = this.§!'§[_loc4_] * param2;
         var _loc5_:*;
         this.§!'§[_loc5_ = _loc3_++] = this.§!'§[_loc5_] * param2;
         var _loc6_:*;
         this.§!'§[_loc6_ = _loc3_++] = this.§!'§[_loc6_] * param2;
         this.§!'§[_loc3_] *= param2;
      }
      
      public function §'"§(param1:int) : uint
      {
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc2_:int = param1 * §5M§;
         var _loc3_:Number = !!this.§+i§ ? Number(this.§!'§[_loc2_ + 3]) : Number(1);
         if(_loc3_ == 0)
         {
            return 0;
         }
         _loc3_ = 255 / _loc3_;
         _loc4_ = this.§!'§[_loc2_] * _loc3_;
         _loc5_ = this.§!'§[_loc2_ + 1] * _loc3_;
         _loc6_ = this.§!'§[_loc2_ + 2] * _loc3_;
         return §,&§.§'+§(_loc4_,_loc5_,_loc6_);
      }
      
      public function §#!,§(param1:int, param2:Number) : void
      {
         var _loc3_:int = 0;
         if(param2 < 0.001)
         {
            param2 = 0.001;
         }
         if(this.§+i§)
         {
            this.§2;§(param1,this.§'"§(param1),param2);
         }
         else
         {
            _loc3_ = param1 * §5M§ + 3;
            this.§!'§[_loc3_] = param2;
         }
      }
      
      public function §!e§(param1:int) : Number
      {
         var _loc2_:int = param1 * §5M§ + 3;
         return this.§!'§[_loc2_];
      }
      
      public function setTexCoords(param1:int, param2:Number, param3:Number) : void
      {
         var _loc4_:int = param1 * §6!@§;
         this.§"F§.position = _loc4_ * §3L§;
         this.§"F§.writeFloat(param2);
         this.§"F§.writeFloat(param3);
      }
      
      public function getTexCoords(param1:int, param2:Point) : void
      {
         var _loc3_:int = param1 * §6!@§;
         this.§"F§.position = _loc3_ * §3L§;
         param2.x = this.§"F§.readFloat();
         param2.y = this.§"F§.readFloat();
      }
      
      public function §''§(param1:int, param2:Number, param3:Number, param4:Number = 0.0) : void
      {
         var _loc5_:int = param1 * § null§;
         var _loc6_:*;
         this.§&s§[_loc6_ = _loc5_++] = this.§&s§[_loc6_] + param2;
         var _loc7_:*;
         this.§&s§[_loc7_ = _loc5_++] = this.§&s§[_loc7_] + param3;
         this.§&s§[_loc5_] += param4;
      }
      
      public function §9!@§(param1:int, param2:Matrix3D) : void
      {
         var _loc3_:int = param1 * § null§;
         this.§&!4§[0] = this.§&s§[_loc3_];
         this.§&!4§[1] = this.§&s§[_loc3_ + 1];
         this.§&!4§[2] = this.§&s§[_loc3_ + 2];
         param2.transformVectors(this.§&!4§,this.§&!4§);
         this.§&s§[_loc3_] = this.§&!4§[0];
         this.§&s§[_loc3_ + 1] = this.§&!4§[1];
         this.§&s§[_loc3_ + 2] = this.§&!4§[2];
      }
      
      public function §]D§(param1:uint, param2:Number = 1.0) : void
      {
         if(param2 < 0.001)
         {
            param2 = 0.001;
         }
         var _loc3_:Number = !!this.§+i§ ? Number(param2) : Number(1);
         _loc3_ /= 255;
         var _loc4_:Number = §,&§.§^B§(param1) * _loc3_;
         var _loc5_:Number = §,&§.§^!;§(param1) * _loc3_;
         var _loc6_:Number = §,&§.§`!H§(param1) * _loc3_;
         this.§%&§(0,this.§7U§,_loc4_,_loc5_,_loc6_,param2);
      }
      
      public function §4i§(param1:int, param2:int, param3:Number, param4:Number, param5:Number, param6:Number = 1.0) : void
      {
         if(param6 < 0.001)
         {
            param6 = 0.001;
         }
         var _loc7_:int = param1 * §5M§;
         var _loc8_:int = 0;
         while(_loc8_ < param2)
         {
            var _loc9_:*;
            this.§!'§[_loc9_ = _loc7_++] = param3;
            var _loc10_:*;
            this.§!'§[_loc10_ = _loc7_++] = param4;
            var _loc11_:*;
            this.§!'§[_loc11_ = _loc7_++] = param5;
            var _loc12_:*;
            this.§!'§[_loc12_ = _loc7_++] = param6;
            _loc8_++;
         }
      }
      
      public function §;M§(param1:int, param2:Number) : void
      {
         var _loc4_:Number = NaN;
         if(param2 < 0.001)
         {
            param2 = 0.001;
         }
         if(param2 == 1)
         {
            return;
         }
         var _loc3_:int = param1 * §5M§;
         if(this.§+i§)
         {
            _loc4_ = this.§!'§[_loc3_ + 3];
            _loc4_ = param2 / _loc4_;
            this.§!'§[_loc3_] *= _loc4_;
            this.§!'§[_loc3_ + 1] *= _loc4_;
            this.§!'§[_loc3_ + 2] *= _loc4_;
            this.§!'§[_loc3_ + 3] = param2;
         }
         else
         {
            this.§!'§[_loc3_ + 3] *= param2;
         }
      }
      
      public function §8b§(param1:Boolean, param2:Boolean = true) : void
      {
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc7_:Number = NaN;
         if(param1 == this.§+i§)
         {
            return;
         }
         if(param2)
         {
            _loc3_ = this.§7U§ * §5M§;
            _loc4_ = 0;
            while(_loc4_ < _loc3_)
            {
               _loc5_ = this.§!e§(0);
               _loc6_ = !!this.§+i§ ? Number(_loc5_) : Number(1);
               _loc7_ = !!param1 ? Number(_loc5_) : Number(1);
               if(_loc6_ != 0)
               {
                  _loc7_ /= _loc6_;
                  this.§@D§(_loc4_,_loc7_);
               }
               _loc4_++;
            }
         }
         this.§+i§ = param1;
      }
      
      public function get premultipliedAlpha() : Boolean
      {
         return this.§+i§;
      }
      
      public function get §"!"§() : int
      {
         return this.§7U§;
      }
      
      public function set §"!"§(param1:int) : void
      {
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         _loc2_ = param1 * §6!@§ * §3L§ - this.§"F§.length;
         _loc4_ = this.§"F§.length;
         this.§"F§.length += _loc2_;
         if(_loc2_ > 0)
         {
            this.§"F§.position = _loc4_;
            _loc3_ = 0;
            while(_loc3_ < _loc2_ / §3L§)
            {
               this.§"F§.writeFloat(0);
               _loc3_++;
            }
         }
         this.§!'§.fixed = false;
         _loc2_ = param1 * §5M§ - this.§!'§.length;
         _loc3_ = 0;
         while(_loc3_ < _loc2_)
         {
            this.§!'§.push(0);
            _loc3_++;
         }
         _loc3_ = _loc2_;
         while(_loc3_ < 0)
         {
            this.§!'§.pop();
            _loc3_++;
         }
         this.§!'§.fixed = true;
         this.§&s§.fixed = false;
         _loc2_ = param1 * § null§ - this.§&s§.length;
         _loc3_ = 0;
         while(_loc3_ < _loc2_)
         {
            this.§&s§.push(0);
            _loc3_++;
         }
         _loc3_ = _loc2_;
         while(_loc3_ < 0)
         {
            this.§&s§.pop();
            _loc3_++;
         }
         this.§&s§.fixed = true;
         this.§7U§ = param1;
      }
      
      public function get §,7§() : ByteArray
      {
         this.§"F§.position = 0;
         return this.§"F§;
      }
      
      public function get §9o§() : Vector.<Number>
      {
         return this.§!'§;
      }
      
      public function get §2!$§() : Vector.<Number>
      {
         return this.§&s§;
      }
   }
}
