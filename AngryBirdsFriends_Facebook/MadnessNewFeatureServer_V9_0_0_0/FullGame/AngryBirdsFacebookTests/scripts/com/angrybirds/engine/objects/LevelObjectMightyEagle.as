package com.angrybirds.engine.objects
{
   ...
   public class LevelObjectMightyEagle extends LevelObjectBird
   {
      ...
      override public function applyDamage(damage:Number, updateManager:ILevelObjectUpdateManager, damaginObject:LevelObject, addScore:Boolean = true, ignoreDefence:Boolean = false) : Number
      {
         return health;
      }
      ...
      {
         updateManager.addParticle(LevelParticle.PARTICLE_NAME_BIRD_DESTRUCTION,LevelParticleManager.PARTICLE_GROUP_GAME_EFFECTS,LevelParticle.PARTICLE_TYPE_KINETIC_PARTICLE,getBody().GetPosition().x - 3 + Math.random() * (3 * 2),getBody().GetPosition().y - 3 + Math.random() * (3 * 2),3500,"",LevelParticle.getParticleMaterialFromEngineMaterial(itemName),0,0,1,0,4);
      }
