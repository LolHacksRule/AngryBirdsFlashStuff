package com.angrybirds.data.level.item
{
   ...
   public class LevelItemFriends extends LevelItemSpaceLua
   {
      ...
      override public function playSoundLua(soundGroupName:String, soundChannel:String = null, loopCount:Number = 0, startTime:Number = 0) : SoundEffect
      {
         var resource:LevelItemSoundResource = null;
         var playedSound:SoundEffect = super.playSoundLua(soundGroupName,soundChannel,loopCount,startTime);
         if(!playedSound)
         {
            resource = AngryBirdsBase.singleton.getSoundResource(soundGroupName);
            if(resource)
            {
               playedSound = mSoundManagerLua.playSound(resource.getDefaultSounds(),soundChannel,loopCount,startTime);
            }
         }
         return playedSound;
      }
