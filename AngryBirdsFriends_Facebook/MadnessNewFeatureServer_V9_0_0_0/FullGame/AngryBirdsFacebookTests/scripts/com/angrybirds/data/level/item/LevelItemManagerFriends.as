package com.angrybirds.data.level.item
{
   ...
   public class LevelItemManagerFriends extends LevelItemManagerSpace
   {
      ...
      public function addBrandedBlocks(tournamentBlocksLuaObject:Object, markBrandedOverrides:Boolean = false) : Array
      {
         ...
         if(tournamentBlocksLuaObject.dynamicBlocks)
         {
            for(blockName in tournamentBlocksLuaObject.dynamicBlocks.blocks)
            {
               ...
               if(blockObject.spawnBlock != undefined)
               {
                  levelItemSpace1.setSpawnBlockDefinition(blockObject.spawnBlock);
               }
               if(blockObject.spawnParticles != undefined)
               {
                  levelItemSpace1.setSpawnParticlesDefinition(blockObject.spawnParticles);
               }
               if(markBrandedOverrides)
               {
                  if(mItems.hasOwnProperty(blockName.toLocaleLowerCase()))
                  {
                     levelItemSpace1.isBrandedOverride = true;
                  }
               }
