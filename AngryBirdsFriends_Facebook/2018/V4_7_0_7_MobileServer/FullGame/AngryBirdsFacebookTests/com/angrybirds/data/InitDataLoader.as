package com.angrybirds.data
{
   ...
   public class InitDataLoader
   {
      ...
      public static var userLocale:String = "en-US";
      
      public static var userLocaleShort:String = "en";
      ...
      private static function onLoginDataLoaded(e:Event) : void
      {
         ...
         if(sData.error)
         {
            throw new Error("Server error of type \'" + sData.type + "\':\n" + sData.message,ErrorCode.AUTHENTICATION_ERROR);
         }
         if(sData.uid != sFacebookUserId)
         {
            throw new Error("Facebook IDs mismatch. Flashvar:\'" + sFacebookUserId + "\', server:\'" + sData.uid + "\', accessToken:\'" + sAccessToken + "\'.",ErrorCode.FACEBOOK_ID_MISMATCH);
         }
         if(sData.name == null)
         {
      ...
      {
         var loader:URLLoader = new URLLoader(AngryBirdsFacebook.sSingleton.graphAPICaller.createGraphAPIRequest("https://graph.facebook.com/" + AngryBirdsFacebook.FB_API_VERSION + "/me?fields=locale&"));
         loader.addEventListener(Event.COMPLETE,onGDPRLocalizationLoaded);
         loader.addEventListener(IOErrorEvent.IO_ERROR,onGDPRLocalizationLoaded);
         loader.addEventListener(SecurityErrorEvent.SECURITY_ERROR,onGDPRLocalizationLoaded);
         ++sDataLoaderCounter;
      }
      
      private static function onGDPRLocalizationLoaded(e:Event) : void
      {
         var jsonOb:Object = null;
         var loader:URLLoader = e.currentTarget as URLLoader;
         loader.removeEventListener(Event.COMPLETE,onGDPRLocalizationLoaded);
         loader.removeEventListener(IOErrorEvent.IO_ERROR,onGDPRLocalizationLoaded);
         loader.removeEventListener(SecurityErrorEvent.SECURITY_ERROR,onGDPRLocalizationLoaded);
         if(e.target && e.target.data)
         {
            jsonOb = JSON.parse(e.target.data);
            if(jsonOb && jsonOb.locale)
            {
               userLocale = jsonOb.locale.split("_").join("-");
               userLocaleShort = jsonOb.locale.substr(0,2);
            }
         }
         ...
         for each(consentDataObj in loadedConsentsData.consents)
         {
            if(consentDataObj.consentId == "tos")
            {
               if(consentDataObj.answer != "YES")
               {
                  ...
                  {
                     popupContent["locale"] = userLocaleShort;
                     for each(section in consentDataObj.consent.sections)
                     {
                        section.locale = section.locale.substr(0,2);
                        if(section.locale == "en" || section.locale == userLocaleShort)
                        {
                           popupContent[section.id] = section.text;
                           if(section.url != null)
                           {
