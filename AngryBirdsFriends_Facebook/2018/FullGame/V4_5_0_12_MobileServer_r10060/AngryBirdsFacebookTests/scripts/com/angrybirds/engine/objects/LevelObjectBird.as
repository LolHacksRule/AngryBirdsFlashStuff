package com.angrybirds.engine.objects
{
   ...
   public class LevelObjectBird extends LevelObjectAnimal
   {
      ...
      protected var mBirdDataObject:Object;
      ...
      public function LevelObjectBird(sprite:Sprite, animation:Animation, world:b2World, levelItem:LevelItem, levelObjectModel:LevelObjectModel, scale:Number = 1.0, tryToScream:Boolean = true)
      {
         ...
         this.mBirdDataObject = new Object();
      ...
      override public function applyDamage(damage:Number, updateManager:ILevelObjectUpdateManager, damagingObject:LevelObject, addScore:Boolean = true) : Number
      {
         ...
         {
            this.mCollisionEffect.collisionActivated(damage,getBody().GetMass(),updateManager,getBody().GetPosition().x + mRenderer.width * 0.5 * LevelMain.PIXEL_TO_B2_SCALE,getBody().GetPosition().y + mRenderer.width * 0.5 * LevelMain.PIXEL_TO_B2_SCALE,getBody().GetAngle(),getBody().GetLinearVelocity());
         }
      ...
      override public function addDamageParticles(updateManager:ILevelObjectUpdateManager, damage:int) : void
      {
         ...
         {
            angle += Math.random() * (720 / count);
            updateManager.addParticle(this.destructionBlockName,LevelParticleManager.PARTICLE_GROUP_GAME_EFFECTS,LevelParticle.PARTICLE_TYPE_KINETIC_PARTICLE,getBody().GetPosition().x,getBody().GetPosition().y,1000,"",0,Math.cos(angle) * scale,0,5,speed * 5,1);
         }
         for(i = 0; i < count; i++)
         {
            angle += Math.random() * (720 / count);
      ...
      public function getBirdDataObject(name:String) : Object
      {
         return this.mBirdDataObject[name];
      }
      
      public function addBirdDataObject(name:String, value:Object) : void
      {
         this.mBirdDataObject[name] = value;
      }
