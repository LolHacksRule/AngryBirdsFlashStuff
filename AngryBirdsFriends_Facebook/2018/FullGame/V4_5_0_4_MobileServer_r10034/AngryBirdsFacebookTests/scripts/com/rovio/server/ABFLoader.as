   ...
   public class ABFLoader extends SessionRetryingURLLoader
   {
      ...
      override protected function initData() : Boolean
      {
         ...
         if(data.hasOwnProperty("errorCode"))
         {
            ...
            {
               AngryBirdsBase.singleton.popupManager.openPopup(new ErrorPopup(PopupLayerIndexFacebook.ERROR,PopupPriorityType.OVERRIDE,ErrorPopup.ERROR_INVALID_ACCESSTOKEN));
               return true;
            }
            if(data.errorCode == ServerErrorCodes.PRODUCT_WAS_NOT_FOUND)
            {
               AngryBirdsBase.singleton.popupManager.openPopup(new ErrorPopup(PopupLayerIndexFacebook.ERROR,PopupPriorityType.OVERRIDE,ErrorPopup.ERROR_PRODUCT_NOT_FOUND));
               return true;
            }
            if(data.hasOwnProperty("errorMessage"))
            {
               ...
               throw new Error("Server returned error code \'" + data.errorCode + "\', message: \'" + data.errorMessage + additionalMessage + "\' for URL \'" + mRequest.url + "\'.#CLIENT#");
            }
         }
         return false;
      }
