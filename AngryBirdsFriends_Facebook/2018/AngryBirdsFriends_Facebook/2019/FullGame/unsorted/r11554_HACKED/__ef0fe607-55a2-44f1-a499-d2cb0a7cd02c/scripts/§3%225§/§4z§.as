package §3"5§
{
   ...
   public class §4z§
   {
      ...
      public function §4z§(param1:*)
      {
         super();
         this.jsonString = this.§8$A§(param1);
      }
      ...
      private function §8$A§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§&"S§(param1 as String);
         }
         if(param1 is Number)
         {
      ...
      private function §[#6§(param1:Array) : String
      {
         var _loc2_:* = "";
         var _loc3_:int = 0;
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§&"S§(v.@name.toString()) + ":" + this.§8$A§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
