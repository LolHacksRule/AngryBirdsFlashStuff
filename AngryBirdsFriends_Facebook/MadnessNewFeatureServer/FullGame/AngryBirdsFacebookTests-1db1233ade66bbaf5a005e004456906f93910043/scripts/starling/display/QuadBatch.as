package starling.display
{
   ...
   public class QuadBatch extends DisplayObject
   {
      ...
      private function syncBuffers() : void
      {
         ...
         {
            this.mVertexBufferPosition.uploadFromVector(this.mVertexData.rawDataPosition,0,this.mNumQuads * 4);
            if(this.mTexture == null || this.tinted)
            {
               this.mVertexBufferColor.uploadFromVector(this.mVertexData.rawDataColor,0,this.mNumQuads * 4);
            }
            if(this.mTexture)
            {
               this.mVertexBufferTexture.uploadFromVector(this.mVertexData.rawDataTexture,0,this.mNumQuads * 4);
            }
            this.mSyncRequired = false;
         }
      }
      ...
      public function renderCustom(support:RenderSupport, mvpMatrix:Matrix, parentAlpha:Number = 1.0, blendMode:int = -1) : void
      {
         ...
         if(Starling.current.disposed || this.mDisposed)
         {
            return;
         }
