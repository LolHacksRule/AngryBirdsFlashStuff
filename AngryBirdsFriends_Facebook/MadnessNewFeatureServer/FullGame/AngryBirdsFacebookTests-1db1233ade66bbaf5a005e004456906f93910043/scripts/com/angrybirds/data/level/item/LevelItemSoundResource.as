package com.angrybirds.data.level.item
{
   public class LevelItemSoundResource
   {
      ...
      private var mDefaultSounds:Vector.<String>;
      
      public function LevelItemSoundResource(id:String, soundChannelName:String, collisionSound:XMLList, damagedSound:XMLList, launchSound:XMLList, specialSound:XMLList, selectionSound:XMLList, idleSound:XMLList, destroyedSound:XMLList, screamSound:XMLList, rollingSound:XMLList, slippingSound:XMLList, defaultSounds:XMLList)
      {
         var soundIndex:* = null;
         super();
         this.mId = id;
         ...
         if(defaultSounds)
         {
            this.mDefaultSounds = new Vector.<String>();
            for(soundIndex in defaultSounds.item)
            {
               this.mDefaultSounds.push(defaultSounds.item[soundIndex]);
            }
         }
      }
      ...
      public function getDefaultSounds() : String
      {
         if(this.mDefaultSounds && this.mDefaultSounds.length > 0)
         {
            return this.mDefaultSounds[int(this.mDefaultSounds.length * Math.random())];
         }
         return null;
      }
   }
