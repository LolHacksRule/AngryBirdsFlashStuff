package com.angrybirds.states.tournament
{
   ...
   public class StateTournamentLevelEnd extends StateFacebookLevelEnd
   {
      ...
      private static const SHARE_MODE_BRAG:int = 0;
      ...
      override public function activate(previousState:String) : void
      {
         ...
         {
            leagueOrderNumber = LeagueType.getLeagueOrderNumber(LeagueModel.instance.currentLeague().id);
            if(leagueOrderNumber > -1)
            {
               fbPixelEventName = "join_tier_" + leagueOrderNumber;
               sentEvent = true;
               alreadySentEvents = DataModelFriends(AngryBirdsBase.singleton.dataModel).clientStorage.getData(ClientStorage.FB_PIXEL_REPORT_STORAGE_NAME);
               ...
               if(sentEvent)
               {
                  ExternalInterfaceHandler.performCall("trackFBPixelEvent",fbPixelEventName);
                  DataModelFriends(AngryBirdsBase.singleton.dataModel).clientStorage.storeData(ClientStorage.FB_PIXEL_REPORT_STORAGE_NAME,[fbPixelEventName]);
               }
            }
         }
      ...
      {
         var canStartNextLevel:Boolean = false;
