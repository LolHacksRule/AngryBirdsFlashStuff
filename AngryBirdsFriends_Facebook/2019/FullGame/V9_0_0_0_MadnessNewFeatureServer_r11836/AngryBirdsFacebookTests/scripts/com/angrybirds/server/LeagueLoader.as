package com.angrybirds.server
{
   ...
   public class LeagueLoader extends EventDispatcher implements ILeagueLoader
   {
      ...
      private static var smPreviouslyLoadedTournamentId:int;
      ...
      public function loadLeagueStandings(tournamentId:int) : void
      {
         if(this.useCacheForStandingsLoading() && LeagueModel.instance.getCachedLeagueStandingsData() && tournamentId == smPreviouslyLoadedTournamentId)
         {
         ...
         smPreviouslyLoadedTournamentId = tournamentId;
      }
      
      public function loadLeagueProfile(tournamentId:int) : void
      {
      ...
      public function loadAllUnconcluded(tournamentId:int) : void
      {
         var urlRequestLeague:URLRequest = URLRequestFactory.getNonCachingURLRequest(AngryBirdsBase.SERVER_ROOT + this.PATH_ALL_UNCONCLUDED + "?st=" + SessionRetryingURLLoader.sessionToken);
         urlRequestLeague.method = URLRequestMethod.GET;
         urlRequestLeague.contentType = this.getContentType();
         if(tournamentId > 0)
         {
            urlRequestLeague.url += "&tournamentId=" + tournamentId;
         }
      ...
      public function claimAllRewards(tournamentId:int) : void
      {
         var urlRequest:URLRequest = URLRequestFactory.getNonCachingURLRequest(AngryBirdsBase.SERVER_ROOT + this.PATH_ALL_CLAIM_REWARDS + "?st=" + SessionRetryingURLLoader.sessionToken);
         if(tournamentId <= 0)
         {
            urlRequest.url += "&tournamentId=" + TournamentModel.instance.getCurrentTournamentId();
         }
      ...
      private function onLeagueRefreshTimerCompleted(e:TimerEvent) : void
      {
         ...
         this.loadLeagueStandings(TournamentModel.instance.getCurrentTournamentId());
      }
      ...
      public function getScoresForLevel(levelId:String, useCache:Boolean = true) : CachedFacebookFriends
      {
         ...
         return this.mLevelsScores[levelId] = new CachedFacebookFriends((AngryBirdsBase.singleton.dataModel.userProgress as FacebookUserProgress).userID,(AngryBirdsBase.singleton.dataModel.userProgress as FacebookUserProgress).userName,AngryBirdsBase.SERVER_ROOT + PATH_LEAGUE + PATH_CURRENT_LEVEL_STANDINGS + "?level=" + levelId + "&" + TournamentModel.instance.getCurrentTournamentIdParameter(),false,null,true,false,true);
      }
      ...
      {
         var now:Number = NaN;
         var seconds:int = TournamentModel.instance.getSecondsLeft();
         ...
         seconds = (now - smStandingsLastLoadingTime) / 1000;
      ...
      public function resetPreviouslyLoadedTournamentId() : void
      {
         smPreviouslyLoadedTournamentId = -1;
      }
