package §else §
{
   ...
   public class §,$&§
   {
      ...
      {
         super();
         this.jsonString = this.§5!R§(param1);
      }
      ...
      private function §5!R§(param1:*) : String
      {
      ...
      private function §1!'§(param1:Array) : String
      {
         var _loc2_:* = "";
         var _loc3_:int = 0;
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§]!-§(v.@name.toString()) + ":" + this.§5!R§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
