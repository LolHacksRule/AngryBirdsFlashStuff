package §package§
{
   import §8"'§.§2!H§;
   import flash.utils.Dictionary;
   
   public class §4!1§
   {
      
      public static const §@#m§:String = "linear";
      
      public static const §4#6§:String = "easeIn";
      
      public static const §8t§:String = "easeOut";
      
      public static const §;$B§:String = "easeInOut";
      
      public static const § H§:String = "easeOutIn";
      
      public static const §`!7§:String = "easeInBack";
      
      public static const §-#N§:String = "easeOutBack";
      
      public static const §@"[§:String = "easeInOutBack";
      
      public static const §+f§:String = "easeOutInBack";
      
      public static const §%!J§:String = "easeInElastic";
      
      public static const §-L§:String = "easeOutElastic";
      
      public static const §!$2§:String = "easeInOutElastic";
      
      public static const §4Q§:String = "easeOutInElastic";
      
      public static const §37§:String = "easeInBounce";
      
      public static const §0N§:String = "easeOutBounce";
      
      public static const §^#G§:String = "easeInOutBounce";
      
      public static const §7!a§:String = "easeOutInBounce";
      
      private static var §;!7§:Dictionary;
       
      
      public function §4!1§()
      {
         super();
         throw new §2!H§();
      }
      
      public static function §;#C§(param1:String) : Function
      {
         if(§;!7§ == null)
         {
            §=!'§();
         }
         return §;!7§[param1];
      }
      
      public static function §^"7§(param1:String, param2:Function) : void
      {
         if(§;!7§ == null)
         {
            §=!'§();
         }
         §;!7§[param1] = param2;
      }
      
      private static function §=!'§() : void
      {
         §;!7§ = new Dictionary();
         §^"7§(§@#m§,linear);
         §^"7§(§4#6§,easeIn);
         §^"7§(§8t§,easeOut);
         §^"7§(§;$B§,easeInOut);
         §^"7§(§ H§,easeOutIn);
         §^"7§(§`!7§,easeInBack);
         §^"7§(§-#N§,easeOutBack);
         §^"7§(§@"[§,easeInOutBack);
         §^"7§(§+f§,easeOutInBack);
         §^"7§(§%!J§,easeInElastic);
         §^"7§(§-L§,easeOutElastic);
         §^"7§(§!$2§,easeInOutElastic);
         §^"7§(§4Q§,easeOutInElastic);
         §^"7§(§37§,easeInBounce);
         §^"7§(§0N§,easeOutBounce);
         §^"7§(§^#G§,easeInOutBounce);
         §^"7§(§7!a§,easeOutInBounce);
      }
      
      protected static function linear(param1:Number) : Number
      {
         return param1;
      }
      
      protected static function easeIn(param1:Number) : Number
      {
         return param1 * param1 * param1;
      }
      
      protected static function easeOut(param1:Number) : Number
      {
         var _loc2_:Number = param1 - 1;
         return _loc2_ * _loc2_ * _loc2_ + 1;
      }
      
      protected static function easeInOut(param1:Number) : Number
      {
         return §?#T§(easeIn,easeOut,param1);
      }
      
      protected static function easeOutIn(param1:Number) : Number
      {
         return §?#T§(easeOut,easeIn,param1);
      }
      
      protected static function easeInBack(param1:Number) : Number
      {
         return Math.pow(param1,2) * ((1.70158 + 1) * param1 - 1.70158);
      }
      
      protected static function easeOutBack(param1:Number) : Number
      {
         var _loc2_:Number = param1 - 1;
         return Math.pow(_loc2_,2) * ((1.70158 + 1) * _loc2_ + 1.70158) + 1;
      }
      
      protected static function easeInOutBack(param1:Number) : Number
      {
         return §?#T§(easeInBack,easeOutBack,param1);
      }
      
      protected static function easeOutInBack(param1:Number) : Number
      {
         return §?#T§(easeOutBack,easeInBack,param1);
      }
      
      protected static function easeInElastic(param1:Number) : Number
      {
         var _loc2_:Number = NaN;
         var _loc3_:Number = NaN;
         var _loc4_:Number = NaN;
         if(param1 == 0 || param1 == 1)
         {
            return param1;
         }
         _loc2_ = 0.3;
         _loc3_ = _loc2_ / 4;
         _loc4_ = param1 - 1;
         return -1 * Math.pow(2,10 * _loc4_) * Math.sin((_loc4_ - _loc3_) * (2 * Math.PI) / _loc2_);
      }
      
      protected static function easeOutElastic(param1:Number) : Number
      {
         var _loc2_:Number = NaN;
         var _loc3_:Number = NaN;
         if(param1 == 0 || param1 == 1)
         {
            return param1;
         }
         _loc2_ = 0.3;
         _loc3_ = _loc2_ / 4;
         return Math.pow(2,-10 * param1) * Math.sin((param1 - _loc3_) * (2 * Math.PI) / _loc2_) + 1;
      }
      
      protected static function easeInOutElastic(param1:Number) : Number
      {
         return §?#T§(easeInElastic,easeOutElastic,param1);
      }
      
      protected static function easeOutInElastic(param1:Number) : Number
      {
         return §?#T§(easeOutElastic,easeInElastic,param1);
      }
      
      protected static function easeInBounce(param1:Number) : Number
      {
         return 1 - easeOutBounce(1 - param1);
      }
      
      protected static function easeOutBounce(param1:Number) : Number
      {
         var _loc4_:Number = NaN;
         if(param1 < 1 / 2.75)
         {
            _loc4_ = 7.5625 * Math.pow(param1,2);
         }
         else if(param1 < 2 / 2.75)
         {
            param1 -= 1.5 / 2.75;
            _loc4_ = 7.5625 * Math.pow(param1,2) + 0.75;
         }
         else if(param1 < 2.5 / 2.75)
         {
            param1 -= 2.25 / 2.75;
            _loc4_ = 7.5625 * Math.pow(param1,2) + 0.9375;
         }
         else
         {
            param1 -= 2.625 / 2.75;
            _loc4_ = 7.5625 * Math.pow(param1,2) + 0.984375;
         }
         return _loc4_;
      }
      
      protected static function easeInOutBounce(param1:Number) : Number
      {
         return §?#T§(easeInBounce,easeOutBounce,param1);
      }
      
      protected static function easeOutInBounce(param1:Number) : Number
      {
         return §?#T§(easeOutBounce,easeInBounce,param1);
      }
      
      protected static function §?#T§(param1:Function, param2:Function, param3:Number) : Number
      {
         if(param3 < 0.5)
         {
            return 0.5 * param1(param3 * 2);
         }
         return 0.5 * param2((param3 - 0.5) * 2) + 0.5;
      }
   }
}
