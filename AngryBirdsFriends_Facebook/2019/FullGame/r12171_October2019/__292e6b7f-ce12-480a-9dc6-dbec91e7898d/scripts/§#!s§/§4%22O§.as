package §#!s§
{
   ...
   public class §4"O§
   {
      ...
      public function §4"O§(param1:*)
      {
         super();
         this.jsonString = this.§="4§(param1);
      }
      ...
      private function §="4§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§]"3§(param1 as String);
         }
         if(param1 is Number)
         {
         ...
         if(param1 is Array)
         {
            return this.§6$4§(param1 as Array);
         }
      ...
      private function §6$4§(param1:Array) : String
      {
         var _loc2_:* = "";
         var _loc3_:int = 0;
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§]"3§(v.@name.toString()) + ":" + this.§="4§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
