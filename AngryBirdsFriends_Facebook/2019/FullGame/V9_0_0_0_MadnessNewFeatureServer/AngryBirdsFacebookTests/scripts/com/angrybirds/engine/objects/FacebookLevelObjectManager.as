package com.angrybirds.engine.objects
{
   ...
   public class FacebookLevelObjectManager extends LevelObjectManagerSpace
   {
      ...
      override protected function addObjectFromModel(model:LevelObjectModel, id:int, trail:Boolean = false, activeObject:Boolean = false, tryToScream:Boolean = true, scale:Number = 1.0, overlay:Boolean = false, inFrontObject:Boolean = false) : LevelObjectBase
      {
         var returnObject:LevelObjectBase = null;
         try
         {
      ...
      override protected function isFoundEasterEgg(modelType:String) : Boolean
      {
         var splitItemName:Array = null;
         var lastPart:String = null;
         var easterEggId:String = null;
         var returnValue:Boolean = false;
         if(modelType.indexOf("MISC_EASTER_EGG_") == 0)
         {
            splitItemName = modelType.split("_");
            lastPart = splitItemName[splitItemName.length - 1];
            easterEggId = "1000-" + lastPart;
            if((AngryBirdsBase.singleton.dataModel.userProgress as FacebookUserProgress).isEggUnlocked(easterEggId))
            {
               returnValue = true;
            }
         }
      ...
      override protected function createObjectInstance(model:LevelObjectModel, sprite:Sprite, tryToScream:Boolean = true, scale:Number = 1.0) : LevelObjectBase
      {
         ...
         else
         {
            ...
            if(model.type.indexOf("FIRE_BLOCK") == 0)
            {
               ...
               {
                  return new FacebookLevelObjectFireBlockSpark(sprite,animation,mLevelMain.mLevelEngine.mWorld,levelItem,model,scale);
               }
               return new FacebookLevelObjectFireBlock(sprite,animation,mLevelMain.mLevelEngine.mWorld,levelItem,model,scale);
            }
      ...
      {
         if((this.mLevelManager as FacebookLevelManager).isCurrentEpisodeTournament() && TournamentModel.instance.isZombiePigsEnabled())
         {
            return new FacebookLevelObjectInfectedPig(sprite,animation,mLevelMain.mLevelEngine.mWorld,levelItem,model,scale);
         }
      ...
      {
         super.applyExplosionDamage(object,damage,Boolean(true));
      }
      ...
      {
         setCameraShaking(true,0.2,force / 900 * 4,force / 900 * 400);
      }
      ...
      {
         this.mushroomsAndZombiePigsToSpawn(deltaTimeMilliSeconds);
         this.generatePortalTexture();
         super.updateObjects(deltaTimeMilliSeconds);
      }
      
      protected function mushroomsAndZombiePigsToSpawn(deltaTimeMilliSeconds:Number) : void
      {
         var i:int = 0;
         var spawnObj:Object = null;
