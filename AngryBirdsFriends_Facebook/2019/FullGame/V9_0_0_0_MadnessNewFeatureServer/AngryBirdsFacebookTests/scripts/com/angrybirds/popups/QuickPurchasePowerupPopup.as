package com.angrybirds.popups
{
   ...
   public class QuickPurchasePowerupPopup extends AbstractPopup implements INavigable, IVirtualPageView, IWalletContainer
   {
      ...
      private static var smContextualOfferData:Object;
      
      private static var smPurchaceRequested:Boolean;
      ...
      private var mContextualOfferLoader:ABFLoader;
      
      private var mBuyItemWithPremiumCurrency:BuyItemWithPremiumCurrency;
      
      private var mTargetedOfferRefreshTimer:Timer;
      ...
      override protected function hide(useTransition:Boolean = false, waitForAnimationsToStop:Boolean = false) : void
      {
         ...
         if(this.mContextualOfferLoader)
         {
            this.mContextualOfferLoader.removeEventListener(Event.COMPLETE,this.onContextualOfferLoaded);
            this.mContextualOfferLoader.removeEventListener(ErrorEvent.ERROR,this.onError);
            this.mContextualOfferLoader.removeEventListener(IOErrorEvent.IO_ERROR,this.onError);
            this.mContextualOfferLoader = null;
         }
         mContainer.mClip.TargetedOfferButton.removeEventListener(MouseEvent.CLICK,this.buyTargetedOffer);
         smContextualOfferData = null;
         ...
         this.stopTargetedOfferRefreshTimer();
      ...
      private function enableBuyArea(value:Boolean) : void
      {
         ...
         for(var i:int = 1; i <= MAX_TAB_AMOUNT; i++)
         {
            ...
            if(pp)
            {
               if(pp.campaignPrice && pp.campaignPrice > 0)
               {
      ...
      private function onBuyClick(e:MouseEvent) : void
      {
         ...
         var price:Number = pp.campaignPrice > 0 ? Number(pp.campaignPrice) : Number(pp.price);
      ...
      protected function onBuyComplete(e:BuyItemEvent) : void
      {
         ...
         var price:Number = buyItem.pricePoint.campaignPrice > 0 ? Number(buyItem.pricePoint.campaignPrice) : Number(buyItem.pricePoint.price);
      ...
      private function loadContextualOffer() : void
      {
         if(this.mContextualOfferLoader || smPurchaceRequested || !mContainer)
         {
            return;
         }
         mContainer.mClip.TargetedOfferLoading.visible = true;
         mContainer.mClip.TargetedOfferButton.visible = false;
         this.showTargetedOfferButtonPurchasedIcon(false);
         this.mContextualOfferLoader = new ABFLoader();
         this.mContextualOfferLoader.addEventListener(Event.COMPLETE,this.onContextualOfferLoaded);
         this.mContextualOfferLoader.addEventListener(ErrorEvent.ERROR,this.onError);
         this.mContextualOfferLoader.addEventListener(IOErrorEvent.IO_ERROR,this.onError);
         this.mContextualOfferLoader.dataFormat = URLLoaderDataFormat.TEXT;
         var urlReq:URLRequest = URLRequestFactory.getNonCachingURLRequest(AngryBirdsBase.SERVER_ROOT + "/targetedoffer");
         this.mContextualOfferLoader.load(urlReq);
      }
      
      private function onContextualOfferLoaded(e:Event) : void
      {
         var item:Object = null;
         var dataModel:DataModelFriends = null;
         var nextRefreshIn:Number = NaN;
         var itemDef:PowerupDefinition = null;
         if(this.mContextualOfferLoader)
         {
            this.mContextualOfferLoader.removeEventListener(Event.COMPLETE,this.onContextualOfferLoaded);
            this.mContextualOfferLoader.removeEventListener(ErrorEvent.ERROR,this.onError);
         ...
         smContextualOfferData = e.target.data;
         this.stopTargetedOfferRefreshTimer();
         if(smContextualOfferData && smContextualOfferData.items)
         {
            for each(item in smContextualOfferData.items)
            {
               if(item.id == "VirtualCurrency")
               {
                  this.changeTargetedOfferButtonText("VirtualCurrencyQuantity","" + item.q);
               }
               else
               {
                  itemDef = PowerupType.getPowerupByID(item.id);
                  if(itemDef)
                  {
                     this.changeTargetedOfferButtonText("PowerupQuantity","x" + item.q);
                     this.changeTargetedOfferButtonPowerupIcon(itemDef.identifier);
                  }
               }
            }
            dataModel = AngryBirdsBase.singleton.dataModel as DataModelFriends;
            this.changeTargetedOfferButtonText("prc",dataModel.currencyModel.getPriceTag(smContextualOfferData.prc,true,"",smContextualOfferData.cur));
            mContainer.mClip.TargetedOfferLoading.visible = false;
            mContainer.mClip.TargetedOfferButton.visible = true;
            if(smContextualOfferData.pcd)
            {
               this.showTargetedOfferButtonPurchasedIcon(true);
               mContainer.mClip.TargetedOfferButton.enabled = false;
               mContainer.mClip.TargetedOfferButton.removeEventListener(MouseEvent.CLICK,this.buyTargetedOffer);
            }
            else
            {
               this.showTargetedOfferButtonPurchasedIcon(false);
               mContainer.mClip.TargetedOfferButton.enabled = true;
               if(!mContainer.mClip.TargetedOfferButton.hasEventListener(MouseEvent.CLICK))
               {
                  mContainer.mClip.TargetedOfferButton.addEventListener(MouseEvent.CLICK,this.buyTargetedOffer);
               }
            }
            nextRefreshIn = smContextualOfferData.rfs * 1000 - new Date().time;
            if(nextRefreshIn < 1000)
            {
               nextRefreshIn = 1000;
            }
            this.mTargetedOfferRefreshTimer = new Timer(nextRefreshIn,1);
            this.mTargetedOfferRefreshTimer.addEventListener(TimerEvent.TIMER,this.onTargetedOfferRefreshTimer);
            this.mTargetedOfferRefreshTimer.start();
         }
         else
         {
            mContainer.mClip.TargetedOfferButton.visible = false;
      ...
      {
         AngryBirdsBase.singleton.popupManager.openPopup(new ErrorPopup(ErrorPopup.ERROR_GENERAL,"Contextual offer loading error:" + e.text + " id: " + e.errorID));
      }
      
      private function changeTargetedOfferButtonText(textfieldName:String, text:String) : void
      {
         var stateDoc:DisplayObjectContainer = mContainer.mClip.TargetedOfferButton.upState as DisplayObjectContainer;
         var tf:TextField = stateDoc.getChildByName(textfieldName) as TextField;
         tf.text = text;
         stateDoc = mContainer.mClip.TargetedOfferButton.overState;
         tf = stateDoc.getChildByName(textfieldName) as TextField;
         tf.text = text;
         stateDoc = mContainer.mClip.TargetedOfferButton.downState;
         tf = stateDoc.getChildByName(textfieldName) as TextField;
         tf.text = text;
      }
      
      private function changeTargetedOfferButtonPowerupIcon(powerupIconName:String) : void
      {
         var stateDoc:DisplayObjectContainer = mContainer.mClip.TargetedOfferButton.upState;
         var powerupIcons:MovieClip = stateDoc.getChildByName("TargetedOfferPowerupIcons") as MovieClip;
         for(var i:int = 0; i < powerupIcons.numChildren; i++)
         {
            powerupIcons.getChildAt(i).visible = powerupIcons.getChildAt(i).name == powerupIconName;
         }
         stateDoc = mContainer.mClip.TargetedOfferButton.overState;
         powerupIcons = stateDoc.getChildByName("TargetedOfferPowerupIcons") as MovieClip;
         for(i = 0; i < powerupIcons.numChildren; i++)
         {
            powerupIcons.getChildAt(i).visible = powerupIcons.getChildAt(i).name == powerupIconName;
         }
         stateDoc = mContainer.mClip.TargetedOfferButton.downState;
      ...
      private function showTargetedOfferButtonPurchasedIcon(value:Boolean) : void
      {
         mContainer.mClip.purchasedIcon.visible = value;
      }
      
      private function buyTargetedOffer(e:MouseEvent) : void
      {
         var shopItem:ShopItem = null;
         var pricePoint:PricePoint = null;
         if(!smPurchaceRequested && !this.mContextualOfferLoader && smContextualOfferData)
         {
            ...
            shopItem = new ShopItem(smContextualOfferData.cid,[],false,smContextualOfferData.cur,smContextualOfferData.ogo);
            pricePoint = new PricePoint(1,0,smContextualOfferData.prc);
            this.mBuyItemWithPremiumCurrency = new BuyItemWithPremiumCurrency(shopItem,pricePoint,BuyItemWithPremiumCurrency.PAYMENT_TYPE_CASH,ID);
            this.mBuyItemWithPremiumCurrency.addEventListener(BuyItemEvent.ITEM_BOUGHT_PREMIUM_CURRENCY,this.onBuyTargetedOfferCompleted);
            this.mBuyItemWithPremiumCurrency.addEventListener(BuyItemEvent.ITEM_BOUGHT_FAILED,this.onBuyTargetedOfferFailed);
            this.mBuyItemWithPremiumCurrency.addEventListener(BuyItemWithPremiumCurrency.EVENT_PURCHASE_TIMER_COMPLETED,this.onBuyTargetedOfferCompleted);
         }
      }
      
      private function onBuyTargetedOfferCompleted(e:Event) : void
      {
         smPurchaceRequested = false;
         this.loadContextualOffer();
      }
      
      private function onBuyTargetedOfferFailed(event:BuyItemEvent) : void
      {
         var popup:WarningPopup = new WarningPopup(PopupLayerIndexFacebook.ALERT,PopupPriorityType.TOP,event.errorMessage,event.errorTitle,event.errorCode.toString(),false);
         AngryBirdsBase.singleton.popupManager.openPopup(popup);
         smPurchaceRequested = false;
      }
      
      private function onTargetedOfferRefreshTimer(e:TimerEvent) : void
      {
         if(this.mTargetedOfferRefreshTimer)
         {
            this.stopTargetedOfferRefreshTimer();
            this.loadContextualOffer();
         }
      ...
      private function stopTargetedOfferRefreshTimer() : void
      {
         if(this.mTargetedOfferRefreshTimer)
         {
            this.mTargetedOfferRefreshTimer.stop();
            this.mTargetedOfferRefreshTimer.removeEventListener(TimerEvent.TIMER,this.onTargetedOfferRefreshTimer);
            this.mTargetedOfferRefreshTimer = null;
         }
      }
   }
}
