package com.angrybirds.server
{
   ...
   public class TournamentLoader extends EventDispatcher implements ITournamentLoader
   {
      ...
      private const PATH_TOURNAMENTS_INFO:String = "/tournaments/info";
      ...
      private var mCachedOverlappingTournamentsInfo:Object;
      ...
      public function loadOverlappingTournaments() : void
      {
         var urlRequest:URLRequest = URLRequestFactory.getNonCachingURLRequest(AngryBirdsBase.SERVER_ROOT + this.PATH_TOURNAMENTS_INFO + "?st=" + SessionRetryingURLLoader.sessionToken);
         urlRequest.method = URLRequestMethod.GET;
         urlRequest.contentType = this.getContentType();
         var abfLoader:ABFLoader = new ABFLoader();
         abfLoader.addEventListener(Event.COMPLETE,this.onOverlappingTournamentsLoaded);
         abfLoader.addEventListener(IOErrorEvent.IO_ERROR,this.onOverlappingTournamentError);
         abfLoader.addEventListener(SecurityErrorEvent.SECURITY_ERROR,this.onOverlappingTournamentError);
         abfLoader.dataFormat = URLLoaderDataFormat.TEXT;
         abfLoader.load(urlRequest);
      }
      
      protected function onOverlappingTournamentsLoaded(e:Event) : void
      {
         if(!e || !e.currentTarget)
         {
            AngryBirdsBase.singleton.popupManager.openPopup(new ErrorPopup(ErrorPopup.ERROR_GENERAL,"Can\'t load the overlapping tournaments."));
            return;
         }
         e.currentTarget.removeEventListener(Event.COMPLETE,this.onOverlappingTournamentsLoaded);
         e.currentTarget.removeEventListener(IOErrorEvent.IO_ERROR,this.onOverlappingTournamentError);
         e.currentTarget.removeEventListener(SecurityErrorEvent.SECURITY_ERROR,this.onOverlappingTournamentError);
         this.mCachedOverlappingTournamentsInfo = e.currentTarget.data;
         dispatchEvent(new TournamentEvent(TournamentEvent.OVERLAPPING_TOURNAMENTS_INFO_LOADED,this.mCachedOverlappingTournamentsInfo));
      }
      
      public function loadTournament(tournamentId:int = 0) : void
      {
         var urlRequest:URLRequest = URLRequestFactory.getNonCachingURLRequest(AngryBirdsBase.SERVER_ROOT + this.PATH_TOURNAMENT + this.PATH_CURRENT_TOURNAMENT_INFO + "?st=" + SessionRetryingURLLoader.sessionToken);
         urlRequest.method = URLRequestMethod.GET;
         urlRequest.contentType = this.getContentType();
         var abfLoader:ABFLoader = new ABFLoader();
         if(tournamentId > 0)
         {
            urlRequest.url += "&tournamentId=" + tournamentId;
         }
         abfLoader.addEventListener(Event.COMPLETE,this.onTournamentLoaded);
         abfLoader.addEventListener(IOErrorEvent.IO_ERROR,this.onError);
         abfLoader.addEventListener(SecurityErrorEvent.SECURITY_ERROR,this.onError);
         abfLoader.dataFormat = URLLoaderDataFormat.TEXT;
         abfLoader.load(urlRequest);
      }
      
      protected function onTournamentLoaded(e:Event) : void
      {
         e.currentTarget.removeEventListener(Event.COMPLETE,this.onTournamentLoaded);
         e.currentTarget.removeEventListener(IOErrorEvent.IO_ERROR,this.onError);
         e.currentTarget.removeEventListener(SecurityErrorEvent.SECURITY_ERROR,this.onError);
         if(e && e.currentTarget)
         {
            if(!e.currentTarget.data.id)
            {
               AngryBirdsBase.singleton.popupManager.openPopup(new ErrorPopup(ErrorPopup.ERROR_GENERAL,"Tournament not found."));
               return;
            }
            if(!this.mCachedTournamentData)
            {
               this.mCachedTournamentData = new Object();
            }
            this.mCachedTournamentData[e.currentTarget.data.id] = e.currentTarget.data;
            this.loadCachedTournament(e.currentTarget.data.id);
         }
         else
         {
            this.loadCachedTournament(0);
         }
      }
      
      public function loadCachedTournament(tournamentId:int = 0) : void
      {
         TournamentEventManager.instance.setData(this.mCachedTournamentData[tournamentId].eventInfo);
         dispatchEvent(new TournamentEvent(TournamentEvent.TOURNAMENT_LOADED,this.mCachedTournamentData[tournamentId]));
      ...
      public function getCachedTournament(tournamentId:int) : Object
      {
         if(this.mCachedTournamentData[tournamentId])
         {
            return this.mCachedTournamentData[tournamentId];
         }
         return null;
      }
      ...
      public function loadPreviousTournamentResults() : void
      {
         ...
         if(TournamentModel.instance.getCurrentTournamentId() > 0)
         {
            urlRequest.url += "&tournamentId=" + TournamentModel.instance.getCurrentTournamentId();
         }
         urlRequest.method = URLRequestMethod.GET;
         urlRequest.contentType = this.getContentType();
      ...
      public function loadPlayersCurrentTournamentLevelScores(tournamentId:int) : void
      {
      ...
      {
         AngryBirdsBase.singleton.popupManager.openPopup(new ErrorPopup(ErrorPopup.ERROR_GENERAL,"Tournament loading error:" + e.text + " id: " + e.errorID));
      }
      
      protected function onOverlappingTournamentError(e:ErrorEvent) : void
      {
         dispatchEvent(new TournamentEvent(TournamentEvent.OVERLAPPING_TOURNAMENTS_INFO_NOT_FOUND));
         dispatchEvent(new TournamentEvent(TournamentEvent.OVERLAPPING_TOURNAMENTS_INFO_LOADED,null));
      }
         ...
         if(mLevelManager)
         {
            for each(levelObj in levels)
            {
               ...
               if(level == null)
               {
                  ...
                  {
                     levelURL = AngryBirdsFacebook.TOURNAMENT_JSON_LEVELS_FOLDER_PREFIX + levelObj.levelId.substr(0,6) + "/Level" + levelObj.levelId + ".lvl";
                  }
      ...
      public function isTournamentCached(tournamentId:int) : Boolean
      {
         return this.mCachedTournamentData && this.mCachedTournamentData[tournamentId];
      }
      
      public function loadMainMenuData(tournamentId:int) : void
      {
         this.loadPlayersCurrentTournamentLevelScores(tournamentId);
         this.loadCurrentTournamentStandings(tournamentId);
      }
   }
}
