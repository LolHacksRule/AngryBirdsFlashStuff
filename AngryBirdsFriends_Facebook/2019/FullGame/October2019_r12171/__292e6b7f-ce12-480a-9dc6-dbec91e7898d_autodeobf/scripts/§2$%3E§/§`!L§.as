package §2$>§
{
   import flash.geom.Rectangle;
   
   public class §`!L§
   {
      
      public static const VERSION:String = "1.2.0";
       
      
      private var §>"h§:int = 0;
      
      private var §"#G§:int = 0;
      
      private var §3!h§:int = 8;
      
      private var §5!#§:int = 0;
      
      private var §@#>§:int = 0;
      
      private var §8$B§:Array;
      
      private var §^"v§:Vector.<§&$6§>;
      
      private var §`$<§:Vector.<§&$6§>;
      
      private var §'3§:Vector.<§&$6§>;
      
      private var §<$C§:§&$6§;
      
      private var §+n§:Vector.<§&"l§>;
      
      private var §[#l§:Vector.<§&$6§>;
      
      public function §`!L§(param1:int, param2:int, param3:int = 0)
      {
         this.§8$B§ = [];
         this.§^"v§ = new Vector.<§&$6§>();
         this.§`$<§ = new Vector.<§&$6§>();
         this.§'3§ = new Vector.<§&$6§>();
         this.§+n§ = new Vector.<§&"l§>();
         this.§[#l§ = new Vector.<§&$6§>();
         super();
         this.§<$C§ = new §&$6§(param1 + 1,param2 + 1,0,0);
         this.reset(param1,param2,param3);
      }
      
      public function get §@"-§() : int
      {
         return this.§^"v§.length;
      }
      
      public function get §3#t§() : int
      {
         return this.§5!#§;
      }
      
      public function get §,@§() : int
      {
         return this.§@#>§;
      }
      
      public function get §-!Y§() : int
      {
         return this.§3!h§;
      }
      
      public function reset(param1:int, param2:int, param3:int = 0) : void
      {
         while(this.§^"v§.length)
         {
            this.§^"T§(this.§^"v§.pop());
         }
         while(this.§`$<§.length)
         {
            this.§^"T§(this.§`$<§.pop());
         }
         this.§>"h§ = param1;
         this.§"#G§ = param2;
         this.§5!#§ = 0;
         this.§@#>§ = 0;
         this.§`$<§[0] = this.§,"%§(0,0,this.§>"h§,this.§"#G§);
         while(this.§8$B§.length)
         {
            this.§'j§(this.§8$B§.pop());
         }
         this.§3!h§ = param3;
      }
      
      public function §9!x§(param1:int, param2:Rectangle) : Rectangle
      {
         var _loc3_:§&$6§ = this.§^"v§[param1];
         if(param2)
         {
            param2.x = _loc3_.x;
            param2.y = _loc3_.y;
            param2.width = _loc3_.width;
            param2.height = _loc3_.height;
            return param2;
         }
         return new Rectangle(_loc3_.x,_loc3_.y,_loc3_.width,_loc3_.height);
      }
      
      public function §!"1§(param1:int) : int
      {
         var _loc2_:§&$6§ = this.§^"v§[param1];
         return _loc2_.id;
      }
      
      public function §7"N§(param1:int, param2:int, param3:int) : void
      {
         var _loc4_:§&"l§ = this.§'"u§(param1,param2,param3);
         this.§8$B§.push(_loc4_);
      }
      
      public function § !H§(param1:Boolean = true) : int
      {
         var _loc2_:§&"l§ = null;
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc6_:§&$6§ = null;
         var _loc7_:§&$6§ = null;
         if(param1)
         {
            this.§8$B§.sortOn("width",Array.NUMERIC);
         }
         while(this.§8$B§.length > 0)
         {
            _loc2_ = this.§8$B§.pop() as §&"l§;
            _loc3_ = _loc2_.width;
            _loc4_ = _loc2_.height;
            if((_loc5_ = this.§!"7§(_loc3_,_loc4_)) >= 0)
            {
               _loc6_ = this.§`$<§[_loc5_];
               (_loc7_ = this.§,"%§(_loc6_.x,_loc6_.y,_loc3_,_loc4_)).id = _loc2_.id;
               this.§["%§(_loc7_,this.§`$<§,this.§'3§);
               while(this.§'3§.length > 0)
               {
                  this.§`$<§[this.§`$<§.length] = this.§'3§.pop();
               }
               this.§^"v§[this.§^"v§.length] = _loc7_;
               if(_loc7_.right > this.§5!#§)
               {
                  this.§5!#§ = _loc7_.right;
               }
               if(_loc7_.bottom > this.§@#>§)
               {
                  this.§@#>§ = _loc7_.bottom;
               }
            }
            this.§'j§(_loc2_);
         }
         return this.§@"-§;
      }
      
      private function §;$4§(param1:Vector.<§&$6§>) : void
      {
         var _loc3_:§&$6§ = null;
         var _loc4_:int = 0;
         var _loc5_:§&$6§ = null;
         var _loc6_:§&$6§ = null;
         var _loc2_:int = param1.length - 1;
         while(_loc2_ >= 0)
         {
            _loc3_ = param1[_loc2_];
            _loc4_ = param1.length - 1;
            while(_loc4_ >= 0)
            {
               if(_loc2_ != _loc4_)
               {
                  _loc5_ = param1[_loc4_];
                  if(_loc3_.x >= _loc5_.x && _loc3_.y >= _loc5_.y && _loc3_.right <= _loc5_.right && _loc3_.bottom <= _loc5_.bottom)
                  {
                     this.§^"T§(_loc3_);
                     _loc6_ = param1.pop();
                     if(_loc2_ < param1.length)
                     {
                        param1[_loc2_] = _loc6_;
                        break;
                     }
                     break;
                  }
               }
               _loc4_--;
            }
            _loc2_--;
         }
      }
      
      private function §["%§(param1:§&$6§, param2:Vector.<§&$6§>, param3:Vector.<§&$6§>) : void
      {
         var _loc10_:§&$6§ = null;
         var _loc11_:§&$6§ = null;
         var _loc4_:int = param1.x;
         var _loc5_:int = param1.y;
         var _loc6_:int = param1.right + 1 + this.§3!h§;
         var _loc7_:int = param1.bottom + 1 + this.§3!h§;
         var _loc8_:§&$6§ = null;
         if(this.§3!h§ == 0)
         {
            _loc8_ = param1;
         }
         var _loc9_:int = param2.length - 1;
         while(_loc9_ >= 0)
         {
            _loc10_ = param2[_loc9_];
            if(!(_loc4_ >= _loc10_.right || _loc6_ <= _loc10_.x || _loc5_ >= _loc10_.bottom || _loc7_ <= _loc10_.y))
            {
               if(!_loc8_)
               {
                  _loc8_ = this.§,"%§(param1.x,param1.y,param1.width + this.§3!h§,param1.height + this.§3!h§);
               }
               this.§'#u§(_loc8_,_loc10_,param3);
               _loc11_ = param2.pop();
               if(_loc9_ < param2.length)
               {
                  param2[_loc9_] = _loc11_;
               }
            }
            _loc9_--;
         }
         if(_loc8_ && _loc8_ != param1)
         {
            this.§^"T§(_loc8_);
         }
         this.§;$4§(param3);
      }
      
      private function §'#u§(param1:§&$6§, param2:§&$6§, param3:Vector.<§&$6§>) : void
      {
         var _loc4_:int = 0;
         var _loc5_:int;
         if((_loc5_ = param2.right - param1.right) > 0)
         {
            param3[param3.length] = this.§,"%§(param1.right,param2.y,_loc5_,param2.height);
            _loc4_++;
         }
         var _loc6_:int;
         if((_loc6_ = param1.x - param2.x) > 0)
         {
            param3[param3.length] = this.§,"%§(param2.x,param2.y,_loc6_,param2.height);
            _loc4_++;
         }
         var _loc7_:int;
         if((_loc7_ = param2.bottom - param1.bottom) > 0)
         {
            param3[param3.length] = this.§,"%§(param2.x,param1.bottom,param2.width,_loc7_);
            _loc4_++;
         }
         var _loc8_:int;
         if((_loc8_ = param1.y - param2.y) > 0)
         {
            param3[param3.length] = this.§,"%§(param2.x,param2.y,param2.width,_loc8_);
            _loc4_++;
         }
         if(_loc4_ == 0 && (param1.width < param2.width || param1.height < param2.height))
         {
            param3[param3.length] = param2;
         }
         else
         {
            this.§^"T§(param2);
         }
      }
      
      private function §!"7§(param1:int, param2:int) : int
      {
         var _loc9_:§&$6§ = null;
         var _loc3_:§&$6§ = this.§<$C§;
         var _loc4_:int = -1;
         var _loc5_:int = param1 + this.§3!h§;
         var _loc6_:int = param2 + this.§3!h§;
         var _loc7_:int;
         var _loc8_:int = (_loc7_ = this.§`$<§.length) - 1;
         while(_loc8_ >= 0)
         {
            if((_loc9_ = this.§`$<§[_loc8_]).x < this.§5!#§ || _loc9_.y < this.§@#>§)
            {
               if(_loc9_.x < _loc3_.x && _loc5_ <= _loc9_.width && _loc6_ <= _loc9_.height)
               {
                  _loc4_ = _loc8_;
                  if(_loc5_ == _loc9_.width && _loc9_.width <= _loc9_.height && _loc9_.right < this.§>"h§ || _loc6_ == _loc9_.height && _loc9_.height <= _loc9_.width)
                  {
                     break;
                  }
                  _loc3_ = _loc9_;
               }
            }
            else if(_loc9_.x < _loc3_.x && param1 <= _loc9_.width && param2 <= _loc9_.height)
            {
               _loc4_ = _loc8_;
               if(param1 == _loc9_.width && _loc9_.width <= _loc9_.height && _loc9_.right < this.§>"h§ || param2 == _loc9_.height && _loc9_.height <= _loc9_.width)
               {
                  break;
               }
               _loc3_ = _loc9_;
            }
            _loc8_--;
         }
         return _loc4_;
      }
      
      private function §,"%§(param1:int, param2:int, param3:int, param4:int) : §&$6§
      {
         var _loc5_:§&$6§ = null;
         if(this.§[#l§.length > 0)
         {
            (_loc5_ = this.§[#l§.pop()).x = param1;
            _loc5_.y = param2;
            _loc5_.width = param3;
            _loc5_.height = param4;
            _loc5_.right = param1 + param3;
            _loc5_.bottom = param2 + param4;
            return _loc5_;
         }
         return new §&$6§(param1,param2,param3,param4);
      }
      
      private function §^"T§(param1:§&$6§) : void
      {
         this.§[#l§[this.§[#l§.length] = param1;
      }
      
      private function §'"u§(param1:int, param2:int, param3:int) : §&"l§
      {
         var _loc4_:§&"l§ = null;
         if(this.§+n§.length > 0)
         {
            (_loc4_ = this.§+n§.pop()).width = param1;
            _loc4_.height = param2;
            _loc4_.id = param3;
            return _loc4_;
         }
         return new §&"l§(param1,param2,param3);
      }
      
      private function §'j§(param1:§&"l§) : void
      {
         this.§+n§[this.§+n§.length] = param1;
      }
   }
}
