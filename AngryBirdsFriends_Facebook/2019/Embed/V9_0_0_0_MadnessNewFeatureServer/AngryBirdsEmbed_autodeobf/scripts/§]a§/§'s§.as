package §]a§
{
   ...
   public class §'s§
   {
      ...
      private function §;+§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§6!<§(param1 as String);
         }
         if(param1 is Number)
         {
      ...
      private function §6r§(param1:Object) : String
      {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§6!<§(v.@name.toString()) + ":" + this.§;+§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
