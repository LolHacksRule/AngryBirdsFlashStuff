package com.rovio.utils
{
   ...
   public class LuaUtils
   {
      ...
      private static function fixArrays(jsonString:String, emptyObjectAsArray:Boolean) : String
      {
         ...
         var i:int = 0;
         var opening:int = jsonString.indexOf("{");
         var openingMax:int = opening;
         var closing:int = jsonString.indexOf("}");
         while(opening >= 0 || closing >= 0)
         {
            ...
            else
            {
               ...
               if(part.indexOf("{",1) < 0)
               {
                  ...
                  {
                     arrayParts = part.substr(1,part.length - 2).split(",");
                     mixed = false;
                     for(i = 0; i < arrayParts.length; i++)
                     {
                        if(arrayParts[i].indexOf(":") < 0)
                        {
                           mixed = true;
                           break;
                        }
               ...
               else
               {
                  ...
                  {
                     partOpeningBrace = part.indexOf("{",1);
               ...
               closing = jsonString.indexOf("}",closing + 1);
            }
            opening = jsonString.indexOf("{",openingMax + 1);
         }
         ...
         for(i = newClosingBraces.length - 1; i >= 0; i--)
         {
            ...
            jsonString = partBefore + "}" + partAfter;
