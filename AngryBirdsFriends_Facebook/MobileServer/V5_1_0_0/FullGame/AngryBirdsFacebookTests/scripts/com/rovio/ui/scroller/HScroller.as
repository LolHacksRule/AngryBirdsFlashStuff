package com.rovio.ui.scroller
{
   ...
   public class HScroller
   {
      ...
      {
         this.mMaskHeight = this.mSprite.mask.height = value;
      }
      
      protected function populate(resetOffset:Boolean = true) : void
      {
         var renderer:ScrollerItemRenderer = null;
         if(resetOffset)
         {
         ...
         for(var i:int = 0; i < this.mItemCount; i++)
         {
            ...
            renderer.index = i;
            this.setRendererData(renderer,this.data[i]);
            this.mItemRenderers.push(renderer);
         }
         this.updatePositions();
      }
      
      protected function setRendererData(renderer:ScrollerItemRenderer, data:Object) : void
      {
         renderer.data = data;
      }
      ...
      public function setData(data:Array, resetOffset:Boolean = true) : void
      {
         this.mData = data;
         this.clear();
         if(this.mData)
         {
            this.populate(resetOffset);
         }
      }
      ...
      protected function updatePositions() : void
      {
         ...
         for(i = 0; i < this.mData.length; i++)
         {
            ...
            if(x > -this.itemRendererWidth && x < this.mMaskWidth && !this.hasRendererWithIndex(i))
            {
               newRenderer = this.getRenderer();
               this.mSprite.addChild(newRenderer);
               this.setRendererData(newRenderer,this.mData[i]);
               newRenderer.index = i;
               newRenderer.x = x;
      ...
      protected function hide(renderer:ScrollerItemRenderer) : void
      {
         this.mSprite.removeChild(renderer);
      ...
      public function refresh() : void
      {
         ...
         {
            this.setRendererData(renderer,this.mData[renderer.index]);
         }
      }
      ...
      {
         this.setRendererData(renderer,null);
