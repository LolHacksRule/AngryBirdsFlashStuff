package §9s§
{
   import §!!,§.§`Q§;
   
   public class §^u§
   {
      
      public static const MIGHTY_EAGLE_USE_SHADE:Boolean = true;
      
      public static const MIGHTY_EAGLE_FLYING_ANGLE:Number = -22.9;
      
      public static const MIGHTY_EAGLE_FLYING_SPEED:Number = 0.128;
      
      public static const MIGHTY_EAGLE_ROTATION_SPEED:Number = 8;
      
      public static const MIGHTY_EAGLE_WAIT_TIME:Number = 2400;
      
      public static const MIGHTY_EAGLE_SOUND_DELAY:Number = 1000;
      
      public static const MIGHTY_EAGLE_SHADING_DELAY:Number = 2900;
      
      public static const MIGHTY_EAGLE_STARTING_DISTANCE_X:Number = 140;
      
      public static const MIGHTY_EAGLE_STARTING_DISTANCE_Y:Number = 140;
      
      public static const MIGHTY_EAGLE_Y_CHANGE:Number = Math.sin(§`Q§.§`L§(MIGHTY_EAGLE_FLYING_ANGLE));
      
      public static const MIGHTY_EAGLE_X_DIRECTION:Number = 1;
      
      public static const §#f§:Number = 1500;
      
      public static const §8!B§:Number = 0.7;
      
      public static const §@F§:Number = 0.2;
      
      public static const §switch§:Number = 8.8;
      
      public static const §+F§:Number = 2400;
      
      public static const §@!H§:Number = 1;
      
      public static const §>!O§:Number = 0.15;
      
      public static const §4M§:Number = 4;
      
      public static const §8=§:Number = 60;
      
      public static const §%!,§:Number = 16;
      
      public static const §!G§:Number = 6 * §%!,§;
      
      public static const SARDINE_CAN_INITIAL_ROTATION_SPEED:Number = 0.07;
      
      public static const SARDINE_CAN_ROTATION_ACCELERATION:Number = 0.0002;
      
      public static const SARDINE_CAN_MAX_ROTATION_SPEED:Number = SARDINE_CAN_INITIAL_ROTATION_SPEED * 4.4;
      
      public static const SARDINE_CAN_DELAY_AFTER_HIT:int = 3000;
      
      public static const §+!4§:Number = 5;
      
      public static var §]c§:int = 2;
      
      public static var §`!D§:Boolean = false;
      
      public static const §;!M§:Number = 600;
      
      public static const §'!D§:Number = 1.3;
      
      public static const §9-§:Number = 1.3;
      
      public static const §7!-§:Number = 1.3;
      
      public static const §"!>§:Number = 1.3;
      
      public static const §[n§:Number = 1.3;
      
      public static const §4l§:Number = 1.3;
      
      public static const §2G§:Number = 1.5;
      
      public static const §5!9§:Number = 1.2;
      
      public static const §,f§:Number = 1.25;
      
      public static const §`!I§:Number = 1.2;
      
      public static const §2!6§:Number = 1.25;
      
      public static var §?p§:int = 60;
      
      public static const §6H§:Number = 9;
      
      public static const §[!I§:Number = 5;
      
      public static const §4>§:Number = 3000;
      
      public static const § !F§:Number = 60;
      
      public static const §'E§:Number = 6;
      
      public static const §^!5§:Number = 3;
      
      public static const §&!+§:Number = 1 / 30;
      
      public static const §8q§:int = 25;
      
      public static const §-[§:int = 1;
      
      public static const §2x§:Number = 70;
      
      public static const §?;§:Number = 10;
      
      public static const §'y§:Number = 1000;
      
      public static const §"T§:Number = 1.3;
      
      public static const §&r§:Number = 7.5;
      
      public static const §<#§:Number = 300;
      
      public static const §>%§:Number = 0.5;
      
      public static const §"!§:Number = 6;
      
      public static const §,!&§:Number = 9;
      
      public static const §?!G§:Number = 0.15;
      
      public static const §]!9§:Number = 10;
       
      
      public function §^u§()
      {
         super();
      }
   }
}
