package §48§
{
   import §!D§.*;
   import §#]§.*;
   import §4!!§.*;
   import §55§.*;
   import §8k§.*;
   import §<!`§.*;
   import §<!c§.b2Controller;
   import §<!c§.b2ControllerEdge;
   
   use namespace b2internal;
   
   public class b2World
   {
      
      private static var s_timestep2:b2TimeStep = new b2TimeStep();
      
      private static var §#_§:b2Transform = new b2Transform();
      
      private static var §&"7§:b2Sweep = new b2Sweep();
      
      private static var §3!X§:b2Sweep = new b2Sweep();
      
      private static var §`!%§:b2TimeStep = new b2TimeStep();
      
      private static var §`"2§:Vector.<b2Body> = new Vector.<b2Body>();
      
      private static var §0!0§:b2Color = new b2Color(0.5,0.8,0.8);
      
      private static var §`!1§:Boolean;
      
      private static var §&!j§:Boolean;
      
      public static const §3g§:int = 1;
      
      public static const §+"F§:int = 2;
       
      
      private var §2A§:Vector.<b2Body>;
      
      b2internal var §`">§:int;
      
      b2internal var §1Y§:b2ContactManager;
      
      private var §<8§:b2ContactSolver;
      
      private var § !N§:b2Island;
      
      b2internal var §'!7§:b2Body;
      
      private var §3n§:b2Joint;
      
      b2internal var §>"#§:b2Contact;
      
      private var §]!M§:int;
      
      b2internal var §3M§:int;
      
      private var §0!$§:int;
      
      private var §+q§:b2Controller;
      
      private var §,"4§:int;
      
      private var §8!m§:b2Vec2;
      
      private var §>!L§:Boolean;
      
      b2internal var m_groundBody:b2Body;
      
      private var §]!v§:b2DestructionListener;
      
      private var §8!?§:b2DebugDraw;
      
      private var §&^§:Number;
      
      public function b2World(param1:b2Vec2, param2:Boolean)
      {
         this.§2A§ = new Vector.<b2Body>();
         this.§1Y§ = new b2ContactManager();
         this.§<8§ = new b2ContactSolver();
         this.§ !N§ = new b2Island();
         super();
         this.§]!v§ = null;
         this.§8!?§ = null;
         this.§'!7§ = null;
         this.§>"#§ = null;
         this.§3n§ = null;
         this.§+q§ = null;
         this.§]!M§ = 0;
         this.§3M§ = 0;
         this.§0!$§ = 0;
         this.§,"4§ = 0;
         §`!1§ = true;
         §&!j§ = true;
         this.§>!L§ = param2;
         this.§8!m§ = param1;
         this.§&^§ = 0;
         this.§1Y§.m_world = this;
         var _loc3_:b2BodyDef = new b2BodyDef();
         this.m_groundBody = this.§,P§(_loc3_);
      }
      
      public function §<">§(param1:b2DestructionListener) : void
      {
         this.§]!v§ = param1;
      }
      
      public function §1"C§(param1:b2ContactFilter) : void
      {
         this.§1Y§.§1!9§ = param1;
      }
      
      public function §!!L§(param1:b2ContactListener) : void
      {
         this.§1Y§.§&!Y§ = param1;
      }
      
      public function §+"C§(param1:b2DebugDraw) : void
      {
         this.§8!?§ = param1;
      }
      
      public function §<"5§(param1:§0!K§) : void
      {
         var _loc4_:b2Fixture = null;
         var _loc2_:§0!K§ = this.§1Y§.§"!q§;
         this.§1Y§.§"!q§ = param1;
         var _loc3_:b2Body = this.§'!7§;
         while(_loc3_)
         {
            _loc4_ = _loc3_.§^F§;
            while(_loc4_)
            {
               _loc4_.§=!;§ = param1.§'X§(_loc2_.§-Z§(_loc4_.§=!;§),_loc4_);
               _loc4_ = _loc4_.§%"@§;
            }
            _loc3_ = _loc3_.§%"@§;
         }
      }
      
      public function §5G§() : void
      {
         this.§1Y§.§"!q§.§5G§();
      }
      
      public function §9g§() : int
      {
         return this.§1Y§.§"!q§.§9g§();
      }
      
      public function §,P§(param1:b2BodyDef) : b2Body
      {
         if(this.§8Z§() == true)
         {
            return null;
         }
         var _loc2_:b2Body = new b2Body(param1,this);
         _loc2_.§-!Q§ = null;
         _loc2_.§%"@§ = this.§'!7§;
         if(this.§'!7§)
         {
            this.§'!7§.§-!Q§ = _loc2_;
         }
         this.§'!7§ = _loc2_;
         ++this.§]!M§;
         return _loc2_;
      }
      
      public function §""=§(param1:b2Body) : void
      {
         var _loc6_:b2JointEdge = null;
         var _loc7_:b2ControllerEdge = null;
         var _loc8_:b2ContactEdge = null;
         var _loc9_:b2Fixture = null;
         if(this.§8Z§() == true)
         {
            return;
         }
         var _loc2_:b2JointEdge = param1.§3n§;
         while(_loc2_)
         {
            _loc6_ = _loc2_;
            _loc2_ = _loc2_.§5"&§;
            if(this.§]!v§)
            {
               this.§]!v§.§8X§(_loc6_.§;!W§);
            }
            this.§7! §(_loc6_.§;!W§);
         }
         var _loc3_:b2ControllerEdge = param1.§+q§;
         while(_loc3_)
         {
            _loc7_ = _loc3_;
            _loc3_ = _loc3_.§'0§;
            _loc7_.§'" §.§<V§(param1);
         }
         var _loc4_:b2ContactEdge = param1.§>"#§;
         while(_loc4_)
         {
            _loc8_ = _loc4_;
            _loc4_ = _loc4_.§5"&§;
            this.§1Y§.§'S§(_loc8_.§0q§);
         }
         param1.§>"#§ = null;
         var _loc5_:b2Fixture = param1.§^F§;
         while(_loc5_)
         {
            _loc9_ = _loc5_;
            _loc5_ = _loc5_.§%"@§;
            if(this.§]!v§)
            {
               this.§]!v§.§ !i§(_loc9_);
            }
            _loc9_.§,K§(this.§1Y§.§"!q§);
            _loc9_.§'S§();
         }
         param1.§^F§ = null;
         param1.§<!b§ = 0;
         if(param1.§-!Q§)
         {
            param1.§-!Q§.§%"@§ = param1.§%"@§;
         }
         if(param1.§%"@§)
         {
            param1.§%"@§.§-!Q§ = param1.§-!Q§;
         }
         if(param1 == this.§'!7§)
         {
            this.§'!7§ = param1.§%"@§;
         }
         --this.§]!M§;
      }
      
      public function §?[§(param1:b2JointDef) : b2Joint
      {
         var _loc5_:b2ContactEdge = null;
         var _loc2_:b2Joint = b2Joint.§?!Q§(param1,null);
         _loc2_.§-!Q§ = null;
         _loc2_.§%"@§ = this.§3n§;
         if(this.§3n§)
         {
            this.§3n§.§-!Q§ = _loc2_;
         }
         this.§3n§ = _loc2_;
         ++this.§0!$§;
         _loc2_.§`t§.§;!W§ = _loc2_;
         _loc2_.§`t§.§ `§ = _loc2_.§4"0§;
         _loc2_.§`t§.§[g§ = null;
         _loc2_.§`t§.§5"&§ = _loc2_.§8!Z§.§3n§;
         if(_loc2_.§8!Z§.§3n§)
         {
            _loc2_.§8!Z§.§3n§.§[g§ = _loc2_.§`t§;
         }
         _loc2_.§8!Z§.§3n§ = _loc2_.§`t§;
         _loc2_.§%!P§.§;!W§ = _loc2_;
         _loc2_.§%!P§.§ `§ = _loc2_.§8!Z§;
         _loc2_.§%!P§.§[g§ = null;
         _loc2_.§%!P§.§5"&§ = _loc2_.§4"0§.§3n§;
         if(_loc2_.§4"0§.§3n§)
         {
            _loc2_.§4"0§.§3n§.§[g§ = _loc2_.§%!P§;
         }
         _loc2_.§4"0§.§3n§ = _loc2_.§%!P§;
         var _loc3_:b2Body = param1.§!!v§;
         var _loc4_:b2Body = param1.§'!4§;
         if(param1.collideConnected == false)
         {
            _loc5_ = _loc4_.§0!@§();
            while(_loc5_)
            {
               if(_loc5_.§ `§ == _loc3_)
               {
                  _loc5_.§0q§.§6"F§();
               }
               _loc5_ = _loc5_.§5"&§;
            }
         }
         return _loc2_;
      }
      
      public function §7! §(param1:b2Joint) : void
      {
         var _loc5_:b2ContactEdge = null;
         var _loc2_:Boolean = param1.§6!p§;
         if(param1.§-!Q§)
         {
            param1.§-!Q§.§%"@§ = param1.§%"@§;
         }
         if(param1.§%"@§)
         {
            param1.§%"@§.§-!Q§ = param1.§-!Q§;
         }
         if(param1 == this.§3n§)
         {
            this.§3n§ = param1.§%"@§;
         }
         var _loc3_:b2Body = param1.§8!Z§;
         var _loc4_:b2Body = param1.§4"0§;
         _loc3_.SetAwake(true);
         _loc4_.SetAwake(true);
         if(param1.§`t§.§[g§)
         {
            param1.§`t§.§[g§.§5"&§ = param1.§`t§.§5"&§;
         }
         if(param1.§`t§.§5"&§)
         {
            param1.§`t§.§5"&§.§[g§ = param1.§`t§.§[g§;
         }
         if(param1.§`t§ == _loc3_.§3n§)
         {
            _loc3_.§3n§ = param1.§`t§.§5"&§;
         }
         param1.§`t§.§[g§ = null;
         param1.§`t§.§5"&§ = null;
         if(param1.§%!P§.§[g§)
         {
            param1.§%!P§.§[g§.§5"&§ = param1.§%!P§.§5"&§;
         }
         if(param1.§%!P§.§5"&§)
         {
            param1.§%!P§.§5"&§.§[g§ = param1.§%!P§.§[g§;
         }
         if(param1.§%!P§ == _loc4_.§3n§)
         {
            _loc4_.§3n§ = param1.§%!P§.§5"&§;
         }
         param1.§%!P§.§[g§ = null;
         param1.§%!P§.§5"&§ = null;
         b2Joint.§'S§(param1,null);
         --this.§0!$§;
         if(_loc2_ == false)
         {
            _loc5_ = _loc4_.§0!@§();
            while(_loc5_)
            {
               if(_loc5_.§ `§ == _loc3_)
               {
                  _loc5_.§0q§.§6"F§();
               }
               _loc5_ = _loc5_.§5"&§;
            }
         }
      }
      
      public function §`]§(param1:b2Controller) : b2Controller
      {
         param1.§%"@§ = this.§+q§;
         param1.§-!Q§ = null;
         this.§+q§ = param1;
         param1.m_world = this;
         ++this.§,"4§;
         return param1;
      }
      
      public function §^!H§(param1:b2Controller) : void
      {
         if(param1.§-!Q§)
         {
            param1.§-!Q§.§%"@§ = param1.§%"@§;
         }
         if(param1.§%"@§)
         {
            param1.§%"@§.§-!Q§ = param1.§-!Q§;
         }
         if(this.§+q§ == param1)
         {
            this.§+q§ = param1.§%"@§;
         }
         --this.§,"4§;
      }
      
      public function §>!a§(param1:b2Controller) : b2Controller
      {
         if(param1.m_world != this)
         {
            throw new Error("Controller can only be a member of one world");
         }
         param1.§%"@§ = this.§+q§;
         param1.§-!Q§ = null;
         if(this.§+q§)
         {
            this.§+q§.§-!Q§ = param1;
         }
         this.§+q§ = param1;
         ++this.§,"4§;
         param1.m_world = this;
         return param1;
      }
      
      public function §%g§(param1:b2Controller) : void
      {
         param1.§;!R§();
         if(param1.§%"@§)
         {
            param1.§%"@§.§-!Q§ = param1.§-!Q§;
         }
         if(param1.§-!Q§)
         {
            param1.§-!Q§.§%"@§ = param1.§%"@§;
         }
         if(param1 == this.§+q§)
         {
            this.§+q§ = param1.§%"@§;
         }
         --this.§,"4§;
      }
      
      public function §1=§(param1:Boolean) : void
      {
         §`!1§ = param1;
      }
      
      public function §;h§(param1:Boolean) : void
      {
         §&!j§ = param1;
      }
      
      public function §;!=§() : int
      {
         return this.§]!M§;
      }
      
      public function §'m§() : int
      {
         return this.§0!$§;
      }
      
      public function §,!N§() : int
      {
         return this.§3M§;
      }
      
      public function §%$§(param1:b2Vec2) : void
      {
         this.§8!m§ = param1;
      }
      
      public function §0u§() : b2Vec2
      {
         return this.§8!m§;
      }
      
      public function §^!a§() : b2Body
      {
         return this.m_groundBody;
      }
      
      public function §4$§(param1:Number, param2:int, param3:int) : void
      {
         if(this.§`">§ & §3g§)
         {
            this.§1Y§.§>!=§();
            this.§`">§ &= ~§3g§;
         }
         this.§`">§ |= §+"F§;
         var _loc4_:b2TimeStep;
         (_loc4_ = s_timestep2).§4"?§ = param1;
         _loc4_.§ get§ = param2;
         _loc4_.§7!;§ = param3;
         if(param1 > 0)
         {
            _loc4_.§,Q§ = 1 / param1;
         }
         else
         {
            _loc4_.§,Q§ = 0;
         }
         _loc4_.§ ,§ = this.§&^§ * param1;
         _loc4_.§'i§ = §`!1§;
         this.§1Y§.§ ! §();
         if(_loc4_.§4"?§ > 0)
         {
            this.§#!6§(_loc4_);
         }
         if(§&!j§ && _loc4_.§4"?§ > 0)
         {
            this.§`!!§(_loc4_);
         }
         if(_loc4_.§4"?§ > 0)
         {
            this.§&^§ = _loc4_.§,Q§;
         }
         this.§`">§ &= ~§+"F§;
      }
      
      public function §4,§() : void
      {
         var _loc1_:b2Body = this.§'!7§;
         while(_loc1_)
         {
            _loc1_.§&e§.§;v§();
            _loc1_.§;!+§ = 0;
            _loc1_ = _loc1_.§%"@§;
         }
      }
      
      public function §?!U§() : void
      {
         var _loc3_:b2Body = null;
         var _loc4_:b2Fixture = null;
         var _loc5_:b2Shape = null;
         var _loc6_:b2Joint = null;
         var _loc7_:§0!K§ = null;
         var _loc11_:b2Transform = null;
         var _loc16_:b2Controller = null;
         var _loc17_:b2Contact = null;
         var _loc18_:b2Fixture = null;
         var _loc19_:b2Fixture = null;
         var _loc20_:b2Vec2 = null;
         var _loc21_:b2Vec2 = null;
         var _loc22_:b2AABB = null;
         if(this.§8!?§ == null)
         {
            return;
         }
         this.§8!?§.§0`§.graphics.clear();
         var _loc1_:uint = this.§8!?§.§&K§();
         var _loc8_:b2Vec2 = new b2Vec2();
         var _loc9_:b2Vec2 = new b2Vec2();
         var _loc10_:b2Vec2 = new b2Vec2();
         var _loc12_:b2AABB = new b2AABB();
         var _loc13_:b2AABB = new b2AABB();
         var _loc14_:Array = [new b2Vec2(),new b2Vec2(),new b2Vec2(),new b2Vec2()];
         var _loc15_:b2Color = new b2Color(0,0,0);
         if(_loc1_ & b2DebugDraw.§6!e§)
         {
            _loc3_ = this.§'!7§;
            while(_loc3_)
            {
               _loc11_ = _loc3_.m_xf;
               _loc4_ = _loc3_.GetFixtureList();
               while(_loc4_)
               {
                  _loc5_ = _loc4_.GetShape();
                  if(_loc3_.IsActive() == false)
                  {
                     _loc15_.Set(0.5,0.5,0.3);
                     this.§#",§(_loc5_,_loc11_,_loc15_);
                  }
                  else if(_loc3_.§"r§() == b2Body.b2_staticBody)
                  {
                     _loc15_.Set(0.5,0.9,0.5);
                     this.§#",§(_loc5_,_loc11_,_loc15_);
                  }
                  else if(_loc3_.§"r§() == b2Body.b2_kinematicBody)
                  {
                     _loc15_.Set(0.5,0.5,0.9);
                     this.§#",§(_loc5_,_loc11_,_loc15_);
                  }
                  else if(_loc3_.IsAwake() == false)
                  {
                     _loc15_.Set(0.6,0.6,0.6);
                     this.§#",§(_loc5_,_loc11_,_loc15_);
                  }
                  else
                  {
                     _loc15_.Set(0.9,0.7,0.7);
                     this.§#",§(_loc5_,_loc11_,_loc15_);
                  }
                  _loc4_ = _loc4_.§%"@§;
               }
               _loc3_ = _loc3_.§%"@§;
            }
         }
         if(_loc1_ & b2DebugDraw.§]!L§)
         {
            _loc6_ = this.§3n§;
            while(_loc6_)
            {
               this.§ F§(_loc6_);
               _loc6_ = _loc6_.§%"@§;
            }
         }
         if(_loc1_ & b2DebugDraw.§<"3§)
         {
            _loc16_ = this.§+q§;
            while(_loc16_)
            {
               _loc16_.§!Y§(this.§8!?§);
               _loc16_ = _loc16_.§%"@§;
            }
         }
         if(_loc1_ & b2DebugDraw.§?!J§)
         {
            _loc15_.Set(0.3,0.9,0.9);
            _loc17_ = this.§1Y§.§>"#§;
            while(_loc17_)
            {
               _loc18_ = _loc17_.§5§();
               _loc19_ = _loc17_.§]h§();
               _loc20_ = _loc18_.§["#§().§2"F§();
               _loc21_ = _loc19_.§["#§().§2"F§();
               this.§8!?§.§ !L§(_loc20_,_loc21_,_loc15_);
               _loc17_ = _loc17_.§8!+§();
            }
         }
         if(_loc1_ & b2DebugDraw.§8!I§)
         {
            _loc7_ = this.§1Y§.§"!q§;
            _loc14_ = [new b2Vec2(),new b2Vec2(),new b2Vec2(),new b2Vec2()];
            _loc3_ = this.§'!7§;
            while(_loc3_)
            {
               if(_loc3_.IsActive() != false)
               {
                  _loc4_ = _loc3_.GetFixtureList();
                  while(_loc4_)
                  {
                     _loc22_ = _loc7_.§-Z§(_loc4_.§=!;§);
                     _loc14_[0].Set(_loc22_.§!8§.x,_loc22_.§!8§.y);
                     _loc14_[1].Set(_loc22_.§,f§.x,_loc22_.§!8§.y);
                     _loc14_[2].Set(_loc22_.§,f§.x,_loc22_.§,f§.y);
                     _loc14_[3].Set(_loc22_.§!8§.x,_loc22_.§,f§.y);
                     this.§8!?§.§98§(_loc14_,4,_loc15_);
                     _loc4_ = _loc4_.§8!+§();
                  }
               }
               _loc3_ = _loc3_.§8!+§();
            }
         }
         if(_loc1_ & b2DebugDraw.§8o§)
         {
            _loc3_ = this.§'!7§;
            while(_loc3_)
            {
               (_loc11_ = §#_§).R = _loc3_.m_xf.R;
               _loc11_.position = _loc3_.§6!j§();
               this.§8!?§.§#b§(_loc11_);
               _loc3_ = _loc3_.§%"@§;
            }
         }
      }
      
      public function §]w§(param1:Function, param2:b2AABB) : void
      {
         var broadPhase:§0!K§ = null;
         var WorldQueryWrapper:Function = null;
         var callback:Function = param1;
         var aabb:b2AABB = param2;
         WorldQueryWrapper = function(param1:*):Boolean
         {
            return callback(broadPhase.GetUserData(param1));
         };
         broadPhase = this.§1Y§.§"!q§;
         broadPhase.§const§(WorldQueryWrapper,aabb);
      }
      
      public function §&H§(param1:Function, param2:b2Shape, param3:b2Transform = null) : void
      {
         var broadPhase:§0!K§ = null;
         var WorldQueryWrapper:Function = null;
         var callback:Function = param1;
         var shape:b2Shape = param2;
         var transform:b2Transform = param3;
         WorldQueryWrapper = function(param1:*):Boolean
         {
            var _loc2_:b2Fixture = broadPhase.GetUserData(param1) as b2Fixture;
            if(b2Shape.§3§(shape,transform,_loc2_.GetShape(),_loc2_.GetBody().GetTransform()))
            {
               return callback(_loc2_);
            }
            return true;
         };
         if(transform == null)
         {
            transform = new b2Transform();
            transform.§8[§();
         }
         broadPhase = this.§1Y§.§"!q§;
         var aabb:b2AABB = new b2AABB();
         shape.ComputeAABB(aabb,transform);
         broadPhase.§const§(WorldQueryWrapper,aabb);
      }
      
      public function §8",§(param1:Function, param2:b2Vec2) : void
      {
         var broadPhase:§0!K§ = null;
         var WorldQueryWrapper:Function = null;
         var callback:Function = param1;
         var p:b2Vec2 = param2;
         WorldQueryWrapper = function(param1:*):Boolean
         {
            var _loc2_:b2Fixture = broadPhase.GetUserData(param1) as b2Fixture;
            if(_loc2_.TestPoint(p))
            {
               return callback(_loc2_);
            }
            return true;
         };
         broadPhase = this.§1Y§.§"!q§;
         var aabb:b2AABB = new b2AABB();
         aabb.§!8§.Set(p.x - b2Settings.b2_linearSlop,p.y - b2Settings.b2_linearSlop);
         aabb.§,f§.Set(p.x + b2Settings.b2_linearSlop,p.y + b2Settings.b2_linearSlop);
         broadPhase.§const§(WorldQueryWrapper,aabb);
      }
      
      public function RayCast(param1:Function, param2:b2Vec2, param3:b2Vec2) : void
      {
         var broadPhase:§0!K§ = null;
         var output:b2RayCastOutput = null;
         var RayCastWrapper:Function = null;
         var callback:Function = param1;
         var point1:b2Vec2 = param2;
         var point2:b2Vec2 = param3;
         RayCastWrapper = function(param1:b2RayCastInput, param2:*):Number
         {
            var _loc6_:Number = NaN;
            var _loc7_:b2Vec2 = null;
            var _loc3_:* = broadPhase.GetUserData(param2);
            var _loc5_:Boolean;
            var _loc4_:b2Fixture;
            if(_loc5_ = (_loc4_ = _loc3_ as b2Fixture).RayCast(output,param1))
            {
               _loc6_ = output.fraction;
               _loc7_ = new b2Vec2((1 - _loc6_) * point1.x + _loc6_ * point2.x,(1 - _loc6_) * point1.y + _loc6_ * point2.y);
               return callback(_loc4_,_loc7_,output.normal,_loc6_);
            }
            return param1.§,!§;
         };
         broadPhase = this.§1Y§.§"!q§;
         output = new b2RayCastOutput();
         var input:b2RayCastInput = new b2RayCastInput(point1,point2);
         broadPhase.RayCast(RayCastWrapper,input);
      }
      
      public function §4"4§(param1:b2Vec2, param2:b2Vec2) : b2Fixture
      {
         var result:b2Fixture = null;
         var RayCastOneWrapper:Function = null;
         var point1:b2Vec2 = param1;
         var point2:b2Vec2 = param2;
         RayCastOneWrapper = function(param1:b2Fixture, param2:b2Vec2, param3:b2Vec2, param4:Number):Number
         {
            result = param1;
            return param4;
         };
         this.RayCast(RayCastOneWrapper,point1,point2);
         return result;
      }
      
      public function §&!b§(param1:b2Vec2, param2:b2Vec2) : Vector.<b2Fixture>
      {
         var result:Vector.<b2Fixture> = null;
         var RayCastAllWrapper:Function = null;
         var point1:b2Vec2 = param1;
         var point2:b2Vec2 = param2;
         RayCastAllWrapper = function(param1:b2Fixture, param2:b2Vec2, param3:b2Vec2, param4:Number):Number
         {
            result[result.length] = param1;
            return 1;
         };
         result = new Vector.<b2Fixture>();
         this.RayCast(RayCastAllWrapper,point1,point2);
         return result;
      }
      
      public function §8F§() : b2Body
      {
         return this.§'!7§;
      }
      
      public function §1$§() : b2Joint
      {
         return this.§3n§;
      }
      
      public function §0!@§() : b2Contact
      {
         return this.§>"#§;
      }
      
      public function §8Z§() : Boolean
      {
         return (this.§`">§ & §+"F§) > 0;
      }
      
      b2internal function §#!6§(param1:b2TimeStep) : void
      {
         var _loc2_:b2Body = null;
         var _loc10_:int = 0;
         var _loc11_:int = 0;
         var _loc12_:b2Body = null;
         var _loc13_:b2ContactEdge = null;
         var _loc14_:b2JointEdge = null;
         var _loc3_:b2Controller = this.§+q§;
         while(_loc3_)
         {
            _loc3_.§4$§(param1);
            _loc3_ = _loc3_.§%"@§;
         }
         var _loc4_:b2Island;
         (_loc4_ = this.§ !N§).§`!r§(this.§]!M§,this.§3M§,this.§0!$§,null,this.§1Y§.§&!Y§,this.§<8§);
         _loc2_ = this.§'!7§;
         while(_loc2_)
         {
            _loc2_.§`">§ &= ~b2Body.§1!t§;
            _loc2_ = _loc2_.§%"@§;
         }
         var _loc5_:b2Contact = this.§>"#§;
         while(_loc5_)
         {
            _loc5_.§`">§ &= ~b2Contact.§1!t§;
            _loc5_ = _loc5_.§%"@§;
         }
         var _loc6_:b2Joint = this.§3n§;
         while(_loc6_)
         {
            _loc6_.§`3§ = false;
            _loc6_ = _loc6_.§%"@§;
         }
         var _loc7_:int = this.§]!M§;
         var _loc8_:Vector.<b2Body> = this.§2A§;
         var _loc9_:b2Body = this.§'!7§;
         while(_loc9_)
         {
            if(!(_loc9_.§`">§ & b2Body.§1!t§))
            {
               if(!(_loc9_.IsAwake() == false || _loc9_.IsActive() == false))
               {
                  if(_loc9_.§"r§() != b2Body.b2_staticBody)
                  {
                     _loc4_.§;!R§();
                     _loc10_ = 0;
                     var _loc15_:*;
                     _loc8_[_loc15_ = _loc10_++] = _loc9_;
                     _loc9_.§`">§ |= b2Body.§1!t§;
                     while(_loc10_ > 0)
                     {
                        _loc2_ = _loc8_[--_loc10_];
                        _loc4_.§53§(_loc2_);
                        if(_loc2_.IsAwake() == false)
                        {
                           _loc2_.SetAwake(true);
                        }
                        if(_loc2_.§"r§() != b2Body.b2_staticBody)
                        {
                           _loc13_ = _loc2_.§>"#§;
                           while(_loc13_)
                           {
                              if(!(_loc13_.§0q§.§`">§ & b2Contact.§1!t§))
                              {
                                 if(!(_loc13_.§0q§.§ !,§() == true || _loc13_.§0q§.§#e§() == false || _loc13_.§0q§.§[l§() == false))
                                 {
                                    _loc4_.§1!@§(_loc13_.§0q§);
                                    _loc13_.§0q§.§`">§ |= b2Contact.§1!t§;
                                    if(!((_loc12_ = _loc13_.§ `§).§`">§ & b2Body.§1!t§))
                                    {
                                       var _loc16_:*;
                                       _loc8_[_loc16_ = _loc10_++] = _loc12_;
                                       _loc12_.§`">§ |= b2Body.§1!t§;
                                    }
                                 }
                              }
                              _loc13_ = _loc13_.§5"&§;
                           }
                           _loc14_ = _loc2_.§3n§;
                           while(_loc14_)
                           {
                              if(_loc14_.§;!W§.§`3§ != true)
                              {
                                 if((_loc12_ = _loc14_.§ `§).IsActive() != false)
                                 {
                                    _loc4_.§^!g§(_loc14_.§;!W§);
                                    _loc14_.§;!W§.§`3§ = true;
                                    if(!(_loc12_.§`">§ & b2Body.§1!t§))
                                    {
                                       _loc8_[_loc16_ = _loc10_++] = _loc12_;
                                       _loc12_.§`">§ |= b2Body.§1!t§;
                                    }
                                 }
                              }
                              _loc14_ = _loc14_.§5"&§;
                           }
                        }
                     }
                     _loc4_.§#!6§(param1,this.§8!m§,this.§>!L§);
                     _loc11_ = 0;
                     while(_loc11_ < _loc4_.§]!M§)
                     {
                        _loc2_ = _loc4_.§1!l§[_loc11_];
                        if(_loc2_.§"r§() == b2Body.b2_staticBody)
                        {
                           _loc2_.§`">§ &= ~b2Body.§1!t§;
                        }
                        _loc11_++;
                     }
                  }
               }
            }
            _loc9_ = _loc9_.§%"@§;
         }
         _loc11_ = 0;
         while(_loc11_ < _loc8_.length)
         {
            if(!_loc8_[_loc11_])
            {
               break;
            }
            _loc8_[_loc11_] = null;
            _loc11_++;
         }
         _loc2_ = this.§'!7§;
         while(_loc2_)
         {
            if(!(_loc2_.IsAwake() == false || _loc2_.IsActive() == false))
            {
               if(_loc2_.§"r§() != b2Body.b2_staticBody)
               {
                  _loc2_.§^!X§();
               }
            }
            _loc2_ = _loc2_.§%"@§;
         }
         this.§1Y§.§>!=§();
      }
      
      b2internal function §`!!§(param1:b2TimeStep) : void
      {
         var _loc2_:b2Body = null;
         var _loc3_:b2Fixture = null;
         var _loc4_:b2Fixture = null;
         var _loc5_:b2Body = null;
         var _loc6_:b2Body = null;
         var _loc7_:b2ContactEdge = null;
         var _loc8_:b2Joint = null;
         var _loc11_:b2Contact = null;
         var _loc12_:b2Contact = null;
         var _loc13_:Number = NaN;
         var _loc14_:b2Body = null;
         var _loc15_:int = 0;
         var _loc16_:int = 0;
         var _loc17_:b2TimeStep = null;
         var _loc18_:int = 0;
         var _loc19_:Number = NaN;
         var _loc20_:Number = NaN;
         var _loc21_:b2JointEdge = null;
         var _loc22_:b2Body = null;
         var _loc9_:b2Island;
         (_loc9_ = this.§ !N§).§`!r§(this.§]!M§,b2Settings.b2_maxTOIContactsPerIsland,b2Settings.b2_maxTOIJointsPerIsland,null,this.§1Y§.§&!Y§,this.§<8§);
         var _loc10_:Vector.<b2Body> = §`"2§;
         _loc2_ = this.§'!7§;
         while(_loc2_)
         {
            _loc2_.§`">§ &= ~b2Body.§1!t§;
            _loc2_.m_sweep.§0!O§ = 0;
            _loc2_ = _loc2_.§%"@§;
         }
         _loc11_ = this.§>"#§;
         while(_loc11_)
         {
            _loc11_.§`">§ &= ~(b2Contact.§?8§ | b2Contact.§1!t§);
            _loc11_ = _loc11_.§%"@§;
         }
         _loc8_ = this.§3n§;
         while(_loc8_)
         {
            _loc8_.§`3§ = false;
            _loc8_ = _loc8_.§%"@§;
         }
         while(true)
         {
            _loc12_ = null;
            _loc13_ = 1;
            _loc11_ = this.§>"#§;
            while(_loc11_)
            {
               if(!(_loc11_.§ !,§() == true || _loc11_.§#e§() == false || _loc11_.§^O§() == false))
               {
                  _loc19_ = 1;
                  if(_loc11_.§`">§ & b2Contact.§?8§)
                  {
                     _loc19_ = _loc11_.§`!>§;
                  }
                  else
                  {
                     _loc3_ = _loc11_.§3t§;
                     _loc4_ = _loc11_.§?!p§;
                     _loc5_ = _loc3_.m_body;
                     _loc6_ = _loc4_.m_body;
                     if((_loc5_.§"r§() != b2Body.b2_dynamicBody || _loc5_.IsAwake() == false) && (_loc6_.§"r§() != b2Body.b2_dynamicBody || _loc6_.IsAwake() == false))
                     {
                        addr636:
                        _loc11_ = _loc11_.§%"@§;
                        continue;
                     }
                     _loc20_ = _loc5_.m_sweep.§0!O§;
                     if(_loc5_.m_sweep.§0!O§ < _loc6_.m_sweep.§0!O§)
                     {
                        _loc20_ = _loc6_.m_sweep.§0!O§;
                        _loc5_.m_sweep.§61§(_loc20_);
                     }
                     else if(_loc6_.m_sweep.§0!O§ < _loc5_.m_sweep.§0!O§)
                     {
                        _loc20_ = _loc5_.m_sweep.§0!O§;
                        _loc6_.m_sweep.§61§(_loc20_);
                     }
                     _loc19_ = _loc11_.§7!M§(_loc5_.m_sweep,_loc6_.m_sweep);
                     b2Settings.b2Assert(0 <= _loc19_ && _loc19_ <= 1);
                     if(_loc19_ > 0 && _loc19_ < 1)
                     {
                        if((_loc19_ = (1 - _loc19_) * _loc20_ + _loc19_) > 1)
                        {
                           _loc19_ = 1;
                        }
                     }
                     _loc11_.§`!>§ = _loc19_;
                     _loc11_.§`">§ |= b2Contact.§?8§;
                     §§goto(addr636);
                  }
                  if(Number.MIN_VALUE < _loc19_ && _loc19_ < _loc13_)
                  {
                     _loc12_ = _loc11_;
                     _loc13_ = _loc19_;
                  }
               }
               §§goto(addr636);
            }
            if(_loc12_ == null || 1 - 100 * Number.MIN_VALUE < _loc13_)
            {
               break;
            }
            _loc3_ = _loc12_.§3t§;
            _loc4_ = _loc12_.§?!p§;
            _loc5_ = _loc3_.m_body;
            _loc6_ = _loc4_.m_body;
            §&"7§.Set(_loc5_.m_sweep);
            §3!X§.Set(_loc6_.m_sweep);
            _loc5_.§61§(_loc13_);
            _loc6_.§61§(_loc13_);
            _loc12_.§?"!§(this.§1Y§.§&!Y§);
            _loc12_.§`">§ &= ~b2Contact.§?8§;
            if(_loc12_.§ !,§() == true || _loc12_.§#e§() == false)
            {
               _loc5_.m_sweep.Set(§&"7§);
               _loc6_.m_sweep.Set(§3!X§);
               _loc5_.§,6§();
               _loc6_.§,6§();
            }
            else if(_loc12_.§[l§() != false)
            {
               if((_loc14_ = _loc5_).§"r§() != b2Body.b2_dynamicBody)
               {
                  _loc14_ = _loc6_;
               }
               _loc9_.§;!R§();
               _loc15_ = 0;
               _loc16_ = 0;
               _loc10_[_loc15_ + _loc16_++] = _loc14_;
               _loc14_.§`">§ |= b2Body.§1!t§;
               while(_loc16_ > 0)
               {
                  _loc2_ = _loc10_[_loc15_++];
                  _loc16_--;
                  _loc9_.§53§(_loc2_);
                  if(_loc2_.IsAwake() == false)
                  {
                     _loc2_.SetAwake(true);
                  }
                  if(_loc2_.§"r§() == b2Body.b2_dynamicBody)
                  {
                     _loc7_ = _loc2_.§>"#§;
                     while(_loc7_)
                     {
                        if(_loc9_.§3M§ == _loc9_.§`E§)
                        {
                           break;
                        }
                        if(!(_loc7_.§0q§.§`">§ & b2Contact.§1!t§))
                        {
                           if(!(_loc7_.§0q§.§ !,§() == true || _loc7_.§0q§.§#e§() == false || _loc7_.§0q§.§[l§() == false))
                           {
                              _loc9_.§1!@§(_loc7_.§0q§);
                              _loc7_.§0q§.§`">§ |= b2Contact.§1!t§;
                              if(!((_loc22_ = _loc7_.§ `§).§`">§ & b2Body.§1!t§))
                              {
                                 if(_loc22_.§"r§() != b2Body.b2_staticBody)
                                 {
                                    _loc22_.§61§(_loc13_);
                                    _loc22_.SetAwake(true);
                                 }
                                 _loc10_[_loc15_ + _loc16_] = _loc22_;
                                 _loc16_++;
                                 _loc22_.§`">§ |= b2Body.§1!t§;
                              }
                           }
                        }
                        _loc7_ = _loc7_.§5"&§;
                     }
                     _loc21_ = _loc2_.§3n§;
                     while(_loc21_)
                     {
                        if(_loc9_.§0!$§ != _loc9_.§4c§)
                        {
                           if(_loc21_.§;!W§.§`3§ != true)
                           {
                              if((_loc22_ = _loc21_.§ `§).IsActive() != false)
                              {
                                 _loc9_.§^!g§(_loc21_.§;!W§);
                                 _loc21_.§;!W§.§`3§ = true;
                                 if(!(_loc22_.§`">§ & b2Body.§1!t§))
                                 {
                                    if(_loc22_.§"r§() != b2Body.b2_staticBody)
                                    {
                                       _loc22_.§61§(_loc13_);
                                       _loc22_.SetAwake(true);
                                    }
                                    _loc10_[_loc15_ + _loc16_] = _loc22_;
                                    _loc16_++;
                                    _loc22_.§`">§ |= b2Body.§1!t§;
                                 }
                              }
                           }
                        }
                        _loc21_ = _loc21_.§5"&§;
                     }
                  }
               }
               (_loc17_ = §`!%§).§'i§ = false;
               _loc17_.§4"?§ = (1 - _loc13_) * param1.§4"?§;
               _loc17_.§,Q§ = 1 / _loc17_.§4"?§;
               _loc17_.§ ,§ = 0;
               _loc17_.§ get§ = param1.§ get§;
               _loc17_.§7!;§ = param1.§7!;§;
               _loc9_.§`!!§(_loc17_);
               _loc18_ = 0;
               while(_loc18_ < _loc9_.§]!M§)
               {
                  _loc2_ = _loc9_.§1!l§[_loc18_];
                  _loc2_.§`">§ &= ~b2Body.§1!t§;
                  if(_loc2_.IsAwake() != false)
                  {
                     if(_loc2_.§"r§() == b2Body.b2_dynamicBody)
                     {
                        _loc2_.§^!X§();
                        _loc7_ = _loc2_.§>"#§;
                        while(_loc7_)
                        {
                           _loc7_.§0q§.§`">§ &= ~b2Contact.§?8§;
                           _loc7_ = _loc7_.§5"&§;
                        }
                     }
                  }
                  _loc18_++;
               }
               _loc18_ = 0;
               while(_loc18_ < _loc9_.§3M§)
               {
                  _loc11_ = _loc9_.§2!S§[_loc18_];
                  _loc11_.§`">§ &= ~(b2Contact.§?8§ | b2Contact.§1!t§);
                  _loc18_++;
               }
               _loc18_ = 0;
               while(_loc18_ < _loc9_.§0!$§)
               {
                  (_loc8_ = _loc9_.§ try§[_loc18_]).§`3§ = false;
                  _loc18_++;
               }
               this.§1Y§.§>!=§();
            }
         }
      }
      
      b2internal function § F§(param1:b2Joint) : void
      {
         var _loc11_:b2PulleyJoint = null;
         var _loc12_:b2Vec2 = null;
         var _loc13_:b2Vec2 = null;
         var _loc2_:b2Body = param1.§6p§();
         var _loc3_:b2Body = param1.§]%§();
         var _loc4_:b2Transform = _loc2_.m_xf;
         var _loc5_:b2Transform = _loc3_.m_xf;
         var _loc6_:b2Vec2 = _loc4_.position;
         var _loc7_:b2Vec2 = _loc5_.position;
         var _loc8_:b2Vec2 = param1.GetAnchorA();
         var _loc9_:b2Vec2 = param1.GetAnchorB();
         var _loc10_:b2Color = §0!0§;
         switch(param1.§<";§)
         {
            case b2Joint.§1!A§:
               this.§8!?§.§ !L§(_loc8_,_loc9_,_loc10_);
               break;
            case b2Joint.§3!u§:
               _loc12_ = (_loc11_ = param1 as b2PulleyJoint).§@n§();
               _loc13_ = _loc11_.§=r§();
               this.§8!?§.§ !L§(_loc12_,_loc8_,_loc10_);
               this.§8!?§.§ !L§(_loc13_,_loc9_,_loc10_);
               this.§8!?§.§ !L§(_loc12_,_loc13_,_loc10_);
               break;
            case b2Joint.§ !0§:
               this.§8!?§.§ !L§(_loc8_,_loc9_,_loc10_);
               break;
            default:
               if(_loc2_ != this.m_groundBody)
               {
                  this.§8!?§.§ !L§(_loc6_,_loc8_,_loc10_);
               }
               this.§8!?§.§ !L§(_loc8_,_loc9_,_loc10_);
               if(_loc3_ != this.m_groundBody)
               {
                  this.§8!?§.§ !L§(_loc7_,_loc9_,_loc10_);
                  break;
               }
         }
      }
      
      b2internal function §#",§(param1:b2Shape, param2:b2Transform, param3:b2Color) : void
      {
         var _loc4_:b2CircleShape = null;
         var _loc5_:b2Vec2 = null;
         var _loc6_:Number = NaN;
         var _loc7_:b2Vec2 = null;
         var _loc8_:int = 0;
         var _loc9_:b2PolygonShape = null;
         var _loc10_:int = 0;
         var _loc11_:Vector.<b2Vec2> = null;
         var _loc12_:Vector.<b2Vec2> = null;
         var _loc13_:b2EdgeShape = null;
         switch(param1.§<";§)
         {
            case b2Shape.§'!U§:
               _loc4_ = param1 as b2CircleShape;
               _loc5_ = b2Math.§2C§(param2,_loc4_.§ m§);
               _loc6_ = _loc4_.§%I§;
               _loc7_ = param2.R.col1;
               this.§8!?§.§3B§(_loc5_,_loc6_,_loc7_,param3);
               break;
            case b2Shape.§@!#§:
               _loc10_ = (_loc9_ = param1 as b2PolygonShape).§^x§();
               _loc11_ = _loc9_.§9y§();
               _loc12_ = new Vector.<b2Vec2>(_loc10_);
               _loc8_ = 0;
               while(_loc8_ < _loc10_)
               {
                  _loc12_[_loc8_] = b2Math.§2C§(param2,_loc11_[_loc8_]);
                  _loc8_++;
               }
               this.§8!?§.§-"!§(_loc12_,_loc10_,param3);
               break;
            case b2Shape.§`M§:
               _loc13_ = param1 as b2EdgeShape;
               this.§8!?§.§ !L§(b2Math.§2C§(param2,_loc13_.GetVertex1()),b2Math.§2C§(param2,_loc13_.GetVertex2()),param3);
         }
      }
   }
}
