package starling.events
{
   import §]!6§.DisplayObject;
   import §]!6§.DisplayObjectContainer;
   
   public class TouchEvent extends Event
   {
      
      public static const §0!;§:String = "touch";
       
      
      private var §]L§:Vector.<§[9§>;
      
      private var § !7§:Boolean;
      
      private var §8!H§:Boolean;
      
      private var §%E§:Number;
      
      public function TouchEvent(param1:String, param2:Vector.<§[9§>, param3:Boolean = false, param4:Boolean = false, param5:Boolean = true)
      {
         super(param1,param5);
         this.§]L§ = !!param2 ? param2 : new Vector.<§[9§>(0);
         this.§ !7§ = param3;
         this.§8!H§ = param4;
         this.§%E§ = -1;
         var _loc6_:int = param2.length;
         var _loc7_:int = 0;
         while(_loc7_ < _loc6_)
         {
            if(param2[_loc7_].timestamp > this.§%E§)
            {
               this.§%E§ = param2[_loc7_].timestamp;
            }
            _loc7_++;
         }
      }
      
      public function § !"§(param1:DisplayObject, param2:String = null) : Vector.<§[9§>
      {
         var _loc6_:§[9§ = null;
         var _loc7_:Boolean = false;
         var _loc8_:Boolean = false;
         var _loc3_:Vector.<§[9§> = new Vector.<§[9§>(0);
         var _loc4_:int = this.§]L§.length;
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            _loc7_ = (_loc6_ = this.§]L§[_loc5_]).target == param1 || param1 is DisplayObjectContainer && (param1 as DisplayObjectContainer).contains(_loc6_.target);
            _loc8_ = param2 == null || param2 == _loc6_.phase;
            if(_loc7_ && _loc8_)
            {
               _loc3_.push(_loc6_);
            }
            _loc5_++;
         }
         return _loc3_;
      }
      
      public function §]!D§(param1:DisplayObject, param2:String = null) : §[9§
      {
         var _loc3_:Vector.<§[9§> = this.§ !"§(param1,param2);
         if(_loc3_.length > 0)
         {
            return _loc3_[0];
         }
         return null;
      }
      
      public function §]2§(param1:DisplayObject) : Boolean
      {
         var _loc2_:Vector.<§[9§> = null;
         var _loc3_:int = 0;
         if(this.§]!D§(param1) == null)
         {
            return false;
         }
         _loc2_ = this.§ !"§(param1);
         _loc3_ = _loc2_.length - 1;
         while(_loc3_ >= 0)
         {
            if(_loc2_[_loc3_].phase != §?c§.§,F§)
            {
               return true;
            }
            _loc3_--;
         }
         return false;
      }
      
      public function get timestamp() : Number
      {
         return this.§%E§;
      }
      
      public function get §0!0§() : Vector.<§[9§>
      {
         return this.§]L§.concat();
      }
      
      public function get shiftKey() : Boolean
      {
         return this.§ !7§;
      }
      
      public function get ctrlKey() : Boolean
      {
         return this.§8!H§;
      }
   }
}
