package §1g§
{
   import §[V§.§80§;
   import flash.utils.*;
   
   public class §3[§
   {
      
      public static const §2!0§:int = 0;
      
      public static const §<A§:int = 1;
      
      public static const COMPRESSION_REDUCED_1:int = 2;
      
      public static const COMPRESSION_REDUCED_2:int = 3;
      
      public static const COMPRESSION_REDUCED_3:int = 4;
      
      public static const COMPRESSION_REDUCED_4:int = 5;
      
      public static const §`'§:int = 6;
      
      public static const §62§:int = 7;
      
      public static const §#b§:int = 8;
      
      public static const §>!<§:int = 9;
      
      public static const §class§:int = 10;
      
      protected static var § K§:Boolean = describeType(ByteArray).factory.method.(@name == "uncompress").parameter.length() > 0;
      
      protected static var §;`§:Boolean = describeType(ByteArray).factory.method.(@name == "inflate").length() > 0;
       
      
      protected var §8e§:int = 0;
      
      protected var §'!@§:String = "2.0";
      
      protected var §5V§:int = 8;
      
      protected var §^!+§:Boolean = false;
      
      protected var §#!;§:int = -1;
      
      protected var §&4§:int = -1;
      
      protected var §2%§:int = -1;
      
      protected var §[S§:Boolean = false;
      
      protected var §-N§:Boolean = false;
      
      protected var §<i§:Date;
      
      protected var §>!@§:uint;
      
      protected var §2!>§:Boolean = false;
      
      protected var §;J§:uint = 0;
      
      protected var §!x§:uint = 0;
      
      protected var §=`§:String = "";
      
      protected var §=!1§:String;
      
      protected var §[W§:Dictionary;
      
      protected var §9j§:String = "";
      
      protected var §["§:ByteArray;
      
      var §;S§:uint;
      
      var §=U§:uint = 0;
      
      var §5U§:uint = 0;
      
      protected var §^?§:Boolean = false;
      
      protected var §^;§:Function;
      
      public function §3[§(param1:String = "utf-8")
      {
         this.§^;§ = this.§,!G§;
         super();
         this.§=!1§ = param1;
         this.§[W§ = new Dictionary();
         this.§["§ = new ByteArray();
         this.§["§.endian = Endian.BIG_ENDIAN;
      }
      
      public function get §!!"§() : Date
      {
         return this.§<i§;
      }
      
      public function set §!!"§(param1:Date) : void
      {
         this.§<i§ = param1 != null ? param1 : new Date();
      }
      
      public function get §<_§() : String
      {
         return this.§=`§;
      }
      
      public function set §<_§(param1:String) : void
      {
         this.§=`§ = param1;
      }
      
      function get §'r§() : Boolean
      {
         return this.§[S§;
      }
      
      public function get content() : ByteArray
      {
         if(this.§^?§)
         {
            this.uncompress();
         }
         return this.§["§;
      }
      
      public function set content(param1:ByteArray) : void
      {
         this.§8Y§(param1);
      }
      
      public function §8Y§(param1:ByteArray, param2:Boolean = true) : void
      {
         if(param1 != null && param1.length > 0)
         {
            param1.position = 0;
            param1.readBytes(this.§["§,0,param1.length);
            this.§;S§ = §80§.§<2§(this.§["§);
            this.§2!>§ = false;
         }
         else
         {
            this.§["§.length = 0;
            this.§["§.position = 0;
            this.§^?§ = false;
         }
         if(param2)
         {
            this.compress();
         }
         else
         {
            this.§5U§ = this.§=U§ = this.§["§.length;
         }
      }
      
      public function get §8,§() : String
      {
         return this.§'!@§;
      }
      
      public function get §2T§() : uint
      {
         return this.§=U§;
      }
      
      public function get §4Y§() : uint
      {
         return this.§5U§;
      }
      
      public function §[a§(param1:Boolean = true, param2:String = "utf-8") : String
      {
         var _loc3_:String = null;
         if(this.§^?§)
         {
            this.uncompress();
         }
         this.§["§.position = 0;
         if(param2 == "utf-8")
         {
            _loc3_ = this.§["§.readUTFBytes(this.§["§.bytesAvailable);
         }
         else
         {
            _loc3_ = this.§["§.readMultiByte(this.§["§.bytesAvailable,param2);
         }
         this.§["§.position = 0;
         if(param1)
         {
            this.compress();
         }
         return _loc3_;
      }
      
      public function §5Y§(param1:String, param2:String = "utf-8", param3:Boolean = true) : void
      {
         this.§["§.length = 0;
         this.§["§.position = 0;
         this.§^?§ = false;
         if(param1 != null && param1.length > 0)
         {
            if(param2 == "utf-8")
            {
               this.§["§.writeUTFBytes(param1);
            }
            else
            {
               this.§["§.writeMultiByte(param1,param2);
            }
            this.§;S§ = §80§.§<2§(this.§["§);
            this.§2!>§ = false;
         }
         if(param3)
         {
            this.compress();
         }
         else
         {
            this.§5U§ = this.§=U§ = this.§["§.length;
         }
      }
      
      public function §&b§(param1:IDataOutput, param2:Boolean, param3:Boolean = false, param4:uint = 0) : uint
      {
         var _loc10_:* = null;
         var _loc15_:ByteArray = null;
         var _loc16_:Boolean = false;
         if(param1 == null)
         {
            return 0;
         }
         if(param3)
         {
            param1.writeUnsignedInt(§]R§.§;k§);
            param1.writeShort(this.§8e§ << 8 | 20);
         }
         else
         {
            param1.writeUnsignedInt(§]R§.§09§);
         }
         param1.writeShort(this.§8e§ << 8 | 20);
         param1.writeShort(this.§=!1§ == "utf-8" ? 2048 : 0);
         param1.writeShort(!!this.§^?§ ? int(§#b§) : int(§2!0§));
         var _loc5_:Date = this.§<i§ != null ? this.§<i§ : new Date();
         var _loc6_:uint = uint(_loc5_.getSeconds()) | uint(_loc5_.getMinutes()) << 5 | uint(_loc5_.getHours()) << 11;
         var _loc7_:uint = uint(_loc5_.getDate()) | uint(_loc5_.getMonth() + 1) << 5 | uint(_loc5_.getFullYear() - 1980) << 9;
         param1.writeShort(_loc6_);
         param1.writeShort(_loc7_);
         param1.writeUnsignedInt(this.§;S§);
         param1.writeUnsignedInt(this.§=U§);
         param1.writeUnsignedInt(this.§5U§);
         var _loc8_:ByteArray;
         (_loc8_ = new ByteArray()).endian = Endian.LITTLE_ENDIAN;
         if(this.§=!1§ == "utf-8")
         {
            _loc8_.writeUTFBytes(this.§=`§);
         }
         else
         {
            _loc8_.writeMultiByte(this.§=`§,this.§=!1§);
         }
         var _loc9_:uint = _loc8_.position;
         for(_loc10_ in this.§[W§)
         {
            if((_loc15_ = this.§[W§[_loc10_] as ByteArray) != null)
            {
               _loc8_.writeShort(uint(_loc10_));
               _loc8_.writeShort(uint(_loc15_.length));
               _loc8_.writeBytes(_loc15_);
            }
         }
         if(param2)
         {
            if(!this.§2!>§)
            {
               if(_loc16_ = this.§^?§)
               {
                  this.uncompress();
               }
               this.§>!@§ = §80§.§8Z§(this.§["§,0,this.§["§.length);
               this.§2!>§ = true;
               if(_loc16_)
               {
                  this.compress();
               }
            }
            _loc8_.writeShort(56026);
            _loc8_.writeShort(4);
            _loc8_.writeUnsignedInt(this.§>!@§);
         }
         var _loc11_:uint = _loc8_.position - _loc9_;
         if(param3 && this.§9j§.length > 0)
         {
            if(this.§=!1§ == "utf-8")
            {
               _loc8_.writeUTFBytes(this.§9j§);
            }
            else
            {
               _loc8_.writeMultiByte(this.§9j§,this.§=!1§);
            }
         }
         var _loc12_:uint = _loc8_.position - _loc9_ - _loc11_;
         param1.writeShort(_loc9_);
         param1.writeShort(_loc11_);
         if(param3)
         {
            param1.writeShort(_loc12_);
            param1.writeShort(0);
            param1.writeShort(0);
            param1.writeUnsignedInt(0);
            param1.writeUnsignedInt(param4);
         }
         if(_loc9_ + _loc11_ + _loc12_ > 0)
         {
            param1.writeBytes(_loc8_);
         }
         var _loc13_:uint = 0;
         if(!param3 && this.§["§.length > 0)
         {
            if(this.§^?§)
            {
               if(§ K§ || §;`§)
               {
                  _loc13_ = this.§["§.length;
                  param1.writeBytes(this.§["§,0,_loc13_);
               }
               else
               {
                  _loc13_ = this.§["§.length - 6;
                  param1.writeBytes(this.§["§,2,_loc13_);
               }
            }
            else
            {
               _loc13_ = this.§["§.length;
               param1.writeBytes(this.§["§,0,_loc13_);
            }
         }
         var _loc14_:uint = 30 + _loc9_ + _loc11_ + _loc12_ + _loc13_;
         if(param3)
         {
            _loc14_ += 16;
         }
         return _loc14_;
      }
      
      function parse(param1:IDataInput) : Boolean
      {
         while(param1.bytesAvailable && this.§^;§(param1))
         {
         }
         return this.§^;§ === this.§6!3§;
      }
      
      protected function §6!3§(param1:IDataInput) : Boolean
      {
         return false;
      }
      
      protected function §,!G§(param1:IDataInput) : Boolean
      {
         if(param1.bytesAvailable >= 30)
         {
            this.§%!F§(param1);
            if(this.§;J§ + this.§!x§ > 0)
            {
               this.§^;§ = this.§3D§;
            }
            else
            {
               this.§^;§ = this.§,6§;
            }
            return true;
         }
         return false;
      }
      
      protected function §3D§(param1:IDataInput) : Boolean
      {
         if(param1.bytesAvailable >= this.§;J§ + this.§!x§)
         {
            this.§6!;§(param1);
            this.§^;§ = this.§,6§;
            return true;
         }
         return false;
      }
      
      protected function §,6§(param1:IDataInput) : Boolean
      {
         var _loc2_:Boolean = true;
         if(this.§[S§)
         {
            this.§^;§ = this.§6!3§;
            _loc2_ = false;
         }
         else if(this.§=U§ == 0)
         {
            this.§^;§ = this.§6!3§;
         }
         else if(param1.bytesAvailable >= this.§=U§)
         {
            this.§,m§(param1);
            this.§^;§ = this.§6!3§;
         }
         else
         {
            _loc2_ = false;
         }
         return _loc2_;
      }
      
      protected function §%!F§(param1:IDataInput) : void
      {
         var _loc2_:uint = param1.readUnsignedShort();
         this.§8e§ = _loc2_ >> 8;
         this.§'!@§ = Math.floor((_loc2_ & 255) / 10) + "." + (_loc2_ & 255) % 10;
         var _loc3_:uint = param1.readUnsignedShort();
         this.§5V§ = param1.readUnsignedShort();
         this.§^!+§ = (_loc3_ & 1) !== 0;
         this.§[S§ = (_loc3_ & 8) !== 0;
         this.§-N§ = (_loc3_ & 32) !== 0;
         if((_loc3_ & 800) !== 0)
         {
            this.§=!1§ = "utf-8";
         }
         if(this.§5V§ === §`'§)
         {
            this.§#!;§ = (_loc3_ & 2) !== 0 ? 8192 : 4096;
            this.§&4§ = (_loc3_ & 4) !== 0 ? 3 : 2;
         }
         else if(this.§5V§ === §#b§)
         {
            this.§2%§ = (_loc3_ & 6) >> 1;
         }
         var _loc4_:uint = param1.readUnsignedShort();
         var _loc5_:uint = param1.readUnsignedShort();
         var _loc6_:* = _loc4_ & 31;
         var _loc7_:* = (_loc4_ & 2016) >> 5;
         var _loc8_:* = (_loc4_ & 63488) >> 11;
         var _loc9_:* = _loc5_ & 31;
         var _loc10_:* = (_loc5_ & 480) >> 5;
         var _loc11_:int = ((_loc5_ & 65024) >> 9) + 1980;
         this.§<i§ = new Date(_loc11_,_loc10_ - 1,_loc9_,_loc8_,_loc7_,_loc6_,0);
         this.§;S§ = param1.readUnsignedInt();
         this.§=U§ = param1.readUnsignedInt();
         this.§5U§ = param1.readUnsignedInt();
         this.§;J§ = param1.readUnsignedShort();
         this.§!x§ = param1.readUnsignedShort();
      }
      
      protected function §6!;§(param1:IDataInput) : void
      {
         var _loc3_:uint = 0;
         var _loc4_:uint = 0;
         var _loc5_:ByteArray = null;
         if(this.§=!1§ == "utf-8")
         {
            this.§=`§ = param1.readUTFBytes(this.§;J§);
         }
         else
         {
            this.§=`§ = param1.readMultiByte(this.§;J§,this.§=!1§);
         }
         var _loc2_:uint = this.§!x§;
         while(_loc2_ > 4)
         {
            _loc3_ = param1.readUnsignedShort();
            if((_loc4_ = param1.readUnsignedShort()) > _loc2_)
            {
               throw new Error("Parse error in file " + this.§=`§ + ": Extra field data size too big.");
            }
            if(_loc3_ === 56026 && _loc4_ === 4)
            {
               this.§>!@§ = param1.readUnsignedInt();
               this.§2!>§ = true;
            }
            else if(_loc4_ > 0)
            {
               _loc5_ = new ByteArray();
               param1.readBytes(_loc5_,0,_loc4_);
               this.§[W§[_loc3_] = _loc5_;
            }
            _loc2_ -= _loc4_ + 4;
         }
         if(_loc2_ > 0)
         {
            param1.readBytes(new ByteArray(),0,_loc2_);
         }
      }
      
      function §,m§(param1:IDataInput) : void
      {
         var _loc2_:uint = 0;
         if(this.§5V§ === §#b§ && !this.§^!+§)
         {
            if(§ K§ || §;`§)
            {
               param1.readBytes(this.§["§,0,this.§=U§);
            }
            else
            {
               if(!this.§2!>§)
               {
                  throw new Error("Adler32 checksum not found.");
               }
               this.§["§.writeByte(120);
               _loc2_ = ~this.§2%§ << 6 & 192;
               _loc2_ += 31 - (120 << 8 | _loc2_) % 31;
               this.§["§.writeByte(_loc2_);
               param1.readBytes(this.§["§,2,this.§=U§);
               this.§["§.position = this.§["§.length;
               this.§["§.writeUnsignedInt(this.§>!@§);
            }
            this.§^?§ = true;
         }
         else
         {
            if(this.§5V§ != §2!0§)
            {
               throw new Error("Compression method " + this.§5V§ + " is not supported.");
            }
            param1.readBytes(this.§["§,0,this.§=U§);
            this.§^?§ = false;
         }
         this.§["§.position = 0;
      }
      
      protected function compress() : void
      {
         if(!this.§^?§)
         {
            if(this.§["§.length > 0)
            {
               this.§["§.position = 0;
               this.§5U§ = this.§["§.length;
               if(§;`§)
               {
                  this.§["§.deflate();
                  this.§=U§ = this.§["§.length;
               }
               else if(§ K§)
               {
                  this.§["§.compress.apply(this.§["§,["deflate"]);
                  this.§=U§ = this.§["§.length;
               }
               else
               {
                  this.§["§.compress();
                  this.§=U§ = this.§["§.length - 6;
               }
               this.§["§.position = 0;
               this.§^?§ = true;
            }
            else
            {
               this.§=U§ = 0;
               this.§5U§ = 0;
            }
         }
      }
      
      protected function uncompress() : void
      {
         if(this.§^?§ && this.§["§.length > 0)
         {
            this.§["§.position = 0;
            if(§;`§)
            {
               this.§["§.inflate();
            }
            else if(§ K§)
            {
               this.§["§.uncompress.apply(this.§["§,["deflate"]);
            }
            else
            {
               this.§["§.uncompress();
            }
            this.§["§.position = 0;
            this.§^?§ = false;
         }
      }
      
      public function toString() : String
      {
         return "[FZipFile]" + "\n  name:" + this.§=`§ + "\n  date:" + this.§<i§ + "\n  sizeCompressed:" + this.§=U§ + "\n  sizeUncompressed:" + this.§5U§ + "\n  versionHost:" + this.§8e§ + "\n  versionNumber:" + this.§'!@§ + "\n  compressionMethod:" + this.§5V§ + "\n  encrypted:" + this.§^!+§ + "\n  hasDataDescriptor:" + this.§[S§ + "\n  hasCompressedPatchedData:" + this.§-N§ + "\n  filenameEncoding:" + this.§=!1§ + "\n  crc32:" + this.§;S§.toString(16) + "\n  adler32:" + this.§>!@§.toString(16);
      }
   }
}
