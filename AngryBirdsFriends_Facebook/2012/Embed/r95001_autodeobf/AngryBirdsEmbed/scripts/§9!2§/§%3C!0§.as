package §9!2§
{
   import §"=§.§@!#§;
   import §%1§.AGALMiniAssembler;
   import §2!+§.§5!§;
   import §<-§.§-t§;
   import §<-§.Texture;
   import §<k§.§!1§;
   import §<k§.§"!=§;
   import §<k§.DisplayObject;
   import §<k§.DisplayObjectContainer;
   import flash.display3D.Context3D;
   import flash.display3D.Context3DProgramType;
   import flash.display3D.Context3DVertexBufferFormat;
   import flash.display3D.IndexBuffer3D;
   import flash.display3D.VertexBuffer3D;
   import flash.geom.Matrix3D;
   import flash.utils.getQualifiedClassName;
   
   public class §<!0§
   {
      
      private static var §;'§:Vector.<Number> = new <Number>[1,1,1,1];
       
      
      private var §#&§:int;
      
      private var §>J§:Texture;
      
      private var §>z§:String;
      
      private var §0!<§:§5!§;
      
      private var §+#§:VertexBuffer3D;
      
      private var §1!C§:VertexBuffer3D;
      
      private var §="§:VertexBuffer3D;
      
      private var §<5§:Vector.<uint>;
      
      private var §7d§:IndexBuffer3D;
      
      private var § else§:Boolean = false;
      
      private var §5!6§:Boolean = false;
      
      public function §<!0§()
      {
         super();
         § J§();
         this.§0!<§ = new §5!§(0,true);
         this.§<5§ = new Vector.<uint>(0);
         this.§#&§ = 0;
         this.§22§();
      }
      
      public static function §6h§(param1:DisplayObjectContainer, param2:Vector.<§<!0§>) : void
      {
         §4q§(param1,param2,-1,new Matrix3D());
      }
      
      private static function §4q§(param1:DisplayObject, param2:Vector.<§<!0§>, param3:int, param4:Matrix3D, param5:Number = 1.0) : int
      {
         var _loc6_:int = 0;
         var _loc8_:DisplayObjectContainer = null;
         var _loc9_:int = 0;
         var _loc10_:Matrix3D = null;
         var _loc11_:DisplayObject = null;
         var _loc12_:§!1§ = null;
         var _loc13_:§"!=§ = null;
         var _loc14_:Texture = null;
         var _loc15_:String = null;
         var _loc16_:§<!0§ = null;
         var _loc7_:Boolean = false;
         if(param3 == -1)
         {
            _loc7_ = true;
            param3 = 0;
            if(param2.length == 0)
            {
               param2.push(new §<!0§());
            }
            else
            {
               param2[0].reset();
            }
         }
         else if(param1.alpha == 0 || !param1.visible)
         {
            return param3;
         }
         if(param1 is DisplayObjectContainer)
         {
            _loc9_ = (_loc8_ = param1 as DisplayObjectContainer).numChildren;
            _loc10_ = new Matrix3D();
            param5 *= param1.alpha;
            _loc6_ = 0;
            while(_loc6_ < _loc9_)
            {
               _loc11_ = _loc8_.getChildAt(_loc6_);
               _loc10_.copyFrom(param4);
               §>P§.§![§(_loc10_,_loc11_);
               param3 = §4q§(_loc11_,param2,param3,_loc10_,param5);
               _loc6_++;
            }
         }
         else
         {
            if(!(param1 is §!1§))
            {
               throw new Error("Unsupported display object: " + getQualifiedClassName(param1));
            }
            _loc14_ = !!(_loc13_ = (_loc12_ = param1 as §!1§) as §"!=§) ? _loc13_.texture : null;
            _loc15_ = !!_loc13_ ? _loc13_.smoothing : null;
            if((_loc16_ = param2[param3]).§`!0§(_loc12_,_loc14_,_loc15_))
            {
               param3++;
               if(param2.length <= param3)
               {
                  param2.push(new §<!0§());
               }
               (_loc16_ = param2[param3]).reset();
            }
            _loc16_.§<§(_loc12_,param5,_loc14_,_loc15_,param4);
         }
         if(_loc7_)
         {
            _loc6_ = param2.length - 1;
            while(_loc6_ > param3)
            {
               param2[_loc6_].dispose();
               delete param2[_loc6_];
               _loc6_--;
            }
         }
         return param3;
      }
      
      private static function § J§() : void
      {
         var _loc4_:* = null;
         var _loc5_:String = null;
         var _loc6_:Boolean = false;
         var _loc7_:Boolean = false;
         var _loc8_:Array = null;
         var _loc9_:Boolean = false;
         var _loc10_:Boolean = false;
         var _loc11_:String = null;
         var _loc12_:Array = null;
         var _loc1_:§7N§ = §7N§.§17§;
         if(_loc1_.§<@§(§6w§(true)))
         {
            return;
         }
         var _loc2_:AGALMiniAssembler = new AGALMiniAssembler();
         var _loc3_:AGALMiniAssembler = new AGALMiniAssembler();
         for each(_loc6_ in [true,false])
         {
            _loc4_ = "m44 op, va0, vc0  \n" + "mov v0, va2       \n";
            _loc5_ = !!_loc6_ ? "mul ft0, v0, fc0  \n" + "mov oc, ft0       \n" : "mov oc, v0        \n";
            _loc2_.§;!@§(Context3DProgramType.VERTEX,_loc4_);
            _loc3_.§;!@§(Context3DProgramType.FRAGMENT,_loc5_);
            _loc1_.§#"§(§6w§(_loc6_),_loc2_.§]G§,_loc3_.§]G§);
            for each(_loc7_ in [true,false])
            {
               _loc4_ = !!_loc7_ ? "m44 op, va0, vc0  \n" + "mov v0, va1 \n" + "mov v1, va2       \n" : "m44 op, va0, vc0  \n" + "mov v0, va1 \n";
               if(_loc7_)
               {
                  _loc5_ = !!_loc6_ ? "tex ft1, v0, fs0 <???>  \n" + "mul ft1, ft1, v1        \n" + "mul oc, ft1, fc0        \n" : "tex ft1, v0, fs0 <???>  \n" + "mul ft1, ft1, v1        \n" + "mov oc, ft1 \n";
               }
               else
               {
                  _loc5_ = !!_loc6_ ? "tex ft1, v0, fs0 <???>  \n" + "mul oc, ft1, fc0        \n" : "tex ft1, v0, fs0 <???>  \n" + "mov oc, ft1 \n";
               }
               _loc2_.§;!@§(Context3DProgramType.VERTEX,_loc4_);
               _loc8_ = [§-t§.NONE,§-t§.§1!=§,§-t§.§&!7§];
               for each(_loc9_ in [true,false])
               {
                  for each(_loc10_ in [true,false])
                  {
                     for each(_loc11_ in _loc8_)
                     {
                        _loc12_ = ["2d",!!_loc9_ ? "repeat" : "clamp"];
                        if(_loc11_ == §-t§.NONE)
                        {
                           _loc12_.push("nearest",!!_loc10_ ? "mipnearest" : "mipnone");
                        }
                        else if(_loc11_ == §-t§.§1!=§)
                        {
                           _loc12_.push("linear",!!_loc10_ ? "mipnearest" : "mipnone");
                        }
                        else
                        {
                           _loc12_.push("linear",!!_loc10_ ? "miplinear" : "mipnone");
                        }
                        _loc3_.§;!@§(Context3DProgramType.FRAGMENT,_loc5_.replace("???",_loc12_.join()));
                        _loc1_.§#"§(§%§(_loc6_,_loc10_,_loc9_,_loc11_,_loc7_),_loc2_.§]G§,_loc3_.§]G§);
                     }
                  }
               }
            }
         }
      }
      
      public static function §6w§(param1:Boolean) : String
      {
         return !!param1 ? "QB_q*" : "QB_q\'";
      }
      
      public static function §%§(param1:Boolean, param2:Boolean = true, param3:Boolean = false, param4:String = "bilinear", param5:Boolean = false) : String
      {
         var _loc6_:* = !!param1 ? "QB_i*" : "QB_i\'";
         if(!param2)
         {
            _loc6_ += "N";
         }
         if(param3)
         {
            _loc6_ += "R";
         }
         if(param5)
         {
            _loc6_ += "C";
         }
         if(param4 != §-t§.§1!=§)
         {
            _loc6_ += param4.charAt(0);
         }
         return _loc6_;
      }
      
      public function get §9!6§() : int
      {
         return this.§#&§;
      }
      
      public function dispose() : void
      {
         if(this.§+#§)
         {
            this.§+#§.dispose();
         }
         if(this.§1!C§)
         {
            this.§1!C§.dispose();
         }
         if(this.§="§)
         {
            this.§="§.dispose();
         }
         if(this.§7d§)
         {
            this.§7d§.dispose();
         }
      }
      
      private function §22§() : void
      {
         var _loc1_:int = this.§0!<§.§;"§ / 4;
         var _loc2_:int = _loc1_ == 0 ? 16 : int(_loc1_ * 2);
         this.§0!<§.§;"§ = _loc2_ * 4;
         var _loc3_:int = _loc1_;
         while(_loc3_ < _loc2_)
         {
            this.§<5§[int(_loc3_ * 6)] = _loc3_ * 4;
            this.§<5§[int(_loc3_ * 6 + 1)] = _loc3_ * 4 + 1;
            this.§<5§[int(_loc3_ * 6 + 2)] = _loc3_ * 4 + 2;
            this.§<5§[int(_loc3_ * 6 + 3)] = _loc3_ * 4 + 1;
            this.§<5§[int(_loc3_ * 6 + 4)] = _loc3_ * 4 + 3;
            this.§<5§[int(_loc3_ * 6 + 5)] = _loc3_ * 4 + 2;
            _loc3_++;
         }
         this.§5!6§ = true;
      }
      
      private function §!!5§(param1:Context3D) : Boolean
      {
         if(!this.§5!6§)
         {
            return false;
         }
         if(param1 == null)
         {
            throw new §@!#§();
         }
         if(this.§+#§)
         {
            this.§+#§.dispose();
         }
         if(this.§1!C§)
         {
            this.§1!C§.dispose();
         }
         if(this.§="§)
         {
            this.§="§.dispose();
         }
         if(this.§7d§)
         {
            this.§7d§.dispose();
         }
         var _loc2_:int = this.§0!<§.§;"§ / 4;
         this.§+#§ = param1.createVertexBuffer(_loc2_ * 4,§5!§.§,w§);
         this.§+#§.uploadFromByteArray(this.§0!<§.§8Q§,0,0,_loc2_ * 4);
         this.§1!C§ = param1.createVertexBuffer(_loc2_ * 4,§5!§.§]!7§);
         this.§1!C§.uploadFromVector(this.§0!<§.§0_§,0,_loc2_ * 4);
         this.§="§ = param1.createVertexBuffer(_loc2_ * 4,§5!§.§5!>§);
         this.§="§.uploadFromVector(this.§0!<§.§-W§,0,_loc2_ * 4);
         this.§7d§ = param1.createIndexBuffer(_loc2_ * 6);
         this.§7d§.uploadFromVector(this.§<5§,0,_loc2_ * 6);
         this.§5!6§ = false;
         return true;
      }
      
      private function §!!3§(param1:Context3D) : void
      {
         if(!this.§!!5§(param1))
         {
            this.§+#§.uploadFromByteArray(this.§0!<§.§8Q§,0,0,this.§#&§ * 4);
            if(this.§ else§)
            {
               this.§1!C§.uploadFromVector(this.§0!<§.§0_§,0,this.§#&§ * 4);
            }
            this.§="§.uploadFromVector(this.§0!<§.§-W§,0,this.§#&§ * 4);
         }
      }
      
      public function render(param1:Context3D, param2:Matrix3D, param3:Number = 1.0) : void
      {
         if(this.§#&§ == 0)
         {
            return;
         }
         var _loc4_:Boolean = this.§0!<§.premultipliedAlpha;
         var _loc5_:* = param3 != 1;
         var _loc6_:String = !!this.§>J§ ? §%§(_loc5_,this.§>J§.mipMapping,this.§>J§.repeat,this.§>z§,this.§ else§) : §6w§(_loc5_);
         §>P§.§&1§(param1,_loc4_);
         this.§!!3§(param1);
         param1.setProgram(§7N§.§17§.§5!+§(_loc6_));
         param1.setVertexBufferAt(0,this.§="§,§5!§.§0I§,Context3DVertexBufferFormat.FLOAT_3);
         if(this.§>J§ == null || this.§ else§)
         {
            param1.setVertexBufferAt(2,this.§1!C§,§5!§.§#o§,Context3DVertexBufferFormat.FLOAT_4);
         }
         param1.setProgramConstantsFromMatrix(Context3DProgramType.VERTEX,0,param2,true);
         if(_loc5_)
         {
            §;'§[0] = §;'§[1] = §;'§[2] = !!_loc4_ ? Number(param3) : Number(1);
            §;'§[3] = param3;
            param1.setProgramConstantsFromVector(Context3DProgramType.FRAGMENT,0,§;'§,1);
         }
         if(this.§>J§)
         {
            param1.setTextureAt(0,this.§>J§.getBase(param1));
            param1.setVertexBufferAt(1,this.§+#§,§5!§.§?p§,Context3DVertexBufferFormat.FLOAT_2);
         }
         else
         {
            param1.setTextureAt(0,null);
         }
         param1.drawTriangles(this.§7d§,0,this.§#&§ * 2);
         if(this.§>J§)
         {
            param1.setTextureAt(0,null);
            param1.setVertexBufferAt(1,null);
         }
         if(this.§>J§ == null || this.§ else§)
         {
            param1.setVertexBufferAt(2,null);
         }
         param1.setVertexBufferAt(0,null);
      }
      
      public function reset() : void
      {
         this.§#&§ = 0;
         this.§ else§ = false;
         this.§>J§ = null;
         this.§>z§ = null;
      }
      
      public function §<§(param1:§!1§, param2:Number, param3:Texture, param4:String, param5:Matrix3D) : void
      {
         if(this.§#&§ + 1 > this.§0!<§.§;"§ / 4)
         {
            this.§22§();
         }
         if(this.§#&§ == 0)
         {
            this.§>J§ = param3;
            this.§>z§ = param4;
            this.§0!<§.setPremultipliedAlpha(!!param3 ? Boolean(param3.premultipliedAlpha) : true,false);
         }
         var _loc6_:int = this.§#&§ * 4;
         param2 *= param1.alpha;
         param1.copyVertexDataTo(this.§0!<§,_loc6_,param2,param5);
         if(param2 != 1 || param1.useColor)
         {
            this.§ else§ = true;
         }
         ++this.§#&§;
      }
      
      public function §`!0§(param1:§!1§, param2:Texture, param3:String) : Boolean
      {
         if(this.§>J§ != null && param2 != null)
         {
            return this.§>J§.root != param2.root || this.§>J§.repeat != param2.repeat || this.§>z§ != param3 || this.§ else§ != param1.useColor || this.§#&§ == 8192;
         }
         if(this.§>J§ == null && param2 == null)
         {
            return false;
         }
         return this.§#&§ != 0 || this.§ else§ != param1.useColor;
      }
   }
}
