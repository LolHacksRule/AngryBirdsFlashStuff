package §<k§
{
   import §2!+§.§5!§;
   import §2!+§.transformCoords;
   import §9!2§.§>P§;
   import flash.display.BitmapData;
   import flash.geom.Matrix;
   import flash.geom.Matrix3D;
   import flash.geom.Point;
   import flash.geom.Rectangle;
   import flash.geom.Vector3D;
   
   public class §!1§ extends DisplayObject
   {
      
      private static var §!!§:Vector3D = new Vector3D();
      
      private static var §7!E§:Point = new Point();
      
      private static var §;+§:Matrix = new Matrix();
       
      
      protected var §0!<§:§5!§;
      
      private var §%-§:Number = 0.0;
      
      private var §;r§:Number = 0.0;
      
      private var §%L§:uint = 16777215;
      
      public function §!1§(param1:Number, param2:Number, param3:uint = 16777215, param4:Boolean = true)
      {
         super();
         this.§0!<§ = new §5!§(4,param4);
         this.updateVertexData(param1,param2,param3,param4);
         this.§%L§ = param3;
      }
      
      public function get highQuality() : Boolean
      {
         return false;
      }
      
      protected function updateVertexData(param1:Number, param2:Number, param3:uint, param4:Boolean) : void
      {
         this.§0!<§.setPremultipliedAlpha(param4);
         this.§0!<§.setPosition(0,0,0);
         this.§0!<§.setPosition(1,param1,0);
         this.§0!<§.setPosition(2,0,param2);
         this.§0!<§.setPosition(3,param1,param2);
         this.§0!<§.§9Z§(param3);
         this.§%-§ = param1;
         this.§;r§ = param2;
      }
      
      override public function getBounds(param1:DisplayObject, param2:Rectangle = null) : Rectangle
      {
         var _loc7_:int = 0;
         if(param2 == null)
         {
            param2 = new Rectangle();
         }
         var _loc3_:Number = Number.MAX_VALUE;
         var _loc4_:Number = -Number.MAX_VALUE;
         var _loc5_:Number = Number.MAX_VALUE;
         var _loc6_:Number = -Number.MAX_VALUE;
         if(param1 == this)
         {
            _loc7_ = 0;
            while(_loc7_ < 4)
            {
               this.§0!<§.§"!?§(_loc7_,§!!§);
               _loc3_ = _loc3_ < §!!§.x ? Number(_loc3_) : Number(§!!§.x);
               _loc4_ = _loc4_ > §!!§.x ? Number(_loc4_) : Number(§!!§.x);
               _loc5_ = _loc5_ < §!!§.y ? Number(_loc5_) : Number(§!!§.y);
               _loc6_ = _loc6_ > §!!§.y ? Number(_loc6_) : Number(§!!§.y);
               _loc7_++;
            }
         }
         else
         {
            §!!?§(param1,§;+§);
            _loc7_ = 0;
            while(_loc7_ < 4)
            {
               this.§0!<§.§"!?§(_loc7_,§!!§);
               transformCoords(§;+§,§!!§.x,§!!§.y,§7!E§);
               _loc3_ = _loc3_ < §7!E§.x ? Number(_loc3_) : Number(§7!E§.x);
               _loc4_ = _loc4_ > §7!E§.x ? Number(_loc4_) : Number(§7!E§.x);
               _loc5_ = _loc5_ < §7!E§.y ? Number(_loc5_) : Number(§7!E§.y);
               _loc6_ = _loc6_ > §7!E§.y ? Number(_loc6_) : Number(§7!E§.y);
               _loc7_++;
            }
         }
         param2.x = _loc3_;
         param2.y = _loc5_;
         param2.width = _loc4_ - _loc3_;
         param2.height = _loc6_ - _loc5_;
         return param2;
      }
      
      public function §]b§(param1:int) : uint
      {
         return this.§0!<§.§-!'§(param1);
      }
      
      public function setVertexColor(param1:int, param2:uint) : void
      {
         this.§0!<§.§,1§(param1,param2);
      }
      
      public function §-n§(param1:int) : Number
      {
         return this.§0!<§.§?n§(param1);
      }
      
      public function setVertexAlpha(param1:int, param2:Number) : void
      {
         this.§0!<§.§`>§(param1,param2);
      }
      
      public function get color() : uint
      {
         return this.§%L§;
      }
      
      override public function set color(param1:uint) : void
      {
         if(this.§%L§ != param1)
         {
            this.§%L§ = param1;
            this.§0!<§.§9Z§(param1,1);
         }
      }
      
      public function copyVertexDataTo(param1:§5!§, param2:int = 0, param3:Number = 1.0, param4:Matrix3D = null) : void
      {
         this.§0!<§.copyTo(param1,param2,param3,true,param4);
      }
      
      override public function render(param1:§>P§, param2:Number) : void
      {
         param1.batchQuad(this,param2);
      }
      
      public function get useColor() : Boolean
      {
         return true;
      }
      
      public function get clippedBitmapData() : BitmapData
      {
         return null;
      }
      
      public function get initialWidth() : Number
      {
         return this.§%-§;
      }
      
      public function get initialHeight() : Number
      {
         return this.§;r§;
      }
   }
}
