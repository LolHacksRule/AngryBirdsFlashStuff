package §-'§
{
   import flash.utils.getTimer;
   
   public class §[g§ extends §in§
   {
      
      private static const §`p§:Boolean = AngryBirdsFP11.§,!%§;
      
      private static const §#g§:String = "warning";
      
      private static const §6!G§:String = "full-game-link";
      
      private static const §7z§:String = "framerate";
      
      private static const §<!3§:String = "error";
      
      public static const §"x§:String = "slingshot-used";
      
      private static const §@4§:String = "download-failed";
      
      private static const §4z§:String = "invalid-level";
      
      private static const §4n§:String = "flash-var-missing";
      
      private static const SAMPLE_1_PERCENT_FLASH_ACTIONS:Array = [];
      
      private static const SAMPLE_100_PERCENT_FLASH_ACTIONS:Array = [ACTION_APPLICATION_CRASH,§]X§,§!U§];
      
      private static const SAMPLE_1_PERCENT_CATEGORIES:Array = [];
      
      private static const SAMPLE_100_PERCENT_CATEGORIES:Array = [§<!3§];
      
      private static const SAMPLE_1_PERCENT:String = " x 100";
      
      private static const SAMPLE_10_PERCENT:String = " x 10";
      
      private static var sSample10Percent:Boolean = false;
      
      private static var sSample1Percent:Boolean = false;
      
      private static var §%!0§:Array = [];
      
      private static var §'!8§:Boolean = false;
       
      
      public function §[g§()
      {
         super();
      }
      
      public static function §34§() : void
      {
         if(Math.random() <= 0.1)
         {
            sSample10Percent = true;
         }
         if(Math.random() <= 0.01)
         {
            sSample1Percent = true;
         }
      }
      
      public static function §;!4§(param1:int, param2:int) : void
      {
         §>W§(§in§.ACTION_APPLICATION_CRASH,param1.toString(),param2);
      }
      
      public static function §>W§(param1:String, param2:String = null, param3:int = 0) : void
      {
         var _loc4_:String = null;
         if(enabled)
         {
            if((_loc4_ = §"C§(param1,SAMPLE_100_PERCENT_FLASH_ACTIONS,SAMPLE_1_PERCENT_FLASH_ACTIONS)) != null)
            {
               trackEvent(§4w§,param1 + _loc4_,param2,param3);
            }
         }
      }
      
      public static function §<9§(param1:int, param2:String, param3:Boolean) : void
      {
         var _loc5_:String = null;
         if(§%!0§.indexOf(param2) >= 0)
         {
            return;
         }
         var _loc4_:* = §%!0§.length == 0;
         §%!0§.push(param2);
         if(param3)
         {
            _loc5_ = §in§.ACTION_CPU_FPS_REPORT;
         }
         else
         {
            _loc5_ = §in§.ACTION_GPU_FPS_REPORT;
         }
         if(_loc4_)
         {
            §>W§(_loc5_,param1.toString(),param1);
         }
         §&!0§(§7z§,_loc5_,param2,param1);
      }
      
      public static function §95§(param1:String) : void
      {
         §&!0§(§#g§,§@4§,param1,0);
      }
      
      public static function §6G§(param1:String) : void
      {
         var _loc2_:int = §&2§();
         §&!0§(§6!G§,param1,_loc2_.toString(),_loc2_);
      }
      
      public static function §&!B§(param1:String) : void
      {
         if(param1 == null)
         {
            param1 = "[null]";
         }
         else if(param1.length == 0)
         {
            param1 = "[empty]";
         }
         §&!0§(§#g§,§4z§,param1,0);
      }
      
      public static function §^+§(param1:String) : void
      {
         §&!0§(§#g§,§4n§,param1,0);
      }
      
      public static function §5X§(param1:String) : void
      {
         §&!0§(§<!3§,param1,"",0,false);
      }
      
      public static function §9,§() : void
      {
         if(§'!8§)
         {
            return;
         }
         §'!8§ = true;
         var _loc1_:int = §&2§();
         §>W§(§"x§,_loc1_.toString(),_loc1_);
      }
      
      private static function §&!0§(param1:String, param2:String, param3:String, param4:int, param5:Boolean = true) : void
      {
         var _loc6_:String = null;
         if(enabled)
         {
            _loc6_ = "";
            if(param5)
            {
               _loc6_ = §"C§(param1,SAMPLE_100_PERCENT_CATEGORIES,SAMPLE_1_PERCENT_CATEGORIES);
            }
            if(_loc6_ != null)
            {
               trackEvent(param1,param2 + _loc6_,param3,param4);
            }
         }
      }
      
      private static function §"C§(param1:String, param2:Array, param3:Array) : String
      {
         if(param3.indexOf(param1) >= 0)
         {
            if(sSample1Percent)
            {
               return SAMPLE_1_PERCENT;
            }
            return null;
         }
         if(param2.indexOf(param1) < 0)
         {
            if(sSample10Percent)
            {
               return SAMPLE_10_PERCENT;
            }
            return null;
         }
         return "";
      }
      
      private static function §&2§(param1:int = 0) : int
      {
         return getTimer() / 1000 - param1;
      }
   }
}
