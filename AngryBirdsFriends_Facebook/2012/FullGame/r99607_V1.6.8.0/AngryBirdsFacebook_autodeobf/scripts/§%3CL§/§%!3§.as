package §<L§
{
   import §+"6§.§5!H§;
   import flash.display.BitmapData;
   import flash.display3D.Context3D;
   import flash.display3D.Context3DTextureFormat;
   import flash.display3D.textures.Texture;
   import flash.display3D.textures.TextureBase;
   import flash.utils.ByteArray;
   import starling.events.Event;
   
   public class §%!3§ extends §<L§.Texture
   {
       
      
      private var §?h§:TextureBase;
      
      private var §]"A§:int;
      
      private var §>!>§:int;
      
      private var §<g§:Boolean;
      
      private var §-!8§:Boolean;
      
      private var §]!%§:Boolean;
      
      private var § 9§:Object;
      
      private var §-5§:Boolean = false;
      
      public function §%!3§(param1:TextureBase, param2:int, param3:int, param4:Boolean, param5:Boolean, param6:Boolean = false)
      {
         super();
         this.§?h§ = param1;
         this.§]"A§ = param2;
         this.§>!>§ = param3;
         this.§<g§ = param4;
         this.§-!8§ = param5;
         this.§]!%§ = param6;
      }
      
      override public function dispose() : void
      {
         if(this.§?h§)
         {
            this.§?h§.dispose();
         }
         this.§1!]§(null);
         super.dispose();
      }
      
      public function §1!]§(param1:Object) : void
      {
         if(this.§ 9§ == null && param1 != null)
         {
            §5!H§.§%n§.addEventListener(Event.CONTEXT3D_CREATE,this.§0"<§);
         }
         if(param1 == null)
         {
            §5!H§.§%n§.removeEventListener(Event.CONTEXT3D_CREATE,this.§0"<§);
         }
         this.§ 9§ = param1;
      }
      
      private function §0"<§(param1:Event) : void
      {
         this.requestBaseTextureUpdate(null);
      }
      
      override public function requestBaseTextureUpdate(param1:Object) : void
      {
         this.§-5§ = true;
         if(param1 != null)
         {
            this.§1!]§(param1);
         }
      }
      
      public function get § !s§() : Boolean
      {
         return this.§]!%§;
      }
      
      override public function getBase(param1:Context3D) : TextureBase
      {
         this.§&!$§(param1);
         return this.§?h§;
      }
      
      override public function get root() : §<L§.Texture
      {
         return this;
      }
      
      private function §&!$§(param1:Context3D) : void
      {
         var _loc4_:flash.display3D.textures.Texture = null;
         var _loc5_:String = null;
         if(!this.§-5§)
         {
            return;
         }
         var _loc2_:BitmapData = this.§ 9§ as BitmapData;
         var _loc3_:ByteArray = this.§ 9§ as ByteArray;
         if(_loc2_)
         {
            _loc4_ = param1.createTexture(this.§]"A§,this.§>!>§,Context3DTextureFormat.BGRA,this.§]!%§);
            §<L§.Texture.uploadBitmapData(_loc4_,_loc2_,this.§<g§);
         }
         else if(_loc3_)
         {
            _loc5_ = _loc3_[6] == 2 ? Context3DTextureFormat.COMPRESSED : Context3DTextureFormat.BGRA;
            _loc4_ = param1.createTexture(this.§]"A§,this.§>!>§,_loc5_,this.§]!%§);
            §<L§.Texture.uploadAtfData(_loc4_,_loc3_);
         }
         this.§?h§ = _loc4_;
         this.§-5§ = false;
      }
      
      override public function get width() : Number
      {
         return this.§]"A§;
      }
      
      override public function get height() : Number
      {
         return this.§>!>§;
      }
      
      override public function get mipMapping() : Boolean
      {
         return this.§<g§;
      }
      
      override public function get premultipliedAlpha() : Boolean
      {
         return this.§-!8§;
      }
      
      override public function get bitmapData() : BitmapData
      {
         return this.§ 9§ as BitmapData;
      }
      
      override public function get clippedBitmapData() : BitmapData
      {
         return this.bitmapData;
      }
   }
}
