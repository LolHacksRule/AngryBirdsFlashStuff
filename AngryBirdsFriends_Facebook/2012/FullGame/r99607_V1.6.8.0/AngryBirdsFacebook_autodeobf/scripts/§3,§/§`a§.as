package §3,§
{
   import §'_§.§9V§;
   
   public class §`a§
   {
      
      public static const MIGHTY_EAGLE_USE_SHADE:Boolean = true;
      
      public static const MIGHTY_EAGLE_FLYING_ANGLE:Number = -22.9;
      
      public static const MIGHTY_EAGLE_FLYING_SPEED:Number = 0.128;
      
      public static const MIGHTY_EAGLE_ROTATION_SPEED:Number = 8;
      
      public static const MIGHTY_EAGLE_WAIT_TIME:Number = 2400;
      
      public static const MIGHTY_EAGLE_SOUND_DELAY:Number = 1000;
      
      public static const MIGHTY_EAGLE_SHADING_DELAY:Number = 2900;
      
      public static const MIGHTY_EAGLE_STARTING_DISTANCE_X:Number = 140;
      
      public static const MIGHTY_EAGLE_STARTING_DISTANCE_Y:Number = 140;
      
      public static const MIGHTY_EAGLE_Y_CHANGE:Number = Math.sin(§9V§.§6!h§(MIGHTY_EAGLE_FLYING_ANGLE));
      
      public static const MIGHTY_EAGLE_X_DIRECTION:Number = 1;
      
      public static const §1!x§:Number = 1500;
      
      public static const §`l§:Number = 0.7;
      
      public static const §-B§:Number = 0.2;
      
      public static const §`c§:Number = 8.8;
      
      public static const §8]§:Number = 2400;
      
      public static const §#!Y§:Number = 1;
      
      public static const §+&§:Number = 0.15;
      
      public static const §;!]§:Number = 4;
      
      public static const §3w§:Number = 60;
      
      public static const §"f§:Number = 16;
      
      public static const §#"A§:Number = 6 * §"f§;
      
      public static const SARDINE_CAN_INITIAL_ROTATION_SPEED:Number = 0.07;
      
      public static const SARDINE_CAN_ROTATION_ACCELERATION:Number = 0.0002;
      
      public static const SARDINE_CAN_MAX_ROTATION_SPEED:Number = SARDINE_CAN_INITIAL_ROTATION_SPEED * 4.4;
      
      public static const SARDINE_CAN_DELAY_AFTER_HIT:int = 3000;
      
      public static const §="2§:Number = 5;
      
      public static var § &§:int = 2;
      
      public static var §^"A§:Boolean = false;
      
      public static const §4$§:Number = 600;
      
      public static const §7!M§:Number = 1.3;
      
      public static const §?!§:Number = 1.3;
      
      public static const §9I§:Number = 1.3;
      
      public static const §-"%§:Number = 1.3;
      
      public static const §26§:Number = 1.3;
      
      public static const §=E§:Number = 1.3;
      
      public static const §?&§:Number = 1.5;
      
      public static const §05§:Number = 1.2;
      
      public static var §]W§:int = 60;
      
      public static const §>!U§:Number = 9;
      
      public static const §?E§:Number = 5;
      
      public static const §0!M§:Number = 3000;
      
      public static const §<5§:Number = 60;
      
      public static const §["$§:Number = 6;
      
      public static const §6!U§:Number = 3;
      
      public static const §;!w§:Number = 1 / 30;
      
      public static const §@!n§:int = 100;
      
      public static const §9S§:int = 3;
      
      public static const §6E§:Number = 70;
      
      public static const §%X§:Number = 10;
      
      public static const §#"6§:Number = 1000;
      
      public static const §!! §:Number = 1.3;
      
      public static const §]9§:Number = 7.5;
      
      public static const §,!C§:Number = 300;
      
      public static const §5r§:Number = 0.5;
      
      public static const static:Number = 6;
      
      public static const §;Z§:Number = 9;
      
      public static const § !=§:Number = 0.15;
      
      public static const §%"B§:Number = 10;
       
      
      public function §`a§()
      {
         super();
      }
   }
}
