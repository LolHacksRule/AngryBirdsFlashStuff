package §94§
{
   import §%#A§.§&§;
   import §%#A§.§,#w§;
   import §%#A§.§=!k§;
   import §%#A§.§]!_§;
   import §%#A§.§^!4§;
   import §&$!§.§+#!§;
   import §1#W§.§!#&§;
   import §2`§.§<"I§;
   import §6"r§.§!#A§;
   import §8#D§.§#E§;
   import §8#D§.§=>§;
   import §8Z§.§-#C§;
   import §8Z§.§6"w§;
   import §<!b§.§&!i§;
   import §<"1§.§'"S§;
   import §<"1§.§@!N§;
   import §@#§.§9##§;
   import §@#§.§^#Q§;
   import com.rovio.assets.§+$#§;
   import flash.display.MovieClip;
   import flash.events.KeyboardEvent;
   import flash.events.MouseEvent;
   import flash.ui.Keyboard;
   import flash.utils.Dictionary;
   
   public class §7!c§ extends §`"z§
   {
      
      public static const STATE_NAME:String = "LevelSelectionState";
      
      protected static const §1,§:Number = 0.5;
      
      public static var §<r§:String = "";
       
      
      protected var §6&§:Boolean = false;
      
      protected var §2#H§:§]!_§;
      
      protected var §?!W§:§]!_§;
      
      protected var §6#%§:MovieClip;
      
      protected var §'#-§:Array;
      
      protected var §"$'§:Array;
      
      protected var §>"H§:int = 0;
      
      protected var §1!#§:int = 0;
      
      protected var §%#e§:int = 0;
      
      protected var §]"L§:Boolean = false;
      
      protected var §-!Z§:§-#C§ = null;
      
      protected var §4g§:Dictionary;
      
      protected var §3!N§:Array;
      
      protected var §^$ §:Array;
      
      protected var §`#Y§:§&!i§;
      
      protected var §@$+§:Number = 0;
      
      public function §7!c§(param1:§^#Q§, param2:§+#!§, param3:Boolean = false, param4:String = "LevelSelectionState")
      {
         super(param1,param3,param4,param2);
      }
      
      override protected function init() : void
      {
         super.init();
         this.initView();
         while(this.§?!W§.mClip.numChildren > 0)
         {
            this.§?!W§.mClip.removeChildAt(0);
         }
         this.§"$'§ = [];
         this.§'#-§ = [];
      }
      
      protected function initView() : void
      {
         §?Q§ = new §#E§(this);
         §?Q§.init(§=>§.§%" §.Views.View_LevelSelection[0]);
         this.§?!W§ = §?Q§.getItemByName("Container_LevelRepeaters") as §]!_§;
         this.§2#H§ = §?Q§.getItemByName("Container_LevelSelection") as §]!_§;
      }
      
      override public function activate(param1:String) : void
      {
         super.activate(param1);
         §!#A§.§#F§.clearLevel();
         §!#A§.§#F§.§8"d§(false);
         this.initLevelsRepeater();
         AngryBirdsBase.singleton.playThemeMusic();
         if(this.§"$'§.length == 1)
         {
            (§?Q§.getItemByName("Button_Prev") as §,#w§).setVisibility(false);
            (§?Q§.getItemByName("Button_Next") as §,#w§).setVisibility(false);
            (§?Q§.getItemByName("TextField_LevelNumberSmall") as §^!4§).setVisibility(false);
         }
         else
         {
            (§?Q§.getItemByName("Button_Prev") as §,#w§).setVisibility(true);
            (§?Q§.getItemByName("Button_Next") as §,#w§).setVisibility(true);
            (§?Q§.getItemByName("TextField_LevelNumberSmall") as §^!4§).setVisibility(true);
         }
         this.§@$+§ = this.§?!W§.x;
         if(§?Q§.stage)
         {
            §?Q§.stage.addEventListener(KeyboardEvent.KEY_DOWN,this.onKeyEvent);
         }
      }
      
      protected function onKeyEvent(param1:KeyboardEvent) : void
      {
         if(param1.keyCode == Keyboard.LEFT)
         {
            this.gotoPrevPage();
         }
         else if(param1.keyCode == Keyboard.RIGHT)
         {
            this.gotoNextPage();
         }
      }
      
      override protected function update(param1:Number) : void
      {
         super.update(param1);
         this.§&$9§();
         this.§@P§();
      }
      
      protected function §&$9§() : void
      {
         var _loc2_:int = 0;
         var _loc1_:int = 0;
         while(_loc1_ < this.§'#-§.length)
         {
            if(-this.§?!W§.x >= this.§"$'§[_loc1_].x && -this.§@$+§ < this.§"$'§[_loc1_].x)
            {
               _loc2_ = 0;
               while(_loc2_ < this.§'#-§.length)
               {
                  this.§'#-§[_loc2_].gotoAndStop("UnSelected");
                  _loc2_++;
               }
               this.§%#e§ = _loc1_;
               this.§'#-§[_loc1_].gotoAndStop("Selected");
               (§?Q§.getItemByName("TextField_LevelNumberSmall") as §^!4§).x = this.§'#-§[_loc1_].x;
               this.updatePageNumber(_loc1_);
            }
            if(-this.§?!W§.x <= this.§"$'§[_loc1_].x && -this.§@$+§ > this.§"$'§[_loc1_].x)
            {
               _loc2_ = 0;
               while(_loc2_ < this.§'#-§.length)
               {
                  this.§'#-§[_loc2_].gotoAndStop("UnSelected");
                  _loc2_++;
               }
               this.§%#e§ = _loc1_;
               this.§'#-§[_loc1_].gotoAndStop("Selected");
               (§?Q§.getItemByName("TextField_LevelNumberSmall") as §^!4§).x = this.§'#-§[_loc1_].x;
               this.updatePageNumber(_loc1_);
            }
            _loc1_++;
         }
         if(this.§%#e§ != this.§>"H§)
         {
            if(this.§>"H§ > this.§%#e§)
            {
               this.§1!#§ = this.§%#e§ + 1;
            }
            else if(this.§>"H§ < this.§%#e§)
            {
               this.§1!#§ = this.§%#e§ - 1;
            }
            else
            {
               this.§1!#§ = this.§%#e§;
            }
            this.§`#Y§.§[#u§(this.§3!N§[this.§1!#§].red,this.§3!N§[this.§1!#§].green,this.§3!N§[this.§1!#§].blue);
         }
         this.§@$+§ = this.§?!W§.x;
      }
      
      protected function §@P§() : void
      {
         var _loc2_:Number = NaN;
         var _loc1_:int = 0;
         while(_loc1_ < this.§"$'§.length)
         {
            _loc2_ = this.§?!W§.x + this.§"$'§[_loc1_].x;
            _loc2_ = Math.abs(_loc2_);
            if(_loc2_ > 1000)
            {
               _loc2_ = 1000;
            }
            if(Math.abs(this.§"$'§[_loc1_].mClip.alpha - (1 - _loc2_ / 1000)) > 0.0001)
            {
               this.§"$'§[_loc1_].mClip.alpha = 1 - _loc2_ / 1000;
            }
            if(this.§"$'§[_loc1_].mClip.alpha < 1)
            {
               (this.§"$'§[_loc1_] as §&#4§).setEnabled(false);
            }
            else
            {
               (this.§"$'§[_loc1_] as §&#4§).setEnabled(true);
            }
            _loc1_++;
         }
      }
      
      override public function deActivate() : void
      {
         if(§?Q§.stage)
         {
            §?Q§.stage.removeEventListener(KeyboardEvent.KEY_DOWN,this.onKeyEvent);
         }
         super.deActivate();
         this.§!$=§();
         (§?Q§.getItemByName("Button_Back") as §,#w§).setComponentVisualState(§@!N§.COMPONENT_STATE_ACTIVE_DEFAULT);
         mLevelManager.resetPreviousLevel();
      }
      
      override public function cleanup() : void
      {
         if(this.§-!Z§)
         {
            this.§-!Z§.stop();
            this.§-!Z§ = null;
         }
         super.cleanup();
      }
      
      override protected function onUIInteraction(param1:int, param2:String, param3:§'"S§) : void
      {
         if(param2.length > 0 && param3 is §=!k§)
         {
            if((param3 as §=!k§).mParentContainer.mParentContainer.name == "Repeater_LevelSelection" || (param3 as §=!k§).mParentContainer.mParentContainer.name == "Repeater_LevelSelection15")
            {
               if(!this.§]"L§)
               {
                  mLevelManager.loadLevel(mLevelManager.getValidLevelId(param2.toLowerCase()));
                  §5"p§(StateCutScene.STATE_NAME);
               }
            }
         }
         switch(param2)
         {
            case "BACK":
               §!#&§.playSound("Menu_Back",§!#&§.§6#h§);
               §5"p§(§4#M§.STATE_NAME);
               break;
            case "NEXT":
               this.gotoNextPage();
               break;
            case "PREV":
               this.gotoPrevPage();
               break;
            case "FULLSCREEN_BUTTON":
               §!#&§.playSound("Menu_Confirm",§!#&§.§6#h§);
               AngryBirdsBase.singleton.§6"0§();
         }
      }
      
      protected function gotoNextPage() : void
      {
         §!#&§.playSound("Menu_Confirm",§!#&§.§6#h§);
         if(!this.§]"L§)
         {
            ++this.§>"H§;
            this.moveToPage(this.§>"H§);
         }
      }
      
      protected function gotoPrevPage() : void
      {
         §!#&§.playSound("Menu_Confirm",§!#&§.§6#h§);
         if(!this.§]"L§)
         {
            --this.§>"H§;
            this.moveToPage(this.§>"H§);
         }
      }
      
      public function initLevelsRepeater() : void
      {
         var _loc4_:Array = null;
         var _loc5_:Class = null;
         var _loc6_:MovieClip = null;
         var _loc7_:Class = null;
         var _loc8_:MovieClip = null;
         var _loc1_:Number = 0;
         this.§3!N§ = [];
         this.§^$ § = [];
         var _loc2_:§9##§ = mLevelManager.getEpisode(mLevelManager.currentEpisode);
         this.§>"H§ = _loc2_.currentPage;
         this.§1!#§ = this.§>"H§;
         this.§%#e§ = this.§>"H§;
         var _loc3_:Number = 0;
         while(_loc3_ < _loc2_.§]!7§)
         {
            _loc4_ = _loc2_.§%!k§(_loc3_);
            this.§3!N§.push(_loc2_.§?#v§(_loc3_));
            this.§^$ §.push(_loc2_.§8$5§(_loc3_));
            _loc1_ = this.§8!"§(_loc4_,_loc3_,_loc1_,_loc2_);
            _loc3_++;
         }
         this.§`#Y§ = new §&!i§(this.§3!N§[this.§>"H§].red,this.§3!N§[this.§>"H§].green,this.§3!N§[this.§>"H§].blue,1);
         §?Q§.getItemByName("MovieClip_ColorFade").changeMovieClip(this.§`#Y§);
         if(_loc2_.§"5§)
         {
            _loc6_ = new (_loc5_ = §+$#§.§["`§(_loc2_.§"5§))();
            §?Q§.getItemByName("MovieClip_ThemeLeft").changeMovieClip(_loc6_);
         }
         else
         {
            §?Q§.getItemByName("MovieClip_ThemeLeft").changeMovieClip(new MovieClip());
         }
         if(_loc2_.§4#t§)
         {
            _loc8_ = new (_loc7_ = §+$#§.§["`§(_loc2_.§4#t§))();
            §?Q§.getItemByName("MovieClip_ThemeRight").changeMovieClip(_loc8_);
         }
         else
         {
            §?Q§.getItemByName("MovieClip_ThemeRight").changeMovieClip(new MovieClip());
         }
         this.§%!I§();
         if(§<r§ == StateCutScene.STATE_NAME)
         {
            §<r§ = "";
            this.moveToPage(this.§%#e§);
         }
         else
         {
            this.moveToPage(this.§>"H§,true);
         }
      }
      
      protected function §8!"§(param1:Array, param2:int, param3:Number, param4:§9##§) : Number
      {
         var _loc8_:XML = null;
         var _loc10_:String = null;
         var _loc11_:Boolean = false;
         var _loc12_:MovieClip = null;
         var _loc13_:XML = null;
         var _loc5_:Array;
         (_loc5_ = new Array())[0] = new Array();
         var _loc6_:Class = §+$#§.§["`§(this.§^$ §[param2]);
         var _loc7_:Number = 0;
         while(_loc7_ < param1.length)
         {
            _loc10_ = param1[_loc7_];
            _loc11_ = AngryBirdsBase.singleton.dataModel.userProgress.isLevelOpen(_loc10_);
            _loc12_ = this.makeButtonForLevel(_loc10_,_loc11_,_loc6_,_loc7_,param2);
            (_loc13_ = <Button/>).@name = _loc10_;
            if(_loc11_ || AngryBirdsBase.DEBUG_MODE_ENABLED)
            {
               _loc13_.@MouseUp = _loc10_;
            }
            _loc13_.@scaleOnMouseOver = "True";
            _loc5_[0].push(new Array(_loc13_,null,_loc12_));
            if(_loc11_)
            {
               _loc12_.MovieClip_Stars.mouseEnabled = false;
            }
            _loc12_.TextField_LevelNum.text.mouseEnabled = false;
            _loc7_++;
         }
         _loc8_ = <Repeater/>;
         if(param1.length == 15)
         {
            _loc8_.@name = "Repeater_LevelSelection15";
         }
         else if(param1.length == 12)
         {
            _loc8_.@name = "Repeater_LevelSelection12";
         }
         else
         {
            _loc8_.@name = "Repeater_LevelSelection";
         }
         _loc8_.@button = this.§^$ §[param2];
         _loc8_.@enabled = "True";
         _loc8_.@buttonSelectionType = "NO_SELECTION";
         _loc8_.@fromLibrary = "true";
         var _loc9_:§&#4§;
         (_loc9_ = new §&#4§(_loc8_,this.§?!W§,null,null)).§6"`§(_loc5_);
         _loc9_.setVisibility(true);
         _loc9_.x += param3;
         this.§?!W§.§%!t§(_loc9_);
         this.§"$'§.push(_loc9_);
         return Number(param3 + AngryBirdsBase.screenWidth);
      }
      
      protected function makeButtonForLevel(param1:String, param2:Boolean, param3:Class, param4:int, param5:int) : MovieClip
      {
         var _loc7_:Number = NaN;
         var _loc6_:MovieClip;
         (_loc6_ = new param3()).TextField_LevelNum.text.text = (param4 + 1).toString();
         if(param2)
         {
            _loc6_.gotoAndStop("Open");
            _loc6_.MovieClip_MEInUse.visible = false;
            if(AngryBirdsBase.singleton.dataModel.userProgress.getEagleScoreForLevel(param1) == 100)
            {
               _loc6_.MovieClip_Feather.gotoAndStop("Visible");
               _loc6_.MovieClip_Feather.mouseEnabled = false;
            }
            else
            {
               _loc6_.MovieClip_Feather.gotoAndStop("Hidden");
               _loc6_.MovieClip_Feather.mouseEnabled = false;
            }
         }
         else
         {
            _loc6_.gotoAndStop("Locked");
         }
         _loc6_.isOpen = param2;
         if(param2)
         {
            _loc7_ = AngryBirdsBase.singleton.dataModel.userProgress.getStarsForLevel(param1);
            _loc6_.MovieClip_Stars.gotoAndStop(_loc7_.toString() + "_stars");
         }
         return _loc6_;
      }
      
      protected function §%!I§() : void
      {
         var _loc1_:Class = null;
         var _loc2_:MovieClip = null;
         if(this.§"$'§.length == 1)
         {
            return;
         }
         var _loc3_:Number = 0;
         this.§4g§ = new Dictionary();
         var _loc4_:int = 0;
         while(_loc4_ < this.§"$'§.length)
         {
            _loc1_ = §+$#§.§["`§("Button_Dot");
            _loc2_ = new _loc1_();
            _loc2_.x = AngryBirdsBase.screenWidth / 2 + _loc3_ - this.§"$'§.length * _loc2_.width / 2;
            _loc2_.y = (§?Q§.getItemByName("Button_Next") as §,#w§).y - _loc2_.height / 2;
            if(_loc4_ == this.§>"H§)
            {
               _loc2_.gotoAndStop("Selected");
               (§?Q§.getItemByName("TextField_LevelNumberSmall") as §^!4§).x = _loc2_.x;
               (§?Q§.getItemByName("TextField_LevelNumberSmall") as §^!4§).y = _loc2_.y - _loc2_.height;
               this.updatePageNumber(_loc4_);
            }
            else
            {
               _loc2_.gotoAndStop("UnSelected");
            }
            this.§2#H§.mClip.addChild(_loc2_);
            this.§4g§[_loc2_] = _loc4_;
            this.§'#-§.push(_loc2_);
            _loc3_ += _loc2_.width;
            _loc2_.addEventListener(MouseEvent.MOUSE_DOWN,this.§@!6§);
            _loc2_.buttonMode = true;
            _loc4_++;
         }
         _loc3_ = this.§"$'§.length * _loc2_.width / 2 + _loc2_.width * 1.5;
         (§?Q§.getItemByName("Button_Next") as §,#w§).x = AngryBirdsBase.screenWidth / 2;
         (§?Q§.getItemByName("Button_Prev") as §,#w§).x = AngryBirdsBase.screenWidth / 2;
         (§?Q§.getItemByName("Button_Next") as §,#w§).x = (§?Q§.getItemByName("Button_Next") as §,#w§).x + (_loc3_ + 10);
         (§?Q§.getItemByName("Button_Prev") as §,#w§).x = (§?Q§.getItemByName("Button_Prev") as §,#w§).x - (_loc3_ + 10);
      }
      
      protected function §!$=§() : void
      {
         if(this.§`#Y§)
         {
            this.§`#Y§.§;!?§();
         }
         var _loc1_:int = 0;
         while(_loc1_ < this.§'#-§.length)
         {
            if(this.§2#H§.mClip.contains(this.§'#-§[_loc1_]))
            {
               this.§'#-§[_loc1_].removeEventListener(MouseEvent.MOUSE_DOWN,this.§@!6§);
               this.§2#H§.mClip.removeChild(this.§'#-§[_loc1_]);
            }
            _loc1_++;
         }
         this.§'#-§ = [];
         this.§?!W§.§`+§();
         this.§"$'§ = [];
      }
      
      protected function §@!6§(param1:MouseEvent) : void
      {
         if(!this.§]"L§)
         {
            this.moveToPage(this.§4g§[param1.target]);
         }
      }
      
      protected function §=c§(param1:MouseEvent) : void
      {
         if(!this.§]"L§)
         {
            §5"p§(§7!c§.STATE_NAME);
         }
      }
      
      protected function moveToPage(param1:int, param2:Boolean = false) : void
      {
         this.§]"L§ = true;
         if(param1 > this.§"$'§.length - 1)
         {
            param1 = this.§"$'§.length - 1;
         }
         else if(param1 < 0)
         {
            param1 = 0;
         }
         this.§>"H§ = param1;
         if(this.§>"H§ > this.§%#e§)
         {
            this.§1!#§ = this.§%#e§ + 1;
         }
         else if(this.§>"H§ < this.§%#e§)
         {
            this.§1!#§ = this.§%#e§ - 1;
         }
         var _loc3_:Number = -this.§"$'§[param1].x;
         var _loc4_:Number = this.§"$'§[param1].x + this.§?!W§.x;
         var _loc5_:Number = (_loc5_ = (_loc5_ = Math.abs(_loc4_)) / 1024) * §1,§;
         if(this.§-!Z§ != null)
         {
            this.§-!Z§.stop();
         }
         if(param2)
         {
            this.§?!W§.x = _loc3_;
         }
         else
         {
            this.§-!Z§ = §6"w§.§ "D§.§""3§(this.§?!W§,{"x":_loc3_},null,_loc5_,§<"I§.easeOut);
            this.§-!Z§.onComplete = this.§;"a§;
         }
         if(param2)
         {
            this.§;"a§();
         }
         else
         {
            this.§-!Z§.play();
         }
      }
      
      protected function §;"a§() : void
      {
         var _loc1_:int = 0;
         while(_loc1_ < this.§'#-§.length)
         {
            if(_loc1_ == this.§>"H§)
            {
               this.§'#-§[_loc1_].gotoAndStop("Selected");
               (§?Q§.getItemByName("TextField_LevelNumberSmall") as §^!4§).x = this.§'#-§[_loc1_].x;
               this.updatePageNumber(_loc1_);
            }
            else
            {
               this.§'#-§[_loc1_].gotoAndStop("UnSelected");
            }
            _loc1_++;
         }
         this.§]"L§ = false;
         mLevelManager.getEpisode(mLevelManager.currentEpisode).currentPage = this.§>"H§;
      }
      
      protected function updatePageNumber(param1:int) : void
      {
         (§?Q§.getItemByName("TextField_LevelNumberSmall") as §^!4§).§[!&§.text = mLevelManager.getEpisode(mLevelManager.currentEpisode).getPageIndex(param1).toString();
      }
   }
}
