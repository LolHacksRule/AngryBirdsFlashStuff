   ...
   public class LevelItemManager
   {
      ...
      public function loadItems(itemData:XML) : void
      {
         ...
         for each(item in this.mItemDataTable.Items.Item)
         {
            ...
            this.addItem(id,type,material,sounds,shape,destroyedScoreInc,floatingScoreFont,damageScore,category,strength,scale,front,particleJsonId,particleVariationCount,particleAmount,disableBirdPassThrough,collision,textureType);
         }
         this.loadSoundChannels(this.mItemDataTable.SoundChannels);
      }
      
      public function addItem(id:String, type:int, material:String, resourcePathsSound:String, shape:String, destroyedScoreInc:int, floatingScoreFont:String, damageScore:int, category:String, strength:int, scale:Number, front:Boolean, particleJSONId:String = "", particleVariationCount:int = 1, particleAmount:int = -1, disableBirdPassThrough:Boolean = false, collision:Boolean = true, textureType:String = null) : void
      {
         ...
         this.mItems[id.toLowerCase()] = new LevelItem(id,type,this.mMaterialManager.getMaterial(material),this.mSoundManager.getSoundResource(resourcePathsSound),itemShape,destroyedScoreInc,floatingScoreFont,damageScore,category,strength,scale,front,particleJSONId,particleVariationCount,particleAmount,disableBirdPassThrough,false,collision,textureType);
      }
