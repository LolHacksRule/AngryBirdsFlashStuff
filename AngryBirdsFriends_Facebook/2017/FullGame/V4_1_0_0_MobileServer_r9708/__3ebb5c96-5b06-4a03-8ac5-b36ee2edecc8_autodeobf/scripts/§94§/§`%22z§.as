package §94§
{
   import §&$!§.§+#!§;
   import §>@§.§'!L§;
   import §@#§.§^#Q§;
   import §]@§.§8"A§;
   
   public class §`"z§ extends §'!L§
   {
       
      
      protected var mLevelManager:§^#Q§;
      
      public function §`"z§(param1:§^#Q§, param2:Boolean, param3:String, param4:§+#!§)
      {
         this.mLevelManager = param1;
         super(param2,param3,param4);
      }
      
      public function prepareToLoadNextClassicLevel() : void
      {
         this.mLevelManager.loadLevel(this.mLevelManager.getNextLevelId());
      }
      
      override public function activate(param1:String) : void
      {
         super.activate(param1);
         AngryBirdsBase.singleton.§0"C§.addLocalizationTarget(this);
      }
      
      override public function deActivate() : void
      {
         super.deActivate();
         AngryBirdsBase.singleton.§0"C§.removeLocalizationTarget(this);
      }
      
      override protected function runAnimations(param1:Number) : void
      {
         if(§ !t§ && §%!b§ != §8"A§.§^"'§ && !AngryBirdsBase.singleton.popupManager.§`$§())
         {
            § !t§.run(param1);
         }
      }
      
      override protected function update(param1:Number) : void
      {
         this.updateUIScale();
      }
      
      protected function updateUIScale() : void
      {
      }
   }
}
