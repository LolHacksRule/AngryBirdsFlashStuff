package §!H§
{
   import §!!B§.b2Body;
   import §!!B§.b2TimeStep;
   import §;0§.b2Settings;
   import §;0§.b2internal;
   import §;U§.b2Mat22;
   import §;U§.b2Mat33;
   import §;U§.b2Math;
   import §;U§.b2Vec2;
   import §;U§.b2Vec3;
   
   use namespace b2internal;
   
   public class b2RevoluteJoint extends b2Joint
   {
      
      private static var §^!C§:b2Vec2 = new b2Vec2();
       
      
      private var §+"§:b2Mat22;
      
      private var K1:b2Mat22;
      
      private var K2:b2Mat22;
      
      private var K3:b2Mat22;
      
      private var impulse3:b2Vec3;
      
      private var impulse2:b2Vec2;
      
      private var §!!H§:b2Vec2;
      
      b2internal var m_localAnchor1:b2Vec2;
      
      b2internal var m_localAnchor2:b2Vec2;
      
      private var §9$§:b2Vec3;
      
      private var §3!N§:Number;
      
      private var §^"§:b2Mat33;
      
      private var §&!J§:Number;
      
      private var §@=§:Boolean;
      
      private var §use §:Number;
      
      private var §!!_§:Number;
      
      private var §0b§:Boolean;
      
      private var §1!H§:Number;
      
      private var §47§:Number;
      
      private var §%!!§:Number;
      
      private var §7!8§:int;
      
      public function b2RevoluteJoint(param1:b2RevoluteJointDef)
      {
         this.§+"§ = new b2Mat22();
         this.K1 = new b2Mat22();
         this.K2 = new b2Mat22();
         this.K3 = new b2Mat22();
         this.impulse3 = new b2Vec3();
         this.impulse2 = new b2Vec2();
         this.§!!H§ = new b2Vec2();
         this.m_localAnchor1 = new b2Vec2();
         this.m_localAnchor2 = new b2Vec2();
         this.§9$§ = new b2Vec3();
         this.§^"§ = new b2Mat33();
         super(param1);
         this.m_localAnchor1.SetV(param1.§7p§);
         this.m_localAnchor2.SetV(param1.§"!C§);
         this.§1!H§ = param1.§#!V§;
         this.§9$§.§4S§();
         this.§3!N§ = 0;
         this.§47§ = param1.§0q§;
         this.§%!!§ = param1.§'!'§;
         this.§use § = param1.§[4§;
         this.§!!_§ = param1.motorSpeed;
         this.§0b§ = param1.§^+§;
         this.§@=§ = param1.§+!6§;
         this.§7!8§ = b2internal::29;
      }
      
      override public function GetAnchorA() : b2Vec2
      {
         return b2internal::!!!.GetWorldPoint(this.m_localAnchor1);
      }
      
      override public function GetAnchorB() : b2Vec2
      {
         return b2internal::%!.GetWorldPoint(this.m_localAnchor2);
      }
      
      override public function GetReactionForce(param1:Number) : b2Vec2
      {
         return new b2Vec2(param1 * this.§9$§.x,param1 * this.§9$§.y);
      }
      
      override public function GetReactionTorque(param1:Number) : Number
      {
         return param1 * this.§9$§.z;
      }
      
      public function §>D§() : Number
      {
         return b2internal::%!.m_sweep.a - b2internal::!!!.m_sweep.a - this.§1!H§;
      }
      
      public function §9!=§() : Number
      {
         return b2internal::%!.m_angularVelocity - b2internal::!!!.m_angularVelocity;
      }
      
      public function §5!C§() : Boolean
      {
         return this.§0b§;
      }
      
      public function §3!K§(param1:Boolean) : void
      {
         this.§0b§ = param1;
      }
      
      public function §,!-§() : Number
      {
         return this.§47§;
      }
      
      public function §+-§() : Number
      {
         return this.§%!!§;
      }
      
      public function §[o§(param1:Number, param2:Number) : void
      {
         this.§47§ = param1;
         this.§%!!§ = param2;
      }
      
      public function §;3§() : Boolean
      {
         b2internal::!!!.SetAwake(true);
         b2internal::%!.SetAwake(true);
         return this.§@=§;
      }
      
      public function §+!'§(param1:Boolean) : void
      {
         this.§@=§ = param1;
      }
      
      public function §-!3§(param1:Number) : void
      {
         b2internal::!!!.SetAwake(true);
         b2internal::%!.SetAwake(true);
         this.§!!_§ = param1;
      }
      
      public function §82§() : Number
      {
         return this.§!!_§;
      }
      
      public function §0@§(param1:Number) : void
      {
         this.§use § = param1;
      }
      
      public function §3!'§() : Number
      {
         return this.§use §;
      }
      
      override b2internal function InitVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc2_:b2Body = null;
         var _loc3_:b2Body = null;
         var _loc4_:b2Mat22 = null;
         var _loc5_:Number = NaN;
         var _loc7_:Number = NaN;
         var _loc14_:Number = NaN;
         var _loc15_:Number = NaN;
         var _loc16_:Number = NaN;
         _loc2_ = b2internal::!!!;
         _loc3_ = b2internal::%!;
         if(this.§@=§ || this.§0b§)
         {
         }
         _loc4_ = _loc2_.m_xf.R;
         var _loc6_:Number = this.m_localAnchor1.x - _loc2_.m_sweep.localCenter.x;
         _loc7_ = this.m_localAnchor1.y - _loc2_.m_sweep.localCenter.y;
         _loc5_ = _loc4_.col1.x * _loc6_ + _loc4_.col2.x * _loc7_;
         _loc7_ = _loc4_.col1.y * _loc6_ + _loc4_.col2.y * _loc7_;
         _loc6_ = _loc5_;
         _loc4_ = _loc3_.m_xf.R;
         var _loc8_:Number = this.m_localAnchor2.x - _loc3_.m_sweep.localCenter.x;
         var _loc9_:Number = this.m_localAnchor2.y - _loc3_.m_sweep.localCenter.y;
         _loc5_ = _loc4_.col1.x * _loc8_ + _loc4_.col2.x * _loc9_;
         _loc9_ = _loc4_.col1.y * _loc8_ + _loc4_.col2.y * _loc9_;
         _loc8_ = _loc5_;
         var _loc10_:Number = _loc2_.§?t§;
         var _loc11_:Number = _loc3_.§?t§;
         var _loc12_:Number = _loc2_.§<!V§;
         var _loc13_:Number = _loc3_.§<!V§;
         this.§^"§.col1.x = _loc10_ + _loc11_ + _loc7_ * _loc7_ * _loc12_ + _loc9_ * _loc9_ * _loc13_;
         this.§^"§.col2.x = -_loc7_ * _loc6_ * _loc12_ - _loc9_ * _loc8_ * _loc13_;
         this.§^"§.col3.x = -_loc7_ * _loc12_ - _loc9_ * _loc13_;
         this.§^"§.col1.y = this.§^"§.col2.x;
         this.§^"§.col2.y = _loc10_ + _loc11_ + _loc6_ * _loc6_ * _loc12_ + _loc8_ * _loc8_ * _loc13_;
         this.§^"§.col3.y = _loc6_ * _loc12_ + _loc8_ * _loc13_;
         this.§^"§.col1.z = this.§^"§.col3.x;
         this.§^"§.col2.z = this.§^"§.col3.y;
         this.§^"§.col3.z = _loc12_ + _loc13_;
         this.§&!J§ = 1 / (_loc12_ + _loc13_);
         if(this.§@=§ == false)
         {
            this.§3!N§ = 0;
         }
         if(this.§0b§)
         {
            _loc14_ = _loc3_.m_sweep.a - _loc2_.m_sweep.a - this.§1!H§;
            if(b2Math.§[!I§(this.§%!!§ - this.§47§) < 2 * b2Settings.b2_angularSlop)
            {
               this.§7!8§ = b2internal::"!^;
            }
            else if(_loc14_ <= this.§47§)
            {
               if(this.§7!8§ != b2internal::@")
               {
                  this.§9$§.z = 0;
               }
               this.§7!8§ = b2internal::@";
            }
            else if(_loc14_ >= this.§%!!§)
            {
               if(this.§7!8§ != b2internal::5!P)
               {
                  this.§9$§.z = 0;
               }
               this.§7!8§ = b2internal::5!P;
            }
            else
            {
               this.§7!8§ = b2internal::29;
               this.§9$§.z = 0;
            }
         }
         else
         {
            this.§7!8§ = b2internal::29;
         }
         if(param1.§7!Z§)
         {
            this.§9$§.x *= param1.§+k§;
            this.§9$§.y *= param1.§+k§;
            this.§3!N§ *= param1.§+k§;
            _loc15_ = this.§9$§.x;
            _loc16_ = this.§9$§.y;
            _loc2_.§7W§.x -= _loc10_ * _loc15_;
            _loc2_.§7W§.y -= _loc10_ * _loc16_;
            _loc2_.m_angularVelocity -= _loc12_ * (_loc6_ * _loc16_ - _loc7_ * _loc15_ + this.§3!N§ + this.§9$§.z);
            _loc3_.§7W§.x += _loc11_ * _loc15_;
            _loc3_.§7W§.y += _loc11_ * _loc16_;
            _loc3_.m_angularVelocity += _loc13_ * (_loc8_ * _loc16_ - _loc9_ * _loc15_ + this.§3!N§ + this.§9$§.z);
         }
         else
         {
            this.§9$§.§4S§();
            this.§3!N§ = 0;
         }
      }
      
      override b2internal function SolveVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc4_:b2Mat22 = null;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc7_:Number = NaN;
         var _loc8_:Number = NaN;
         var _loc9_:Number = NaN;
         var _loc10_:Number = NaN;
         var _loc19_:Number = NaN;
         var _loc20_:Number = NaN;
         var _loc21_:Number = NaN;
         var _loc22_:Number = NaN;
         var _loc23_:Number = NaN;
         var _loc24_:Number = NaN;
         var _loc25_:Number = NaN;
         var _loc26_:Number = NaN;
         var _loc27_:Number = NaN;
         var _loc2_:b2Body = b2internal::!!!;
         var _loc3_:b2Body = b2internal::%!;
         var _loc11_:b2Vec2 = _loc2_.§7W§;
         var _loc12_:Number = _loc2_.m_angularVelocity;
         var _loc13_:b2Vec2 = _loc3_.§7W§;
         var _loc14_:Number = _loc3_.m_angularVelocity;
         var _loc15_:Number = _loc2_.§?t§;
         var _loc16_:Number = _loc3_.§?t§;
         var _loc17_:Number = _loc2_.§<!V§;
         var _loc18_:Number = _loc3_.§<!V§;
         if(this.§@=§ && this.§7!8§ != b2internal::"!^)
         {
            _loc19_ = _loc14_ - _loc12_ - this.§!!_§;
            _loc20_ = this.§&!J§ * -_loc19_;
            _loc21_ = this.§3!N§;
            _loc22_ = param1.§1-§ * this.§use §;
            this.§3!N§ = b2Math.§1!<§(this.§3!N§ + _loc20_,-_loc22_,_loc22_);
            _loc20_ = this.§3!N§ - _loc21_;
            _loc12_ -= _loc17_ * _loc20_;
            _loc14_ += _loc18_ * _loc20_;
         }
         if(this.§0b§ && this.§7!8§ != b2internal::29)
         {
            _loc4_ = _loc2_.m_xf.R;
            _loc7_ = this.m_localAnchor1.x - _loc2_.m_sweep.localCenter.x;
            _loc8_ = this.m_localAnchor1.y - _loc2_.m_sweep.localCenter.y;
            _loc5_ = _loc4_.col1.x * _loc7_ + _loc4_.col2.x * _loc8_;
            _loc8_ = _loc4_.col1.y * _loc7_ + _loc4_.col2.y * _loc8_;
            _loc7_ = _loc5_;
            _loc4_ = _loc3_.m_xf.R;
            _loc9_ = this.m_localAnchor2.x - _loc3_.m_sweep.localCenter.x;
            _loc10_ = this.m_localAnchor2.y - _loc3_.m_sweep.localCenter.y;
            _loc5_ = _loc4_.col1.x * _loc9_ + _loc4_.col2.x * _loc10_;
            _loc10_ = _loc4_.col1.y * _loc9_ + _loc4_.col2.y * _loc10_;
            _loc9_ = _loc5_;
            _loc23_ = _loc13_.x + -_loc14_ * _loc10_ - _loc11_.x - -_loc12_ * _loc8_;
            _loc24_ = _loc13_.y + _loc14_ * _loc9_ - _loc11_.y - _loc12_ * _loc7_;
            _loc25_ = _loc14_ - _loc12_;
            this.§^"§.Solve33(this.impulse3,-_loc23_,-_loc24_,-_loc25_);
            if(this.§7!8§ == b2internal::"!^)
            {
               this.§9$§.§<!A§(this.impulse3);
            }
            else if(this.§7!8§ == b2internal::@")
            {
               if((_loc6_ = this.§9$§.z + this.impulse3.z) < 0)
               {
                  this.§^"§.Solve22(this.§!!H§,-_loc23_,-_loc24_);
                  this.impulse3.x = this.§!!H§.x;
                  this.impulse3.y = this.§!!H§.y;
                  this.impulse3.z = -this.§9$§.z;
                  this.§9$§.x += this.§!!H§.x;
                  this.§9$§.y += this.§!!H§.y;
                  this.§9$§.z = 0;
               }
            }
            else if(this.§7!8§ == b2internal::5!P)
            {
               if((_loc6_ = this.§9$§.z + this.impulse3.z) > 0)
               {
                  this.§^"§.Solve22(this.§!!H§,-_loc23_,-_loc24_);
                  this.impulse3.x = this.§!!H§.x;
                  this.impulse3.y = this.§!!H§.y;
                  this.impulse3.z = -this.§9$§.z;
                  this.§9$§.x += this.§!!H§.x;
                  this.§9$§.y += this.§!!H§.y;
                  this.§9$§.z = 0;
               }
            }
            _loc11_.x -= _loc15_ * this.impulse3.x;
            _loc11_.y -= _loc15_ * this.impulse3.y;
            _loc12_ -= _loc17_ * (_loc7_ * this.impulse3.y - _loc8_ * this.impulse3.x + this.impulse3.z);
            _loc13_.x += _loc16_ * this.impulse3.x;
            _loc13_.y += _loc16_ * this.impulse3.y;
            _loc14_ += _loc18_ * (_loc9_ * this.impulse3.y - _loc10_ * this.impulse3.x + this.impulse3.z);
         }
         else
         {
            _loc4_ = _loc2_.m_xf.R;
            _loc7_ = this.m_localAnchor1.x - _loc2_.m_sweep.localCenter.x;
            _loc8_ = this.m_localAnchor1.y - _loc2_.m_sweep.localCenter.y;
            _loc5_ = _loc4_.col1.x * _loc7_ + _loc4_.col2.x * _loc8_;
            _loc8_ = _loc4_.col1.y * _loc7_ + _loc4_.col2.y * _loc8_;
            _loc7_ = _loc5_;
            _loc4_ = _loc3_.m_xf.R;
            _loc9_ = this.m_localAnchor2.x - _loc3_.m_sweep.localCenter.x;
            _loc10_ = this.m_localAnchor2.y - _loc3_.m_sweep.localCenter.y;
            _loc5_ = _loc4_.col1.x * _loc9_ + _loc4_.col2.x * _loc10_;
            _loc10_ = _loc4_.col1.y * _loc9_ + _loc4_.col2.y * _loc10_;
            _loc9_ = _loc5_;
            _loc26_ = _loc13_.x + -_loc14_ * _loc10_ - _loc11_.x - -_loc12_ * _loc8_;
            _loc27_ = _loc13_.y + _loc14_ * _loc9_ - _loc11_.y - _loc12_ * _loc7_;
            this.§^"§.Solve22(this.impulse2,-_loc26_,-_loc27_);
            this.§9$§.x += this.impulse2.x;
            this.§9$§.y += this.impulse2.y;
            _loc11_.x -= _loc15_ * this.impulse2.x;
            _loc11_.y -= _loc15_ * this.impulse2.y;
            _loc12_ -= _loc17_ * (_loc7_ * this.impulse2.y - _loc8_ * this.impulse2.x);
            _loc13_.x += _loc16_ * this.impulse2.x;
            _loc13_.y += _loc16_ * this.impulse2.y;
            _loc14_ += _loc18_ * (_loc9_ * this.impulse2.y - _loc10_ * this.impulse2.x);
         }
         _loc2_.§7W§.SetV(_loc11_);
         _loc2_.m_angularVelocity = _loc12_;
         _loc3_.§7W§.SetV(_loc13_);
         _loc3_.m_angularVelocity = _loc14_;
      }
      
      override b2internal function SolvePositionConstraints(param1:Number) : Boolean
      {
         var _loc2_:Number = NaN;
         var _loc3_:Number = NaN;
         var _loc4_:b2Mat22 = null;
         var _loc9_:Number = NaN;
         var _loc10_:Number = NaN;
         var _loc11_:Number = NaN;
         var _loc25_:Number = NaN;
         var _loc26_:Number = NaN;
         var _loc27_:Number = NaN;
         var _loc28_:Number = NaN;
         var _loc29_:Number = NaN;
         var _loc30_:Number = NaN;
         var _loc31_:Number = NaN;
         var _loc5_:b2Body = b2internal::!!!;
         var _loc6_:b2Body = b2internal::%!;
         var _loc7_:Number = 0;
         var _loc8_:Number = 0;
         if(this.§0b§ && this.§7!8§ != b2internal::29)
         {
            _loc25_ = _loc6_.m_sweep.a - _loc5_.m_sweep.a - this.§1!H§;
            _loc26_ = 0;
            if(this.§7!8§ == b2internal::"!^)
            {
               _loc3_ = b2Math.§1!<§(_loc25_ - this.§47§,-b2Settings.b2_maxAngularCorrection,b2Settings.b2_maxAngularCorrection);
               _loc26_ = -this.§&!J§ * _loc3_;
               _loc7_ = b2Math.§[!I§(_loc3_);
            }
            else if(this.§7!8§ == b2internal::@")
            {
               _loc3_ = _loc25_ - this.§47§;
               _loc7_ = -_loc3_;
               _loc3_ = b2Math.§1!<§(_loc3_ + b2Settings.b2_angularSlop,-b2Settings.b2_maxAngularCorrection,0);
               _loc26_ = -this.§&!J§ * _loc3_;
            }
            else if(this.§7!8§ == b2internal::5!P)
            {
               _loc3_ = _loc25_ - this.§%!!§;
               _loc7_ = _loc3_;
               _loc3_ = b2Math.§1!<§(_loc3_ - b2Settings.b2_angularSlop,0,b2Settings.b2_maxAngularCorrection);
               _loc26_ = -this.§&!J§ * _loc3_;
            }
            _loc5_.m_sweep.a -= _loc5_.§<!V§ * _loc26_;
            _loc6_.m_sweep.a += _loc6_.§<!V§ * _loc26_;
            _loc5_.§9!8§();
            _loc6_.§9!8§();
         }
         _loc4_ = _loc5_.m_xf.R;
         var _loc12_:Number = this.m_localAnchor1.x - _loc5_.m_sweep.localCenter.x;
         var _loc13_:Number = this.m_localAnchor1.y - _loc5_.m_sweep.localCenter.y;
         _loc9_ = _loc4_.col1.x * _loc12_ + _loc4_.col2.x * _loc13_;
         _loc13_ = _loc4_.col1.y * _loc12_ + _loc4_.col2.y * _loc13_;
         _loc12_ = _loc9_;
         _loc4_ = _loc6_.m_xf.R;
         var _loc14_:Number = this.m_localAnchor2.x - _loc6_.m_sweep.localCenter.x;
         var _loc15_:Number = this.m_localAnchor2.y - _loc6_.m_sweep.localCenter.y;
         _loc9_ = _loc4_.col1.x * _loc14_ + _loc4_.col2.x * _loc15_;
         _loc15_ = _loc4_.col1.y * _loc14_ + _loc4_.col2.y * _loc15_;
         _loc14_ = _loc9_;
         var _loc16_:Number = _loc6_.m_sweep.c.x + _loc14_ - _loc5_.m_sweep.c.x - _loc12_;
         var _loc17_:Number = _loc6_.m_sweep.c.y + _loc15_ - _loc5_.m_sweep.c.y - _loc13_;
         var _loc18_:Number = _loc16_ * _loc16_ + _loc17_ * _loc17_;
         var _loc19_:Number;
         _loc8_ = _loc19_ = Math.sqrt(_loc18_);
         var _loc20_:Number = _loc5_.§?t§;
         var _loc21_:Number = _loc6_.§?t§;
         var _loc22_:Number = _loc5_.§<!V§;
         var _loc23_:Number = _loc6_.§<!V§;
         var _loc24_:Number = 10 * b2Settings.b2_linearSlop;
         if(_loc18_ > _loc24_ * _loc24_)
         {
            _loc27_ = _loc16_ / _loc19_;
            _loc28_ = _loc17_ / _loc19_;
            _loc29_ = _loc20_ + _loc21_;
            _loc10_ = (_loc30_ = 1 / _loc29_) * -_loc16_;
            _loc11_ = _loc30_ * -_loc17_;
            _loc31_ = 0.5;
            _loc5_.m_sweep.c.x -= _loc31_ * _loc20_ * _loc10_;
            _loc5_.m_sweep.c.y -= _loc31_ * _loc20_ * _loc11_;
            _loc6_.m_sweep.c.x += _loc31_ * _loc21_ * _loc10_;
            _loc6_.m_sweep.c.y += _loc31_ * _loc21_ * _loc11_;
            _loc16_ = _loc6_.m_sweep.c.x + _loc14_ - _loc5_.m_sweep.c.x - _loc12_;
            _loc17_ = _loc6_.m_sweep.c.y + _loc15_ - _loc5_.m_sweep.c.y - _loc13_;
         }
         this.K1.col1.x = _loc20_ + _loc21_;
         this.K1.col2.x = 0;
         this.K1.col1.y = 0;
         this.K1.col2.y = _loc20_ + _loc21_;
         this.K2.col1.x = _loc22_ * _loc13_ * _loc13_;
         this.K2.col2.x = -_loc22_ * _loc12_ * _loc13_;
         this.K2.col1.y = -_loc22_ * _loc12_ * _loc13_;
         this.K2.col2.y = _loc22_ * _loc12_ * _loc12_;
         this.K3.col1.x = _loc23_ * _loc15_ * _loc15_;
         this.K3.col2.x = -_loc23_ * _loc14_ * _loc15_;
         this.K3.col1.y = -_loc23_ * _loc14_ * _loc15_;
         this.K3.col2.y = _loc23_ * _loc14_ * _loc14_;
         this.§+"§.§4!;§(this.K1);
         this.§+"§.§5A§(this.K2);
         this.§+"§.§5A§(this.K3);
         this.§+"§.§??§(§^!C§,-_loc16_,-_loc17_);
         _loc10_ = §^!C§.x;
         _loc11_ = §^!C§.y;
         _loc5_.m_sweep.c.x -= _loc5_.§?t§ * _loc10_;
         _loc5_.m_sweep.c.y -= _loc5_.§?t§ * _loc11_;
         _loc5_.m_sweep.a -= _loc5_.§<!V§ * (_loc12_ * _loc11_ - _loc13_ * _loc10_);
         _loc6_.m_sweep.c.x += _loc6_.§?t§ * _loc10_;
         _loc6_.m_sweep.c.y += _loc6_.§?t§ * _loc11_;
         _loc6_.m_sweep.a += _loc6_.§<!V§ * (_loc14_ * _loc11_ - _loc15_ * _loc10_);
         _loc5_.§9!8§();
         _loc6_.§9!8§();
         return _loc8_ <= b2Settings.b2_linearSlop && _loc7_ <= b2Settings.b2_angularSlop;
      }
   }
}
