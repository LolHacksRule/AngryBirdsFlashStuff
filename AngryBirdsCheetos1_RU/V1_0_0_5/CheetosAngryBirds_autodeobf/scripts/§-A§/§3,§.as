package §-A§
{
   import §]_§.§!F§;
   import flash.display.BitmapData;
   import flash.display3D.Context3D;
   import flash.display3D.Context3DTextureFormat;
   import flash.display3D.textures.Texture;
   import flash.display3D.textures.TextureBase;
   import flash.utils.ByteArray;
   import starling.events.Event;
   
   public class §3,§ extends §-A§.Texture
   {
       
      
      private var §0!F§:TextureBase;
      
      private var §`!S§:int;
      
      private var §]n§:int;
      
      private var §!f§:Boolean;
      
      private var §-N§:Boolean;
      
      private var §7x§:Boolean;
      
      private var §<F§:Object;
      
      private var §%L§:Boolean = false;
      
      public function §3,§(param1:TextureBase, param2:int, param3:int, param4:Boolean, param5:Boolean, param6:Boolean = false)
      {
         super();
         this.§0!F§ = param1;
         this.§`!S§ = param2;
         this.§]n§ = param3;
         this.§!f§ = param4;
         this.§-N§ = param5;
         this.§7x§ = param6;
      }
      
      override public function dispose() : void
      {
         if(this.§0!F§)
         {
            this.§0!F§.dispose();
         }
         this.§+!9§(null);
         super.dispose();
      }
      
      public function §+!9§(param1:Object) : void
      {
         if(this.§<F§ == null && param1 != null)
         {
            §!F§.§[M§.addEventListener(Event.CONTEXT3D_CREATE,this.§?6§);
         }
         if(param1 == null)
         {
            §!F§.§[M§.removeEventListener(Event.CONTEXT3D_CREATE,this.§?6§);
         }
         this.§<F§ = param1;
      }
      
      private function §?6§(param1:Event) : void
      {
         this.requestBaseTextureUpdate(null);
      }
      
      override public function requestBaseTextureUpdate(param1:Object) : void
      {
         this.§%L§ = true;
         if(param1 != null)
         {
            this.§+!9§(param1);
         }
      }
      
      public function get §2[§() : Boolean
      {
         return this.§7x§;
      }
      
      override public function getBase(param1:Context3D) : TextureBase
      {
         this.§4J§(param1);
         return this.§0!F§;
      }
      
      override public function get root() : §-A§.Texture
      {
         return this;
      }
      
      private function §4J§(param1:Context3D) : void
      {
         var _loc4_:flash.display3D.textures.Texture = null;
         var _loc5_:String = null;
         if(!this.§%L§)
         {
            return;
         }
         var _loc2_:BitmapData = this.§<F§ as BitmapData;
         var _loc3_:ByteArray = this.§<F§ as ByteArray;
         if(_loc2_)
         {
            _loc4_ = param1.createTexture(this.§`!S§,this.§]n§,Context3DTextureFormat.BGRA,this.§7x§);
            §-A§.Texture.uploadBitmapData(_loc4_,_loc2_,this.§!f§);
         }
         else if(_loc3_)
         {
            _loc5_ = _loc3_[6] == 2 ? Context3DTextureFormat.COMPRESSED : Context3DTextureFormat.BGRA;
            _loc4_ = param1.createTexture(this.§`!S§,this.§]n§,_loc5_,this.§7x§);
            §-A§.Texture.uploadAtfData(_loc4_,_loc3_);
         }
         this.§0!F§ = _loc4_;
         this.§%L§ = false;
      }
      
      override public function get width() : Number
      {
         return this.§`!S§;
      }
      
      override public function get height() : Number
      {
         return this.§]n§;
      }
      
      override public function get mipMapping() : Boolean
      {
         return this.§!f§;
      }
      
      override public function get premultipliedAlpha() : Boolean
      {
         return this.§-N§;
      }
      
      override public function get bitmapData() : BitmapData
      {
         return this.§<F§ as BitmapData;
      }
      
      override public function get clippedBitmapData() : BitmapData
      {
         return this.bitmapData;
      }
   }
}
