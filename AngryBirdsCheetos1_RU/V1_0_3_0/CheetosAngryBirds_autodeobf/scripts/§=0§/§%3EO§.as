package §=0§
{
   import §+R§.§#!O§;
   import §+R§.§[`§;
   import §+a§.§9e§;
   import §+a§.Texture;
   import §>N§.§"u§;
   import flash.display.BitmapData;
   import flash.events.EventDispatcher;
   import flash.geom.Point;
   import flash.geom.Rectangle;
   import flash.utils.Dictionary;
   
   public class §>O§ extends EventDispatcher
   {
      
      private static const §[#§:int = 2;
      
      private static const §+n§:int = 2048;
      
      private static var § !,§:§>O§;
       
      
      private var §!p§:Dictionary;
      
      private var §,Y§:Vector.<§#!O§>;
      
      private var §`X§:Dictionary;
      
      private var §6R§:Boolean;
      
      private var §%`§:String;
      
      private var §6!V§:Texture;
      
      private var §<K§:int;
      
      private var §]+§:int;
      
      public function §>O§(param1:String = null)
      {
         super();
         this.§,Y§ = new Vector.<§#!O§>();
         this.§!p§ = new Dictionary();
         this.§`X§ = new Dictionary();
         this.§%`§ = param1;
      }
      
      public static function get §<?§() : §>O§
      {
         if(!§ !,§)
         {
            § !,§ = new §>O§();
         }
         return § !,§;
      }
      
      public function get §4e§() : int
      {
         return this.§<K§;
      }
      
      public function get §`R§() : int
      {
         return this.§]+§;
      }
      
      public function get id() : String
      {
         return this.§%`§;
      }
      
      public function dispose() : void
      {
         var _loc1_:§^L§ = null;
         var _loc2_:§#!O§ = null;
         if(this.§!p§)
         {
            for each(_loc1_ in this.§!p§)
            {
               this.§]%§(_loc1_.texture.parent);
               _loc1_.dispose();
            }
            this.§!p§ = null;
         }
         if(this.§,Y§)
         {
            for each(_loc2_ in this.§,Y§)
            {
               _loc2_.dispose();
            }
            this.§,Y§ = null;
         }
         if(this.§`X§)
         {
            this.§`X§ = null;
         }
         if(this.§6!V§)
         {
            this.§6!V§.dispose();
            this.§6!V§ = null;
         }
         this.§,Y§ = new Vector.<§#!O§>();
         this.§!p§ = new Dictionary();
         this.§`X§ = new Dictionary();
      }
      
      public function §"e§(param1:§#!O§) : void
      {
         if(this.§6R§)
         {
            return;
         }
         if(this.§,Y§.indexOf(param1) >= 0)
         {
            return;
         }
         this.§,Y§.push(param1);
      }
      
      private function §"b§(param1:Array) : Rectangle
      {
         var _loc9_:§#!O§ = null;
         var _loc10_:BitmapData = null;
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc6_:int = 0;
         var _loc7_:Number = 1;
         var _loc8_:int = 0;
         while(_loc8_ < param1.length)
         {
            _loc9_ = param1[_loc8_];
            if(_loc8_ == 0)
            {
               _loc7_ = _loc9_.scale;
            }
            else if(_loc9_.scale != _loc7_)
            {
               break;
            }
            _loc10_ = _loc9_.bitmapData;
            if(_loc2_ + _loc10_.width + §[#§ >= §+n§)
            {
               _loc2_ = 0;
               _loc3_ += _loc6_;
               _loc6_ = 0;
            }
            if(_loc3_ + _loc10_.height + §[#§ >= §+n§)
            {
               break;
            }
            _loc2_ += _loc10_.width + §[#§;
            if(_loc2_ > _loc4_)
            {
               _loc4_ = _loc2_;
            }
            if(_loc10_.height + §[#§ > _loc6_)
            {
               _loc6_ = _loc10_.height + §[#§;
               _loc5_ = _loc3_ + _loc6_;
            }
            _loc8_++;
         }
         return new Rectangle(0,0,_loc4_,_loc5_);
      }
      
      private function §[c§(param1:Array, param2:BitmapData) : Array
      {
         var _loc8_:§#!O§ = null;
         var _loc9_:BitmapData = null;
         var _loc10_:§&!$§ = null;
         var _loc11_:int = 0;
         var _loc12_:int = 0;
         var _loc13_:§[`§ = null;
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc6_:Array = [];
         var _loc7_:Number = NaN;
         while(param1.length > 0)
         {
            _loc8_ = param1[0];
            if(!isNaN(_loc7_) && _loc7_ != _loc8_.scale)
            {
               break;
            }
            _loc7_ = _loc8_.scale;
            _loc9_ = _loc8_.bitmapData;
            if(_loc3_ + _loc9_.width + §[#§ > param2.width)
            {
               _loc3_ = 0;
               _loc4_ += _loc5_;
               _loc5_ = 0;
            }
            if(_loc4_ + _loc9_.height + §[#§ > param2.height)
            {
               break;
            }
            param2.copyPixels(_loc9_,_loc9_.rect,new Point(_loc3_,_loc4_),null,null,true);
            (_loc10_ = new §&!$§()).offset = new Point(_loc3_,_loc4_);
            _loc10_.§=!7§ = _loc8_;
            _loc6_.push(_loc10_);
            _loc11_ = _loc8_.§=r§;
            _loc12_ = 0;
            while(_loc12_ < _loc11_)
            {
               (_loc13_ = _loc8_.§[a§(_loc12_)).§=!R§ = null;
               _loc13_.§1@§.offset(_loc3_,_loc4_);
               _loc12_++;
            }
            _loc3_ += _loc9_.width + §[#§;
            if(_loc9_.height + §[#§ > _loc5_)
            {
               _loc5_ = _loc9_.height + §[#§;
            }
            param1.splice(0,1);
         }
         return _loc6_;
      }
      
      public function §'6§() : Boolean
      {
         var _loc1_:§#!O§ = null;
         var _loc3_:Rectangle = null;
         var _loc4_:BitmapData = null;
         var _loc5_:Array = null;
         var _loc6_:Texture = null;
         var _loc7_:int = 0;
         var _loc8_:§&!$§ = null;
         var _loc9_:int = 0;
         var _loc10_:int = 0;
         var _loc11_:§[`§ = null;
         if(!§"u§.§9b§())
         {
            return false;
         }
         if(this.§6R§)
         {
            return true;
         }
         var _loc2_:Array = [];
         for each(_loc1_ in this.§,Y§)
         {
            if(_loc1_.§=r§ > 0)
            {
               _loc2_.push(_loc1_);
            }
         }
         while(_loc2_.length > 0)
         {
            _loc3_ = this.§"b§(_loc2_);
            if(_loc3_.width * _loc3_.height == 0)
            {
               throw new Error("Found too large sprite sheet image!");
            }
            _loc4_ = new BitmapData(_loc3_.width,_loc3_.height,true,0);
            if((_loc5_ = this.§[c§(_loc2_,_loc4_)).length == 0)
            {
               throw new Error("Found too large sprite sheet image!");
            }
            _loc6_ = this.§1u§(_loc4_);
            _loc7_ = 0;
            while(_loc7_ < _loc5_.length)
            {
               _loc1_ = (_loc8_ = _loc5_[_loc7_]).§=!7§;
               _loc9_ = _loc1_.§=r§;
               _loc10_ = 0;
               while(_loc10_ < _loc9_)
               {
                  _loc11_ = _loc1_.§[a§(_loc10_);
                  this.§]D§(_loc11_,_loc6_,_loc4_,_loc1_.scale);
                  _loc10_++;
               }
               _loc7_++;
            }
         }
         for each(_loc1_ in this.§,Y§)
         {
            _loc1_.dispose();
         }
         this.§,Y§ = new Vector.<§#!O§>();
         this.§6R§ = true;
         return true;
      }
      
      private function §]D§(param1:§[`§, param2:Texture, param3:BitmapData, param4:Number) : void
      {
         var _loc6_:Rectangle = null;
         var _loc7_:§9e§ = null;
         var _loc5_:§^L§;
         if(!(_loc5_ = this.§!p§[param1.mName]))
         {
            _loc6_ = param1.§1@§.clone();
            _loc7_ = new §9e§(param2,_loc6_,false);
            _loc5_ = new §^L§(_loc7_,param3,_loc6_,param1.§=3§,param1.§`!G§,param4);
            this.§!p§[param1.mName] = _loc5_;
         }
      }
      
      public function §"R§() : void
      {
         var bd:Object = null;
         var texture:Texture = null;
         if(!§"u§.§3"§)
         {
            for(bd in this.§`X§)
            {
               try
               {
                  texture = this.§`X§[bd];
                  texture.requestBaseTextureUpdate(bd as BitmapData);
               }
               catch(e:Error)
               {
               }
            }
         }
      }
      
      public function § r§(param1:String) : §^L§
      {
         return this.§!p§[param1];
      }
      
      public function §1u§(param1:BitmapData, param2:Boolean = true) : Texture
      {
         var _loc3_:Texture = this.§`X§[param1];
         if(_loc3_)
         {
            return _loc3_;
         }
         _loc3_ = §"u§.§`[§(param1,param2);
         this.§`X§[param1] = _loc3_;
         this.§0!%§(_loc3_,param1,true);
         return _loc3_;
      }
      
      public function §0!%§(param1:Texture, param2:BitmapData, param3:Boolean) : void
      {
         var _loc5_:* = 0;
         var _loc6_:* = 0;
         var _loc4_:int = 1;
         if(!param3)
         {
            _loc4_ = -1;
         }
         if(param2)
         {
            this.§]+§ += _loc4_ * param2.width * param2.height * 4;
         }
         if(param1)
         {
            _loc5_ = int(param1.width);
            _loc6_ = int(param1.height);
            while(_loc5_ >= 1 && _loc6_ >= 1)
            {
               this.§<K§ += _loc4_ * _loc5_ * _loc6_ * 4;
               _loc5_ >>= 1;
               _loc6_ >>= 1;
            }
         }
      }
      
      public function §^!W§() : Texture
      {
         if(!this.§6!V§)
         {
            this.§6!V§ = this.§1u§(new BitmapData(40,40,false,16711935));
         }
         return this.§6!V§;
      }
      
      public function §]%§(param1:Texture) : void
      {
         var _loc2_:* = null;
         var _loc3_:BitmapData = null;
         for(_loc2_ in this.§`X§)
         {
            if(this.§`X§[_loc2_] == param1)
            {
               _loc3_ = _loc2_ as BitmapData;
               this.§0!%§(param1,_loc3_,false);
               if(_loc3_)
               {
                  _loc3_.dispose();
               }
               delete this.§`X§[_loc2_];
               param1.dispose();
               return;
            }
         }
      }
      
      public function §1!V§(param1:String) : BitmapData
      {
         var _loc2_:§^L§ = this.§ r§(param1);
         if(_loc2_ == null)
         {
            _loc2_ = this.§ r§(param1 + "_1");
         }
         if(_loc2_ == null)
         {
            _loc2_ = this.§ r§(param1 + "_01");
         }
         if(_loc2_ == null)
         {
            return null;
         }
         return _loc2_.bitmapData;
      }
   }
}
