package §^9§
{
   import flash.utils.Dictionary;
   
   public class override
   {
       
      
      private var §?!!§:String = "";
      
      private var mName:String = "";
      
      private var §]E§:String = "";
      
      private var §'!-§:String = "";
      
      private var §8!>§:String = "";
      
      private var §[!>§:Array = null;
      
      private var §]L§:int = 0;
      
      private var §`P§:Array = null;
      
      private var §7n§:Array;
      
      private var §;h§:Array;
      
      private var §2H§:int = 0;
      
      private var §#>§:Dictionary;
      
      public function override()
      {
         this.§#>§ = new Dictionary();
         super();
      }
      
      public function §'!U§() : int
      {
         return this.pageIndexes.length * this.levelsPerPage;
      }
      
      public function §]!^§(param1:String) : Array
      {
         var _loc4_:String = null;
         var _loc2_:Array = new Array();
         var _loc3_:Number = 0;
         while(_loc3_ < this.levelsPerPage)
         {
            _loc4_ = param1 + "-" + (_loc3_ + 1);
            _loc2_.push(_loc4_);
            _loc3_++;
         }
         return _loc2_;
      }
      
      public function §&e§(param1:int) : Object
      {
         return this.§7n§[param1];
      }
      
      public function §^!`§(param1:int) : String
      {
         return this.§;h§[param1];
      }
      
      public function §3!P§(param1:String) : Boolean
      {
         return this.§>A§().indexOf(param1) != -1;
      }
      
      public function §>A§() : Array
      {
         var _loc1_:Array = null;
         var _loc2_:String = null;
         var _loc3_:Array = null;
         var _loc4_:String = null;
         if(this.§`P§ == null)
         {
            _loc1_ = new Array();
            for each(_loc2_ in this.pageIndexes)
            {
               _loc3_ = this.§]!^§(_loc2_);
               for each(_loc4_ in _loc3_)
               {
                  _loc1_.push(_loc4_);
               }
            }
            this.§`P§ = _loc1_;
            return _loc1_;
         }
         return this.§`P§;
      }
      
      public function §1!U§(param1:String) : int
      {
         var _loc3_:String = null;
         var _loc2_:int = 0;
         while(_loc2_ < this.pageIndexes.length)
         {
            _loc3_ = this.pageIndexes[_loc2_];
            if(param1.indexOf(_loc3_ + "-") == 0)
            {
               return _loc2_;
            }
            _loc2_++;
         }
         return -1;
      }
      
      public function §6m§(param1:String) : String
      {
         var _loc4_:int = 0;
         var _loc2_:String = param1.substring(0,param1.indexOf("-"));
         var _loc3_:int = parseInt(param1.substring(param1.indexOf("-") + 1));
         if(_loc3_ < this.levelsPerPage)
         {
            return _loc2_ + "-" + (_loc3_ + 1);
         }
         _loc4_ = 0;
         while(_loc4_ < this.pageIndexes.length)
         {
            if(this.pageIndexes[_loc4_] == _loc2_)
            {
               if(_loc4_ + 1 < this.pageIndexes.length)
               {
                  return this.pageIndexes[_loc4_ + 1] + "-" + 1;
               }
               break;
            }
            _loc4_++;
         }
         return null;
      }
      
      public function get name() : String
      {
         return this.mName;
      }
      
      public function set name(param1:String) : void
      {
         this.mName = param1;
      }
      
      public function get menuImage() : String
      {
         return this.§]E§;
      }
      
      public function set menuImage(param1:String) : void
      {
         this.§]E§ = param1;
      }
      
      public function get levelsPerPage() : int
      {
         return this.§]L§;
      }
      
      public function set levelsPerPage(param1:int) : void
      {
         this.§]L§ = param1;
      }
      
      public function get §["§() : Array
      {
         return this.§`P§;
      }
      
      public function get §5#§() : Array
      {
         return this.§7n§;
      }
      
      public function set §5#§(param1:Array) : void
      {
         this.§7n§ = param1;
      }
      
      public function get currentPage() : int
      {
         return this.§2H§;
      }
      
      public function set currentPage(param1:int) : void
      {
         if(param1 >= 0 && param1 < this.pageIndexes.length)
         {
            this.§2H§ = param1;
         }
      }
      
      public function get pageIndexes() : Array
      {
         return this.§[!>§;
      }
      
      public function set pageIndexes(param1:Array) : void
      {
         this.§[!>§ = param1;
      }
      
      public function §%!3§(param1:String, param2:String) : void
      {
         this.§#>§[param1] = param2;
      }
      
      public function §9Z§(param1:String) : String
      {
         return this.§#>§[param1];
      }
      
      public function get levelButtons() : Array
      {
         return this.§;h§;
      }
      
      public function set levelButtons(param1:Array) : void
      {
         this.§;h§ = param1;
      }
      
      public function get §9!<§() : String
      {
         return this.§'!-§;
      }
      
      public function set §9!<§(param1:String) : void
      {
         this.§'!-§ = param1;
      }
      
      public function get §2!A§() : String
      {
         return this.§8!>§;
      }
      
      public function set §2!A§(param1:String) : void
      {
         this.§8!>§ = param1;
      }
      
      public function get writtenName() : String
      {
         return this.§?!!§;
      }
      
      public function set writtenName(param1:String) : void
      {
         this.§?!!§ = param1;
      }
   }
}
