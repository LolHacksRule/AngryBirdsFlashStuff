package §%W§
{
   import §%I§.§?!P§;
   
   public class §1! §
   {
      
      public static const §4!I§:uint = 16777215;
      
      public static const §,!X§:uint = 12632256;
      
      public static const §>!G§:uint = 8421504;
      
      public static const §?%§:uint = 0;
      
      public static const §^3§:uint = 16711680;
      
      public static const §71§:uint = 8388608;
      
      public static const §!]§:uint = 16776960;
      
      public static const §<!2§:uint = 8421376;
      
      public static const §^7§:uint = 65280;
      
      public static const §40§:uint = 32768;
      
      public static const §#!0§:uint = 65535;
      
      public static const §@!#§:uint = 32896;
      
      public static const §9!^§:uint = 255;
      
      public static const §"f§:uint = 128;
      
      public static const §,t§:uint = 16711935;
      
      public static const §,k§:uint = 8388736;
       
      
      public function §1! §()
      {
         super();
         throw new §?!P§();
      }
      
      public static function §"!+§(param1:uint) : int
      {
         return param1 >> 24 & 255;
      }
      
      public static function §+a§(param1:uint) : int
      {
         return param1 >> 16 & 255;
      }
      
      public static function §'!5§(param1:uint) : int
      {
         return param1 >> 8 & 255;
      }
      
      public static function §6!1§(param1:uint) : int
      {
         return param1 & 255;
      }
      
      public static function §<W§(param1:int, param2:int, param3:int) : uint
      {
         return param1 << 16 | param2 << 8 | param3;
      }
      
      public static function §]y§(param1:int, param2:int, param3:int, param4:int) : uint
      {
         return param1 << 24 | param2 << 16 | param3 << 8 | param4;
      }
   }
}
