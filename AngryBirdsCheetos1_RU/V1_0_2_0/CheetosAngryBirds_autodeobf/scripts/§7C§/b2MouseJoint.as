package §7C§
{
   import §&!+§.b2Mat22;
   import §&!+§.b2Vec2;
   import §4!&§.b2Body;
   import §4!&§.b2TimeStep;
   import §8!%§.b2internal;
   
   use namespace b2internal;
   
   public class b2MouseJoint extends b2Joint
   {
       
      
      private var §!n§:b2Mat22;
      
      private var K1:b2Mat22;
      
      private var K2:b2Mat22;
      
      private var §0;§:b2Vec2;
      
      private var §'!R§:b2Vec2;
      
      private var §[!8§:b2Vec2;
      
      private var §[`§:b2Mat22;
      
      private var §6V§:b2Vec2;
      
      private var § !a§:Number;
      
      private var §?!6§:Number;
      
      private var §]!Y§:Number;
      
      private var §9!$§:Number;
      
      private var §,!9§:Number;
      
      public function b2MouseJoint(param1:b2MouseJointDef)
      {
         var _loc2_:Number = NaN;
         var _loc4_:b2Mat22 = null;
         this.§!n§ = new b2Mat22();
         this.K1 = new b2Mat22();
         this.K2 = new b2Mat22();
         this.§0;§ = new b2Vec2();
         this.§'!R§ = new b2Vec2();
         this.§[!8§ = new b2Vec2();
         this.§[`§ = new b2Mat22();
         this.§6V§ = new b2Vec2();
         super(param1);
         this.§'!R§.SetV(param1.target);
         _loc2_ = this.§'!R§.x - b2internal::[L.m_xf.position.x;
         var _loc3_:Number = this.§'!R§.y - b2internal::[L.m_xf.position.y;
         _loc4_ = b2internal::[L.m_xf.R;
         this.§0;§.x = _loc2_ * _loc4_.col1.x + _loc3_ * _loc4_.col1.y;
         this.§0;§.y = _loc2_ * _loc4_.col2.x + _loc3_ * _loc4_.col2.y;
         this.§ !a§ = param1.§=R§;
         this.§[!8§.§8Q§();
         this.§?!6§ = param1.§5!0§;
         this.§]!Y§ = param1.§,!=§;
         this.§9!$§ = 0;
         this.§,!9§ = 0;
      }
      
      override public function GetAnchorA() : b2Vec2
      {
         return this.§'!R§;
      }
      
      override public function GetAnchorB() : b2Vec2
      {
         return b2internal::[L.GetWorldPoint(this.§0;§);
      }
      
      override public function GetReactionForce(param1:Number) : b2Vec2
      {
         return new b2Vec2(param1 * this.§[!8§.x,param1 * this.§[!8§.y);
      }
      
      override public function GetReactionTorque(param1:Number) : Number
      {
         return 0;
      }
      
      public function §,!$§() : b2Vec2
      {
         return this.§'!R§;
      }
      
      public function §9!0§(param1:b2Vec2) : void
      {
         if(b2internal::[L.IsAwake() == false)
         {
            b2internal::[L.SetAwake(true);
         }
         this.§'!R§ = param1;
      }
      
      public function § !M§() : Number
      {
         return this.§ !a§;
      }
      
      public function §#!I§(param1:Number) : void
      {
         this.§ !a§ = param1;
      }
      
      public function § H§() : Number
      {
         return this.§?!6§;
      }
      
      public function §1!S§(param1:Number) : void
      {
         this.§?!6§ = param1;
      }
      
      public function §<>§() : Number
      {
         return this.§]!Y§;
      }
      
      public function § !`§(param1:Number) : void
      {
         this.§]!Y§ = param1;
      }
      
      override b2internal function InitVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc7_:b2Mat22 = null;
         var _loc11_:Number = NaN;
         var _loc12_:Number = NaN;
         var _loc2_:b2Body = b2internal::[L;
         var _loc3_:Number = _loc2_.GetMass();
         var _loc4_:Number = 2 * Math.PI * this.§?!6§;
         var _loc5_:Number = 2 * _loc3_ * this.§]!Y§ * _loc4_;
         var _loc6_:Number = _loc3_ * _loc4_ * _loc4_;
         this.§,!9§ = param1.§2!2§ * (_loc5_ + param1.§2!2§ * _loc6_);
         this.§,!9§ = this.§,!9§ != 0 ? Number(1 / this.§,!9§) : Number(0);
         this.§9!$§ = param1.§2!2§ * _loc6_ * this.§,!9§;
         _loc7_ = _loc2_.m_xf.R;
         var _loc8_:Number = this.§0;§.x - _loc2_.m_sweep.localCenter.x;
         var _loc9_:Number = this.§0;§.y - _loc2_.m_sweep.localCenter.y;
         var _loc10_:Number = _loc7_.col1.x * _loc8_ + _loc7_.col2.x * _loc9_;
         _loc9_ = _loc7_.col1.y * _loc8_ + _loc7_.col2.y * _loc9_;
         _loc8_ = _loc10_;
         _loc11_ = _loc2_.§=!P§;
         _loc12_ = _loc2_.§`!6§;
         this.K1.col1.x = _loc11_;
         this.K1.col2.x = 0;
         this.K1.col1.y = 0;
         this.K1.col2.y = _loc11_;
         this.K2.col1.x = _loc12_ * _loc9_ * _loc9_;
         this.K2.col2.x = -_loc12_ * _loc8_ * _loc9_;
         this.K2.col1.y = -_loc12_ * _loc8_ * _loc9_;
         this.K2.col2.y = _loc12_ * _loc8_ * _loc8_;
         this.§!n§.§2!7§(this.K1);
         this.§!n§.§1!^§(this.K2);
         this.§!n§.col1.x += this.§,!9§;
         this.§!n§.col2.y += this.§,!9§;
         this.§!n§.§<t§(this.§[`§);
         this.§6V§.x = _loc2_.m_sweep.c.x + _loc8_ - this.§'!R§.x;
         this.§6V§.y = _loc2_.m_sweep.c.y + _loc9_ - this.§'!R§.y;
         _loc2_.m_angularVelocity *= 0.98;
         this.§[!8§.x *= param1.§'7§;
         this.§[!8§.y *= param1.§'7§;
         _loc2_.§5+§.x += _loc11_ * this.§[!8§.x;
         _loc2_.§5+§.y += _loc11_ * this.§[!8§.y;
         _loc2_.m_angularVelocity += _loc12_ * (_loc8_ * this.§[!8§.y - _loc9_ * this.§[!8§.x);
      }
      
      override b2internal function SolveVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc3_:b2Mat22 = null;
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc2_:b2Body = b2internal::[L;
         _loc3_ = _loc2_.m_xf.R;
         var _loc6_:Number = this.§0;§.x - _loc2_.m_sweep.localCenter.x;
         var _loc7_:Number = this.§0;§.y - _loc2_.m_sweep.localCenter.y;
         _loc4_ = _loc3_.col1.x * _loc6_ + _loc3_.col2.x * _loc7_;
         _loc7_ = _loc3_.col1.y * _loc6_ + _loc3_.col2.y * _loc7_;
         _loc6_ = _loc4_;
         var _loc8_:Number = _loc2_.§5+§.x + -_loc2_.m_angularVelocity * _loc7_;
         var _loc9_:Number = _loc2_.§5+§.y + _loc2_.m_angularVelocity * _loc6_;
         _loc3_ = this.§[`§;
         _loc4_ = _loc8_ + this.§9!$§ * this.§6V§.x + this.§,!9§ * this.§[!8§.x;
         _loc5_ = _loc9_ + this.§9!$§ * this.§6V§.y + this.§,!9§ * this.§[!8§.y;
         var _loc10_:Number = -(_loc3_.col1.x * _loc4_ + _loc3_.col2.x * _loc5_);
         var _loc11_:Number = -(_loc3_.col1.y * _loc4_ + _loc3_.col2.y * _loc5_);
         var _loc12_:Number = this.§[!8§.x;
         var _loc13_:Number = this.§[!8§.y;
         this.§[!8§.x += _loc10_;
         this.§[!8§.y += _loc11_;
         var _loc14_:Number = param1.§2!2§ * this.§ !a§;
         if(this.§[!8§.§,=§() > _loc14_ * _loc14_)
         {
            this.§[!8§.§5!P§(_loc14_ / this.§[!8§.§8;§());
         }
         _loc10_ = this.§[!8§.x - _loc12_;
         _loc11_ = this.§[!8§.y - _loc13_;
         _loc2_.§5+§.x += _loc2_.§=!P§ * _loc10_;
         _loc2_.§5+§.y += _loc2_.§=!P§ * _loc11_;
         _loc2_.m_angularVelocity += _loc2_.§`!6§ * (_loc6_ * _loc11_ - _loc7_ * _loc10_);
      }
      
      override b2internal function SolvePositionConstraints(param1:Number) : Boolean
      {
         return true;
      }
   }
}
