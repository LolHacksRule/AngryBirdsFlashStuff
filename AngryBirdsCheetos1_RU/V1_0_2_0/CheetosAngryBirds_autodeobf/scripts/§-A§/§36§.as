package §-A§
{
   import §;z§.§!+§;
   
   public class §36§
   {
      
      public static const MIGHTY_EAGLE_USE_SHADE:Boolean = true;
      
      public static const MIGHTY_EAGLE_FLYING_ANGLE:Number = -22.9;
      
      public static const MIGHTY_EAGLE_FLYING_SPEED:Number = 0.128;
      
      public static const MIGHTY_EAGLE_ROTATION_SPEED:Number = 8;
      
      public static const MIGHTY_EAGLE_WAIT_TIME:Number = 2400;
      
      public static const MIGHTY_EAGLE_SOUND_DELAY:Number = 1000;
      
      public static const MIGHTY_EAGLE_SHADING_DELAY:Number = 2900;
      
      public static const MIGHTY_EAGLE_STARTING_DISTANCE_X:Number = 140;
      
      public static const MIGHTY_EAGLE_STARTING_DISTANCE_Y:Number = 140;
      
      public static const MIGHTY_EAGLE_Y_CHANGE:Number = Math.sin(§!+§.§4S§(MIGHTY_EAGLE_FLYING_ANGLE));
      
      public static const MIGHTY_EAGLE_X_DIRECTION:Number = 1;
      
      public static const § !0§:Number = 1500;
      
      public static const §9!S§:Number = 0.7;
      
      public static const §9!I§:Number = 0.2;
      
      public static const §;!I§:Number = 8.8;
      
      public static const §?'§:Number = 2400;
      
      public static const §13§:Number = 1;
      
      public static const §!!K§:Number = 0.15;
      
      public static const §,k§:Number = 4;
      
      public static const §=!V§:Number = 60;
      
      public static const §44§:Number = 16;
      
      public static const §&?§:Number = 6 * §44§;
      
      public static const SARDINE_CAN_INITIAL_ROTATION_SPEED:Number = 0.07;
      
      public static const SARDINE_CAN_ROTATION_ACCELERATION:Number = 0.0002;
      
      public static const SARDINE_CAN_MAX_ROTATION_SPEED:Number = SARDINE_CAN_INITIAL_ROTATION_SPEED * 4.4;
      
      public static const SARDINE_CAN_DELAY_AFTER_HIT:int = 3000;
      
      public static const §8g§:Number = 5;
      
      public static var §1!I§:int = 2;
      
      public static var §4J§:Boolean = false;
      
      public static const §1!%§:Number = 600;
      
      public static const §,!H§:Number = 1.3;
      
      public static const §[u§:Number = 1.3;
      
      public static const §3I§:Number = 1.3;
      
      public static const §1p§:Number = 1.3;
      
      public static const §%3§:Number = 1.3;
      
      public static const §+!J§:Number = 1.3;
      
      public static const § -§:Number = 1.5;
      
      public static var §0T§:int = 60;
      
      public static const §+l§:Number = 9;
      
      public static const §<!]§:Number = 5;
      
      public static const §2?§:Number = 3000;
      
      public static const §%!#§:Number = 60;
      
      public static const §7'§:Number = 6;
      
      public static const §;@§:Number = 3;
      
      public static const §9!Y§:Number = 1 / 30;
      
      public static const §%!!§:int = 100;
      
      public static const §-I§:int = 3;
      
      public static const §'!_§:Number = 70;
      
      public static const §65§:Number = 10;
      
      public static const §?!Q§:Number = 1000;
      
      public static const §"!M§:Number = 1.3;
      
      public static const §#m§:Number = 7.5;
      
      public static const §;^§:Number = 300;
      
      public static const §,!?§:Number = 0.5;
      
      public static const §@s§:Number = 6;
      
      public static const §!9§:Number = 9;
      
      public static const §1#§:Number = 0.15;
      
      public static const §'!1§:Number = 10;
       
      
      public function §36§()
      {
         super();
      }
   }
}
