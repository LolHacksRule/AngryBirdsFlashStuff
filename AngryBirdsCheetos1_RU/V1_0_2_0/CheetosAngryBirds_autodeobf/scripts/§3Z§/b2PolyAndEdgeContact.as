package §3Z§
{
   import § y§.b2EdgeShape;
   import § y§.b2PolygonShape;
   import § y§.b2Shape;
   import §&!+§.b2Transform;
   import §4!&§.b2Body;
   import §4!&§.b2Fixture;
   import §8!%§.b2Settings;
   import §8!%§.b2internal;
   import §<!^§.b2Manifold;
   
   use namespace b2internal;
   
   public class b2PolyAndEdgeContact extends b2Contact
   {
       
      
      public function b2PolyAndEdgeContact()
      {
         super();
      }
      
      public static function §?j§(param1:*) : b2Contact
      {
         return new b2PolyAndEdgeContact();
      }
      
      public static function §4c§(param1:b2Contact, param2:*) : void
      {
      }
      
      public function §]!S§(param1:b2Fixture, param2:b2Fixture) : void
      {
         super.§]!S§(param1,param2);
         b2Settings.b2Assert(param1.§?!S§() == b2Shape.§7F§);
         b2Settings.b2Assert(param2.§?!S§() == b2Shape.§1!C§);
      }
      
      override b2internal function Evaluate() : void
      {
         var _loc1_:b2Body = b2internal::>^.GetBody();
         var _loc2_:b2Body = b2internal::;!Z.GetBody();
         this.§=m§(b2internal::2!=,b2internal::>^.GetShape() as b2PolygonShape,_loc1_.m_xf,b2internal::;!Z.GetShape() as b2EdgeShape,_loc2_.m_xf);
      }
      
      private function §=m§(param1:b2Manifold, param2:b2PolygonShape, param3:b2Transform, param4:b2EdgeShape, param5:b2Transform) : void
      {
      }
   }
}
