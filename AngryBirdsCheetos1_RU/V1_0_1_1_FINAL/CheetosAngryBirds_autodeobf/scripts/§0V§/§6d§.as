package §0V§
{
   import §3d§.§20§;
   
   public class §6d§
   {
      
      public static const MIGHTY_EAGLE_USE_SHADE:Boolean = true;
      
      public static const MIGHTY_EAGLE_FLYING_ANGLE:Number = -22.9;
      
      public static const MIGHTY_EAGLE_FLYING_SPEED:Number = 0.128;
      
      public static const MIGHTY_EAGLE_ROTATION_SPEED:Number = 8;
      
      public static const MIGHTY_EAGLE_WAIT_TIME:Number = 2400;
      
      public static const MIGHTY_EAGLE_SOUND_DELAY:Number = 1000;
      
      public static const MIGHTY_EAGLE_SHADING_DELAY:Number = 2900;
      
      public static const MIGHTY_EAGLE_STARTING_DISTANCE_X:Number = 140;
      
      public static const MIGHTY_EAGLE_STARTING_DISTANCE_Y:Number = 140;
      
      public static const MIGHTY_EAGLE_Y_CHANGE:Number = Math.sin(§20§.§6b§(MIGHTY_EAGLE_FLYING_ANGLE));
      
      public static const MIGHTY_EAGLE_X_DIRECTION:Number = 1;
      
      public static const §]m§:Number = 1500;
      
      public static const §5!]§:Number = 0.7;
      
      public static const §]P§:Number = 0.2;
      
      public static const §2!U§:Number = 8.8;
      
      public static const §0!%§:Number = 2400;
      
      public static const §[F§:Number = 1;
      
      public static const §<7§:Number = 0.15;
      
      public static const §6;§:Number = 4;
      
      public static const §+S§:Number = 60;
      
      public static const §,x§:Number = 16;
      
      public static const §,U§:Number = 6 * §,x§;
      
      public static const SARDINE_CAN_INITIAL_ROTATION_SPEED:Number = 0.07;
      
      public static const SARDINE_CAN_ROTATION_ACCELERATION:Number = 0.0002;
      
      public static const SARDINE_CAN_MAX_ROTATION_SPEED:Number = SARDINE_CAN_INITIAL_ROTATION_SPEED * 4.4;
      
      public static const SARDINE_CAN_DELAY_AFTER_HIT:int = 3000;
      
      public static const §`#§:Number = 5;
      
      public static var §@!H§:int = 2;
      
      public static var §5!§:Boolean = false;
      
      public static const §"!<§:Number = 600;
      
      public static const §6@§:Number = 1.3;
      
      public static const §#!G§:Number = 1.3;
      
      public static const §32§:Number = 1.3;
      
      public static const §%6§:Number = 1.3;
      
      public static const §`E§:Number = 1.3;
      
      public static const §%^§:Number = 1.3;
      
      public static const §3!W§:Number = 1.5;
      
      public static const §^u§:Number = 1.4;
      
      public static var §^!5§:int = 60;
      
      public static const §=m§:Number = 9;
      
      public static const §+M§:Number = 5;
      
      public static const §-G§:Number = 3000;
      
      public static const §2S§:Number = 60;
      
      public static const §^?§:Number = 6;
      
      public static const §3!'§:Number = 3;
      
      public static const §<^§:Number = 1 / 30;
      
      public static const §=s§:int = 100;
      
      public static const §9=§:int = 3;
      
      public static const §7![§:Number = 70;
      
      public static const §1"§:Number = 10;
      
      public static const §@#§:Number = 1000;
      
      public static const §&P§:Number = 1.3;
      
      public static const §1!O§:Number = 7.5;
      
      public static const §+L§:Number = 300;
      
      public static const §#!;§:Number = 0.5;
      
      public static const §?!<§:Number = 6;
      
      public static const §%$§:Number = 9;
      
      public static const §]l§:Number = 0.15;
      
      public static const §5!=§:Number = 10;
       
      
      public function §6d§()
      {
         super();
      }
   }
}
