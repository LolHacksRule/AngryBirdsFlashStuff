package §@F§
{
   ...
   public class §'!]§
   {
      ...
      private function §9v§(param1:*) : String
      {
         ...
         if(param1 is Array)
         {
            return this.§7x§(param1 as Array);
         }
      ...
      private function §7x§(param1:Array) : String
      {
         var _loc2_:* = "";
         var _loc3_:int = 0;
         while(_loc3_ < param1.length)
         {
            ...
            _loc2_ += this.§9v§(param1[_loc3_]);
            _loc3_++;
         }
         return "[" + _loc2_ + "]";
      }
      
      private function §4!C§(param1:Object) : String
      {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§&!"§(v.@name.toString()) + ":" + this.§9v§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
