package §&V§
{
   import §%?§.Texture;
   import §%?§.§^!9§;
   import §@!%§.§+8§;
   import §`6§.§'o§;
   import §`6§.§-6§;
   import flash.display.BitmapData;
   import flash.events.EventDispatcher;
   import flash.geom.Point;
   import flash.geom.Rectangle;
   import flash.utils.Dictionary;
   
   public class §,w§ extends EventDispatcher
   {
      
      private static const §,7§:int = 2;
      
      private static const §->§:int = 2048;
      
      private static var §^H§:§,w§;
       
      
      private var §'i§:Dictionary;
      
      private var §1q§:Vector.<§'o§>;
      
      private var §"!1§:Dictionary;
      
      private var §!!6§:Boolean;
      
      private var §7K§:String;
      
      private var §&s§:Texture;
      
      private var §%!G§:int;
      
      private var §4q§:int;
      
      public function §,w§(param1:String = null)
      {
         super();
         this.§1q§ = new Vector.<§'o§>();
         this.§'i§ = new Dictionary();
         this.§"!1§ = new Dictionary();
         this.§7K§ = param1;
      }
      
      public static function get §5!B§() : §,w§
      {
         if(!§^H§)
         {
            §^H§ = new §,w§();
         }
         return §^H§;
      }
      
      public function get §&!$§() : int
      {
         return this.§%!G§;
      }
      
      public function get §@^§() : int
      {
         return this.§4q§;
      }
      
      public function get id() : String
      {
         return this.§7K§;
      }
      
      public function dispose() : void
      {
         var _loc1_:§;Q§ = null;
         var _loc2_:§'o§ = null;
         if(this.§'i§)
         {
            for each(_loc1_ in this.§'i§)
            {
               this.§,?§(_loc1_.texture.parent);
               _loc1_.dispose();
            }
            this.§'i§ = null;
         }
         if(this.§1q§)
         {
            for each(_loc2_ in this.§1q§)
            {
               _loc2_.dispose();
            }
            this.§1q§ = null;
         }
         if(this.§"!1§)
         {
            this.§"!1§ = null;
         }
         if(this.§&s§)
         {
            this.§&s§.dispose();
            this.§&s§ = null;
         }
         this.§1q§ = new Vector.<§'o§>();
         this.§'i§ = new Dictionary();
         this.§"!1§ = new Dictionary();
      }
      
      public function §[K§(param1:§'o§) : void
      {
         if(this.§!!6§)
         {
            return;
         }
         if(this.§1q§.indexOf(param1) >= 0)
         {
            return;
         }
         this.§1q§.push(param1);
      }
      
      private function §8!F§(param1:Array) : Rectangle
      {
         var _loc9_:§'o§ = null;
         var _loc10_:BitmapData = null;
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc6_:int = 0;
         var _loc7_:Number = 1;
         var _loc8_:int = 0;
         while(_loc8_ < param1.length)
         {
            _loc9_ = param1[_loc8_];
            if(_loc8_ == 0)
            {
               _loc7_ = _loc9_.scale;
            }
            else if(_loc9_.scale != _loc7_)
            {
               break;
            }
            _loc10_ = _loc9_.bitmapData;
            if(_loc2_ + _loc10_.width + §,7§ >= §->§)
            {
               _loc2_ = 0;
               _loc3_ += _loc6_;
               _loc6_ = 0;
            }
            if(_loc3_ + _loc10_.height + §,7§ >= §->§)
            {
               break;
            }
            _loc2_ += _loc10_.width + §,7§;
            if(_loc2_ > _loc4_)
            {
               _loc4_ = _loc2_;
            }
            if(_loc10_.height + §,7§ > _loc6_)
            {
               _loc6_ = _loc10_.height + §,7§;
               _loc5_ = _loc3_ + _loc6_;
            }
            _loc8_++;
         }
         return new Rectangle(0,0,_loc4_,_loc5_);
      }
      
      private function §>!§(param1:Array, param2:BitmapData) : Array
      {
         var _loc8_:§'o§ = null;
         var _loc9_:BitmapData = null;
         var _loc10_:§66§ = null;
         var _loc11_:int = 0;
         var _loc12_:int = 0;
         var _loc13_:§-6§ = null;
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc6_:Array = [];
         var _loc7_:Number = NaN;
         while(param1.length > 0)
         {
            _loc8_ = param1[0];
            if(!isNaN(_loc7_) && _loc7_ != _loc8_.scale)
            {
               break;
            }
            _loc7_ = _loc8_.scale;
            _loc9_ = _loc8_.bitmapData;
            if(_loc3_ + _loc9_.width + §,7§ > param2.width)
            {
               _loc3_ = 0;
               _loc4_ += _loc5_;
               _loc5_ = 0;
            }
            if(_loc4_ + _loc9_.height + §,7§ > param2.height)
            {
               break;
            }
            param2.copyPixels(_loc9_,_loc9_.rect,new Point(_loc3_,_loc4_),null,null,true);
            (_loc10_ = new §66§()).offset = new Point(_loc3_,_loc4_);
            _loc10_.§0c§ = _loc8_;
            _loc6_.push(_loc10_);
            _loc11_ = _loc8_.§%u§;
            _loc12_ = 0;
            while(_loc12_ < _loc11_)
            {
               (_loc13_ = _loc8_.§-!B§(_loc12_)).§4"§ = null;
               _loc13_.§-!E§.offset(_loc3_,_loc4_);
               _loc12_++;
            }
            _loc3_ += _loc9_.width + §,7§;
            if(_loc9_.height + §,7§ > _loc5_)
            {
               _loc5_ = _loc9_.height + §,7§;
            }
            param1.splice(0,1);
         }
         return _loc6_;
      }
      
      public function §const§() : Boolean
      {
         var _loc1_:§'o§ = null;
         var _loc3_:Rectangle = null;
         var _loc4_:BitmapData = null;
         var _loc5_:Array = null;
         var _loc6_:Texture = null;
         var _loc7_:int = 0;
         var _loc8_:§66§ = null;
         var _loc9_:int = 0;
         var _loc10_:int = 0;
         var _loc11_:§-6§ = null;
         if(!§+8§.§#!<§())
         {
            return false;
         }
         if(this.§!!6§)
         {
            return true;
         }
         var _loc2_:Array = [];
         for each(_loc1_ in this.§1q§)
         {
            if(_loc1_.§%u§ > 0)
            {
               _loc2_.push(_loc1_);
            }
         }
         while(_loc2_.length > 0)
         {
            _loc3_ = this.§8!F§(_loc2_);
            _loc4_ = new BitmapData(_loc3_.width,_loc3_.height,true,0);
            if((_loc5_ = this.§>!§(_loc2_,_loc4_)).length == 0)
            {
               throw new Error("Found too large sprite sheet image!");
            }
            _loc6_ = this.§[[§(_loc4_);
            _loc7_ = 0;
            while(_loc7_ < _loc5_.length)
            {
               _loc1_ = (_loc8_ = _loc5_[_loc7_]).§0c§;
               _loc9_ = _loc1_.§%u§;
               _loc10_ = 0;
               while(_loc10_ < _loc9_)
               {
                  _loc11_ = _loc1_.§-!B§(_loc10_);
                  this.§`L§(_loc11_,_loc6_,_loc4_,_loc1_.scale);
                  _loc10_++;
               }
               _loc7_++;
            }
         }
         for each(_loc1_ in this.§1q§)
         {
            _loc1_.dispose();
         }
         this.§1q§ = new Vector.<§'o§>();
         this.§!!6§ = true;
         return true;
      }
      
      private function §`L§(param1:§-6§, param2:Texture, param3:BitmapData, param4:Number) : void
      {
         var _loc6_:Rectangle = null;
         var _loc7_:§^!9§ = null;
         var _loc5_:§;Q§;
         if(!(_loc5_ = this.§'i§[param1.mName]))
         {
            _loc6_ = param1.§-!E§.clone();
            _loc7_ = new §^!9§(param2,_loc6_,false);
            _loc5_ = new §;Q§(_loc7_,param3,_loc6_,param1.§#0§,param1.§7I§,param4);
            this.§'i§[param1.mName] = _loc5_;
         }
      }
      
      public function §!!7§() : void
      {
         var bd:Object = null;
         var texture:Texture = null;
         if(!§+8§.§3Q§)
         {
            for(bd in this.§"!1§)
            {
               try
               {
                  texture = this.§"!1§[bd];
                  texture.requestBaseTextureUpdate(bd as BitmapData);
               }
               catch(e:Error)
               {
               }
            }
         }
      }
      
      public function §]i§(param1:String) : §;Q§
      {
         return this.§'i§[param1];
      }
      
      public function §[[§(param1:BitmapData, param2:Boolean = true) : Texture
      {
         var _loc3_:Texture = this.§"!1§[param1];
         if(_loc3_)
         {
            return _loc3_;
         }
         _loc3_ = §+8§.§5D§(param1,param2);
         this.§"!1§[param1] = _loc3_;
         this.§`!5§(_loc3_,param1,true);
         return _loc3_;
      }
      
      public function §`!5§(param1:Texture, param2:BitmapData, param3:Boolean) : void
      {
         var _loc5_:* = 0;
         var _loc6_:* = 0;
         var _loc4_:int = 1;
         if(!param3)
         {
            _loc4_ = -1;
         }
         if(param2)
         {
            this.§4q§ += _loc4_ * param2.width * param2.height * 4;
         }
         if(param1)
         {
            _loc5_ = int(param1.width);
            _loc6_ = int(param1.height);
            while(_loc5_ >= 1 && _loc6_ >= 1)
            {
               this.§%!G§ += _loc4_ * _loc5_ * _loc6_ * 4;
               _loc5_ >>= 1;
               _loc6_ >>= 1;
            }
         }
      }
      
      public function §#w§() : Texture
      {
         if(!this.§&s§)
         {
            this.§&s§ = this.§[[§(new BitmapData(40,40,false,16711935));
         }
         return this.§&s§;
      }
      
      public function §,?§(param1:Texture) : void
      {
         var _loc2_:* = null;
         var _loc3_:BitmapData = null;
         for(_loc2_ in this.§"!1§)
         {
            if(this.§"!1§[_loc2_] == param1)
            {
               _loc3_ = _loc2_ as BitmapData;
               this.§`!5§(param1,_loc3_,false);
               if(_loc3_)
               {
                  _loc3_.dispose();
               }
               delete this.§"!1§[_loc2_];
               param1.dispose();
               return;
            }
         }
      }
      
      public function §0r§(param1:String) : BitmapData
      {
         var _loc2_:§;Q§ = this.§]i§(param1);
         if(_loc2_ == null)
         {
            _loc2_ = this.§]i§(param1 + "_1");
         }
         if(_loc2_ == null)
         {
            _loc2_ = this.§]i§(param1 + "_01");
         }
         if(_loc2_ == null)
         {
            return null;
         }
         return _loc2_.bitmapData;
      }
   }
}
