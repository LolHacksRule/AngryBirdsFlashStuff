package §3!-§
{
   ...
   public class §%E§
   {
      ...
      private function §6! §(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§<"E§(param1 as String);
         }
         if(param1 is Number)
         {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§<"E§(v.@name.toString()) + ":" + this.§6! §(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
