package §?"@§
{
   import §&!]§.§#[§;
   import §&!]§.DisplayObject;
   import §&!]§.Sprite;
   import §'!B§.§8T§;
   import §0i§.§""F§;
   import §3"#§.§45§;
   import §3"#§.§`"8§;
   import §9!v§.§"[§;
   import §9!v§.§6a§;
   import §9!v§.§8M§;
   import §;"0§.§+!E§;
   import §;"0§.§="4§;
   import §`'§.§7!p§;
   import flash.geom.Rectangle;
   
   public class §+;§
   {
       
      
      private var §-!l§:int;
      
      private var §0@§:Number;
      
      private var §,q§:Boolean;
      
      protected var §5"'§:Sprite;
      
      private var §^!;§:Array;
      
      private var §>$§:Boolean = true;
      
      private var §@k§:§6a§;
      
      private var §=!t§:Sprite;
      
      private var §4z§:Sprite;
      
      private var §,"@§:Number;
      
      private var §-n§:Number;
      
      protected var §&!A§:Number = 1.0;
      
      protected var §;;§:Number = 0.0;
      
      protected var §6"C§:Number = 0.0;
      
      protected var §!"-§:Number = 0.0;
      
      protected var §]!d§:Number = 0.0;
      
      protected var §0!3§:Number = 0.0;
      
      protected var §"T§:Number = 0.0;
      
      protected var §-x§:Boolean;
      
      public function §+;§(param1:§="4§, param2:Sprite, param3:§6a§, param4:Number)
      {
         this.§^!;§ = [];
         super();
         this.§@k§ = param3;
         this.§5"'§ = param2;
         this.§0@§ = param1.speed;
         this.§,q§ = param1.foreground;
         this.initialize(param1,param4);
      }
      
      public function get §#!@§() : Number
      {
         return this.§0@§;
      }
      
      public function get §@P§() : Boolean
      {
         return this.§,q§;
      }
      
      protected function get §-!@§() : Number
      {
         return this.§-!l§;
      }
      
      protected function initializePivotFromTexture(param1:§"[§) : void
      {
         this.§6"C§ = param1.pivotY;
         if(!this.§,q§)
         {
            this.§;;§ = param1.pivotX;
         }
         else
         {
            this.§;;§ = 0;
         }
      }
      
      protected function initializePivotFromComposite(param1:DisplayObject) : void
      {
         var _loc2_:Rectangle = param1.§#!A§;
         this.§6"C§ = _loc2_.bottom;
         this.§;;§ = (_loc2_.left + _loc2_.right) / 2;
      }
      
      protected function §5"7§(param1:String, param2:Boolean, param3:Number) : void
      {
         var _loc5_:§"[§ = null;
         var _loc4_:DisplayObject;
         if(!(_loc4_ = §8M§.§;Q§(param1,this.§@k§,param2)))
         {
            if(!(_loc5_ = this.§@k§.getTexture(param1)))
            {
               return;
            }
            this.§&!A§ = param3 * _loc5_.scale;
            this.initializePivotFromTexture(_loc5_);
            this.§-!l§ = _loc5_.width - 2;
         }
         else
         {
            this.§&!A§ = param3;
            this.initializePivotFromComposite(_loc4_);
            this.§-!l§ = _loc4_.width - 2;
            _loc4_.dispose();
         }
         this.§-!l§ = Math.round(this.§-!l§);
      }
      
      protected function §-"@§(param1:String, param2:Boolean, param3:int, param4:int) : Sprite
      {
         var _loc7_:DisplayObject = null;
         var _loc8_:Sprite = null;
         var _loc5_:Sprite = new Sprite();
         var _loc6_:int = param3;
         while(_loc6_ <= param4)
         {
            (_loc7_ = this.§1`§(param1,param2)).x = _loc6_ * this.§-!l§;
            _loc7_.y = 0;
            _loc7_.scaleX = 1;
            _loc7_.scaleY = 1;
            _loc5_.addChild(_loc7_);
            _loc6_++;
         }
         if(§8M§.§;^§(param1))
         {
            _loc8_ = this.§2!E§(_loc5_);
            _loc5_.§7!-§();
            _loc5_.dispose();
            (_loc5_ = _loc8_).flatten();
         }
         return _loc5_;
      }
      
      private function §2!E§(param1:Sprite) : Sprite
      {
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc6_:Sprite = null;
         var _loc7_:DisplayObject = null;
         var _loc2_:Sprite = new Sprite();
         if(param1.numChildren > 0 && param1.getChildAt(0) is Sprite)
         {
            _loc3_ = Sprite(param1.getChildAt(0)).numChildren;
            _loc4_ = _loc3_ - 1;
            while(_loc4_ >= 0)
            {
               _loc5_ = 0;
               while(_loc5_ < param1.numChildren)
               {
                  if(_loc6_ = param1.getChildAt(_loc5_) as Sprite)
                  {
                     _loc7_ = _loc6_.getChildAt(_loc4_);
                     _loc2_.addChildAt(_loc7_,0);
                     _loc7_.x += _loc6_.x;
                  }
                  _loc5_++;
               }
               _loc4_--;
            }
         }
         return _loc2_;
      }
      
      protected function §1`§(param1:String, param2:Boolean) : DisplayObject
      {
         var _loc4_:§"[§ = null;
         var _loc3_:DisplayObject = §8M§.§;Q§(param1,this.§@k§,param2);
         if(!_loc3_)
         {
            _loc4_ = this.§@k§.getTexture(param1);
            _loc3_ = new §#[§(_loc4_.texture,false,param2);
         }
         return _loc3_;
      }
      
      protected function getRepeatCount(param1:Number, param2:int) : int
      {
         return (2 + §`"8§.§3q§ * 1.5 / (param2 * param1)) * 2;
      }
      
      protected function initialize(param1:§="4§, param2:Number) : void
      {
         var _loc7_:int = 0;
         if(param1.scale != 0)
         {
            this.§&!A§ = param1.scale;
         }
         else
         {
            this.§&!A§ = 1;
         }
         this.§5"7§(param1.spriteName,param1.highQuality,this.§&!A§);
         if(this.§-!l§ <= 0)
         {
            return;
         }
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         if(param1.tileable)
         {
            _loc3_ = -(_loc7_ = this.getRepeatCount(param2,this.§-!l§)) / 2 - 1;
            _loc4_ = -_loc3_ - 1;
            this.§-x§ = true;
         }
         this.§0!3§ = param1.xOffset;
         this.§"T§ = param1.yOffset;
         var _loc5_:int = _loc3_;
         while(_loc5_ <= _loc4_)
         {
            this.§!![§(param1,_loc5_ * this.§-!l§);
            _loc5_++;
         }
         var _loc6_:Sprite = this.§-"@§(param1.spriteName,param1.highQuality,_loc3_,_loc4_);
         this.§5"'§.addChild(_loc6_);
         this.§`!o§();
         this.§<!j§();
      }
      
      private function §`!o§() : void
      {
         if(this.§=!t§)
         {
            if(this.§=!t§.numChildren > 0)
            {
               this.§5"'§.addChild(this.§=!t§);
            }
            else
            {
               this.§=!t§.dispose();
               this.§=!t§ = null;
            }
         }
         if(this.§4z§)
         {
            if(this.§4z§.numChildren > 0)
            {
               this.§5"'§.addChildAt(this.§4z§,0);
            }
            else
            {
               this.§4z§.dispose();
               this.§4z§ = null;
            }
         }
      }
      
      private function §!![§(param1:§="4§, param2:Number) : void
      {
         var _loc4_:§+!E§ = null;
         var _loc5_:§45§ = null;
         var _loc6_:int = 0;
         var _loc3_:int = 0;
         while(_loc3_ < param1.§-"5§)
         {
            _loc4_ = param1.§=<§(_loc3_);
            if((_loc5_ = §""F§.§[!R§(_loc4_.id,this.§@k§)) && _loc5_.§2>§ > 0)
            {
               _loc5_.x = param2 + _loc4_.x + param1.xOffset;
               _loc5_.y = _loc4_.y;
               _loc5_.scaleX = _loc5_.scaleY = _loc4_.scale;
               _loc5_.rotation = _loc4_.rotation;
               _loc5_.alpha = _loc4_.alpha;
               _loc5_.start();
               §8T§.§?!c§.add(_loc5_);
               this.§^!;§.push(_loc5_);
               if(_loc4_.§6!B§)
               {
                  if(!this.§4z§)
                  {
                     this.§4z§ = new Sprite();
                  }
                  this.§4z§.addChild(_loc5_);
               }
               else
               {
                  if(!this.§=!t§)
                  {
                     this.§=!t§ = new Sprite();
                  }
                  this.§=!t§.addChild(_loc5_);
               }
               _loc6_ = 0;
               while(_loc6_ < _loc4_.fastForwardsAfterAdd)
               {
                  _loc5_.§4!p§(1 / 20);
                  _loc6_++;
               }
            }
            else if(_loc5_)
            {
               _loc5_.dispose();
            }
            _loc3_++;
         }
      }
      
      public function dispose() : void
      {
         var _loc1_:§7!p§ = null;
         this.§5"'§.§7!-§(0,-1,true);
         for each(_loc1_ in this.§^!;§)
         {
            §8T§.§?!c§.§>!0§(_loc1_);
            _loc1_.dispose();
         }
         this.§^!;§ = [];
         this.§4z§ = null;
         this.§=!t§ = null;
      }
      
      public function setScreenOffset(param1:Number, param2:Number, param3:Number, param4:Number, param5:Number, param6:Number, param7:Number) : void
      {
         this.§,"@§ = param1;
         this.§-n§ = param2;
         this.§5"'§.scaleX = this.§&!A§;
         this.§5"'§.scaleY = this.§&!A§;
         this.§5"'§.x = -param1 * this.§0@§ - this.§;;§ * this.§&!A§ + this.§0!3§;
         this.§5"'§.y = -param2 - this.§6"C§ * this.§&!A§ + this.§"T§;
      }
      
      private function §<!j§() : void
      {
         var _loc1_:§7!p§ = null;
         for each(_loc1_ in this.§^!;§)
         {
            §8T§.§?!c§.add(_loc1_);
            _loc1_.visible = true;
         }
      }
      
      private function §9"+§() : void
      {
         var _loc1_:§7!p§ = null;
         for each(_loc1_ in this.§^!;§)
         {
            §8T§.§?!c§.§>!0§(_loc1_);
            _loc1_.visible = false;
         }
      }
      
      public function §7!+§(param1:Boolean) : void
      {
         if(param1 == this.§>$§)
         {
            return;
         }
         this.§>$§ = param1;
         if(this.§>$§)
         {
            this.§<!j§();
         }
         else
         {
            this.§9"+§();
         }
      }
   }
}
