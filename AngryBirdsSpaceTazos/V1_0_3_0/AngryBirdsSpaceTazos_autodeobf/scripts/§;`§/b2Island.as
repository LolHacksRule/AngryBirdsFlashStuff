package §;`§
{
   import §'!^§.*;
   import §8K§.*;
   import §>!m§.*;
   import §?"#§.*;
   import §[!f§.*;
   
   use namespace b2internal;
   
   public class b2Island
   {
      
      private static var §-!1§:b2ContactImpulse = new b2ContactImpulse();
       
      
      private var §2'§;
      
      private var §3z§:b2ContactListener;
      
      private var §`A§:b2ContactSolver;
      
      b2internal var §="D§:Vector.<b2Body>;
      
      b2internal var §#I§:Vector.<b2Contact>;
      
      b2internal var §;">§:Vector.<b2Joint>;
      
      b2internal var §9!n§:int;
      
      b2internal var §2y§:int;
      
      b2internal var §1A§:int;
      
      private var §&"'§:int;
      
      b2internal var §%P§:int;
      
      b2internal var §""D§:int;
      
      public function b2Island()
      {
         super();
         this.§="D§ = new Vector.<b2Body>();
         this.§#I§ = new Vector.<b2Contact>();
         this.§;">§ = new Vector.<b2Joint>();
      }
      
      public function §'m§(param1:int, param2:int, param3:int, param4:*, param5:b2ContactListener, param6:b2ContactSolver) : void
      {
         var _loc7_:int = 0;
         this.§&"'§ = param1;
         this.§%P§ = param2;
         this.§""D§ = param3;
         this.§9!n§ = 0;
         this.§1A§ = 0;
         this.§2y§ = 0;
         this.§2'§ = param4;
         this.§3z§ = param5;
         this.§`A§ = param6;
         _loc7_ = this.§="D§.length;
         while(_loc7_ < param1)
         {
            this.§="D§[_loc7_] = null;
            _loc7_++;
         }
         _loc7_ = this.§#I§.length;
         while(_loc7_ < param2)
         {
            this.§#I§[_loc7_] = null;
            _loc7_++;
         }
         _loc7_ = this.§;">§.length;
         while(_loc7_ < param3)
         {
            this.§;">§[_loc7_] = null;
            _loc7_++;
         }
      }
      
      public function §""G§() : void
      {
         this.§9!n§ = 0;
         this.§1A§ = 0;
         this.§2y§ = 0;
      }
      
      public function §][§(param1:b2TimeStep, param2:b2Vec2, param3:Boolean) : void
      {
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc6_:b2Body = null;
         var _loc7_:b2Joint = null;
         var _loc9_:Number = NaN;
         var _loc10_:Number = NaN;
         var _loc11_:Number = NaN;
         var _loc12_:Boolean = false;
         var _loc13_:Boolean = false;
         var _loc14_:Boolean = false;
         var _loc15_:Number = NaN;
         var _loc16_:Number = NaN;
         var _loc17_:Number = NaN;
         _loc4_ = 0;
         while(_loc4_ < this.§9!n§)
         {
            if((_loc6_ = this.§="D§[_loc4_]).§^![§() == b2Body.b2_dynamicBody)
            {
               _loc6_.m_sweep.c0.SetV(_loc6_.m_sweep.c);
               _loc6_.m_sweep.a0 = _loc6_.m_sweep.a;
               _loc6_.§0!'§.x += param1.§;!!§ * (param2.x + _loc6_.§9K§ * _loc6_.§5!@§.x);
               _loc6_.§0!'§.y += param1.§;!!§ * (param2.y + _loc6_.§9K§ * _loc6_.§5!@§.y);
               _loc6_.m_angularVelocity += param1.§;!!§ * _loc6_.§%"+§ * _loc6_.§]"@§;
               _loc6_.§0!'§.§7!J§(b2Math.§>!]§(1 - param1.§;!!§ * _loc6_.§,B§,0,1));
               _loc6_.m_angularVelocity *= b2Math.§>!]§(1 - param1.§;!!§ * _loc6_.§+!]§,0,1);
            }
            _loc4_++;
         }
         this.§`A§.§'m§(param1,this.§#I§,this.§1A§,this.§2'§);
         var _loc8_:b2ContactSolver;
         (_loc8_ = this.§`A§).InitVelocityConstraints(param1);
         _loc4_ = 0;
         while(_loc4_ < this.§2y§)
         {
            (_loc7_ = this.§;">§[_loc4_]).InitVelocityConstraints(param1);
            _loc4_++;
         }
         _loc4_ = 0;
         while(_loc4_ < param1.§6!B§)
         {
            _loc5_ = 0;
            while(_loc5_ < this.§2y§)
            {
               (_loc7_ = this.§;">§[_loc5_]).SolveVelocityConstraints(param1);
               _loc5_++;
            }
            _loc8_.SolveVelocityConstraints();
            _loc4_++;
         }
         _loc4_ = 0;
         while(_loc4_ < this.§2y§)
         {
            (_loc7_ = this.§;">§[_loc4_]).§7!V§();
            _loc4_++;
         }
         _loc8_.§7!V§();
         _loc4_ = 0;
         while(_loc4_ < this.§9!n§)
         {
            if((_loc6_ = this.§="D§[_loc4_]).§^![§() != b2Body.b2_staticBody)
            {
               _loc9_ = param1.§;!!§ * _loc6_.§0!'§.x;
               _loc10_ = param1.§;!!§ * _loc6_.§0!'§.y;
               if(_loc9_ * _loc9_ + _loc10_ * _loc10_ > b2Settings.b2_maxTranslationSquared)
               {
                  _loc6_.§0!'§.Normalize();
                  _loc6_.§0!'§.x *= b2Settings.b2_maxTranslation * param1.§&"=§;
                  _loc6_.§0!'§.y *= b2Settings.b2_maxTranslation * param1.§&"=§;
               }
               _loc11_ = param1.§;!!§ * _loc6_.m_angularVelocity;
               if(_loc11_ * _loc11_ > b2Settings.b2_maxRotationSquared)
               {
                  if(_loc6_.m_angularVelocity < 0)
                  {
                     _loc6_.m_angularVelocity = -b2Settings.b2_maxRotation * param1.§&"=§;
                  }
                  else
                  {
                     _loc6_.m_angularVelocity = b2Settings.b2_maxRotation * param1.§&"=§;
                  }
               }
               _loc6_.m_sweep.c0.SetV(_loc6_.m_sweep.c);
               _loc6_.m_sweep.a0 = _loc6_.m_sweep.a;
               _loc6_.m_sweep.c.x += param1.§;!!§ * _loc6_.§0!'§.x;
               _loc6_.m_sweep.c.y += param1.§;!!§ * _loc6_.§0!'§.y;
               _loc6_.m_sweep.a += param1.§;!!§ * _loc6_.m_angularVelocity;
               _loc6_.§"t§();
            }
            _loc4_++;
         }
         _loc4_ = 0;
         while(_loc4_ < param1.§-"'§)
         {
            _loc12_ = _loc8_.SolvePositionConstraints(b2Settings.b2_contactBaumgarte);
            _loc13_ = true;
            _loc5_ = 0;
            while(_loc5_ < this.§2y§)
            {
               _loc14_ = (_loc7_ = this.§;">§[_loc5_]).SolvePositionConstraints(b2Settings.b2_contactBaumgarte);
               _loc13_ = _loc13_ && _loc14_;
               _loc5_++;
            }
            if(_loc12_ && _loc13_)
            {
               break;
            }
            _loc4_++;
         }
         this.§'! §(_loc8_.§%!m§);
         if(param3)
         {
            _loc15_ = Number.MAX_VALUE;
            _loc16_ = b2Settings.b2_linearSleepTolerance * b2Settings.b2_linearSleepTolerance;
            _loc17_ = b2Settings.b2_angularSleepTolerance * b2Settings.b2_angularSleepTolerance;
            _loc4_ = 0;
            while(_loc4_ < this.§9!n§)
            {
               if((_loc6_ = this.§="D§[_loc4_]).§^![§() != b2Body.b2_staticBody)
               {
                  if((_loc6_.§4C§ & b2Body.§&>§) == 0)
                  {
                     _loc6_.§'!2§ = 0;
                     _loc15_ = 0;
                  }
                  if((_loc6_.§4C§ & b2Body.§&>§) == 0 || _loc6_.m_angularVelocity * _loc6_.m_angularVelocity > _loc17_ || b2Math.§0T§(_loc6_.§0!'§,_loc6_.§0!'§) > _loc16_)
                  {
                     _loc6_.§'!2§ = 0;
                     _loc15_ = 0;
                  }
                  else
                  {
                     _loc6_.§'!2§ += param1.§;!!§;
                     _loc15_ = b2Math.§@]§(_loc15_,_loc6_.§'!2§);
                  }
               }
               _loc4_++;
            }
            if(_loc15_ >= b2Settings.b2_timeToSleep)
            {
               _loc4_ = 0;
               while(_loc4_ < this.§9!n§)
               {
                  (_loc6_ = this.§="D§[_loc4_]).SetAwake(false);
                  _loc4_++;
               }
            }
         }
      }
      
      public function §'s§(param1:b2TimeStep) : void
      {
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc6_:Boolean = false;
         var _loc7_:Boolean = false;
         var _loc8_:Boolean = false;
         var _loc9_:b2Body = null;
         var _loc10_:Number = NaN;
         var _loc11_:Number = NaN;
         var _loc12_:Number = NaN;
         this.§`A§.§'m§(param1,this.§#I§,this.§1A§,this.§2'§);
         var _loc4_:b2ContactSolver = this.§`A§;
         _loc2_ = 0;
         while(_loc2_ < this.§2y§)
         {
            this.§;">§[_loc2_].InitVelocityConstraints(param1);
            _loc2_++;
         }
         _loc2_ = 0;
         while(_loc2_ < param1.§-"'§)
         {
            _loc6_ = _loc4_.SolvePositionConstraints(0.75);
            _loc7_ = true;
            _loc3_ = 0;
            while(_loc3_ < this.§2y§)
            {
               _loc8_ = this.§;">§[_loc3_].SolvePositionConstraints(b2Settings.b2_contactBaumgarte);
               _loc7_ = _loc7_ && _loc8_;
               _loc3_++;
            }
            if(_loc6_ && _loc7_)
            {
               break;
            }
            _loc2_++;
         }
         _loc2_ = 0;
         while(_loc2_ < param1.§6!B§)
         {
            _loc4_.SolveVelocityConstraints();
            _loc3_ = 0;
            while(_loc3_ < this.§2y§)
            {
               this.§;">§[_loc3_].SolveVelocityConstraints(param1);
               _loc3_++;
            }
            _loc2_++;
         }
         _loc2_ = 0;
         while(_loc2_ < this.§9!n§)
         {
            if((_loc9_ = this.§="D§[_loc2_]).§^![§() != b2Body.b2_staticBody)
            {
               _loc10_ = param1.§;!!§ * _loc9_.§0!'§.x;
               _loc11_ = param1.§;!!§ * _loc9_.§0!'§.y;
               if(_loc10_ * _loc10_ + _loc11_ * _loc11_ > b2Settings.b2_maxTranslationSquared)
               {
                  _loc9_.§0!'§.Normalize();
                  _loc9_.§0!'§.x *= b2Settings.b2_maxTranslation * param1.§&"=§;
                  _loc9_.§0!'§.y *= b2Settings.b2_maxTranslation * param1.§&"=§;
               }
               _loc12_ = param1.§;!!§ * _loc9_.m_angularVelocity;
               if(_loc12_ * _loc12_ > b2Settings.b2_maxRotationSquared)
               {
                  if(_loc9_.m_angularVelocity < 0)
                  {
                     _loc9_.m_angularVelocity = -b2Settings.b2_maxRotation * param1.§&"=§;
                  }
                  else
                  {
                     _loc9_.m_angularVelocity = b2Settings.b2_maxRotation * param1.§&"=§;
                  }
               }
               _loc9_.m_sweep.c0.SetV(_loc9_.m_sweep.c);
               _loc9_.m_sweep.a0 = _loc9_.m_sweep.a;
               _loc9_.m_sweep.c.x += param1.§;!!§ * _loc9_.§0!'§.x;
               _loc9_.m_sweep.c.y += param1.§;!!§ * _loc9_.§0!'§.y;
               _loc9_.m_sweep.a += param1.§;!!§ * _loc9_.m_angularVelocity;
               _loc9_.§"t§();
            }
            _loc2_++;
         }
         this.§'! §(_loc4_.§%!m§);
      }
      
      public function §'! §(param1:Vector.<b2ContactConstraint>) : void
      {
         var _loc3_:b2Contact = null;
         var _loc4_:b2ContactConstraint = null;
         var _loc5_:int = 0;
         if(this.§3z§ == null)
         {
            return;
         }
         var _loc2_:int = 0;
         while(_loc2_ < this.§1A§)
         {
            _loc3_ = this.§#I§[_loc2_];
            _loc4_ = param1[_loc2_];
            _loc5_ = 0;
            while(_loc5_ < _loc4_.§8"-§)
            {
               §-!1§.§+!c§[_loc5_] = _loc4_.§1!l§[_loc5_].normalImpulse;
               §-!1§.§#!O§[_loc5_] = _loc4_.§1!l§[_loc5_].tangentImpulse;
               _loc5_++;
            }
            this.§3z§.PostSolve(_loc3_,§-!1§);
            _loc2_++;
         }
      }
      
      public function §%!F§(param1:b2Body) : void
      {
         param1.§`!q§ = this.§9!n§;
         var _loc2_:* = this.§9!n§++;
         this.§="D§[_loc2_] = param1;
      }
      
      public function §;J§(param1:b2Contact) : void
      {
         var _loc2_:* = this.§1A§++;
         this.§#I§[_loc2_] = param1;
      }
      
      public function §7!]§(param1:b2Joint) : void
      {
         var _loc2_:* = this.§2y§++;
         this.§;">§[_loc2_] = param1;
      }
   }
}
