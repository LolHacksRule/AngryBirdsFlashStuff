package §?"#§
{
   import §8K§.b2internal;
   import §;`§.b2Body;
   import §;`§.b2TimeStep;
   import §[!f§.b2Mat22;
   import §[!f§.b2Vec2;
   
   use namespace b2internal;
   
   public class b2MouseJoint extends b2Joint
   {
       
      
      private var K:b2Mat22;
      
      private var K1:b2Mat22;
      
      private var K2:b2Mat22;
      
      private var § @§:b2Vec2;
      
      private var §"!5§:b2Vec2;
      
      private var §2""§:b2Vec2;
      
      private var §->§:b2Mat22;
      
      private var §[!V§:b2Vec2;
      
      private var §`""§:Number;
      
      private var §`!y§:Number;
      
      private var §'=§:Number;
      
      private var §@6§:Number;
      
      private var §&!#§:Number;
      
      public function b2MouseJoint(param1:b2MouseJointDef)
      {
         var _loc2_:Number = NaN;
         var _loc4_:b2Mat22 = null;
         this.K = new b2Mat22();
         this.K1 = new b2Mat22();
         this.K2 = new b2Mat22();
         this.§ @§ = new b2Vec2();
         this.§"!5§ = new b2Vec2();
         this.§2""§ = new b2Vec2();
         this.§->§ = new b2Mat22();
         this.§[!V§ = new b2Vec2();
         super(param1);
         this.§"!5§.SetV(param1.target);
         _loc2_ = this.§"!5§.x - b2internal::8"?.m_xf.position.x;
         var _loc3_:Number = this.§"!5§.y - b2internal::8"?.m_xf.position.y;
         _loc4_ = b2internal::8"?.m_xf.R;
         this.§ @§.x = _loc2_ * _loc4_.col1.x + _loc3_ * _loc4_.col1.y;
         this.§ @§.y = _loc2_ * _loc4_.col2.x + _loc3_ * _loc4_.col2.y;
         this.§`""§ = param1.§&"1§;
         this.§2""§.§]#§();
         this.§`!y§ = param1.§#!X§;
         this.§'=§ = param1.dampingRatio;
         this.§@6§ = 0;
         this.§&!#§ = 0;
      }
      
      override public function GetAnchorA() : b2Vec2
      {
         return this.§"!5§;
      }
      
      override public function GetAnchorB() : b2Vec2
      {
         return b2internal::8"?.GetWorldPoint(this.§ @§);
      }
      
      override public function GetReactionForce(param1:Number) : b2Vec2
      {
         return new b2Vec2(param1 * this.§2""§.x,param1 * this.§2""§.y);
      }
      
      override public function GetReactionTorque(param1:Number) : Number
      {
         return 0;
      }
      
      public function §6l§() : b2Vec2
      {
         return this.§"!5§;
      }
      
      public function §,i§(param1:b2Vec2) : void
      {
         if(b2internal::8"?.IsAwake() == false)
         {
            b2internal::8"?.SetAwake(true);
         }
         this.§"!5§ = param1;
      }
      
      public function §;!-§() : Number
      {
         return this.§`""§;
      }
      
      public function §3!&§(param1:Number) : void
      {
         this.§`""§ = param1;
      }
      
      public function §4!w§() : Number
      {
         return this.§`!y§;
      }
      
      public function §9!B§(param1:Number) : void
      {
         this.§`!y§ = param1;
      }
      
      public function §6!S§() : Number
      {
         return this.§'=§;
      }
      
      public function §;C§(param1:Number) : void
      {
         this.§'=§ = param1;
      }
      
      override b2internal function InitVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc7_:b2Mat22 = null;
         var _loc11_:Number = NaN;
         var _loc12_:Number = NaN;
         var _loc2_:b2Body = b2internal::8"?;
         var _loc3_:Number = _loc2_.GetMass();
         var _loc4_:Number = 2 * Math.PI * this.§`!y§;
         var _loc5_:Number = 2 * _loc3_ * this.§'=§ * _loc4_;
         var _loc6_:Number = _loc3_ * _loc4_ * _loc4_;
         this.§&!#§ = param1.§;!!§ * (_loc5_ + param1.§;!!§ * _loc6_);
         this.§&!#§ = this.§&!#§ != 0 ? Number(1 / this.§&!#§) : Number(0);
         this.§@6§ = param1.§;!!§ * _loc6_ * this.§&!#§;
         _loc7_ = _loc2_.m_xf.R;
         var _loc8_:Number = this.§ @§.x - _loc2_.m_sweep.localCenter.x;
         var _loc9_:Number = this.§ @§.y - _loc2_.m_sweep.localCenter.y;
         var _loc10_:Number = _loc7_.col1.x * _loc8_ + _loc7_.col2.x * _loc9_;
         _loc9_ = _loc7_.col1.y * _loc8_ + _loc7_.col2.y * _loc9_;
         _loc8_ = _loc10_;
         _loc11_ = _loc2_.§9K§;
         _loc12_ = _loc2_.§%"+§;
         this.K1.col1.x = _loc11_;
         this.K1.col2.x = 0;
         this.K1.col1.y = 0;
         this.K1.col2.y = _loc11_;
         this.K2.col1.x = _loc12_ * _loc9_ * _loc9_;
         this.K2.col2.x = -_loc12_ * _loc8_ * _loc9_;
         this.K2.col1.y = -_loc12_ * _loc8_ * _loc9_;
         this.K2.col2.y = _loc12_ * _loc8_ * _loc8_;
         this.K.§>!w§(this.K1);
         this.K.§7!W§(this.K2);
         this.K.col1.x += this.§&!#§;
         this.K.col2.y += this.§&!#§;
         this.K.§&"6§(this.§->§);
         this.§[!V§.x = _loc2_.m_sweep.c.x + _loc8_ - this.§"!5§.x;
         this.§[!V§.y = _loc2_.m_sweep.c.y + _loc9_ - this.§"!5§.y;
         _loc2_.m_angularVelocity *= 0.98;
         this.§2""§.x *= param1.§0!`§;
         this.§2""§.y *= param1.§0!`§;
         _loc2_.§0!'§.x += _loc11_ * this.§2""§.x;
         _loc2_.§0!'§.y += _loc11_ * this.§2""§.y;
         _loc2_.m_angularVelocity += _loc12_ * (_loc8_ * this.§2""§.y - _loc9_ * this.§2""§.x);
      }
      
      override b2internal function SolveVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc3_:b2Mat22 = null;
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc2_:b2Body = b2internal::8"?;
         _loc3_ = _loc2_.m_xf.R;
         var _loc6_:Number = this.§ @§.x - _loc2_.m_sweep.localCenter.x;
         var _loc7_:Number = this.§ @§.y - _loc2_.m_sweep.localCenter.y;
         _loc4_ = _loc3_.col1.x * _loc6_ + _loc3_.col2.x * _loc7_;
         _loc7_ = _loc3_.col1.y * _loc6_ + _loc3_.col2.y * _loc7_;
         _loc6_ = _loc4_;
         var _loc8_:Number = _loc2_.§0!'§.x + -_loc2_.m_angularVelocity * _loc7_;
         var _loc9_:Number = _loc2_.§0!'§.y + _loc2_.m_angularVelocity * _loc6_;
         _loc3_ = this.§->§;
         _loc4_ = _loc8_ + this.§@6§ * this.§[!V§.x + this.§&!#§ * this.§2""§.x;
         _loc5_ = _loc9_ + this.§@6§ * this.§[!V§.y + this.§&!#§ * this.§2""§.y;
         var _loc10_:Number = -(_loc3_.col1.x * _loc4_ + _loc3_.col2.x * _loc5_);
         var _loc11_:Number = -(_loc3_.col1.y * _loc4_ + _loc3_.col2.y * _loc5_);
         var _loc12_:Number = this.§2""§.x;
         var _loc13_:Number = this.§2""§.y;
         this.§2""§.x += _loc10_;
         this.§2""§.y += _loc11_;
         var _loc14_:Number = param1.§;!!§ * this.§`""§;
         if(this.§2""§.§8!B§() > _loc14_ * _loc14_)
         {
            this.§2""§.§7!J§(_loc14_ / this.§2""§.§@"9§());
         }
         _loc10_ = this.§2""§.x - _loc12_;
         _loc11_ = this.§2""§.y - _loc13_;
         _loc2_.§0!'§.x += _loc2_.§9K§ * _loc10_;
         _loc2_.§0!'§.y += _loc2_.§9K§ * _loc11_;
         _loc2_.m_angularVelocity += _loc2_.§%"+§ * (_loc6_ * _loc11_ - _loc7_ * _loc10_);
      }
      
      override b2internal function SolvePositionConstraints(param1:Number) : Boolean
      {
         return true;
      }
   }
}
