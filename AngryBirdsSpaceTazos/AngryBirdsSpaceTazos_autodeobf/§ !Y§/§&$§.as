package § !Y§
{
   ...
   public class §&$§
   {
      ...
      public function updateTextFields(param1:§ ",§, param2:String) : void
      {
         ...
         if(group.automatic != undefined)
         {
            ...
            for each(xml in group.automatic.button)
            {
               ...
               if(textXML.length() > 0)
               {
                  button = view.getItemByName(xml.@id) as §@_§;
                  if(button != null)
                  {
      ...
      public function §'">§(param1:String, param2:String) : String
      {
         ...
         xml = this.§^F§.group.(@name == groupId).manual.text.(@id == textId).language.(@id == §^N§).toString();
         if(xml != null)
         {
            return xml.toString();
         }
      ...
      public function §0!z§(param1:§ ",§, param2:String, param3:String, param4:String) : void
      {
         ...
         fontName = this.§^F§.font.language.(@id == §^N§);
         xml = this.§^F§.group.(@name == groupId).manual.text.(@id == textId).language.(@id == §^N§);
         if(xml.length() > 0)
         {
      ...
      public function §3e§(param1:String) : String
      {
         ...
         var chars:String = "";
         for each(text in this.§^F§.group.§*§.§*§.language.(@id == §^N§))
         {
            i = 0;
            while(i < text.length)
            {
