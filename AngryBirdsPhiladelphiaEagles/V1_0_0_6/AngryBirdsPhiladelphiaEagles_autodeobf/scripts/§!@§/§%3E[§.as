package §!@§
{
   ...
   public class §>[§
   {
      ...
      private function §!R§(param1:*) : String
      {
         ...
         if(param1 is Array)
         {
            return this.§4z§(param1 as Array);
         }
      ...
      private function §4z§(param1:Array) : String
      {
         var _loc2_:* = "";
         var _loc3_:int = 0;
         while(_loc3_ < param1.length)
         {
            ...
            _loc2_ += this.§!R§(param1[_loc3_]);
            _loc3_++;
         }
         return "[" + _loc2_ + "]";
      }
      
      private function §<!N§(param1:Object) : String
      {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§?!§(v.@name.toString()) + ":" + this.§!R§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
