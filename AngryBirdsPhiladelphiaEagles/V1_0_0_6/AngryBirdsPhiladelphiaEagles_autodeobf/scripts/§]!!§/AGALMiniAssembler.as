package §]!!§
{
   import flash.utils.ByteArray;
   import flash.utils.Dictionary;
   import flash.utils.Endian;
   import flash.utils.getTimer;
   
   public class AGALMiniAssembler
   {
      
      private static var initialized:Boolean = false;
      
      private static const §4b§:Dictionary = new Dictionary();
      
      private static const §'c§:Dictionary = new Dictionary();
      
      private static const §8^§:Dictionary = new Dictionary();
      
      private static const §9G§:int = 4;
      
      private static const §>!L§:int = 256;
      
      private static const FRAGMENT:String = "fragment";
      
      private static const VERTEX:String = "vertex";
      
      private static const §7q§:uint = 12;
      
      private static const §`!@§:uint = 16;
      
      private static const §`M§:uint = 20;
      
      private static const §'=§:uint = 24;
      
      private static const §[H§:uint = 28;
      
      private static const §>!%§:uint = 1;
      
      private static const §<!<§:uint = 2;
      
      private static const §4X§:uint = 32;
      
      private static const §-o§:uint = 64;
      
      private static const §%=§:uint = 1;
      
      private static const § !9§:uint = 2;
      
      private static const §@8§:uint = 4;
      
      private static const §^-§:uint = 8;
      
      private static const §'!>§:uint = 16;
      
      private static const §<u§:uint = 32;
      
      private static const §-n§:uint = 128;
      
      private static const §%X§:String = "mov";
      
      private static const §-!L§:String = "add";
      
      private static const §,!>§:String = "sub";
      
      private static const §!H§:String = "mul";
      
      private static const §function§:String = "div";
      
      private static const §<j§:String = "rcp";
      
      private static const §=!K§:String = "min";
      
      private static const §=!<§:String = "max";
      
      private static const §-!6§:String = "frc";
      
      private static const §5!@§:String = "sqt";
      
      private static const §-!G§:String = "rsq";
      
      private static const §9>§:String = "pow";
      
      private static const §8!4§:String = "log";
      
      private static const §0k§:String = "exp";
      
      private static const §<!H§:String = "nrm";
      
      private static const §<f§:String = "sin";
      
      private static const §get §:String = "cos";
      
      private static const §4!D§:String = "crs";
      
      private static const DP3:String = "dp3";
      
      private static const DP4:String = "dp4";
      
      private static const §9@§:String = "abs";
      
      private static const §-!,§:String = "neg";
      
      private static const §9a§:String = "sat";
      
      private static const M33:String = "m33";
      
      private static const M44:String = "m44";
      
      private static const M34:String = "m34";
      
      private static const §#J§:String = "ifz";
      
      private static const §>y§:String = "inz";
      
      private static const §&`§:String = "ife";
      
      private static const §?j§:String = "ine";
      
      private static const §-!3§:String = "ifg";
      
      private static const § !"§:String = "ifl";
      
      private static const §#&§:String = "ieg";
      
      private static const § w§:String = "iel";
      
      private static const §1r§:String = "els";
      
      private static const §"g§:String = "eif";
      
      private static const §^!8§:String = "rep";
      
      private static const §7!§:String = "erp";
      
      private static const §20§:String = "brk";
      
      private static const §]b§:String = "kil";
      
      private static const §@%§:String = "tex";
      
      private static const §?S§:String = "sge";
      
      private static const §`!8§:String = "slt";
      
      private static const §0m§:String = "sgn";
      
      private static const §?!%§:String = "va";
      
      private static const §,!5§:String = "vc";
      
      private static const §]f§:String = "vt";
      
      private static const §&-§:String = "op";
      
      private static const §]Y§:String = "v";
      
      private static const §?+§:String = "fc";
      
      private static const §<9§:String = "ft";
      
      private static const §6!?§:String = "fs";
      
      private static const §null§:String = "oc";
      
      private static const D2:String = "2d";
      
      private static const D3:String = "3d";
      
      private static const §>!P§:String = "cube";
      
      private static const §#h§:String = "mipnearest";
      
      private static const §!!<§:String = "miplinear";
      
      private static const §+!<§:String = "mipnone";
      
      private static const §'n§:String = "nomip";
      
      private static const §>h§:String = "nearest";
      
      private static const §?!-§:String = "linear";
      
      private static const §@F§:String = "centroid";
      
      private static const §`!;§:String = "single";
      
      private static const §`!&§:String = "depth";
      
      private static const §>e§:String = "repeat";
      
      private static const §2D§:String = "wrap";
      
      private static const §1!D§:String = "clamp";
       
      
      private var §8v§:ByteArray = null;
      
      private var §%!N§:String = "";
      
      private var §+X§:Boolean = false;
      
      public function AGALMiniAssembler(param1:Boolean = false)
      {
         super();
         this.§+X§ = param1;
         if(!initialized)
         {
            init();
         }
      }
      
      private static function init() : void
      {
         initialized = true;
         §4b§[§%X§] = new OpCode(§%X§,2,0,0);
         §4b§[§-!L§] = new OpCode(§-!L§,3,1,0);
         §4b§[§,!>§] = new OpCode(§,!>§,3,2,0);
         §4b§[§!H§] = new OpCode(§!H§,3,3,0);
         §4b§[§function§] = new OpCode(§function§,3,4,0);
         §4b§[§<j§] = new OpCode(§<j§,2,5,0);
         §4b§[§=!K§] = new OpCode(§=!K§,3,6,0);
         §4b§[§=!<§] = new OpCode(§=!<§,3,7,0);
         §4b§[§-!6§] = new OpCode(§-!6§,2,8,0);
         §4b§[§5!@§] = new OpCode(§5!@§,2,9,0);
         §4b§[§-!G§] = new OpCode(§-!G§,2,10,0);
         §4b§[§9>§] = new OpCode(§9>§,3,11,0);
         §4b§[§8!4§] = new OpCode(§8!4§,2,12,0);
         §4b§[§0k§] = new OpCode(§0k§,2,13,0);
         §4b§[§<!H§] = new OpCode(§<!H§,2,14,0);
         §4b§[§<f§] = new OpCode(§<f§,2,15,0);
         §4b§[§get §] = new OpCode(§get §,2,16,0);
         §4b§[§4!D§] = new OpCode(§4!D§,3,17,0);
         §4b§[DP3] = new OpCode(DP3,3,18,0);
         §4b§[DP4] = new OpCode(DP4,3,19,0);
         §4b§[§9@§] = new OpCode(§9@§,2,20,0);
         §4b§[§-!,§] = new OpCode(§-!,§,2,21,0);
         §4b§[§9a§] = new OpCode(§9a§,2,22,0);
         §4b§[M33] = new OpCode(M33,3,23,§'!>§);
         §4b§[M44] = new OpCode(M44,3,24,§'!>§);
         §4b§[M34] = new OpCode(M34,3,25,§'!>§);
         §4b§[§#J§] = new OpCode(§#J§,1,26,§-n§ | § !9§ | §%=§);
         §4b§[§>y§] = new OpCode(§>y§,1,27,§-n§ | § !9§ | §%=§);
         §4b§[§&`§] = new OpCode(§&`§,2,28,§-n§ | § !9§ | §%=§);
         §4b§[§?j§] = new OpCode(§?j§,2,29,§-n§ | § !9§ | §%=§);
         §4b§[§-!3§] = new OpCode(§-!3§,2,30,§-n§ | § !9§ | §%=§);
         §4b§[§ !"§] = new OpCode(§ !"§,2,31,§-n§ | § !9§ | §%=§);
         §4b§[§#&§] = new OpCode(§#&§,2,32,§-n§ | § !9§ | §%=§);
         §4b§[§ w§] = new OpCode(§ w§,2,33,§-n§ | § !9§ | §%=§);
         §4b§[§1r§] = new OpCode(§1r§,0,34,§-n§ | § !9§ | §@8§);
         §4b§[§"g§] = new OpCode(§"g§,0,35,§-n§ | §@8§);
         §4b§[§^!8§] = new OpCode(§^!8§,1,36,§-n§ | § !9§ | §%=§);
         §4b§[§7!§] = new OpCode(§7!§,0,37,§-n§ | §@8§);
         §4b§[§20§] = new OpCode(§20§,0,38,§-n§);
         §4b§[§]b§] = new OpCode(§]b§,1,39,§-n§ | §<u§);
         §4b§[§@%§] = new OpCode(§@%§,3,40,§<u§ | §^-§);
         §4b§[§?S§] = new OpCode(§?S§,3,41,0);
         §4b§[§`!8§] = new OpCode(§`!8§,3,42,0);
         §4b§[§0m§] = new OpCode(§0m§,2,43,0);
         §'c§[§?!%§] = new Register(§?!%§,"vertex attribute",0,7,§-o§ | §<!<§);
         §'c§[§,!5§] = new Register(§,!5§,"vertex constant",1,127,§-o§ | §<!<§);
         §'c§[§]f§] = new Register(§]f§,"vertex temporary",2,7,§-o§ | §>!%§ | §<!<§);
         §'c§[§&-§] = new Register(§&-§,"vertex output",3,0,§-o§ | §>!%§);
         §'c§[§]Y§] = new Register(§]Y§,"varying",4,7,§-o§ | §4X§ | §<!<§ | §>!%§);
         §'c§[§?+§] = new Register(§?+§,"fragment constant",1,27,§4X§ | §<!<§);
         §'c§[§<9§] = new Register(§<9§,"fragment temporary",2,7,§4X§ | §>!%§ | §<!<§);
         §'c§[§6!?§] = new Register(§6!?§,"texture sampler",5,7,§4X§ | §<!<§);
         §'c§[§null§] = new Register(§null§,"fragment output",3,0,§4X§ | §>!%§);
         §8^§[D2] = new Sampler(D2,§7q§,0);
         §8^§[D3] = new Sampler(D3,§7q§,2);
         §8^§[§>!P§] = new Sampler(§>!P§,§7q§,1);
         §8^§[§#h§] = new Sampler(§#h§,§'=§,1);
         §8^§[§!!<§] = new Sampler(§!!<§,§'=§,2);
         §8^§[§+!<§] = new Sampler(§+!<§,§'=§,0);
         §8^§[§'n§] = new Sampler(§'n§,§'=§,0);
         §8^§[§>h§] = new Sampler(§>h§,§[H§,0);
         §8^§[§?!-§] = new Sampler(§?!-§,§[H§,1);
         §8^§[§@F§] = new Sampler(§@F§,§`!@§,1 << 0);
         §8^§[§`!;§] = new Sampler(§`!;§,§`!@§,1 << 1);
         §8^§[§`!&§] = new Sampler(§`!&§,§`!@§,1 << 2);
         §8^§[§>e§] = new Sampler(§>e§,§`M§,1);
         §8^§[§2D§] = new Sampler(§2D§,§`M§,1);
         §8^§[§1!D§] = new Sampler(§1!D§,§`M§,0);
      }
      
      public function get error() : String
      {
         return this.§%!N§;
      }
      
      public function get §<o§() : ByteArray
      {
         return this.§8v§;
      }
      
      public function §&!2§(param1:String, param2:String, param3:Boolean = false) : ByteArray
      {
         var _loc9_:int = 0;
         var _loc11_:String = null;
         var _loc12_:int = 0;
         var _loc13_:int = 0;
         var _loc14_:Array = null;
         var _loc15_:Array = null;
         var _loc16_:OpCode = null;
         var _loc17_:Array = null;
         var _loc18_:Boolean = false;
         var _loc19_:uint = 0;
         var _loc20_:uint = 0;
         var _loc21_:int = 0;
         var _loc22_:Boolean = false;
         var _loc23_:Array = null;
         var _loc24_:Array = null;
         var _loc25_:Register = null;
         var _loc26_:Array = null;
         var _loc27_:uint = 0;
         var _loc28_:uint = 0;
         var _loc29_:Array = null;
         var _loc30_:Boolean = false;
         var _loc31_:Boolean = false;
         var _loc32_:uint = 0;
         var _loc33_:uint = 0;
         var _loc34_:int = 0;
         var _loc35_:uint = 0;
         var _loc36_:uint = 0;
         var _loc37_:int = 0;
         var _loc38_:Array = null;
         var _loc39_:Register = null;
         var _loc40_:Array = null;
         var _loc41_:Array = null;
         var _loc42_:uint = 0;
         var _loc43_:uint = 0;
         var _loc44_:Number = NaN;
         var _loc45_:Sampler = null;
         var _loc46_:* = null;
         var _loc47_:uint = 0;
         var _loc48_:uint = 0;
         var _loc49_:String = null;
         var _loc4_:uint = getTimer();
         this.§8v§ = new ByteArray();
         this.§%!N§ = "";
         var _loc5_:Boolean = false;
         if(param1 == FRAGMENT)
         {
            _loc5_ = true;
         }
         else if(param1 != VERTEX)
         {
            this.§%!N§ = "ERROR: mode needs to be \"" + FRAGMENT + "\" or \"" + VERTEX + "\" but is \"" + param1 + "\".";
         }
         this.§<o§.endian = Endian.LITTLE_ENDIAN;
         this.§<o§.writeByte(160);
         this.§<o§.writeUnsignedInt(1);
         this.§<o§.writeByte(161);
         this.§<o§.writeByte(!!_loc5_ ? 1 : 0);
         var _loc6_:Array = param2.replace(/[\f\n\r\v]+/g,"\n").split("\n");
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         var _loc10_:int = _loc6_.length;
         _loc9_ = 0;
         while(_loc9_ < _loc10_ && this.§%!N§ == "")
         {
            if((_loc12_ = (_loc11_ = new String(_loc6_[_loc9_])).search("//")) != -1)
            {
               _loc11_ = _loc11_.slice(0,_loc12_);
            }
            if((_loc13_ = _loc11_.search(/<.*>/g)) != -1)
            {
               _loc14_ = _loc11_.slice(_loc13_).match(/([\w\.\-\+]+)/gi);
               _loc11_ = _loc11_.slice(0,_loc13_);
            }
            _loc15_ = _loc11_.match(/^\w{3}/ig);
            _loc16_ = §4b§[_loc15_[0]];
            if(!this.§+X§)
            {
            }
            if(_loc16_ == null)
            {
               if(_loc11_.length >= 3)
               {
               }
            }
            else
            {
               _loc11_ = _loc11_.slice(_loc11_.search(_loc16_.name) + _loc16_.name.length);
               if(_loc16_.flags & §@8§)
               {
                  if(--_loc7_ < 0)
                  {
                     this.§%!N§ = "error: conditional closes without open.";
                     break;
                  }
               }
               if(_loc16_.flags & § !9§)
               {
                  _loc7_++;
                  if(_loc7_ > §9G§)
                  {
                     this.§%!N§ = "error: nesting to deep, maximum allowed is " + §9G§ + ".";
                     break;
                  }
               }
               if(_loc16_.flags & §<u§ && !_loc5_)
               {
                  this.§%!N§ = "error: opcode is only allowed in fragment programs.";
                  break;
               }
               if(!param3)
               {
               }
               this.§<o§.writeUnsignedInt(_loc16_.emitCode);
               _loc8_++;
               if(_loc8_ > §>!L§)
               {
                  this.§%!N§ = "error: too many opcodes. maximum is " + §>!L§ + ".";
                  break;
               }
               if((_loc17_ = _loc11_.match(/vc\[([vof][actps]?)(\d*)?(\.[xyzw](\+\d{1,3})?)?\](\.[xyzw]{1,4})?|([vof][actps]?)(\d*)?(\.[xyzw]{1,4})?/gi)).length != _loc16_.numRegister)
               {
                  this.§%!N§ = "error: wrong number of operands. found " + _loc17_.length + " but expected " + _loc16_.numRegister + ".";
                  break;
               }
               _loc18_ = false;
               _loc19_ = 64 + 64 + 32;
               _loc20_ = _loc17_.length;
               _loc21_ = 0;
               while(_loc21_ < _loc20_)
               {
                  _loc22_ = false;
                  if((_loc23_ = _loc17_[_loc21_].match(/\[.*\]/ig)).length > 0)
                  {
                     _loc17_[_loc21_] = _loc17_[_loc21_].replace(_loc23_[0],"0");
                     if(!param3)
                     {
                     }
                     _loc22_ = true;
                  }
                  _loc24_ = _loc17_[_loc21_].match(/^\b[A-Za-z]{1,2}/ig);
                  _loc25_ = §'c§[_loc24_[0]];
                  if(!this.§+X§)
                  {
                  }
                  if(_loc25_ == null)
                  {
                     this.§%!N§ = "error: could not parse operand " + _loc21_ + " (" + _loc17_[_loc21_] + ").";
                     _loc18_ = true;
                     break;
                  }
                  if(_loc5_)
                  {
                     if(!(_loc25_.flags & §4X§))
                     {
                        this.§%!N§ = "error: register operand " + _loc21_ + " (" + _loc17_[_loc21_] + ") only allowed in vertex programs.";
                        _loc18_ = true;
                        break;
                     }
                     if(_loc22_)
                     {
                        this.§%!N§ = "error: register operand " + _loc21_ + " (" + _loc17_[_loc21_] + ") relative adressing not allowed in fragment programs.";
                        _loc18_ = true;
                        break;
                     }
                  }
                  else if(!(_loc25_.flags & §-o§))
                  {
                     this.§%!N§ = "error: register operand " + _loc21_ + " (" + _loc17_[_loc21_] + ") only allowed in fragment programs.";
                     _loc18_ = true;
                     break;
                  }
                  _loc17_[_loc21_] = _loc17_[_loc21_].slice(_loc17_[_loc21_].search(_loc25_.name) + _loc25_.name.length);
                  _loc26_ = !!_loc22_ ? _loc23_[0].match(/\d+/) : _loc17_[_loc21_].match(/\d+/);
                  _loc27_ = 0;
                  if(_loc26_)
                  {
                     _loc27_ = uint(_loc26_[0]);
                  }
                  if(_loc25_.range < _loc27_)
                  {
                     this.§%!N§ = "error: register operand " + _loc21_ + " (" + _loc17_[_loc21_] + ") index exceeds limit of " + (_loc25_.range + 1) + ".";
                     _loc18_ = true;
                     break;
                  }
                  _loc28_ = 0;
                  _loc29_ = _loc17_[_loc21_].match(/(\.[xyzw]{1,4})/);
                  _loc30_ = _loc21_ == 0 && !(_loc16_.flags & §-n§);
                  _loc31_ = _loc21_ == 2 && _loc16_.flags & §^-§;
                  _loc32_ = 0;
                  _loc33_ = 0;
                  _loc34_ = 0;
                  if(_loc30_ && _loc22_)
                  {
                     this.§%!N§ = "error: relative can not be destination";
                     _loc18_ = true;
                     break;
                  }
                  if(_loc29_)
                  {
                     _loc28_ = 0;
                     _loc36_ = _loc29_[0].length;
                     _loc37_ = 1;
                     while(_loc37_ < _loc36_)
                     {
                        if((_loc35_ = _loc29_[0].charCodeAt(_loc37_) - "x".charCodeAt(0)) > 2)
                        {
                           _loc35_ = 3;
                        }
                        if(_loc30_)
                        {
                           _loc28_ |= 1 << _loc35_;
                        }
                        else
                        {
                           _loc28_ |= _loc35_ << (_loc37_ - 1 << 1);
                        }
                        _loc37_++;
                     }
                     if(!_loc30_)
                     {
                        while(_loc37_ <= 4)
                        {
                           _loc28_ |= _loc35_ << (_loc37_ - 1 << 1);
                           _loc37_++;
                        }
                     }
                  }
                  else
                  {
                     _loc28_ = !!_loc30_ ? uint(15) : uint(228);
                  }
                  if(_loc22_)
                  {
                     _loc38_ = _loc23_[0].match(/[A-Za-z]{1,2}/ig);
                     if((_loc39_ = §'c§[_loc38_[0]]) == null)
                     {
                        this.§%!N§ = "error: bad index register";
                        _loc18_ = true;
                        break;
                     }
                     _loc32_ = _loc39_.emitCode;
                     if((_loc40_ = _loc23_[0].match(/(\.[xyzw]{1,1})/)).length == 0)
                     {
                        this.§%!N§ = "error: bad index register select";
                        _loc18_ = true;
                        break;
                     }
                     if((_loc33_ = _loc40_[0].charCodeAt(1) - "x".charCodeAt(0)) > 2)
                     {
                        _loc33_ = 3;
                     }
                     if((_loc41_ = _loc23_[0].match(/\+\d{1,3}/ig)).length > 0)
                     {
                        _loc34_ = _loc41_[0];
                     }
                     if(_loc34_ < 0 || _loc34_ > 255)
                     {
                        this.§%!N§ = "error: index offset " + _loc34_ + " out of bounds. [0..255]";
                        _loc18_ = true;
                        break;
                     }
                     if(!param3)
                     {
                     }
                  }
                  if(!param3)
                  {
                  }
                  if(_loc30_)
                  {
                     this.§<o§.writeShort(_loc27_);
                     this.§<o§.writeByte(_loc28_);
                     this.§<o§.writeByte(_loc25_.emitCode);
                     _loc19_ -= 32;
                  }
                  else if(_loc31_)
                  {
                     if(!param3)
                     {
                     }
                     _loc42_ = 5;
                     _loc43_ = _loc14_.length;
                     _loc44_ = 0;
                     _loc37_ = 0;
                     while(_loc37_ < _loc43_)
                     {
                        if(!param3)
                        {
                        }
                        if((_loc45_ = §8^§[_loc14_[_loc37_]]) == null)
                        {
                           _loc44_ = Number(_loc14_[_loc37_]);
                           if(!param3)
                           {
                           }
                        }
                        else
                        {
                           if(_loc45_.flag != §`!@§)
                           {
                              _loc42_ &= ~(15 << _loc45_.flag);
                           }
                           _loc42_ |= uint(_loc45_.mask) << uint(_loc45_.flag);
                        }
                        _loc37_++;
                     }
                     this.§<o§.writeShort(_loc27_);
                     this.§<o§.writeByte(int(_loc44_ * 8));
                     this.§<o§.writeByte(0);
                     this.§<o§.writeUnsignedInt(_loc42_);
                     if(!param3)
                     {
                     }
                     _loc19_ -= 64;
                  }
                  else
                  {
                     if(_loc21_ == 0)
                     {
                        this.§<o§.writeUnsignedInt(0);
                        _loc19_ -= 32;
                     }
                     this.§<o§.writeShort(_loc27_);
                     this.§<o§.writeByte(_loc34_);
                     this.§<o§.writeByte(_loc28_);
                     this.§<o§.writeByte(_loc25_.emitCode);
                     this.§<o§.writeByte(_loc32_);
                     this.§<o§.writeShort(!!_loc22_ ? _loc33_ | 1 << 15 : 0);
                     _loc19_ -= 64;
                  }
                  _loc21_++;
               }
               _loc21_ = 0;
               while(_loc21_ < _loc19_)
               {
                  this.§<o§.writeByte(0);
                  _loc21_ += 8;
               }
               if(_loc18_)
               {
                  break;
               }
            }
            _loc9_++;
         }
         if(this.§%!N§ != "")
         {
            this.§%!N§ += "\n  at line " + _loc9_ + " " + _loc6_[_loc9_];
            this.§<o§.length = 0;
         }
         if(this.§+X§)
         {
            _loc46_ = "generated bytecode:";
            _loc47_ = this.§<o§.length;
            _loc48_ = 0;
            while(_loc48_ < _loc47_)
            {
               if(!(_loc48_ % 16))
               {
                  _loc46_ += "\n";
               }
               if(!(_loc48_ % 4))
               {
                  _loc46_ += " ";
               }
               if((_loc49_ = this.§<o§[_loc48_].toString(16)).length < 2)
               {
                  _loc49_ = "0" + _loc49_;
               }
               _loc46_ += _loc49_;
               _loc48_++;
            }
         }
         if(!param3)
         {
         }
         return this.§<o§;
      }
   }
}

class OpCode
{
    
   
   private var _emitCode:uint;
   
   private var _flags:uint;
   
   private var _name:String;
   
   private var _numRegister:uint;
   
   function OpCode(param1:String, param2:uint, param3:uint, param4:uint)
   {
      super();
      this._name = param1;
      this._numRegister = param2;
      this._emitCode = param3;
      this._flags = param4;
   }
   
   public function get emitCode() : uint
   {
      return this._emitCode;
   }
   
   public function get flags() : uint
   {
      return this._flags;
   }
   
   public function get name() : String
   {
      return this._name;
   }
   
   public function get numRegister() : uint
   {
      return this._numRegister;
   }
   
   public function toString() : String
   {
      return "[OpCode name=\"" + this._name + "\", numRegister=" + this._numRegister + ", emitCode=" + this._emitCode + ", flags=" + this._flags + "]";
   }
}

class Register
{
    
   
   private var _emitCode:uint;
   
   private var _name:String;
   
   private var _longName:String;
   
   private var _flags:uint;
   
   private var _range:uint;
   
   function Register(param1:String, param2:String, param3:uint, param4:uint, param5:uint)
   {
      super();
      this._name = param1;
      this._longName = param2;
      this._emitCode = param3;
      this._range = param4;
      this._flags = param5;
   }
   
   public function get emitCode() : uint
   {
      return this._emitCode;
   }
   
   public function get longName() : String
   {
      return this._longName;
   }
   
   public function get name() : String
   {
      return this._name;
   }
   
   public function get flags() : uint
   {
      return this._flags;
   }
   
   public function get range() : uint
   {
      return this._range;
   }
   
   public function toString() : String
   {
      return "[Register name=\"" + this._name + "\", longName=\"" + this._longName + "\", emitCode=" + this._emitCode + ", range=" + this._range + ", flags=" + this._flags + "]";
   }
}

class Sampler
{
    
   
   private var _flag:uint;
   
   private var _mask:uint;
   
   private var _name:String;
   
   function Sampler(param1:String, param2:uint, param3:uint)
   {
      super();
      this._name = param1;
      this._flag = param2;
      this._mask = param3;
   }
   
   public function get flag() : uint
   {
      return this._flag;
   }
   
   public function get mask() : uint
   {
      return this._mask;
   }
   
   public function get name() : String
   {
      return this._name;
   }
   
   public function toString() : String
   {
      return "[Sampler name=\"" + this._name + "\", flag=\"" + this._flag + "\", mask=" + this.mask + "]";
   }
}
