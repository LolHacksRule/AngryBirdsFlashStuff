package §4G§
{
   import §#!5§.§case §;
   import §#!5§.transformCoords;
   import §>w§.§"!,§;
   import flash.display.BitmapData;
   import flash.geom.Matrix;
   import flash.geom.Matrix3D;
   import flash.geom.Point;
   import flash.geom.Rectangle;
   import flash.geom.Vector3D;
   
   public class §5T§ extends DisplayObject
   {
      
      private static var §`I§:Vector3D = new Vector3D();
      
      private static var §^V§:Point = new Point();
      
      private static var §1^§:Matrix = new Matrix();
       
      
      protected var §=i§:§case §;
      
      private var §<N§:Number = 0.0;
      
      private var §+x§:Number = 0.0;
      
      private var §+H§:uint = 16777215;
      
      public function §5T§(param1:Number, param2:Number, param3:uint = 16777215, param4:Boolean = true)
      {
         super();
         this.§=i§ = new §case §(4,param4);
         this.updateVertexData(param1,param2,param3,param4);
         this.§+H§ = param3;
      }
      
      public function get highQuality() : Boolean
      {
         return false;
      }
      
      protected function updateVertexData(param1:Number, param2:Number, param3:uint, param4:Boolean) : void
      {
         this.§=i§.setPremultipliedAlpha(param4);
         this.§=i§.setPosition(0,0,0);
         this.§=i§.setPosition(1,param1,0);
         this.§=i§.setPosition(2,0,param2);
         this.§=i§.setPosition(3,param1,param2);
         this.§=i§.§'!5§(param3);
         this.§<N§ = param1;
         this.§+x§ = param2;
      }
      
      override public function getBounds(param1:DisplayObject, param2:Rectangle = null) : Rectangle
      {
         var _loc7_:int = 0;
         if(param2 == null)
         {
            param2 = new Rectangle();
         }
         var _loc3_:Number = Number.MAX_VALUE;
         var _loc4_:Number = -Number.MAX_VALUE;
         var _loc5_:Number = Number.MAX_VALUE;
         var _loc6_:Number = -Number.MAX_VALUE;
         if(param1 == this)
         {
            _loc7_ = 0;
            while(_loc7_ < 4)
            {
               this.§=i§.§82§(_loc7_,§`I§);
               _loc3_ = _loc3_ < §`I§.x ? Number(_loc3_) : Number(§`I§.x);
               _loc4_ = _loc4_ > §`I§.x ? Number(_loc4_) : Number(§`I§.x);
               _loc5_ = _loc5_ < §`I§.y ? Number(_loc5_) : Number(§`I§.y);
               _loc6_ = _loc6_ > §`I§.y ? Number(_loc6_) : Number(§`I§.y);
               _loc7_++;
            }
         }
         else
         {
            §2Y§(param1,§1^§);
            _loc7_ = 0;
            while(_loc7_ < 4)
            {
               this.§=i§.§82§(_loc7_,§`I§);
               transformCoords(§1^§,§`I§.x,§`I§.y,§^V§);
               _loc3_ = _loc3_ < §^V§.x ? Number(_loc3_) : Number(§^V§.x);
               _loc4_ = _loc4_ > §^V§.x ? Number(_loc4_) : Number(§^V§.x);
               _loc5_ = _loc5_ < §^V§.y ? Number(_loc5_) : Number(§^V§.y);
               _loc6_ = _loc6_ > §^V§.y ? Number(_loc6_) : Number(§^V§.y);
               _loc7_++;
            }
         }
         param2.x = _loc3_;
         param2.y = _loc5_;
         param2.width = _loc4_ - _loc3_;
         param2.height = _loc6_ - _loc5_;
         return param2;
      }
      
      public function §@=§(param1:int) : uint
      {
         return this.§=i§.§<Q§(param1);
      }
      
      public function setVertexColor(param1:int, param2:uint) : void
      {
         this.§=i§.§5Q§(param1,param2);
      }
      
      public function §;s§(param1:int) : Number
      {
         return this.§=i§.§4@§(param1);
      }
      
      public function setVertexAlpha(param1:int, param2:Number) : void
      {
         this.§=i§.§7T§(param1,param2);
      }
      
      public function get color() : uint
      {
         return this.§+H§;
      }
      
      override public function set color(param1:uint) : void
      {
         if(this.§+H§ != param1)
         {
            this.§+H§ = param1;
            this.§=i§.§'!5§(param1,1);
         }
      }
      
      public function copyVertexDataTo(param1:§case §, param2:int = 0, param3:Number = 1.0, param4:Matrix3D = null) : void
      {
         this.§=i§.copyTo(param1,param2,param3,true,param4);
      }
      
      override public function render(param1:§"!,§, param2:Number) : void
      {
         param1.batchQuad(this,param2);
      }
      
      public function get useColor() : Boolean
      {
         return true;
      }
      
      public function get clippedBitmapData() : BitmapData
      {
         return null;
      }
      
      public function get §%>§() : Number
      {
         return this.§<N§;
      }
      
      public function get §%n§() : Number
      {
         return this.§+x§;
      }
   }
}
