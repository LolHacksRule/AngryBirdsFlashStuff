package §%^§
{
   ...
   public class §3h§
   {
      ...
      private function §&@§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§ !I§(param1 as String);
         }
         if(param1 is Number)
         {
      ...
      private function §+G§(param1:Object) : String
      {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§ !I§(v.@name.toString()) + ":" + this.§&@§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
