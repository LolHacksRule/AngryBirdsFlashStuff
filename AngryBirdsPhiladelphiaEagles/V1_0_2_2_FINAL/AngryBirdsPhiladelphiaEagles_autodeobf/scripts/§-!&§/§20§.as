package §-!&§
{
   import § N§.§1!A§;
   import § N§.Sprite;
   import §#e§.§`!9§;
   import §&^§.§[!7§;
   import §&o§.§2=§;
   import §&o§.§3,§;
   import §&o§.§72§;
   import §&o§.§=z§;
   import §-!<§.§5!5§;
   import §-!<§.§8R§;
   import §2G§.§+b§;
   import §2G§.§,6§;
   import §2G§.§0Z§;
   import §2G§.§5!1§;
   import §2G§.§>U§;
   import §5!G§.§>X§;
   import §8z§.Texture;
   import §9T§.§1r§;
   import §9T§.§<!!§;
   import §9T§.Tuner;
   import §9T§.§^=§;
   import §^]§.§[z§;
   import com.rovio.Box2D.Common.Math.b2Vec2;
   import com.rovio.Box2D.Dynamics.Joints.b2PrismaticJoint;
   import com.rovio.Box2D.Dynamics.Joints.b2WeldJoint;
   import flash.display.BitmapData;
   import flash.display.BitmapDataChannel;
   import flash.geom.Matrix;
   import flash.geom.Point;
   import flash.geom.Rectangle;
   
   public class §20§
   {
       
      
      protected var §]!I§:Vector.<§3!,§>;
      
      protected var §7z§:int;
      
      public var §;§:§1r§;
      
      public var § c§:Vector.<§%0§>;
      
      protected var §2!C§:Sprite;
      
      protected var §4!O§:Sprite;
      
      private var §`5§:Sprite;
      
      private var §+!,§:Sprite;
      
      private var §!!Q§:Sprite;
      
      protected var §^E§:Vector.<Texture>;
      
      protected var §0b§:Vector.<§=z§>;
      
      protected var §9Z§:int = 1;
      
      public var mMightyEagleAdded:Boolean = false;
      
      public var mMightyEagleHasTouchedGround:Boolean = false;
      
      public var §?K§:Boolean = false;
      
      public var mSardineCanAdded:Boolean = false;
      
      public var mMightyEagleTimer:Number = 0;
      
      protected var §`D§:Boolean = false;
      
      protected var §;!0§:Number;
      
      protected var §'4§:int;
      
      protected var §0?§:Vector.<§2=§>;
      
      protected var §5i§:int = 0;
      
      private var §&!4§:int = 0;
      
      private var §?j§:int = 0;
      
      private var §1!4§:int;
      
      private var §4!2§:Boolean = true;
      
      private var §?!L§:Boolean = true;
      
      public function §20§(param1:§1r§, param2:§[z§, param3:Sprite)
      {
         var _loc5_:§=z§ = null;
         var _loc6_:§72§ = null;
         var _loc7_:§3!,§ = null;
         var _loc8_:int = 0;
         var _loc9_:int = 0;
         var _loc10_:§3!,§ = null;
         var _loc11_:§3!,§ = null;
         this.§]!I§ = new Vector.<§3!,§>();
         this.§ c§ = new Vector.<§%0§>();
         this.§^E§ = new Vector.<Texture>();
         this.§;!0§ = this.Tuner.SARDINE_CAN_INITIAL_ROTATION_SPEED;
         this.§0?§ = new Vector.<§2=§>();
         super();
         this.§;§ = param1;
         this.§7z§ = 0;
         this.§4!O§ = param3;
         this.§4!O§.§!!"§ = false;
         this.§?!L§ = true;
         this.§4!2§ = true;
         this.§2!C§ = new Sprite();
         this.§`5§ = new Sprite();
         this.§+!,§ = new Sprite();
         this.§!!Q§ = new Sprite();
         this.§4!O§.addChild(this.§2!C§);
         this.§4!O§.addChild(this.§`5§);
         this.§4!O§.addChild(this.§+!,§);
         this.§4!O§.addChild(this.§!!Q§);
         this.addObject(§>X§.§2H§(param2.theme).§0S§,(this.§;§.§9!>§.§?!G§ + this.§;§.§9!>§.§`!!§) / 2,this.§;§.§9!>§.§^!O§ + §^=§.§8p§);
         this.§1!4§ = this.§]!I§.length;
         var _loc4_:int = 0;
         _loc4_ = 0;
         while(_loc4_ < param2.§;J§)
         {
            _loc6_ = param2.§ for§(_loc4_);
            if(_loc7_ = this.addObject(_loc6_.id,_loc6_.x,_loc6_.y,_loc6_.angle,false,false,false,1,_loc6_.front))
            {
               _loc7_.uniqueID = _loc6_.uniqueID;
            }
            _loc4_++;
         }
         this.§!!0§();
         this.§6?§(true);
         this.§0b§ = new Vector.<§=z§>();
         _loc4_ = 0;
         while(_loc4_ < param2.§&,§)
         {
            this.§0b§.push(§=z§.§9#§(param2.§9K§(_loc4_)));
            _loc4_++;
         }
         for each(_loc5_ in this.§0b§)
         {
            _loc8_ = _loc5_.index1 + this.§1!4§;
            _loc9_ = _loc5_.index2 + this.§1!4§;
            if(_loc8_ < this.§]!I§.length && _loc9_ < this.§]!I§.length)
            {
               _loc10_ = this.§]!I§[_loc8_];
               _loc11_ = this.§]!I§[_loc9_];
               if(!(_loc10_ && _loc11_))
               {
                  throw new Error("Joint index out of bounds [" + _loc8_ + ", " + _loc9_ + "]");
               }
               if(_loc5_.type != §3,§.§>R§)
               {
                  _loc5_.§''§ = this.§;§.§-!H§.§8f§.CreateJoint(_loc5_.§<,§(_loc10_,_loc11_));
               }
               else
               {
                  this.§0?§.push(new §2=§(_loc8_,_loc9_,_loc5_.§9"§));
               }
            }
         }
      }
      
      protected function get Tuner() : Class
      {
         return Tuner;
      }
      
      public function get §<-§() : Sprite
      {
         return this.§!!Q§;
      }
      
      public function get §'T§() : Sprite
      {
         return this.§4!O§;
      }
      
      public function dispose() : void
      {
         var _loc1_:Texture = null;
         while(this.§]!I§.length > 0)
         {
            this.§`!%§(0);
         }
         this.§]!I§ = null;
         this.§0?§ = null;
         if(this.§4!O§)
         {
            this.§4!O§.dispose();
            this.§4!O§ = null;
            this.§2!C§ = null;
            this.§!!Q§ = null;
            this.§`5§ = null;
            this.§+!,§ = null;
         }
         while(this.§^E§.length > 0)
         {
            _loc1_ = this.§^E§.pop();
            this.§;§.textureManager.§@C§(_loc1_);
         }
      }
      
      private function §6?§(param1:Boolean) : void
      {
         this.§2!C§.visible = param1;
      }
      
      private function §!!0§() : void
      {
         var _loc3_:§3!,§ = null;
         var _loc4_:Rectangle = null;
         var _loc5_:Number = NaN;
         var _loc6_:int = 0;
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         var _loc9_:int = 0;
         var _loc10_:BitmapData = null;
         var _loc11_:BitmapData = null;
         var _loc12_:Texture = null;
         var _loc13_:§1!A§ = null;
         var _loc1_:Rectangle = null;
         var _loc2_:Vector.<§3!,§> = new Vector.<§3!,§>();
         for each(_loc3_ in this.§]!I§)
         {
            if(_loc3_.isTexture())
            {
               _loc2_.push(_loc3_);
               _loc4_ = _loc3_.sprite.getBounds(this.§4!O§);
               if(_loc1_ == null)
               {
                  _loc1_ = new Rectangle(_loc4_.x,_loc4_.y,_loc4_.width,_loc4_.height);
               }
               else
               {
                  _loc1_ = _loc1_.union(_loc4_);
               }
            }
         }
         if(_loc1_)
         {
            _loc5_ = 1;
            while(_loc1_.width > 2048 || _loc1_.height > 2048)
            {
               _loc1_.left /= 2;
               _loc1_.top /= 2;
               _loc1_.right /= 2;
               _loc1_.bottom /= 2;
               _loc5_ /= 2;
            }
            _loc6_ = _loc1_.width;
            _loc7_ = _loc1_.height;
            _loc8_ = _loc1_.left;
            _loc9_ = _loc1_.top;
            _loc10_ = new BitmapData(_loc6_,_loc7_,true,0);
            this.§?F§(_loc10_.rect,_loc10_,_loc5_);
            _loc11_ = this.§<!L§(_loc2_,_loc6_,_loc7_,_loc8_,_loc9_,_loc5_);
            _loc10_.copyChannel(_loc11_,_loc11_.rect,new Point(0,0),BitmapDataChannel.ALPHA,BitmapDataChannel.ALPHA);
            _loc12_ = this.§;§.textureManager.§'b§(_loc10_);
            this.§^E§.push(_loc12_);
            (_loc13_ = new §1!A§(_loc12_)).x = _loc8_ / _loc5_;
            _loc13_.y = _loc9_ / _loc5_;
            _loc13_.scaleX = 1 / _loc5_;
            _loc13_.scaleY = 1 / _loc5_;
            this.§2!C§.addChild(_loc13_);
            _loc11_.dispose();
         }
      }
      
      private function §<!L§(param1:Vector.<§3!,§>, param2:int, param3:int, param4:int, param5:int, param6:Number) : BitmapData
      {
         var _loc7_:§3!,§ = null;
         var _loc11_:§+b§ = null;
         var _loc12_:Array = null;
         var _loc13_:Rectangle = null;
         var _loc8_:BitmapData = new BitmapData(param2,param3,true,0);
         var _loc9_:BitmapData = new BitmapData(1,1,true,4294967295);
         var _loc10_:Matrix = new Matrix();
         for each(_loc7_ in param1)
         {
            _loc12_ = (_loc11_ = _loc7_.§+!?§.shape).§`!#§();
            _loc13_ = new Rectangle(_loc12_[0].x / §1r§.§[M§ * param6,_loc12_[0].y / §1r§.§[M§ * param6,(_loc12_[1].x - _loc12_[0].x) / §1r§.§[M§ * param6,(_loc12_[1].y - _loc12_[0].y) / §1r§.§[M§ * param6);
            _loc10_.identity();
            _loc10_.scale(_loc13_.width,_loc13_.height);
            _loc10_.translate(-_loc13_.width / 2,-_loc13_.height / 2);
            _loc10_.rotate((360 - _loc7_.§'!I§()) / 180 * Math.PI);
            _loc10_.translate(_loc7_.sprite.x * param6 - param4,_loc7_.sprite.y * param6 - param5);
            _loc8_.draw(_loc9_,_loc10_);
         }
         _loc9_.dispose();
         return _loc8_;
      }
      
      private function §?F§(param1:Rectangle, param2:BitmapData, param3:Number) : void
      {
         var _loc6_:BitmapData = null;
         var _loc7_:Matrix = null;
         var _loc8_:int = 0;
         var _loc9_:int = 0;
         var _loc10_:int = 0;
         var _loc11_:* = 0;
         var _loc12_:int = 0;
         var _loc13_:* = 0;
         var _loc14_:int = 0;
         var _loc15_:int = 0;
         var _loc4_:String = this.§;§.background.§'P§();
         var _loc5_:§`!9§;
         if(_loc5_ = this.§;§.backgroundTextureManager.§&! §(_loc4_))
         {
            _loc6_ = new BitmapData(_loc5_.bitmapData.width * param3,_loc5_.bitmapData.height * param3);
            (_loc7_ = new Matrix()).scale(param3,param3);
            _loc6_.draw(_loc5_.bitmapData,_loc7_);
            _loc8_ = _loc6_.width - 2;
            _loc9_ = _loc6_.height - 2;
            _loc10_ = param1.top / _loc9_;
            if(param1.top < 0)
            {
               _loc10_--;
            }
            _loc11_ = (_loc11_ = int(param1.bottom / _loc9_)) + 1;
            _loc12_ = param1.left / _loc8_;
            if(param1.left < 0)
            {
               _loc12_--;
            }
            _loc13_ = (_loc13_ = int(param1.right / _loc8_)) + 1;
            _loc14_ = _loc12_;
            while(_loc14_ < _loc13_)
            {
               _loc15_ = _loc10_;
               while(_loc15_ < _loc11_)
               {
                  param2.copyPixels(_loc6_,_loc6_.rect,new Point(_loc14_ * _loc8_,_loc15_ * _loc9_));
                  _loc15_++;
               }
               _loc14_++;
            }
            _loc6_.dispose();
         }
      }
      
      public function §[!!§(param1:Sprite, param2:String, param3:Number, param4:Number, param5:Number = 0, param6:Boolean = false, param7:Boolean = false, param8:Boolean = true, param9:Number = 1.0) : §3!,§
      {
         var _loc10_:§3!,§ = null;
         switch(param2)
         {
            case "BIRD_BLACK":
               _loc10_ = new §implements§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param8,param9);
               break;
            case "BIRD_BLUE":
               _loc10_ = new §9!§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param8,param9);
               break;
            case "BIRD_GREEN":
               _loc10_ = new §<!+§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param8,param9);
               break;
            case "BIRD_WHITE":
               _loc10_ = new §5!M§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param8,param9);
               break;
            case "BIRD_YELLOW":
               _loc10_ = new §6s§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param8,param9);
               break;
            case "BIRD_RED":
               _loc10_ = new §0!I§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param8,param9);
               break;
            case "BIRD_REDBIG":
               _loc10_ = new §<_§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param8,param9);
               break;
            default:
               _loc10_ = new §1!"§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param8,param9);
         }
         return _loc10_;
      }
      
      public function hasBirds() : Boolean
      {
         return this.§5i§ > 0;
      }
      
      public function addObject(param1:String, param2:Number, param3:Number, param4:Number = 0, param5:Boolean = false, param6:Boolean = false, param7:Boolean = true, param8:Number = 1.0, param9:Boolean = false) : §3!,§
      {
         var _loc10_:Sprite;
         (_loc10_ = new Sprite()).scaleX = param8;
         _loc10_.scaleY = param8;
         var _loc11_:§3!,§;
         if((_loc11_ = this.§5!J§(_loc10_,param1,param2,param3,param4,param5,param6,param7,param8)).isTexture())
         {
            _loc10_.visible = !this.§4!2§;
            this.§]!I§[this.§]!I§.length] = _loc11_;
         }
         else
         {
            this.§]!I§[this.§]!I§.length] = _loc11_;
         }
         if(_loc11_ is §1!"§ && !_loc11_.§&0§())
         {
            this.§`5§.addChild(_loc10_);
            ++this.§5i§;
         }
         else if(_loc11_.front || param9)
         {
            this.§!!Q§.addChild(_loc10_);
         }
         else
         {
            this.§+!,§.addChild(_loc10_);
         }
         if(param5)
         {
            this.§;§.§<a§(_loc11_);
         }
         if(param6)
         {
            this.§;§.activeObject = _loc11_;
         }
         return _loc11_;
      }
      
      public function §9#§(param1:int, param2:§3!,§, param3:§3!,§) : §=z§
      {
         var _loc6_:§=z§ = null;
         var _loc4_:int = this.§]!I§.indexOf(param2) - this.§1!4§;
         var _loc5_:int = this.§]!I§.indexOf(param3) - this.§1!4§;
         if(_loc4_ >= 0 && _loc5_ >= 0)
         {
            (_loc6_ = new §=z§(§3,§.§'!!§,_loc4_,_loc5_,new Point(0,0),new Point(0,0))).§''§ = this.§;§.§-!H§.§8f§.CreateJoint(_loc6_.§<,§(param2,param3));
            this.§0b§.push(_loc6_);
            return _loc6_;
         }
         return null;
      }
      
      public function §#M§(param1:§=z§) : void
      {
         if(!(param1.§''§ is b2WeldJoint))
         {
            return;
         }
         var _loc2_:§3!,§ = this.§ for§(param1.index1 + this.§1!4§);
         var _loc3_:§3!,§ = this.§ for§(param1.index2 + this.§1!4§);
         this.§;§.§-!H§.§8f§.DestroyJoint(param1.§''§);
         param1.§''§ = this.§;§.§-!H§.§8f§.CreateJoint(param1.§<,§(_loc2_,_loc3_));
      }
      
      public function §07§(param1:§3!,§) : Vector.<§=z§>
      {
         var _loc4_:§=z§ = null;
         var _loc2_:Vector.<§=z§> = new Vector.<§=z§>();
         var _loc3_:int = this.§]!I§.indexOf(param1) - this.§1!4§;
         if(_loc3_ >= 0)
         {
            for each(_loc4_ in this.§0b§)
            {
               if(_loc4_.index1 == _loc3_ || _loc4_.index2 == _loc3_)
               {
                  _loc2_.push(_loc4_);
               }
            }
         }
         return _loc2_;
      }
      
      public function §[=§() : Vector.<§=z§>
      {
         return this.§0b§;
      }
      
      public function §'=§(param1:§3!,§) : void
      {
         var _loc2_:int = this.§]!I§.indexOf(param1) - this.§1!4§;
         var _loc3_:int = this.§0b§.length - 1;
         while(_loc3_ >= 0)
         {
            if(this.§0b§[_loc3_].index1 == _loc2_ || this.§0b§[_loc3_].index2 == _loc2_)
            {
               this.§0b§.splice(_loc3_,1);
            }
            _loc3_--;
         }
      }
      
      public function §`!"§(param1:§3!,§, param2:§3!,§) : Boolean
      {
         var _loc3_:int = this.§]!I§.indexOf(param1) - this.§1!4§;
         var _loc4_:int = this.§]!I§.indexOf(param2) - this.§1!4§;
         var _loc5_:int = this.§0b§.length - 1;
         while(_loc5_ >= 0)
         {
            if(this.§0b§[_loc5_].index1 == _loc3_ && this.§0b§[_loc5_].index2 == _loc4_ || this.§0b§[_loc5_].index1 == _loc4_ && this.§0b§[_loc5_].index2 == _loc3_)
            {
               return true;
            }
            _loc5_--;
         }
         return false;
      }
      
      protected function §5!J§(param1:Sprite, param2:String, param3:Number, param4:Number, param5:Number = 0, param6:Boolean = false, param7:Boolean = false, param8:Boolean = true, param9:Number = 1.0) : §3!,§
      {
         var _loc10_:§3!,§ = null;
         var _loc11_:§5!1§ = null;
         var _loc12_:§5!1§ = null;
         if(param2.indexOf("BIRD") == 0)
         {
            _loc10_ = this.§[!!§(param1,param2,param3,param4,param5,param6,param7,param8,param9);
            ++this.§?j§;
         }
         else if(param2.indexOf("PIG") == 0)
         {
            _loc11_ = §>U§.§=v§(param2);
            _loc10_ = new §25§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param9,_loc11_.particleJSONId,_loc11_.particleVariationCount);
         }
         else
         {
            if((_loc12_ = §>U§.§=v§(param2)) == null && param2.indexOf("MISC_") == 0)
            {
               param2 = "MISC_FOOD_" + param2.substring(5);
               _loc12_ = §>U§.§=v§(param2);
            }
            if(_loc12_.§]^§ == §5!1§.§?m§ || _loc12_.§]^§ == §5!1§.§9D§)
            {
               _loc10_ = new §1Q§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param9,_loc12_.front,_loc12_.particleJSONId,_loc12_.particleVariationCount);
            }
            else
            {
               _loc10_ = new §3!,§(this,param1,this.§;§.§-!H§.§8f§,this.§;§,this.§7z§,param2,param3,param4,param5,param9,_loc12_.front);
            }
         }
         _loc10_.uniqueID = this.§,P§();
         return _loc10_;
      }
      
      private function §,P§() : String
      {
         var _loc3_:§3!,§ = null;
         var _loc1_:int = this.§]!I§.length;
         var _loc2_:Boolean = false;
         while(!_loc2_)
         {
            _loc2_ = true;
            for each(_loc3_ in this.§]!I§)
            {
               if(_loc3_.uniqueID == _loc1_.toString())
               {
                  _loc1_++;
                  _loc2_ = false;
                  break;
               }
            }
         }
         return _loc1_.toString();
      }
      
      public function §@!=§(param1:Number) : void
      {
         var _loc3_:§3!,§ = null;
         var _loc2_:int = this.§]!I§.length - 1;
         while(_loc2_ >= 0)
         {
            _loc3_ = this.§]!I§[_loc2_] as §3!,§;
            if(_loc3_)
            {
               if(_loc3_.§8!2§ <= 0)
               {
                  this.§`!%§(_loc2_,true,true,true);
               }
               else
               {
                  _loc3_.§@y§();
                  _loc3_.§ !4§();
               }
            }
            _loc2_--;
         }
         this.§@a§(param1);
      }
      
      protected function §@a§(param1:Number) : void
      {
         var _loc3_:§=z§ = null;
         var _loc4_:§2=§ = null;
         var _loc5_:int = 0;
         var _loc6_:§3!,§ = null;
         var _loc7_:Number = NaN;
         var _loc8_:Number = NaN;
         var _loc2_:int = this.§0?§.length - 1;
         while(_loc2_ >= 0)
         {
            if((_loc4_ = this.§0?§[_loc2_]).§38§)
            {
               if((_loc5_ = _loc4_.update(param1)) != -1)
               {
                  if(_loc6_ = this.§[!H§("block_" + _loc5_))
                  {
                     this.§^I§(_loc6_,true,true,true);
                  }
                  this.§0?§.splice(_loc2_,1);
               }
            }
            _loc2_--;
         }
         for each(_loc3_ in this.§0b§)
         {
            if(_loc3_.type == §3,§.§0!9§ && _loc3_.§@L§)
            {
               _loc7_ = (_loc3_.§''§ as b2PrismaticJoint).GetJointTranslation();
               _loc8_ = (_loc3_.§''§ as b2PrismaticJoint).GetMotorSpeed();
               if(_loc3_.§%H§ && _loc3_.§9!0§ && (_loc8_ > 0 && _loc7_ >= _loc3_.upperLimit || _loc8_ < 0 && _loc7_ <= _loc3_.lowerLimit))
               {
                  (_loc3_.§''§ as b2PrismaticJoint).SetMotorSpeed(-_loc8_);
               }
            }
         }
      }
      
      public function addExplosions(param1:int, param2:Number, param3:Number) : void
      {
         this.§ c§.push(§%0§.createExplosion(param1,param2,param3));
         §[!7§.playSound("TntExplosions","ChannelExplosions");
      }
      
      public function §'h§(param1:Number, param2:Number) : int
      {
         var _loc4_:§3!,§ = null;
         var _loc3_:int = this.§]!I§.length - 1;
         while(_loc3_ >= 0)
         {
            if(_loc4_ = this.§]!I§[_loc3_])
            {
               if(_loc4_.isInCoordinates(param1,param2))
               {
                  return _loc3_;
               }
            }
            _loc3_--;
         }
         return -1;
      }
      
      public function §9B§(param1:Number, param2:Number) : §3!,§
      {
         var _loc4_:§3!,§ = null;
         var _loc3_:int = this.§]!I§.length - 1;
         while(_loc3_ >= 0)
         {
            if(_loc4_ = this.§]!I§[_loc3_])
            {
               if(_loc4_.isInCoordinates(param1,param2))
               {
                  return _loc4_;
               }
            }
            _loc3_--;
         }
         return null;
      }
      
      public function § for§(param1:int) : §3!,§
      {
         return this.§]!I§[param1];
      }
      
      public function §break§(param1:Number, param2:Number) : void
      {
         var _loc4_:§3!,§ = null;
         var _loc3_:int = this.§]!I§.length - 1;
         while(_loc3_ >= 0)
         {
            (_loc4_ = this.§]!I§[_loc3_] as §3!,§).§break§(param2,param1);
            _loc3_--;
         }
      }
      
      private function §^w§(param1:§3!,§, param2:Number) : Boolean
      {
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc7_:§3!,§ = null;
         if(!this.mSardineCanAdded)
         {
            return true;
         }
         if(this.mMightyEagleAdded)
         {
            return false;
         }
         var _loc3_:Boolean = false;
         if(param1.§8!2§ == param1.§53§)
         {
            if(this.§;!0§ < this.Tuner.SARDINE_CAN_MAX_ROTATION_SPEED)
            {
               this.§;!0§ += param2 * this.Tuner.SARDINE_CAN_ROTATION_ACCELERATION;
            }
            param1.§`%§().SetAngularVelocity(param2 * this.§;!0§);
            this.§'4§ = 0;
         }
         else if(this.§'4§ == 0)
         {
            this.§'4§ = this.§;§.§>,§;
         }
         if(param1.§6D§() || this.§#X§(param1) || this.§'4§ > 0 && this.§;§.§>,§ > this.§'4§ + this.Tuner.SARDINE_CAN_DELAY_AFTER_HIT)
         {
            if(this.mMightyEagleTimer < this.Tuner.MIGHTY_EAGLE_WAIT_TIME)
            {
               _loc4_ = this.mMightyEagleTimer + param2;
               if(this.mMightyEagleTimer < this.Tuner.MIGHTY_EAGLE_SOUND_DELAY && _loc4_ > this.Tuner.MIGHTY_EAGLE_SOUND_DELAY)
               {
                  §[!7§.playSound("Mighty_Eagle_Selected_1","ChannelBird");
                  §[!7§.playSound("Mighty_Eagle_Flying_1","ChannelBird");
               }
               if(_loc4_ >= this.Tuner.MIGHTY_EAGLE_WAIT_TIME)
               {
                  _loc5_ = param1.§`%§().GetPosition().x - this.Tuner.MIGHTY_EAGLE_STARTING_DISTANCE_X;
                  _loc6_ = param1.§`%§().GetPosition().y - this.Tuner.MIGHTY_EAGLE_STARTING_DISTANCE_Y * this.Tuner.MIGHTY_EAGLE_Y_CHANGE * 1.07;
                  if(this.§#X§(param1))
                  {
                     _loc3_ = true;
                  }
                  _loc7_ = this.addObject("BIRD_MIGHTY_EAGLE",_loc5_,_loc6_,this.Tuner.MIGHTY_EAGLE_FLYING_ANGLE * 1.2);
                  this.mMightyEagleAdded = true;
                  this.§'4§ = 0;
                  _loc7_.§3! §.§%!0§(1.82);
               }
               this.mMightyEagleTimer = _loc4_;
            }
         }
         return _loc3_;
      }
      
      private function §@r§(param1:§3!,§, param2:Number) : Boolean
      {
         var _loc5_:§3!,§ = null;
         if(this.Tuner.MIGHTY_EAGLE_USE_SHADE && !this.§`D§ && this.mMightyEagleTimer > this.Tuner.MIGHTY_EAGLE_SHADING_DELAY)
         {
            this.§`D§ = true;
            this.§;§.§0C§();
         }
         this.mMightyEagleTimer += param2;
         this.§;§.particles.addParticle(§8R§.§#'§,§5!5§.§-U§,§8R§.§`R§,param1.§`%§().GetPosition().x - 3 + Math.random() * (3 * 2),param1.§`%§().GetPosition().y - 3 + Math.random() * (3 * 2),3500,"",§8R§.§[q§(param1.§4V§),0,0,1,0,4);
         var _loc4_:int = 1;
         if(this.mMightyEagleHasTouchedGround)
         {
            param1.§8N§(this.Tuner.MIGHTY_EAGLE_ROTATION_SPEED * param2);
            _loc4_ = -1;
            if(this.§?K§)
            {
               this.§?K§ = false;
               this.§;§.§6!-§();
               param1.§,A§(§0Z§.§-C§);
               this.mSardineCanAdded = false;
               param1.§3! §.§1y§ = true;
               param1.§3! §.§08§();
               for each(_loc5_ in this.§]!I§)
               {
                  if(_loc5_ && _loc5_.§&l§())
                  {
                     _loc5_.§`%§().SetAwake(true);
                     _loc5_.§`%§().SetLinearVelocity(new b2Vec2(0,-18));
                  }
               }
               this.§]F§();
            }
            if(this.mMightyEagleTimer > 6000 && this.isPigsAlive())
            {
               for each(_loc5_ in this.§]!I§)
               {
                  if(_loc5_ && _loc5_.§&l§())
                  {
                     _loc5_.applyDamage(_loc5_.§53§ * 2,true,true,true);
                  }
               }
            }
         }
         else
         {
            this.mMightyEagleHasTouchedGround = param1.§`%§().GetPosition().y >= -5.5;
            this.§?K§ = this.mMightyEagleHasTouchedGround;
         }
         param1.§4#§(param2,new Point(this.Tuner.MIGHTY_EAGLE_X_DIRECTION,_loc4_ * this.Tuner.MIGHTY_EAGLE_Y_CHANGE),this.Tuner.MIGHTY_EAGLE_FLYING_SPEED);
         return false;
      }
      
      private function §@,§() : void
      {
         var _loc1_:§%0§ = null;
         var _loc2_:Number = NaN;
         var _loc3_:Number = NaN;
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc6_:§3!,§ = null;
         var _loc7_:int = 0;
         var _loc8_:Number = NaN;
         var _loc9_:Number = NaN;
         var _loc10_:Number = NaN;
         var _loc11_:Number = NaN;
         var _loc12_:Number = NaN;
         var _loc13_:Number = NaN;
         var _loc14_:Number = NaN;
         var _loc15_:Number = NaN;
         var _loc16_:Number = NaN;
         var _loc17_:Number = NaN;
         while(this.§ c§.length > 0)
         {
            _loc1_ = this.§ c§.shift();
            _loc2_ = _loc1_.§>!Q§;
            _loc3_ = _loc1_.x;
            _loc4_ = _loc1_.y;
            _loc5_ = _loc1_.damage;
            for each(_loc6_ in this.§]!I§)
            {
               _loc8_ = _loc6_.§`%§().GetPosition().x - _loc3_;
               _loc9_ = _loc6_.§`%§().GetPosition().y - _loc4_;
               if((_loc10_ = Math.sqrt(_loc8_ * _loc8_ + _loc9_ * _loc9_)) <= _loc1_.§>!Q§)
               {
                  _loc11_ = _loc1_.push;
                  if(_loc10_ > 1)
                  {
                     _loc11_ /= _loc10_;
                  }
                  if(_loc10_ > 0)
                  {
                     _loc12_ = _loc11_ * (_loc8_ / _loc10_);
                     _loc13_ = _loc11_ * (_loc9_ / _loc10_);
                     _loc6_.§`%§().ApplyImpulse(new b2Vec2(_loc12_,_loc13_),new b2Vec2(_loc3_,_loc4_));
                  }
               }
               if(_loc10_ <= _loc1_.§7!F§)
               {
                  _loc14_ = _loc5_;
                  if(_loc10_ > 1)
                  {
                     _loc14_ /= _loc10_;
                  }
                  this.§!m§(_loc6_,_loc14_);
               }
            }
            this.§;§.particles.addParticle(this.§ X§(_loc1_.type),§5!5§.§-U§,§8R§.§0f§,_loc3_,_loc4_,600,"",§8R§.§^!M§,0,0,0,0,1,20,true);
            _loc7_ = 30;
            while(_loc7_ < 150)
            {
               _loc15_ = 0.75 * _loc2_ + Math.random() * _loc2_;
               _loc16_ = 1250 + Math.random() * 750;
               _loc17_ = _loc7_ / (180 / Math.PI);
               this.§;§.particles.addParticle(§8R§.§=6§,§5!5§.§-U§,§8R§.§`R§,_loc3_,_loc4_,_loc16_,"",§8R§.§^!M§,_loc15_ * Math.cos(_loc17_),-_loc15_ * Math.sin(_loc17_),20,0);
               _loc7_ += 5;
            }
         }
      }
      
      protected function §!m§(param1:§3!,§, param2:Number) : void
      {
         param1.applyDamage(param2,false,false,false,true);
      }
      
      protected function § X§(param1:int) : String
      {
         var _loc2_:* = param1;
         switch(0)
         {
         }
         return §8R§.§1d§;
      }
      
      public function updateLevelObjectsComplete(param1:Number) : void
      {
         var _loc2_:§3!,§ = null;
         var _loc3_:int = this.§]!I§.length - 1;
         while(_loc3_ >= 0)
         {
            _loc2_ = this.§]!I§[_loc3_];
            if(_loc2_.§for§())
            {
               if(this.§^w§(_loc2_,param1))
               {
                  this.§`!%§(_loc3_,false,false,false);
               }
            }
            else if(_loc2_.§&0§())
            {
               this.§@r§(_loc2_,param1);
            }
            else if(this.§#X§(_loc2_))
            {
               _loc2_.§,A§(§0Z§.§-q§);
               this.§`!%§(_loc3_,false,false,false);
            }
            else if(_loc2_.isReadyToBeRemoved(param1))
            {
               _loc2_.§,A§(§0Z§.§-q§);
               this.§`!%§(_loc3_,false,true,true);
            }
            else if(_loc2_.§?s§())
            {
               _loc2_.update(param1);
            }
            _loc3_--;
         }
         this.§@,§();
      }
      
      private function §]F§() : void
      {
         var _loc1_:§=z§ = null;
         for each(_loc1_ in this.§0b§)
         {
            if(_loc1_.type != §3,§.§>R§)
            {
               this.§;§.§-!H§.§8f§.DestroyJoint(_loc1_.§''§);
            }
         }
      }
      
      public function §5R§() : Boolean
      {
         var _loc2_:§3!,§ = null;
         var _loc1_:int = 0;
         while(_loc1_ < this.§]!I§.length)
         {
            _loc2_ = this.§]!I§[_loc1_];
            if(_loc2_.explode())
            {
               return true;
            }
            _loc1_++;
         }
         return false;
      }
      
      public function §#X§(param1:§3!,§) : Boolean
      {
         if(param1 && param1.§+!?§.§&'§() != §,6§.§"I§ && this.§;§.§9!>§.§>x§(param1.§`%§().GetPosition().x,param1.§`%§().GetPosition().y))
         {
            return true;
         }
         return false;
      }
      
      public function §`!%§(param1:int, param2:Boolean = false, param3:Boolean = false, param4:Boolean = false) : void
      {
         var _loc6_:§=z§ = null;
         var _loc7_:§2=§ = null;
         if(param1 < 0)
         {
            return;
         }
         var _loc5_:§3!,§;
         if((_loc5_ = this.§]!I§[param1]).§&l§())
         {
            ++this.§&!4§;
         }
         else if(_loc5_.§?s§())
         {
            --this.§5i§;
         }
         if(_loc5_ == this.§;§.activeObject)
         {
            this.§;§.activeObject = null;
         }
         if(param2)
         {
            this.§;§.addScore(_loc5_.§+!?§.score,§<!!§.§;!$§,true,_loc5_.§`%§().GetPosition().x,_loc5_.§`%§().GetPosition().y - 3,§8R§.§4!§(_loc5_.§4V§));
         }
         if(param3)
         {
            this.addDestructionParticles(_loc5_,this.§;§.particles);
         }
         if(param4)
         {
            this.checkExplosions(_loc5_);
         }
         if(!_loc5_.isTexture())
         {
         }
         this.§8J§(_loc5_.sprite);
         this.§'=§(_loc5_);
         for each(_loc6_ in this.§0b§)
         {
            if(_loc6_.index1 >= param1)
            {
               --_loc6_.index1;
            }
            if(_loc6_.index2 >= param1)
            {
               --_loc6_.index2;
            }
         }
         for each(_loc7_ in this.§0?§)
         {
            if(_loc7_.id1 == param1)
            {
               _loc7_.§38§ = true;
            }
            if(_loc7_.id1 >= param1)
            {
               --_loc7_.id1;
            }
         }
         _loc5_.dispose();
         _loc5_ = null;
         this.§]!I§[param1] = null;
         this.§]!I§.splice(param1,1);
      }
      
      protected function addDestructionParticles(param1:§3!,§, param2:§5!5§) : void
      {
         param1.addDestructionParticles(param2);
      }
      
      private function §8J§(param1:Sprite) : void
      {
         if(this.§+!,§.contains(param1))
         {
            this.§+!,§.removeChild(param1);
            return;
         }
         if(this.§`5§.contains(param1))
         {
            this.§`5§.removeChild(param1);
            return;
         }
         if(this.§2!C§.contains(param1))
         {
            this.§2!C§.removeChild(param1);
            return;
         }
         if(this.§!!Q§.contains(param1))
         {
            this.§!!Q§.removeChild(param1);
            return;
         }
      }
      
      protected function checkExplosions(param1:§3!,§) : void
      {
         if(param1.§-X§().toUpperCase() == "WHITE_EGG" || param1.§51§())
         {
            if(param1.§4V§.toUpperCase() == §5!M§.§?!,§)
            {
               this.addExplosions(§%0§.§?N§,param1.§`%§().GetPosition().x,param1.§`%§().GetPosition().y);
            }
            else
            {
               this.addExplosions(§%0§.§9!'§,param1.§`%§().GetPosition().x,param1.§`%§().GetPosition().y);
            }
         }
      }
      
      public function §^I§(param1:Object, param2:Boolean = false, param3:Boolean = false, param4:Boolean = false) : void
      {
         this.§`!%§(this.§]!I§.indexOf(param1),param2,param3,param4);
      }
      
      public function §<E§(param1:Number, param2:Number) : void
      {
         this.§4!O§.x = -param1;
         this.§4!O§.y = -param2;
      }
      
      public function isPigsAlive(param1:Boolean = false) : Boolean
      {
         var _loc3_:§3!,§ = null;
         var _loc2_:int = 0;
         while(_loc2_ < this.§]!I§.length)
         {
            _loc3_ = this.§]!I§[_loc2_] as §3!,§;
            if(_loc3_ && _loc3_.§&l§() && _loc3_.§8!2§ > 0)
            {
               if(!param1 || _loc3_.§3! §.mTryToBlink <= 0 && _loc3_.§3! §.mTryToScream <= 0)
               {
                  return true;
               }
            }
            _loc2_++;
         }
         return false;
      }
      
      public function §=T§(param1:Boolean = false) : int
      {
         var _loc4_:§3!,§ = null;
         var _loc2_:int = 0;
         var _loc3_:int = this.§]!I§.length - 1;
         while(_loc3_ >= 0)
         {
            if((_loc4_ = this.§]!I§[_loc3_] as §3!,§) && _loc4_.§&l§() && _loc4_.§8!2§ > 0)
            {
               if(!param1 || _loc4_.§3! §.mTryToBlink <= 0 && _loc4_.§3! §.mTryToScream <= 0)
               {
                  _loc2_++;
               }
            }
            _loc3_--;
         }
         return _loc2_;
      }
      
      public function §@Q§() : int
      {
         var _loc2_:§3!,§ = null;
         var _loc1_:int = 0;
         for each(_loc2_ in this.§]!I§)
         {
            if(_loc2_ && (_loc2_.§4H§() || _loc2_.§[!F§()))
            {
               _loc1_++;
            }
         }
         return _loc1_;
      }
      
      public function §!!?§() : int
      {
         var _loc2_:§3!,§ = null;
         var _loc1_:int = 0;
         for each(_loc2_ in this.§]!I§)
         {
            if(_loc2_ && _loc2_.isTexture())
            {
               _loc1_++;
            }
         }
         return _loc1_;
      }
      
      public function makePigsSmile(param1:Number = 1) : void
      {
         var _loc3_:§3!,§ = null;
         var _loc2_:int = this.§]!I§.length - 1;
         while(_loc2_ >= 0)
         {
            _loc3_ = this.§]!I§[_loc2_];
            if(_loc3_ && _loc3_.§&l§() && _loc3_.§8!2§ > 0)
            {
               _loc3_.§3! §.mTryToScream = §?k§.§1J§ * param1;
            }
            _loc2_--;
         }
      }
      
      public function § O§() : Boolean
      {
         var _loc2_:§3!,§ = null;
         var _loc1_:int = this.§]!I§.length - 1;
         while(_loc1_ >= 0)
         {
            _loc2_ = this.§]!I§[_loc1_] as §3!,§;
            if(_loc2_ && _loc2_.§?s§() && _loc2_.§8!2§ > 0)
            {
               return true;
            }
            _loc1_--;
         }
         return false;
      }
      
      public function isWorldAtSleep() : Boolean
      {
         var _loc2_:§3!,§ = null;
         var _loc1_:int = this.§]!I§.length - 1;
         while(_loc1_ >= 0)
         {
            _loc2_ = this.§]!I§[_loc1_] as §3!,§;
            if(_loc2_ && _loc2_.§8!2§ > 0 && _loc2_.§]^§ != §5!1§.§2!M§)
            {
               if(_loc2_.§83§() && !_loc2_.§6D§())
               {
                  return false;
               }
               if(_loc2_.§?s§() && _loc2_.§8!2§ > 0)
               {
                  return false;
               }
            }
            _loc1_--;
         }
         return true;
      }
      
      public function §2Y§(param1:Boolean = false) : §3!,§
      {
         var _loc5_:int = 0;
         var _loc6_:§3!,§ = null;
         if(!this.isPigsAlive())
         {
            return null;
         }
         var _loc2_:int = this.§]!I§.length;
         var _loc3_:int = 1 + Math.random() * this.§=T§(param1);
         var _loc4_:int = 0;
         while(_loc4_ < _loc3_)
         {
            _loc5_ = 0;
            while(_loc5_ < _loc2_)
            {
               if((_loc6_ = this.§]!I§[_loc5_]) && _loc6_.§&l§() && _loc6_.§8!2§ > 0)
               {
                  if(!param1 || _loc6_.§3! §.mTryToBlink <= 0 && _loc6_.§3! §.mTryToScream <= 0)
                  {
                     _loc4_++;
                     if(_loc4_ >= _loc3_)
                     {
                        return _loc6_;
                     }
                  }
               }
               _loc5_++;
            }
         }
         return null;
      }
      
      public function §,u§() : int
      {
         var _loc1_:int = 0;
         var _loc2_:int = this.§]!I§.length - 1;
         while(_loc2_ >= 0)
         {
            _loc1_ += (this.§]!I§[_loc2_] as §3!,§).§+!?§.score;
            if((this.§]!I§[_loc2_] as §3!,§).§83§())
            {
               _loc1_ += §1r§.§5m§.getValue() * int((this.§]!I§[_loc2_] as §3!,§).§53§);
            }
            _loc2_--;
         }
         return int(_loc1_ + this.§;§.slingshot.§,u§());
      }
      
      public function §?n§(param1:§3!,§, param2:§3!,§) : Boolean
      {
         if(param1.§?s§() && param2.§?s§())
         {
            return true;
         }
         if(!param1.§@G§() && !param2.§@G§())
         {
            return true;
         }
         return false;
      }
      
      public function mightyEagleUsed() : void
      {
         this.mSardineCanAdded = true;
         this.§5i§ = 0;
      }
      
      public function objectCollision(param1:§3!,§, param2:§3!,§) : Boolean
      {
         var _loc18_:Number = NaN;
         var _loc19_:Number = NaN;
         if(this.mMightyEagleAdded)
         {
            if(param1.§&0§() || param2.§&l§())
            {
               param2.applyDamage(param2.§53§ * 2,true,true,true,false);
               return true;
            }
            if(param2.§&0§() || param1.§&l§())
            {
               param1.applyDamage(param1.§53§ * 2,true,true,true,false);
               return true;
            }
         }
         else if(this.mSardineCanAdded && this.mMightyEagleTimer == this.Tuner.MIGHTY_EAGLE_WAIT_TIME)
         {
            if(param1.§for§() || param2.§for§())
            {
               this.mMightyEagleTimer = 0;
            }
         }
         if(this.§?n§(param1,param2))
         {
            return false;
         }
         var _loc4_:Number = param1.§3!'§(param2.§-X§());
         var _loc5_:Number = param1.§&F§(param2.§-X§());
         var _loc6_:Number = param2.§3!'§(param1.§-X§());
         var _loc7_:Number = param2.§&F§(param1.§-X§());
         var _loc8_:Number = param1.§`%§().GetMass() * param1.§`%§().GetLinearVelocity().x - param2.§`%§().GetMass() * param2.§`%§().GetLinearVelocity().x;
         var _loc9_:Number = param1.§`%§().GetMass() * param1.§`%§().GetLinearVelocity().y - param2.§`%§().GetMass() * param2.§`%§().GetLinearVelocity().y;
         var _loc10_:Number = Math.sqrt(_loc8_ * _loc8_ + _loc9_ * _loc9_) / 10;
         var _loc11_:Number = _loc4_ * _loc10_;
         var _loc12_:Number = _loc6_ * _loc10_;
         var _loc13_:Number = Math.max(0,param1.§8!2§);
         var _loc14_:Number = Math.max(0,param2.§8!2§);
         var _loc15_:Number = param1.applyDamage(_loc12_,true,param2.§?s§(),_loc14_ == param2.§53§);
         var _loc16_:Number = param2.applyDamage(_loc11_,true,param1.§?s§(),_loc13_ == param1.§53§);
         if(_loc15_ <= 0 && _loc12_ != 0)
         {
            if((_loc18_ = (_loc18_ = (_loc12_ - _loc13_) / _loc12_) * _loc7_) > 1)
            {
               _loc18_ = 1;
            }
            param2.§use§(new b2Vec2(param2.§`%§().GetLinearVelocity().x * _loc18_,param2.§`%§().GetLinearVelocity().y * _loc18_));
         }
         if(_loc16_ <= 0 && _loc11_ != 0)
         {
            if((_loc19_ = (_loc19_ = (_loc11_ - _loc14_) / _loc11_) * _loc5_) > 1)
            {
               _loc19_ = 1;
            }
            param1.§use§(new b2Vec2(param1.§`%§().GetLinearVelocity().x * _loc19_,param1.§`%§().GetLinearVelocity().y * _loc19_));
         }
         return _loc15_ <= 0 && _loc16_ <= 0;
      }
      
      public function §`!'§() : void
      {
         var _loc2_:§3!,§ = null;
         var _loc1_:int = this.§]!I§.length - 1;
         while(_loc1_ >= 0)
         {
            _loc2_ = this.§]!I§[_loc1_] as §3!,§;
            if(_loc2_ != null && _loc2_.§&l§())
            {
               this.§`!%§(_loc1_,true,true,true);
            }
            _loc1_--;
         }
      }
      
      public function §]W§() : void
      {
         var _loc2_:§3!,§ = null;
         var _loc1_:int = this.§]!I§.length - 1;
         while(_loc1_ >= 0)
         {
            _loc2_ = this.§]!I§[_loc1_] as §3!,§;
            if(_loc2_ != null && _loc2_.§51§())
            {
               this.§`!%§(_loc1_,true,true,true);
            }
            _loc1_--;
         }
      }
      
      public function §8G§() : int
      {
         return this.§]!I§.length;
      }
      
      public function §^!#§(param1:§[z§) : void
      {
         var _loc4_:§3!,§ = null;
         var _loc5_:§72§ = null;
         var _loc6_:§3,§ = null;
         var _loc7_:§=z§ = null;
         var _loc2_:Number = 0;
         while(_loc2_ < this.§]!I§.length)
         {
            if(!(_loc4_ = this.§]!I§[_loc2_]).isGround())
            {
               (_loc5_ = new §72§()).angle = _loc4_.§'!I§();
               _loc5_.id = _loc4_.§4V§;
               _loc5_.x = _loc4_.§`%§().GetPosition().x;
               _loc5_.y = _loc4_.§`%§().GetPosition().y;
               _loc5_.uniqueID = _loc4_.uniqueID;
               param1.addObject(_loc5_);
            }
            _loc2_++;
         }
         var _loc3_:Number = 0;
         while(_loc3_ < this.§0b§.length)
         {
            _loc7_ = this.§0b§[_loc3_];
            _loc6_ = new §3,§(_loc7_.type,_loc7_.index1,_loc7_.index2,_loc7_.point1,_loc7_.point2,_loc7_.§<!#§,_loc7_.§%H§,_loc7_.lowerLimit,_loc7_.upperLimit,_loc7_.§9!0§,_loc7_.motorSpeed,_loc7_.§@L§,_loc7_.maxTorque);
            param1.§%y§(_loc6_);
            _loc3_++;
         }
      }
      
      public function §7!@§() : void
      {
         var _loc1_:int = 0;
         while(this.§]!I§.length > _loc1_)
         {
            if(this.§]!I§[_loc1_].isGround())
            {
               _loc1_++;
            }
            else
            {
               this.§^I§(this.§]!I§[_loc1_]);
            }
         }
      }
      
      public function §6E§(param1:Point, param2:Point) : Array
      {
         var _loc3_:Array = new Array();
         var _loc4_:Number = 0;
         while(_loc4_ < this.§]!I§.length)
         {
            if(this.§]!I§[_loc4_].isInsideRectangle(param1.y,param2.y,param1.x,param2.x))
            {
               _loc3_.push(this.§]!I§[_loc4_]);
            }
            _loc4_++;
         }
         return _loc3_;
      }
      
      public function setGroundTextureEnabled(param1:Boolean) : void
      {
         this.§4!2§ = param1;
         this.§6?§(param1);
         var _loc2_:Number = 0;
         while(_loc2_ < this.§]!I§.length)
         {
            if(this.§]!I§[_loc2_].isTexture())
            {
               this.§]!I§[_loc2_].sprite.visible = !this.§4!2§;
            }
            _loc2_++;
         }
      }
      
      public function setDamageEnabled(param1:Boolean) : void
      {
         this.§?!L§ = param1;
      }
      
      public function §6!8§() : Boolean
      {
         return this.§?!L§;
      }
      
      public function §,d§() : int
      {
         return this.§?j§;
      }
      
      public function §9Y§() : int
      {
         return this.§&!4§;
      }
      
      public function §[!H§(param1:String) : §3!,§
      {
         var _loc3_:§3!,§ = null;
         var _loc2_:int = 0;
         while(_loc2_ < this.§]!I§.length)
         {
            _loc3_ = this.§]!I§[_loc2_] as §3!,§;
            if(_loc3_.uniqueID == param1)
            {
               return _loc3_;
            }
            _loc2_++;
         }
         return null;
      }
   }
}
