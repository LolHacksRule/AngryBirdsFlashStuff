package §9!§
{
   import §'+§.§1u§;
   import §+S§.b2Vec2;
   import §<!D§.b2CircleShape;
   import §<!D§.b2PolygonShape;
   import §<!D§.b2Shape;
   
   public class §4!j§
   {
      
      public static const §=!Z§:int = 0;
      
      public static const §3§:int = 1;
      
      public static const §@s§:int = 2;
      
      public static const §3!h§:int = 3;
      
      public static const §'!Q§:int = 4;
      
      public static const §<P§:int = 5;
       
      
      public var mName:String;
      
      public var §<a§:int;
      
      public var §9E§:Number;
      
      public var §;`§:Number;
      
      public var §6M§:Number;
      
      public var §3!Y§:Array = null;
      
      public function §4!j§(param1:String, param2:String, param3:Number, param4:Number, param5:Number = 0)
      {
         super();
         this.mName = param1;
         this.§<a§ = this.§?V§(param2);
         this.§9E§ = param3;
         this.§;`§ = param4;
         this.§6M§ = param5;
      }
      
      public function §[!!§() : int
      {
         return this.§<a§;
      }
      
      public function §^!]§() : Number
      {
         return this.§9E§;
      }
      
      public function §&>§() : int
      {
         return this.§;`§;
      }
      
      private function §?V§(param1:String) : int
      {
         if(param1 == "Rectangle")
         {
            return §3!h§;
         }
         if(param1 == "CIRCLE")
         {
            return §'!Q§;
         }
         if(param1 == "POLYGON")
         {
            return §<P§;
         }
         if(param1 == "Image_Rectangle")
         {
            return §=!Z§;
         }
         if(param1 == "Image_Polygon")
         {
            return §3§;
         }
         if(param1 == "Image_Circle")
         {
            return §@s§;
         }
         §1u§.log("WARNING LevelItemShape has invalid Shape Type, name = " + this.mName + " invalid name = " + param1);
         return §3!h§;
      }
      
      public function §3!<§() : Array
      {
         return this.§3!Y§;
      }
      
      public function §8O§(param1:Array) : void
      {
         this.§3!Y§ = param1;
      }
      
      public function §;!c§() : void
      {
         var _loc1_:Array = this.§-g§();
         var _loc2_:b2Vec2 = _loc1_[0] as b2Vec2;
         var _loc3_:b2Vec2 = _loc1_[1] as b2Vec2;
         this.§9E§ = _loc3_.x - _loc2_.x;
         this.§;`§ = _loc3_.y - _loc2_.y;
      }
      
      public function §-g§() : Array
      {
         var _loc6_:b2Vec2 = null;
         var _loc1_:Number = 0;
         var _loc2_:Number = 0;
         var _loc3_:Number = 0;
         var _loc4_:Number = 0;
         var _loc5_:Boolean = true;
         for each(_loc6_ in this.§3!Y§)
         {
            if(_loc5_)
            {
               _loc1_ = _loc3_ = _loc6_.x;
               _loc2_ = _loc4_ = _loc6_.y;
               _loc5_ = false;
            }
            else
            {
               if(_loc6_.x < _loc1_)
               {
                  _loc1_ = _loc6_.x;
               }
               if(_loc6_.x > _loc3_)
               {
                  _loc3_ = _loc6_.x;
               }
               if(_loc6_.y < _loc2_)
               {
                  _loc2_ = _loc6_.y;
               }
               if(_loc6_.y > _loc4_)
               {
                  _loc4_ = _loc6_.y;
               }
            }
         }
         return [new b2Vec2(_loc1_,_loc2_),new b2Vec2(_loc3_,_loc4_)];
      }
      
      public function §<!8§(param1:Number = 1.0) : b2Shape
      {
         var _loc2_:b2CircleShape = null;
         var _loc3_:Array = null;
         var _loc4_:b2PolygonShape = null;
         var _loc5_:Vector.<b2Vec2> = null;
         var _loc6_:b2Vec2 = null;
         switch(this.§<a§)
         {
            case §'!Q§:
               _loc2_ = new b2CircleShape();
               _loc2_.§<!6§(this.§9E§ * param1);
               return _loc2_;
            case §<P§:
               _loc3_ = this.§3!<§();
               (_loc4_ = new b2PolygonShape()).§>2§(_loc3_,_loc3_.length);
               _loc5_ = _loc4_.§4m§();
               for each(_loc6_ in _loc5_)
               {
                  _loc6_.x *= param1;
                  _loc6_.y *= param1;
               }
               return _loc4_;
            default:
               return null;
         }
      }
   }
}
