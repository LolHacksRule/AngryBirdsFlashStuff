package §+P§
{
   ...
   public class §,,§
   {
      ...
      private function §4x§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§@h§(param1 as String);
         }
         if(param1 is Number)
         {
      ...
      private function §"I§(param1:Array) : String
      {
         ...
         while(_loc3_ < param1.length)
         {
            ...
            _loc2_ += this.§4x§(param1[_loc3_]);
            _loc3_++;
         }
         return "[" + _loc2_ + "]";
      }
      
      private function §%e§(param1:Object) : String
      {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§@h§(v.@name.toString()) + ":" + this.§4x§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
