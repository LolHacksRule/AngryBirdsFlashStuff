package §]k§
{
   public class §[!L§
   {
      
      private static var §]R§:Array;
       
      
      public function §[!L§()
      {
         super();
      }
      
      public static function get §<!#§() : Array
      {
         return §]R§;
      }
      
      public static function §;A§(param1:XMLList) : void
      {
         var _loc2_:XML = null;
         §]R§ = new Array();
         for each(_loc2_ in param1.Background)
         {
            §,#§(_loc2_.@id,_loc2_.Layers,_loc2_.@sky,_loc2_.@ground,_loc2_.@sound,_loc2_.@texture,_loc2_.@textureStrokeColor,_loc2_.@icon,_loc2_.@cost,_loc2_.@objectId,_loc2_.@clearGround);
         }
      }
      
      public static function §,#§(param1:String, param2:XMLList, param3:Number, param4:Number, param5:String, param6:String, param7:Number, param8:String, param9:Number, param10:String, param11:Number = 0) : void
      {
         §]R§[§]R§.length] = new §'m§(param1,param2,param3,param4,param5,param6,param7,param8,param9,param10);
      }
      
      public static function §4!7§(param1:String) : §'m§
      {
         var _loc2_:int = 0;
         while(_loc2_ < §]R§.length)
         {
            if((§]R§[_loc2_] as §'m§).mName.toLowerCase() == param1.toLowerCase())
            {
               return §]R§[_loc2_] as §'m§;
            }
            _loc2_++;
         }
         return null;
      }
      
      public static function §@!Y§() : String
      {
         var _loc1_:int = Math.random() * §]R§.length;
         return (§]R§[_loc1_] as §'m§).mName;
      }
   }
}
