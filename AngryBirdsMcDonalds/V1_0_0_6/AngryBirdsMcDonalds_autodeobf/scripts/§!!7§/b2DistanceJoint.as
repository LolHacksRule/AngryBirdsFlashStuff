package §!!7§
{
   import §7!5§.b2Body;
   import §7!5§.b2TimeStep;
   import §7!Y§.b2Settings;
   import §7!Y§.b2internal;
   import §@![§.b2Mat22;
   import §@![§.b2Math;
   import §@![§.b2Vec2;
   
   use namespace b2internal;
   
   public class b2DistanceJoint extends b2Joint
   {
       
      
      private var m_localAnchor1:b2Vec2;
      
      private var m_localAnchor2:b2Vec2;
      
      private var §9&§:b2Vec2;
      
      private var §6z§:Number;
      
      private var §>!l§:Number;
      
      private var §5!i§:Number;
      
      private var §true §:Number;
      
      private var §^!7§:Number;
      
      private var §0!+§:Number;
      
      private var §"!W§:Number;
      
      public function b2DistanceJoint(param1:b2DistanceJointDef)
      {
         var _loc3_:Number = NaN;
         var _loc4_:Number = NaN;
         this.m_localAnchor1 = new b2Vec2();
         this.m_localAnchor2 = new b2Vec2();
         this.§9&§ = new b2Vec2();
         super(param1);
         this.m_localAnchor1.SetV(param1.§'8§);
         this.m_localAnchor2.SetV(param1.§>!_§);
         this.§"!W§ = param1.length;
         this.§6z§ = param1.§6U§;
         this.§>!l§ = param1.§%R§;
         this.§^!7§ = 0;
         this.§5!i§ = 0;
         this.§true § = 0;
      }
      
      override public function GetAnchorA() : b2Vec2
      {
         return b2internal::-!j.GetWorldPoint(this.m_localAnchor1);
      }
      
      override public function GetAnchorB() : b2Vec2
      {
         return b2internal::30.GetWorldPoint(this.m_localAnchor2);
      }
      
      override public function GetReactionForce(param1:Number) : b2Vec2
      {
         return new b2Vec2(param1 * this.§^!7§ * this.§9&§.x,param1 * this.§^!7§ * this.§9&§.y);
      }
      
      override public function GetReactionTorque(param1:Number) : Number
      {
         return 0;
      }
      
      public function §%M§() : Number
      {
         return this.§"!W§;
      }
      
      public function §1`§(param1:Number) : void
      {
         this.§"!W§ = param1;
      }
      
      public function §%!0§() : Number
      {
         return this.§6z§;
      }
      
      public function §'7§(param1:Number) : void
      {
         this.§6z§ = param1;
      }
      
      public function §,&§() : Number
      {
         return this.§>!l§;
      }
      
      public function §5!3§(param1:Number) : void
      {
         this.§>!l§ = param1;
      }
      
      override b2internal function InitVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc2_:b2Mat22 = null;
         var _loc3_:Number = NaN;
         var _loc4_:b2Body = null;
         var _loc5_:b2Body = null;
         var _loc7_:Number = NaN;
         var _loc9_:Number = NaN;
         var _loc14_:Number = NaN;
         var _loc15_:Number = NaN;
         var _loc16_:Number = NaN;
         var _loc17_:Number = NaN;
         var _loc18_:Number = NaN;
         var _loc19_:Number = NaN;
         _loc4_ = b2internal::-!j;
         _loc5_ = b2internal::30;
         _loc2_ = _loc4_.m_xf.R;
         var _loc6_:Number = this.m_localAnchor1.x - _loc4_.m_sweep.localCenter.x;
         _loc7_ = this.m_localAnchor1.y - _loc4_.m_sweep.localCenter.y;
         _loc3_ = _loc2_.col1.x * _loc6_ + _loc2_.col2.x * _loc7_;
         _loc7_ = _loc2_.col1.y * _loc6_ + _loc2_.col2.y * _loc7_;
         _loc6_ = _loc3_;
         _loc2_ = _loc5_.m_xf.R;
         var _loc8_:Number = this.m_localAnchor2.x - _loc5_.m_sweep.localCenter.x;
         _loc9_ = this.m_localAnchor2.y - _loc5_.m_sweep.localCenter.y;
         _loc3_ = _loc2_.col1.x * _loc8_ + _loc2_.col2.x * _loc9_;
         _loc9_ = _loc2_.col1.y * _loc8_ + _loc2_.col2.y * _loc9_;
         _loc8_ = _loc3_;
         this.§9&§.x = _loc5_.m_sweep.c.x + _loc8_ - _loc4_.m_sweep.c.x - _loc6_;
         this.§9&§.y = _loc5_.m_sweep.c.y + _loc9_ - _loc4_.m_sweep.c.y - _loc7_;
         var _loc10_:Number;
         if((_loc10_ = Math.sqrt(this.§9&§.x * this.§9&§.x + this.§9&§.y * this.§9&§.y)) > b2Settings.b2_linearSlop)
         {
            this.§9&§.§<'§(1 / _loc10_);
         }
         else
         {
            this.§9&§.§%!&§();
         }
         var _loc11_:Number = _loc6_ * this.§9&§.y - _loc7_ * this.§9&§.x;
         var _loc12_:Number = _loc8_ * this.§9&§.y - _loc9_ * this.§9&§.x;
         var _loc13_:Number = _loc4_.§0T§ + _loc4_.§#d§ * _loc11_ * _loc11_ + _loc5_.§0T§ + _loc5_.§#d§ * _loc12_ * _loc12_;
         this.§0!+§ = _loc13_ != 0 ? Number(1 / _loc13_) : Number(0);
         if(this.§6z§ > 0)
         {
            _loc14_ = _loc10_ - this.§"!W§;
            _loc15_ = 2 * Math.PI * this.§6z§;
            _loc16_ = 2 * this.§0!+§ * this.§>!l§ * _loc15_;
            _loc17_ = this.§0!+§ * _loc15_ * _loc15_;
            this.§5!i§ = param1.§4l§ * (_loc16_ + param1.§4l§ * _loc17_);
            this.§5!i§ = this.§5!i§ != 0 ? Number(1 / this.§5!i§) : Number(0);
            this.§true § = _loc14_ * param1.§4l§ * _loc17_ * this.§5!i§;
            this.§0!+§ = _loc13_ + this.§5!i§;
            this.§0!+§ = this.§0!+§ != 0 ? Number(1 / this.§0!+§) : Number(0);
         }
         if(param1.§0!J§)
         {
            this.§^!7§ *= param1.§'!P§;
            _loc18_ = this.§^!7§ * this.§9&§.x;
            _loc19_ = this.§^!7§ * this.§9&§.y;
            _loc4_.§?!%§.x -= _loc4_.§0T§ * _loc18_;
            _loc4_.§?!%§.y -= _loc4_.§0T§ * _loc19_;
            _loc4_.m_angularVelocity -= _loc4_.§#d§ * (_loc6_ * _loc19_ - _loc7_ * _loc18_);
            _loc5_.§?!%§.x += _loc5_.§0T§ * _loc18_;
            _loc5_.§?!%§.y += _loc5_.§0T§ * _loc19_;
            _loc5_.m_angularVelocity += _loc5_.§#d§ * (_loc8_ * _loc19_ - _loc9_ * _loc18_);
         }
         else
         {
            this.§^!7§ = 0;
         }
      }
      
      override b2internal function SolveVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc2_:b2Mat22 = null;
         var _loc3_:b2Body = b2internal::-!j;
         var _loc4_:b2Body = b2internal::30;
         _loc2_ = _loc3_.m_xf.R;
         var _loc5_:Number = this.m_localAnchor1.x - _loc3_.m_sweep.localCenter.x;
         var _loc6_:Number = this.m_localAnchor1.y - _loc3_.m_sweep.localCenter.y;
         var _loc7_:Number = _loc2_.col1.x * _loc5_ + _loc2_.col2.x * _loc6_;
         _loc6_ = _loc2_.col1.y * _loc5_ + _loc2_.col2.y * _loc6_;
         _loc5_ = _loc7_;
         _loc2_ = _loc4_.m_xf.R;
         var _loc8_:Number = this.m_localAnchor2.x - _loc4_.m_sweep.localCenter.x;
         var _loc9_:Number = this.m_localAnchor2.y - _loc4_.m_sweep.localCenter.y;
         _loc7_ = _loc2_.col1.x * _loc8_ + _loc2_.col2.x * _loc9_;
         _loc9_ = _loc2_.col1.y * _loc8_ + _loc2_.col2.y * _loc9_;
         _loc8_ = _loc7_;
         var _loc10_:Number = _loc3_.§?!%§.x + -_loc3_.m_angularVelocity * _loc6_;
         var _loc11_:Number = _loc3_.§?!%§.y + _loc3_.m_angularVelocity * _loc5_;
         var _loc12_:Number = _loc4_.§?!%§.x + -_loc4_.m_angularVelocity * _loc9_;
         var _loc13_:Number = _loc4_.§?!%§.y + _loc4_.m_angularVelocity * _loc8_;
         var _loc14_:Number = this.§9&§.x * (_loc12_ - _loc10_) + this.§9&§.y * (_loc13_ - _loc11_);
         var _loc15_:Number = -this.§0!+§ * (_loc14_ + this.§true § + this.§5!i§ * this.§^!7§);
         this.§^!7§ += _loc15_;
         var _loc16_:Number = _loc15_ * this.§9&§.x;
         var _loc17_:Number = _loc15_ * this.§9&§.y;
         _loc3_.§?!%§.x -= _loc3_.§0T§ * _loc16_;
         _loc3_.§?!%§.y -= _loc3_.§0T§ * _loc17_;
         _loc3_.m_angularVelocity -= _loc3_.§#d§ * (_loc5_ * _loc17_ - _loc6_ * _loc16_);
         _loc4_.§?!%§.x += _loc4_.§0T§ * _loc16_;
         _loc4_.§?!%§.y += _loc4_.§0T§ * _loc17_;
         _loc4_.m_angularVelocity += _loc4_.§#d§ * (_loc8_ * _loc17_ - _loc9_ * _loc16_);
      }
      
      override b2internal function SolvePositionConstraints(param1:Number) : Boolean
      {
         var _loc2_:b2Mat22 = null;
         if(this.§6z§ > 0)
         {
            return true;
         }
         var _loc3_:b2Body = b2internal::-!j;
         var _loc4_:b2Body = b2internal::30;
         _loc2_ = _loc3_.m_xf.R;
         var _loc5_:Number = this.m_localAnchor1.x - _loc3_.m_sweep.localCenter.x;
         var _loc6_:Number = this.m_localAnchor1.y - _loc3_.m_sweep.localCenter.y;
         var _loc7_:Number = _loc2_.col1.x * _loc5_ + _loc2_.col2.x * _loc6_;
         _loc6_ = _loc2_.col1.y * _loc5_ + _loc2_.col2.y * _loc6_;
         _loc5_ = _loc7_;
         _loc2_ = _loc4_.m_xf.R;
         var _loc8_:Number = this.m_localAnchor2.x - _loc4_.m_sweep.localCenter.x;
         var _loc9_:Number = this.m_localAnchor2.y - _loc4_.m_sweep.localCenter.y;
         _loc7_ = _loc2_.col1.x * _loc8_ + _loc2_.col2.x * _loc9_;
         _loc9_ = _loc2_.col1.y * _loc8_ + _loc2_.col2.y * _loc9_;
         _loc8_ = _loc7_;
         var _loc10_:Number = _loc4_.m_sweep.c.x + _loc8_ - _loc3_.m_sweep.c.x - _loc5_;
         var _loc11_:Number = _loc4_.m_sweep.c.y + _loc9_ - _loc3_.m_sweep.c.y - _loc6_;
         var _loc12_:Number = Math.sqrt(_loc10_ * _loc10_ + _loc11_ * _loc11_);
         _loc10_ /= _loc12_;
         _loc11_ /= _loc12_;
         var _loc13_:Number = _loc12_ - this.§"!W§;
         _loc13_ = b2Math.§1!P§(_loc13_,-b2Settings.b2_maxLinearCorrection,b2Settings.b2_maxLinearCorrection);
         var _loc14_:Number = -this.§0!+§ * _loc13_;
         this.§9&§.Set(_loc10_,_loc11_);
         var _loc15_:Number = _loc14_ * this.§9&§.x;
         var _loc16_:Number = _loc14_ * this.§9&§.y;
         _loc3_.m_sweep.c.x -= _loc3_.§0T§ * _loc15_;
         _loc3_.m_sweep.c.y -= _loc3_.§0T§ * _loc16_;
         _loc3_.m_sweep.a -= _loc3_.§#d§ * (_loc5_ * _loc16_ - _loc6_ * _loc15_);
         _loc4_.m_sweep.c.x += _loc4_.§0T§ * _loc15_;
         _loc4_.m_sweep.c.y += _loc4_.§0T§ * _loc16_;
         _loc4_.m_sweep.a += _loc4_.§#d§ * (_loc8_ * _loc16_ - _loc9_ * _loc15_);
         _loc3_.§5!%§();
         _loc4_.§5!%§();
         return b2Math.§]!b§(_loc13_) < b2Settings.b2_linearSlop;
      }
   }
}
