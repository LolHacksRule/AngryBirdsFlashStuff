package §8r§
{
   ...
   public class §1!_§
   {
      ...
      private function §#%§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§0Y§(param1 as String);
         }
         if(param1 is Number)
         {
      ...
      private function §0Y§(param1:String) : String
      {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§0Y§(v.@name.toString()) + ":" + this.§#%§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
