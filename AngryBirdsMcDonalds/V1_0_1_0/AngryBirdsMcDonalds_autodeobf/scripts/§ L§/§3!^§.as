package § L§
{
   ...
   public class §3!^§
   {
      ...
      private function §^p§(param1:*) : String
      {
         ...
         if(param1 is Array)
         {
            return this.§1X§(param1 as Array);
         }
      ...
      private function §1X§(param1:Array) : String
      {
         var _loc2_:* = "";
         var _loc3_:int = 0;
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§]#§(v.@name.toString()) + ":" + this.§^p§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
