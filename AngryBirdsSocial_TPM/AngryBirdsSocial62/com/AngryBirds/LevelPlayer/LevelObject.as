package com.AngryBirds.LevelPlayer
{
   ...
   public class LevelObject extends Sprite
   {
      ...
      public function LevelObject(param1:LevelObjects, param2:b2World, param3:int, param4:String, param5:Number, param6:Number, param7:Number)
      {
         ...
         this.mHealth = this.mHealthMax = this.getVolume() * this.mLevelItem.getItemStrenght();
      ...
      public function interpolateRenderer(param1:Number, param2:Number = -1) : void
      {
         ...
         {
            _loc3_ = AngryBirdsSocial.smLevelMain.mLevelParticles.getGroup(LevelParticles.PARTICLE_GROUP_TRAILS);
            _loc4_ = 0;
            while(_loc4_ < _loc3_.length)
            {
               if(!_loc3_[_loc4_].visible && _loc3_[_loc4_].x < this.x)
               {
      ...
      public function applyDamage(param1:Number, param2:Boolean = true, param3:Boolean = false) : Number
      {
         ...
         else
         {
            ...
            if(this.isBird())
            {
               ...
               _loc6_ = 1 + _loc5_ * this.getVolume() * 0.15;
               _loc7_ = 90;
               _loc8_ = 0;
               while(_loc8_ < _loc6_)
               {
      ...
      public function getVolume() : Number
      {
         ...
         {
            return this.mRenderer.mW * this.mRenderer.mH * (LevelMain.PIXEL_TO_B2_SCALE * LevelMain.PIXEL_TO_B2_SCALE);
         }
         return this.getBody().GetMass() / this.mFixture.GetDensity();
      }
   }
}
