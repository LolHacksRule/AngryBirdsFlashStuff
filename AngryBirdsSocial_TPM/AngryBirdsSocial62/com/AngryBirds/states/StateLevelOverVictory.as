package com.AngryBirds.states
{
   ...
   public class StateLevelOverVictory extends StateBase
   {
      ...
      override public function uiInteractionHandler(param1:int, param2:String, param3:UIEventListenerRovio) : void
      {
         ...
         switch(param2)
         {
            ...
            case "LIKE_LEVEL":
               ...
               AngryBirdsServer.setLevelLike(true,AngryBirdsSocial.smLevelMain.mLevelData.levelID.toString(),this.levelRatingResponseReceived);
               break;
            case "DISLIKE_LEVEL":
               (mUIView as ViewLevelEnd).mButtonGroupLikeDislike.buttonSelected((param3 as UIButtonRovio).mName);
               AngryBirdsServer.setLevelLike(false,AngryBirdsSocial.smLevelMain.mLevelData.levelID.toString(),this.levelRatingResponseReceived);
         }
      }
      ...
      private function setScoreData() : void
      {
         ...
         _loc1_[0] = new Array();
         var _loc2_:Number = 0;
         while(_loc2_ < this.mScoreList.length || _loc2_ < 7)
         {
            ...
            {
               _loc6_ = null;
               if(_loc3_.user.userName == AngryBirdsSocial.USER.name)
               {
                  _loc6_ = AssetCache.getAssetFromCache("FriendScore_Component_Player");
               }
               else
               {
                  _loc6_ = AssetCache.getAssetFromCache("FriendScore_Component_Friend");
               }
               _loc4_.@name = "Friend" + _loc2_;
               _loc4_.@MouseUp = "Friend" + _loc2_;
               (_loc5_ = new _loc6_()).Text_Ranking.text = (_loc2_ + 1).toString();
            ...
            {
               _loc5_ = new (_loc7_ = AssetCache.getAssetFromCache("Button_AddFriends"))();
               _loc4_.@name = "Button_AddFriends";
               _loc4_.@MouseUp = "Button_AddFriends";
            }
            (_loc1_[0] as Array).push(new Array(_loc4_,null,_loc5_));
            _loc2_++;
         }
