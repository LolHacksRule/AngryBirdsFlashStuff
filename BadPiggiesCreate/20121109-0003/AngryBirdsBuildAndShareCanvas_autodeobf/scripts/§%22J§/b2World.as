package §"J§
{
   import §%4§.*;
   import §,!0§.*;
   import §0a§.b2Controller;
   import §0a§.b2ControllerEdge;
   import §2k§.*;
   import §;!e§.*;
   import §<!a§.*;
   import §<";§.*;
   
   use namespace b2internal;
   
   public class b2World
   {
      
      private static var s_timestep2:b2TimeStep = new b2TimeStep();
      
      private static var §"r§:b2Transform = new b2Transform();
      
      private static var §false§:b2Sweep = new b2Sweep();
      
      private static var §+z§:b2Sweep = new b2Sweep();
      
      private static var §9V§:b2TimeStep = new b2TimeStep();
      
      private static var §]!E§:Vector.<b2Body> = new Vector.<b2Body>();
      
      private static var §`!X§:b2Color = new b2Color(0.5,0.8,0.8);
      
      private static var §"!9§:Boolean;
      
      private static var §;!I§:Boolean;
      
      public static const §&=§:int = 1;
      
      public static const §7!l§:int = 2;
       
      
      private var §[#§:Vector.<b2Body>;
      
      b2internal var §]!+§:int;
      
      b2internal var §>"7§:b2ContactManager;
      
      private var §else§:b2ContactSolver;
      
      private var §?o§:b2Island;
      
      b2internal var §[_§:b2Body;
      
      private var §,!j§:b2Joint;
      
      b2internal var §<W§:b2Contact;
      
      private var §7!h§:int;
      
      b2internal var §4A§:int;
      
      private var §'#§:int;
      
      private var §`"=§:b2Controller;
      
      private var §-0§:int;
      
      private var §+M§:b2Vec2;
      
      private var §@"6§:Boolean;
      
      b2internal var m_groundBody:b2Body;
      
      private var §,Y§:b2DestructionListener;
      
      private var §[5§:b2DebugDraw;
      
      private var §7"1§:Number;
      
      public function b2World(param1:b2Vec2, param2:Boolean)
      {
         this.§[#§ = new Vector.<b2Body>();
         this.§>"7§ = new b2ContactManager();
         this.§else§ = new b2ContactSolver();
         this.§?o§ = new b2Island();
         super();
         this.§,Y§ = null;
         this.§[5§ = null;
         this.§[_§ = null;
         this.§<W§ = null;
         this.§,!j§ = null;
         this.§`"=§ = null;
         this.§7!h§ = 0;
         this.§4A§ = 0;
         this.§'#§ = 0;
         this.§-0§ = 0;
         §"!9§ = true;
         §;!I§ = true;
         this.§@"6§ = param2;
         this.§+M§ = param1;
         this.§7"1§ = 0;
         this.§>"7§.m_world = this;
         var _loc3_:b2BodyDef = new b2BodyDef();
         this.m_groundBody = this.§`0§(_loc3_);
      }
      
      public function §1!^§(param1:b2DestructionListener) : void
      {
         this.§,Y§ = param1;
      }
      
      public function §1y§(param1:b2ContactFilter) : void
      {
         this.§>"7§.§!!i§ = param1;
      }
      
      public function §-`§(param1:b2ContactListener) : void
      {
         this.§>"7§.§`"!§ = param1;
      }
      
      public function §+!]§(param1:b2DebugDraw) : void
      {
         this.§[5§ = param1;
      }
      
      public function §7!N§(param1:§0w§) : void
      {
         var _loc4_:b2Fixture = null;
         var _loc2_:§0w§ = this.§>"7§.§58§;
         this.§>"7§.§58§ = param1;
         var _loc3_:b2Body = this.§[_§;
         while(_loc3_)
         {
            _loc4_ = _loc3_.§%!9§;
            while(_loc4_)
            {
               _loc4_.§<"3§ = param1.§1S§(_loc2_.§=!p§(_loc4_.§<"3§),_loc4_);
               _loc4_ = _loc4_.§7!'§;
            }
            _loc3_ = _loc3_.§7!'§;
         }
      }
      
      public function §]3§() : void
      {
         this.§>"7§.§58§.§]3§();
      }
      
      public function §?!a§() : int
      {
         return this.§>"7§.§58§.§?!a§();
      }
      
      public function §`0§(param1:b2BodyDef) : b2Body
      {
         if(this.§&<§() == true)
         {
            return null;
         }
         var _loc2_:b2Body = new b2Body(param1,this);
         _loc2_.§+!w§ = null;
         _loc2_.§7!'§ = this.§[_§;
         if(this.§[_§)
         {
            this.§[_§.§+!w§ = _loc2_;
         }
         this.§[_§ = _loc2_;
         ++this.§7!h§;
         return _loc2_;
      }
      
      public function §`E§(param1:b2Body) : void
      {
         var _loc6_:b2JointEdge = null;
         var _loc7_:b2ControllerEdge = null;
         var _loc8_:b2ContactEdge = null;
         var _loc9_:b2Fixture = null;
         if(this.§&<§() == true)
         {
            return;
         }
         var _loc2_:b2JointEdge = param1.§,!j§;
         while(_loc2_)
         {
            _loc6_ = _loc2_;
            _loc2_ = _loc2_.§>"2§;
            if(this.§,Y§)
            {
               this.§,Y§.§1e§(_loc6_.§"!3§);
            }
            this.§ !;§(_loc6_.§"!3§);
         }
         var _loc3_:b2ControllerEdge = param1.§`"=§;
         while(_loc3_)
         {
            _loc7_ = _loc3_;
            _loc3_ = _loc3_.§<1§;
            _loc7_.§=R§.§>"<§(param1);
         }
         var _loc4_:b2ContactEdge = param1.§<W§;
         while(_loc4_)
         {
            _loc8_ = _loc4_;
            _loc4_ = _loc4_.§>"2§;
            this.§>"7§.§3!0§(_loc8_.§6M§);
         }
         param1.§<W§ = null;
         var _loc5_:b2Fixture = param1.§%!9§;
         while(_loc5_)
         {
            _loc9_ = _loc5_;
            _loc5_ = _loc5_.§7!'§;
            if(this.§,Y§)
            {
               this.§,Y§.§-!=§(_loc9_);
            }
            _loc9_.§`q§(this.§>"7§.§58§);
            _loc9_.§3!0§();
         }
         param1.§%!9§ = null;
         param1.§93§ = 0;
         if(param1.§+!w§)
         {
            param1.§+!w§.§7!'§ = param1.§7!'§;
         }
         if(param1.§7!'§)
         {
            param1.§7!'§.§+!w§ = param1.§+!w§;
         }
         if(param1 == this.§[_§)
         {
            this.§[_§ = param1.§7!'§;
         }
         --this.§7!h§;
      }
      
      public function §'!Y§(param1:b2JointDef) : b2Joint
      {
         var _loc5_:b2ContactEdge = null;
         var _loc2_:b2Joint = b2Joint.§`!P§(param1,null);
         _loc2_.§+!w§ = null;
         _loc2_.§7!'§ = this.§,!j§;
         if(this.§,!j§)
         {
            this.§,!j§.§+!w§ = _loc2_;
         }
         this.§,!j§ = _loc2_;
         ++this.§'#§;
         _loc2_.§#Z§.§"!3§ = _loc2_;
         _loc2_.§#Z§.§;5§ = _loc2_.§"!l§;
         _loc2_.§#Z§.§8!?§ = null;
         _loc2_.§#Z§.§>"2§ = _loc2_.§+d§.§,!j§;
         if(_loc2_.§+d§.§,!j§)
         {
            _loc2_.§+d§.§,!j§.§8!?§ = _loc2_.§#Z§;
         }
         _loc2_.§+d§.§,!j§ = _loc2_.§#Z§;
         _loc2_.§package§.§"!3§ = _loc2_;
         _loc2_.§package§.§;5§ = _loc2_.§+d§;
         _loc2_.§package§.§8!?§ = null;
         _loc2_.§package§.§>"2§ = _loc2_.§"!l§.§,!j§;
         if(_loc2_.§"!l§.§,!j§)
         {
            _loc2_.§"!l§.§,!j§.§8!?§ = _loc2_.§package§;
         }
         _loc2_.§"!l§.§,!j§ = _loc2_.§package§;
         var _loc3_:b2Body = param1.§ !T§;
         var _loc4_:b2Body = param1.§import§;
         if(param1.collideConnected == false)
         {
            _loc5_ = _loc4_.§3!$§();
            while(_loc5_)
            {
               if(_loc5_.§;5§ == _loc3_)
               {
                  _loc5_.§6M§.§"N§();
               }
               _loc5_ = _loc5_.§>"2§;
            }
         }
         return _loc2_;
      }
      
      public function § !;§(param1:b2Joint) : void
      {
         var _loc5_:b2ContactEdge = null;
         var _loc2_:Boolean = param1.§6I§;
         if(param1.§+!w§)
         {
            param1.§+!w§.§7!'§ = param1.§7!'§;
         }
         if(param1.§7!'§)
         {
            param1.§7!'§.§+!w§ = param1.§+!w§;
         }
         if(param1 == this.§,!j§)
         {
            this.§,!j§ = param1.§7!'§;
         }
         var _loc3_:b2Body = param1.§+d§;
         var _loc4_:b2Body = param1.§"!l§;
         _loc3_.SetAwake(true);
         _loc4_.SetAwake(true);
         if(param1.§#Z§.§8!?§)
         {
            param1.§#Z§.§8!?§.§>"2§ = param1.§#Z§.§>"2§;
         }
         if(param1.§#Z§.§>"2§)
         {
            param1.§#Z§.§>"2§.§8!?§ = param1.§#Z§.§8!?§;
         }
         if(param1.§#Z§ == _loc3_.§,!j§)
         {
            _loc3_.§,!j§ = param1.§#Z§.§>"2§;
         }
         param1.§#Z§.§8!?§ = null;
         param1.§#Z§.§>"2§ = null;
         if(param1.§package§.§8!?§)
         {
            param1.§package§.§8!?§.§>"2§ = param1.§package§.§>"2§;
         }
         if(param1.§package§.§>"2§)
         {
            param1.§package§.§>"2§.§8!?§ = param1.§package§.§8!?§;
         }
         if(param1.§package§ == _loc4_.§,!j§)
         {
            _loc4_.§,!j§ = param1.§package§.§>"2§;
         }
         param1.§package§.§8!?§ = null;
         param1.§package§.§>"2§ = null;
         b2Joint.§3!0§(param1,null);
         --this.§'#§;
         if(_loc2_ == false)
         {
            _loc5_ = _loc4_.§3!$§();
            while(_loc5_)
            {
               if(_loc5_.§;5§ == _loc3_)
               {
                  _loc5_.§6M§.§"N§();
               }
               _loc5_ = _loc5_.§>"2§;
            }
         }
      }
      
      public function §<!B§(param1:b2Controller) : b2Controller
      {
         param1.§7!'§ = this.§`"=§;
         param1.§+!w§ = null;
         this.§`"=§ = param1;
         param1.m_world = this;
         ++this.§-0§;
         return param1;
      }
      
      public function §#y§(param1:b2Controller) : void
      {
         if(param1.§+!w§)
         {
            param1.§+!w§.§7!'§ = param1.§7!'§;
         }
         if(param1.§7!'§)
         {
            param1.§7!'§.§+!w§ = param1.§+!w§;
         }
         if(this.§`"=§ == param1)
         {
            this.§`"=§ = param1.§7!'§;
         }
         --this.§-0§;
      }
      
      public function §^"8§(param1:b2Controller) : b2Controller
      {
         if(param1.m_world != this)
         {
            throw new Error("Controller can only be a member of one world");
         }
         param1.§7!'§ = this.§`"=§;
         param1.§+!w§ = null;
         if(this.§`"=§)
         {
            this.§`"=§.§+!w§ = param1;
         }
         this.§`"=§ = param1;
         ++this.§-0§;
         param1.m_world = this;
         return param1;
      }
      
      public function §=e§(param1:b2Controller) : void
      {
         param1.§"F§();
         if(param1.§7!'§)
         {
            param1.§7!'§.§+!w§ = param1.§+!w§;
         }
         if(param1.§+!w§)
         {
            param1.§+!w§.§7!'§ = param1.§7!'§;
         }
         if(param1 == this.§`"=§)
         {
            this.§`"=§ = param1.§7!'§;
         }
         --this.§-0§;
      }
      
      public function §7"5§(param1:Boolean) : void
      {
         §"!9§ = param1;
      }
      
      public function §const§(param1:Boolean) : void
      {
         §;!I§ = param1;
      }
      
      public function §7!b§() : int
      {
         return this.§7!h§;
      }
      
      public function §-!M§() : int
      {
         return this.§'#§;
      }
      
      public function § "9§() : int
      {
         return this.§4A§;
      }
      
      public function §#!E§(param1:b2Vec2) : void
      {
         this.§+M§ = param1;
      }
      
      public function §#!w§() : b2Vec2
      {
         return this.§+M§;
      }
      
      public function §8!+§() : b2Body
      {
         return this.m_groundBody;
      }
      
      public function §^!6§(param1:Number, param2:int, param3:int) : void
      {
         if(this.§]!+§ & §&=§)
         {
            this.§>"7§.§8u§();
            this.§]!+§ &= ~§&=§;
         }
         this.§]!+§ |= §7!l§;
         var _loc4_:b2TimeStep;
         (_loc4_ = s_timestep2).§@[§ = param1;
         _loc4_.§ k§ = param2;
         _loc4_.§["4§ = param3;
         if(param1 > 0)
         {
            _loc4_.§!!K§ = 1 / param1;
         }
         else
         {
            _loc4_.§!!K§ = 0;
         }
         _loc4_.§class§ = this.§7"1§ * param1;
         _loc4_.§@!D§ = §"!9§;
         this.§>"7§.§^!Z§();
         if(_loc4_.§@[§ > 0)
         {
            this.§1!R§(_loc4_);
         }
         if(§;!I§ && _loc4_.§@[§ > 0)
         {
            this.§4f§(_loc4_);
         }
         if(_loc4_.§@[§ > 0)
         {
            this.§7"1§ = _loc4_.§!!K§;
         }
         this.§]!+§ &= ~§7!l§;
      }
      
      public function §0^§() : void
      {
         var _loc1_:b2Body = this.§[_§;
         while(_loc1_)
         {
            _loc1_.§1!3§.§?b§();
            _loc1_.§ !v§ = 0;
            _loc1_ = _loc1_.§7!'§;
         }
      }
      
      public function § "+§() : void
      {
         var _loc3_:b2Body = null;
         var _loc4_:b2Fixture = null;
         var _loc5_:b2Shape = null;
         var _loc6_:b2Joint = null;
         var _loc7_:§0w§ = null;
         var _loc11_:b2Transform = null;
         var _loc16_:b2Controller = null;
         var _loc17_:b2Contact = null;
         var _loc18_:b2Fixture = null;
         var _loc19_:b2Fixture = null;
         var _loc20_:b2Vec2 = null;
         var _loc21_:b2Vec2 = null;
         var _loc22_:b2AABB = null;
         if(this.§[5§ == null)
         {
            return;
         }
         this.§[5§.§5!H§.graphics.clear();
         var _loc1_:uint = this.§[5§.§=m§();
         var _loc8_:b2Vec2 = new b2Vec2();
         var _loc9_:b2Vec2 = new b2Vec2();
         var _loc10_:b2Vec2 = new b2Vec2();
         var _loc12_:b2AABB = new b2AABB();
         var _loc13_:b2AABB = new b2AABB();
         var _loc14_:Array = [new b2Vec2(),new b2Vec2(),new b2Vec2(),new b2Vec2()];
         var _loc15_:b2Color = new b2Color(0,0,0);
         if(_loc1_ & b2DebugDraw.§^!F§)
         {
            _loc3_ = this.§[_§;
            while(_loc3_)
            {
               _loc11_ = _loc3_.m_xf;
               _loc4_ = _loc3_.GetFixtureList();
               while(_loc4_)
               {
                  _loc5_ = _loc4_.GetShape();
                  if(_loc3_.§0!9§() == false)
                  {
                     _loc15_.Set(0.5,0.5,0.3);
                     this.§3!2§(_loc5_,_loc11_,_loc15_);
                  }
                  else if(_loc3_.§;!F§() == b2Body.b2_staticBody)
                  {
                     _loc15_.Set(0.5,0.9,0.5);
                     this.§3!2§(_loc5_,_loc11_,_loc15_);
                  }
                  else if(_loc3_.§;!F§() == b2Body.b2_kinematicBody)
                  {
                     _loc15_.Set(0.5,0.5,0.9);
                     this.§3!2§(_loc5_,_loc11_,_loc15_);
                  }
                  else if(_loc3_.IsAwake() == false)
                  {
                     _loc15_.Set(0.6,0.6,0.6);
                     this.§3!2§(_loc5_,_loc11_,_loc15_);
                  }
                  else
                  {
                     _loc15_.Set(0.9,0.7,0.7);
                     this.§3!2§(_loc5_,_loc11_,_loc15_);
                  }
                  _loc4_ = _loc4_.§7!'§;
               }
               _loc3_ = _loc3_.§7!'§;
            }
         }
         if(_loc1_ & b2DebugDraw.§^!M§)
         {
            _loc6_ = this.§,!j§;
            while(_loc6_)
            {
               this.§%!r§(_loc6_);
               _loc6_ = _loc6_.§7!'§;
            }
         }
         if(_loc1_ & b2DebugDraw.§,!I§)
         {
            _loc16_ = this.§`"=§;
            while(_loc16_)
            {
               _loc16_.§'c§(this.§[5§);
               _loc16_ = _loc16_.§7!'§;
            }
         }
         if(_loc1_ & b2DebugDraw.§<!Z§)
         {
            _loc15_.Set(0.3,0.9,0.9);
            _loc17_ = this.§>"7§.§<W§;
            while(_loc17_)
            {
               _loc18_ = _loc17_.§&!K§();
               _loc19_ = _loc17_.§ 1§();
               _loc20_ = _loc18_.§1!x§().§1C§();
               _loc21_ = _loc19_.§1!x§().§1C§();
               this.§[5§.§?Z§(_loc20_,_loc21_,_loc15_);
               _loc17_ = _loc17_.§7"3§();
            }
         }
         if(_loc1_ & b2DebugDraw.§+!1§)
         {
            _loc7_ = this.§>"7§.§58§;
            _loc14_ = [new b2Vec2(),new b2Vec2(),new b2Vec2(),new b2Vec2()];
            _loc3_ = this.§[_§;
            while(_loc3_)
            {
               if(_loc3_.§0!9§() != false)
               {
                  _loc4_ = _loc3_.GetFixtureList();
                  while(_loc4_)
                  {
                     _loc22_ = _loc7_.§=!p§(_loc4_.§<"3§);
                     _loc14_[0].Set(_loc22_.§=!H§.x,_loc22_.§=!H§.y);
                     _loc14_[1].Set(_loc22_.§6P§.x,_loc22_.§=!H§.y);
                     _loc14_[2].Set(_loc22_.§6P§.x,_loc22_.§6P§.y);
                     _loc14_[3].Set(_loc22_.§=!H§.x,_loc22_.§6P§.y);
                     this.§[5§.§ "8§(_loc14_,4,_loc15_);
                     _loc4_ = _loc4_.§7"3§();
                  }
               }
               _loc3_ = _loc3_.§7"3§();
            }
         }
         if(_loc1_ & b2DebugDraw.§#!=§)
         {
            _loc3_ = this.§[_§;
            while(_loc3_)
            {
               (_loc11_ = §"r§).R = _loc3_.m_xf.R;
               _loc11_.position = _loc3_.§?Q§();
               this.§[5§.§8"$§(_loc11_);
               _loc3_ = _loc3_.§7!'§;
            }
         }
      }
      
      public function §@!Q§(param1:Function, param2:b2AABB) : void
      {
         var broadPhase:§0w§ = null;
         var WorldQueryWrapper:Function = null;
         var callback:Function = param1;
         var aabb:b2AABB = param2;
         WorldQueryWrapper = function(param1:*):Boolean
         {
            return callback(broadPhase.GetUserData(param1));
         };
         broadPhase = this.§>"7§.§58§;
         broadPhase.§5%§(WorldQueryWrapper,aabb);
      }
      
      public function §"5§(param1:Function, param2:b2Shape, param3:b2Transform = null) : void
      {
         var broadPhase:§0w§ = null;
         var WorldQueryWrapper:Function = null;
         var callback:Function = param1;
         var shape:b2Shape = param2;
         var transform:b2Transform = param3;
         WorldQueryWrapper = function(param1:*):Boolean
         {
            var _loc2_:b2Fixture = broadPhase.GetUserData(param1) as b2Fixture;
            if(b2Shape.§6m§(shape,transform,_loc2_.GetShape(),_loc2_.GetBody().§60§()))
            {
               return callback(_loc2_);
            }
            return true;
         };
         if(transform == null)
         {
            transform = new b2Transform();
            transform.§@!c§();
         }
         broadPhase = this.§>"7§.§58§;
         var aabb:b2AABB = new b2AABB();
         shape.ComputeAABB(aabb,transform);
         broadPhase.§5%§(WorldQueryWrapper,aabb);
      }
      
      public function §>!E§(param1:Function, param2:b2Vec2) : void
      {
         var broadPhase:§0w§ = null;
         var WorldQueryWrapper:Function = null;
         var callback:Function = param1;
         var p:b2Vec2 = param2;
         WorldQueryWrapper = function(param1:*):Boolean
         {
            var _loc2_:b2Fixture = broadPhase.GetUserData(param1) as b2Fixture;
            if(_loc2_.TestPoint(p))
            {
               return callback(_loc2_);
            }
            return true;
         };
         broadPhase = this.§>"7§.§58§;
         var aabb:b2AABB = new b2AABB();
         aabb.§=!H§.Set(p.x - b2Settings.b2_linearSlop,p.y - b2Settings.b2_linearSlop);
         aabb.§6P§.Set(p.x + b2Settings.b2_linearSlop,p.y + b2Settings.b2_linearSlop);
         broadPhase.§5%§(WorldQueryWrapper,aabb);
      }
      
      public function RayCast(param1:Function, param2:b2Vec2, param3:b2Vec2) : void
      {
         var broadPhase:§0w§ = null;
         var output:b2RayCastOutput = null;
         var RayCastWrapper:Function = null;
         var callback:Function = param1;
         var point1:b2Vec2 = param2;
         var point2:b2Vec2 = param3;
         RayCastWrapper = function(param1:b2RayCastInput, param2:*):Number
         {
            var _loc6_:Number = NaN;
            var _loc7_:b2Vec2 = null;
            var _loc3_:* = broadPhase.GetUserData(param2);
            var _loc5_:Boolean;
            var _loc4_:b2Fixture;
            if(_loc5_ = (_loc4_ = _loc3_ as b2Fixture).RayCast(output,param1))
            {
               _loc6_ = output.fraction;
               _loc7_ = new b2Vec2((1 - _loc6_) * point1.x + _loc6_ * point2.x,(1 - _loc6_) * point1.y + _loc6_ * point2.y);
               return callback(_loc4_,_loc7_,output.normal,_loc6_);
            }
            return param1.§"!7§;
         };
         broadPhase = this.§>"7§.§58§;
         output = new b2RayCastOutput();
         var input:b2RayCastInput = new b2RayCastInput(point1,point2);
         broadPhase.RayCast(RayCastWrapper,input);
      }
      
      public function §-Z§(param1:b2Vec2, param2:b2Vec2) : b2Fixture
      {
         var result:b2Fixture = null;
         var RayCastOneWrapper:Function = null;
         var point1:b2Vec2 = param1;
         var point2:b2Vec2 = param2;
         RayCastOneWrapper = function(param1:b2Fixture, param2:b2Vec2, param3:b2Vec2, param4:Number):Number
         {
            result = param1;
            return param4;
         };
         this.RayCast(RayCastOneWrapper,point1,point2);
         return result;
      }
      
      public function §0!O§(param1:b2Vec2, param2:b2Vec2) : Vector.<b2Fixture>
      {
         var result:Vector.<b2Fixture> = null;
         var RayCastAllWrapper:Function = null;
         var point1:b2Vec2 = param1;
         var point2:b2Vec2 = param2;
         RayCastAllWrapper = function(param1:b2Fixture, param2:b2Vec2, param3:b2Vec2, param4:Number):Number
         {
            result[result.length] = param1;
            return 1;
         };
         result = new Vector.<b2Fixture>();
         this.RayCast(RayCastAllWrapper,point1,point2);
         return result;
      }
      
      public function §&"2§() : b2Body
      {
         return this.§[_§;
      }
      
      public function §-]§() : b2Joint
      {
         return this.§,!j§;
      }
      
      public function §3!$§() : b2Contact
      {
         return this.§<W§;
      }
      
      public function §&<§() : Boolean
      {
         return (this.§]!+§ & §7!l§) > 0;
      }
      
      b2internal function §1!R§(param1:b2TimeStep) : void
      {
         var _loc2_:b2Body = null;
         var _loc10_:int = 0;
         var _loc11_:int = 0;
         var _loc12_:b2Body = null;
         var _loc13_:b2ContactEdge = null;
         var _loc14_:b2JointEdge = null;
         var _loc3_:b2Controller = this.§`"=§;
         while(_loc3_)
         {
            _loc3_.§^!6§(param1);
            _loc3_ = _loc3_.§7!'§;
         }
         var _loc4_:b2Island;
         (_loc4_ = this.§?o§).§&!§(this.§7!h§,this.§4A§,this.§'#§,null,this.§>"7§.§`"!§,this.§else§);
         _loc2_ = this.§[_§;
         while(_loc2_)
         {
            _loc2_.§]!+§ &= ~b2Body.§3Q§;
            _loc2_ = _loc2_.§7!'§;
         }
         var _loc5_:b2Contact = this.§<W§;
         while(_loc5_)
         {
            _loc5_.§]!+§ &= ~b2Contact.§3Q§;
            _loc5_ = _loc5_.§7!'§;
         }
         var _loc6_:b2Joint = this.§,!j§;
         while(_loc6_)
         {
            _loc6_.§=y§ = false;
            _loc6_ = _loc6_.§7!'§;
         }
         var _loc7_:int = this.§7!h§;
         var _loc8_:Vector.<b2Body> = this.§[#§;
         var _loc9_:b2Body = this.§[_§;
         while(_loc9_)
         {
            if(!(_loc9_.§]!+§ & b2Body.§3Q§))
            {
               if(!(_loc9_.IsAwake() == false || _loc9_.§0!9§() == false))
               {
                  if(_loc9_.§;!F§() != b2Body.b2_staticBody)
                  {
                     _loc4_.§"F§();
                     _loc10_ = 0;
                     var _loc15_:*;
                     _loc8_[_loc15_ = _loc10_++] = _loc9_;
                     _loc9_.§]!+§ |= b2Body.§3Q§;
                     while(_loc10_ > 0)
                     {
                        _loc2_ = _loc8_[--_loc10_];
                        _loc4_.§@!O§(_loc2_);
                        if(_loc2_.IsAwake() == false)
                        {
                           _loc2_.SetAwake(true);
                        }
                        if(_loc2_.§;!F§() != b2Body.b2_staticBody)
                        {
                           _loc13_ = _loc2_.§<W§;
                           while(_loc13_)
                           {
                              if(!(_loc13_.§6M§.§]!+§ & b2Contact.§3Q§))
                              {
                                 if(!(_loc13_.§6M§.§;!g§() == true || _loc13_.§6M§.§4!s§() == false || _loc13_.§6M§.§!j§() == false))
                                 {
                                    _loc4_.§2&§(_loc13_.§6M§);
                                    _loc13_.§6M§.§]!+§ |= b2Contact.§3Q§;
                                    if(!((_loc12_ = _loc13_.§;5§).§]!+§ & b2Body.§3Q§))
                                    {
                                       var _loc16_:*;
                                       _loc8_[_loc16_ = _loc10_++] = _loc12_;
                                       _loc12_.§]!+§ |= b2Body.§3Q§;
                                    }
                                 }
                              }
                              _loc13_ = _loc13_.§>"2§;
                           }
                           _loc14_ = _loc2_.§,!j§;
                           while(_loc14_)
                           {
                              if(_loc14_.§"!3§.§=y§ != true)
                              {
                                 if((_loc12_ = _loc14_.§;5§).§0!9§() != false)
                                 {
                                    _loc4_.§&![§(_loc14_.§"!3§);
                                    _loc14_.§"!3§.§=y§ = true;
                                    if(!(_loc12_.§]!+§ & b2Body.§3Q§))
                                    {
                                       _loc8_[_loc16_ = _loc10_++] = _loc12_;
                                       _loc12_.§]!+§ |= b2Body.§3Q§;
                                    }
                                 }
                              }
                              _loc14_ = _loc14_.§>"2§;
                           }
                        }
                     }
                     _loc4_.§1!R§(param1,this.§+M§,this.§@"6§);
                     _loc11_ = 0;
                     while(_loc11_ < _loc4_.§7!h§)
                     {
                        _loc2_ = _loc4_.§`i§[_loc11_];
                        if(_loc2_.§;!F§() == b2Body.b2_staticBody)
                        {
                           _loc2_.§]!+§ &= ~b2Body.§3Q§;
                        }
                        _loc11_++;
                     }
                  }
               }
            }
            _loc9_ = _loc9_.§7!'§;
         }
         _loc11_ = 0;
         while(_loc11_ < _loc8_.length)
         {
            if(!_loc8_[_loc11_])
            {
               break;
            }
            _loc8_[_loc11_] = null;
            _loc11_++;
         }
         _loc2_ = this.§[_§;
         while(_loc2_)
         {
            if(!(_loc2_.IsAwake() == false || _loc2_.§0!9§() == false))
            {
               if(_loc2_.§;!F§() != b2Body.b2_staticBody)
               {
                  _loc2_.§]"!§();
               }
            }
            _loc2_ = _loc2_.§7!'§;
         }
         this.§>"7§.§8u§();
      }
      
      b2internal function §4f§(param1:b2TimeStep) : void
      {
         var _loc2_:b2Body = null;
         var _loc3_:b2Fixture = null;
         var _loc4_:b2Fixture = null;
         var _loc5_:b2Body = null;
         var _loc6_:b2Body = null;
         var _loc7_:b2ContactEdge = null;
         var _loc8_:b2Joint = null;
         var _loc11_:b2Contact = null;
         var _loc12_:b2Contact = null;
         var _loc13_:Number = NaN;
         var _loc14_:b2Body = null;
         var _loc15_:int = 0;
         var _loc16_:int = 0;
         var _loc17_:b2TimeStep = null;
         var _loc18_:int = 0;
         var _loc19_:Number = NaN;
         var _loc20_:Number = NaN;
         var _loc21_:b2JointEdge = null;
         var _loc22_:b2Body = null;
         var _loc9_:b2Island;
         (_loc9_ = this.§?o§).§&!§(this.§7!h§,b2Settings.b2_maxTOIContactsPerIsland,b2Settings.b2_maxTOIJointsPerIsland,null,this.§>"7§.§`"!§,this.§else§);
         var _loc10_:Vector.<b2Body> = §]!E§;
         _loc2_ = this.§[_§;
         while(_loc2_)
         {
            _loc2_.§]!+§ &= ~b2Body.§3Q§;
            _loc2_.m_sweep.§,>§ = 0;
            _loc2_ = _loc2_.§7!'§;
         }
         _loc11_ = this.§<W§;
         while(_loc11_)
         {
            _loc11_.§]!+§ &= ~(b2Contact.§>!u§ | b2Contact.§3Q§);
            _loc11_ = _loc11_.§7!'§;
         }
         _loc8_ = this.§,!j§;
         while(_loc8_)
         {
            _loc8_.§=y§ = false;
            _loc8_ = _loc8_.§7!'§;
         }
         while(true)
         {
            _loc12_ = null;
            _loc13_ = 1;
            _loc11_ = this.§<W§;
            for(; _loc11_; _loc11_ = _loc11_.§7!'§)
            {
               if(!(_loc11_.§;!g§() == true || _loc11_.§4!s§() == false || _loc11_.§'!q§() == false))
               {
                  _loc19_ = 1;
                  if(_loc11_.§]!+§ & b2Contact.§>!u§)
                  {
                     _loc19_ = _loc11_.§1"8§;
                  }
                  else
                  {
                     _loc3_ = _loc11_.§9!<§;
                     _loc4_ = _loc11_.§%,§;
                     _loc5_ = _loc3_.m_body;
                     _loc6_ = _loc4_.m_body;
                     if((_loc5_.§;!F§() != b2Body.b2_dynamicBody || _loc5_.IsAwake() == false) && (_loc6_.§;!F§() != b2Body.b2_dynamicBody || _loc6_.IsAwake() == false))
                     {
                        continue;
                     }
                     _loc20_ = _loc5_.m_sweep.§,>§;
                     if(_loc5_.m_sweep.§,>§ < _loc6_.m_sweep.§,>§)
                     {
                        _loc20_ = _loc6_.m_sweep.§,>§;
                        _loc5_.m_sweep.§%E§(_loc20_);
                     }
                     else if(_loc6_.m_sweep.§,>§ < _loc5_.m_sweep.§,>§)
                     {
                        _loc20_ = _loc5_.m_sweep.§,>§;
                        _loc6_.m_sweep.§%E§(_loc20_);
                     }
                     _loc19_ = _loc11_.§+I§(_loc5_.m_sweep,_loc6_.m_sweep);
                     b2Settings.b2Assert(0 <= _loc19_ && _loc19_ <= 1);
                     if(_loc19_ > 0 && _loc19_ < 1)
                     {
                        if((_loc19_ = (1 - _loc19_) * _loc20_ + _loc19_) > 1)
                        {
                           _loc19_ = 1;
                        }
                     }
                     _loc11_.§1"8§ = _loc19_;
                     _loc11_.§]!+§ |= b2Contact.§>!u§;
                  }
                  if(Number.MIN_VALUE < _loc19_ && _loc19_ < _loc13_)
                  {
                     _loc12_ = _loc11_;
                     _loc13_ = _loc19_;
                  }
               }
            }
            if(_loc12_ == null || 1 - 100 * Number.MIN_VALUE < _loc13_)
            {
               break;
            }
            _loc3_ = _loc12_.§9!<§;
            _loc4_ = _loc12_.§%,§;
            _loc5_ = _loc3_.m_body;
            _loc6_ = _loc4_.m_body;
            §false§.Set(_loc5_.m_sweep);
            §+z§.Set(_loc6_.m_sweep);
            _loc5_.§%E§(_loc13_);
            _loc6_.§%E§(_loc13_);
            _loc12_.§]!J§(this.§>"7§.§`"!§);
            _loc12_.§]!+§ &= ~b2Contact.§>!u§;
            if(_loc12_.§;!g§() == true || _loc12_.§4!s§() == false)
            {
               _loc5_.m_sweep.Set(§false§);
               _loc6_.m_sweep.Set(§+z§);
               _loc5_.§=&§();
               _loc6_.§=&§();
            }
            else if(_loc12_.§!j§() != false)
            {
               if((_loc14_ = _loc5_).§;!F§() != b2Body.b2_dynamicBody)
               {
                  _loc14_ = _loc6_;
               }
               _loc9_.§"F§();
               _loc15_ = 0;
               _loc16_ = 0;
               _loc10_[_loc15_ + _loc16_++] = _loc14_;
               _loc14_.§]!+§ |= b2Body.§3Q§;
               while(_loc16_ > 0)
               {
                  _loc2_ = _loc10_[_loc15_++];
                  _loc16_--;
                  _loc9_.§@!O§(_loc2_);
                  if(_loc2_.IsAwake() == false)
                  {
                     _loc2_.SetAwake(true);
                  }
                  if(_loc2_.§;!F§() == b2Body.b2_dynamicBody)
                  {
                     _loc7_ = _loc2_.§<W§;
                     while(_loc7_)
                     {
                        if(_loc9_.§4A§ == _loc9_.§+!%§)
                        {
                           break;
                        }
                        if(!(_loc7_.§6M§.§]!+§ & b2Contact.§3Q§))
                        {
                           if(!(_loc7_.§6M§.§;!g§() == true || _loc7_.§6M§.§4!s§() == false || _loc7_.§6M§.§!j§() == false))
                           {
                              _loc9_.§2&§(_loc7_.§6M§);
                              _loc7_.§6M§.§]!+§ |= b2Contact.§3Q§;
                              if(!((_loc22_ = _loc7_.§;5§).§]!+§ & b2Body.§3Q§))
                              {
                                 if(_loc22_.§;!F§() != b2Body.b2_staticBody)
                                 {
                                    _loc22_.§%E§(_loc13_);
                                    _loc22_.SetAwake(true);
                                 }
                                 _loc10_[_loc15_ + _loc16_] = _loc22_;
                                 _loc16_++;
                                 _loc22_.§]!+§ |= b2Body.§3Q§;
                              }
                           }
                        }
                        _loc7_ = _loc7_.§>"2§;
                     }
                     _loc21_ = _loc2_.§,!j§;
                     while(_loc21_)
                     {
                        if(_loc9_.§'#§ != _loc9_.§1"=§)
                        {
                           if(_loc21_.§"!3§.§=y§ != true)
                           {
                              if((_loc22_ = _loc21_.§;5§).§0!9§() != false)
                              {
                                 _loc9_.§&![§(_loc21_.§"!3§);
                                 _loc21_.§"!3§.§=y§ = true;
                                 if(!(_loc22_.§]!+§ & b2Body.§3Q§))
                                 {
                                    if(_loc22_.§;!F§() != b2Body.b2_staticBody)
                                    {
                                       _loc22_.§%E§(_loc13_);
                                       _loc22_.SetAwake(true);
                                    }
                                    _loc10_[_loc15_ + _loc16_] = _loc22_;
                                    _loc16_++;
                                    _loc22_.§]!+§ |= b2Body.§3Q§;
                                 }
                              }
                           }
                        }
                        _loc21_ = _loc21_.§>"2§;
                     }
                  }
               }
               (_loc17_ = §9V§).§@!D§ = false;
               _loc17_.§@[§ = (1 - _loc13_) * param1.§@[§;
               _loc17_.§!!K§ = 1 / _loc17_.§@[§;
               _loc17_.§class§ = 0;
               _loc17_.§ k§ = param1.§ k§;
               _loc17_.§["4§ = param1.§["4§;
               _loc9_.§4f§(_loc17_);
               _loc18_ = 0;
               while(_loc18_ < _loc9_.§7!h§)
               {
                  _loc2_ = _loc9_.§`i§[_loc18_];
                  _loc2_.§]!+§ &= ~b2Body.§3Q§;
                  if(_loc2_.IsAwake() != false)
                  {
                     if(_loc2_.§;!F§() == b2Body.b2_dynamicBody)
                     {
                        _loc2_.§]"!§();
                        _loc7_ = _loc2_.§<W§;
                        while(_loc7_)
                        {
                           _loc7_.§6M§.§]!+§ &= ~b2Contact.§>!u§;
                           _loc7_ = _loc7_.§>"2§;
                        }
                     }
                  }
                  _loc18_++;
               }
               _loc18_ = 0;
               while(_loc18_ < _loc9_.§4A§)
               {
                  _loc11_ = _loc9_.§8U§[_loc18_];
                  _loc11_.§]!+§ &= ~(b2Contact.§>!u§ | b2Contact.§3Q§);
                  _loc18_++;
               }
               _loc18_ = 0;
               while(_loc18_ < _loc9_.§'#§)
               {
                  (_loc8_ = _loc9_.§#r§[_loc18_]).§=y§ = false;
                  _loc18_++;
               }
               this.§>"7§.§8u§();
            }
         }
      }
      
      b2internal function §%!r§(param1:b2Joint) : void
      {
         var _loc11_:b2PulleyJoint = null;
         var _loc12_:b2Vec2 = null;
         var _loc13_:b2Vec2 = null;
         var _loc2_:b2Body = param1.§<D§();
         var _loc3_:b2Body = param1.§3@§();
         var _loc4_:b2Transform = _loc2_.m_xf;
         var _loc5_:b2Transform = _loc3_.m_xf;
         var _loc6_:b2Vec2 = _loc4_.position;
         var _loc7_:b2Vec2 = _loc5_.position;
         var _loc8_:b2Vec2 = param1.GetAnchorA();
         var _loc9_:b2Vec2 = param1.GetAnchorB();
         var _loc10_:b2Color = §`!X§;
         switch(param1.§0!,§)
         {
            case b2Joint.§'t§:
               this.§[5§.§?Z§(_loc8_,_loc9_,_loc10_);
               break;
            case b2Joint.§-i§:
               _loc12_ = (_loc11_ = param1 as b2PulleyJoint).§,!e§();
               _loc13_ = _loc11_.§?"&§();
               this.§[5§.§?Z§(_loc12_,_loc8_,_loc10_);
               this.§[5§.§?Z§(_loc13_,_loc9_,_loc10_);
               this.§[5§.§?Z§(_loc12_,_loc13_,_loc10_);
               break;
            case b2Joint.§,s§:
               this.§[5§.§?Z§(_loc8_,_loc9_,_loc10_);
               break;
            default:
               if(_loc2_ != this.m_groundBody)
               {
                  this.§[5§.§?Z§(_loc6_,_loc8_,_loc10_);
               }
               this.§[5§.§?Z§(_loc8_,_loc9_,_loc10_);
               if(_loc3_ != this.m_groundBody)
               {
                  this.§[5§.§?Z§(_loc7_,_loc9_,_loc10_);
               }
         }
      }
      
      b2internal function §3!2§(param1:b2Shape, param2:b2Transform, param3:b2Color) : void
      {
         var _loc4_:b2CircleShape = null;
         var _loc5_:b2Vec2 = null;
         var _loc6_:Number = NaN;
         var _loc7_:b2Vec2 = null;
         var _loc8_:int = 0;
         var _loc9_:b2PolygonShape = null;
         var _loc10_:int = 0;
         var _loc11_:Vector.<b2Vec2> = null;
         var _loc12_:Vector.<b2Vec2> = null;
         var _loc13_:b2EdgeShape = null;
         switch(param1.§0!,§)
         {
            case b2Shape.§;n§:
               _loc4_ = param1 as b2CircleShape;
               _loc5_ = b2Math.§#!K§(param2,_loc4_.§!X§);
               _loc6_ = _loc4_.§&!$§;
               _loc7_ = param2.R.col1;
               this.§[5§.§6"6§(_loc5_,_loc6_,_loc7_,param3);
               break;
            case b2Shape.§<!D§:
               _loc10_ = (_loc9_ = param1 as b2PolygonShape).§]!`§();
               _loc11_ = _loc9_.§`"3§();
               _loc12_ = new Vector.<b2Vec2>(_loc10_);
               _loc8_ = 0;
               while(_loc8_ < _loc10_)
               {
                  _loc12_[_loc8_] = b2Math.§#!K§(param2,_loc11_[_loc8_]);
                  _loc8_++;
               }
               this.§[5§.§8w§(_loc12_,_loc10_,param3);
               break;
            case b2Shape.§=!E§:
               _loc13_ = param1 as b2EdgeShape;
               this.§[5§.§?Z§(b2Math.§#!K§(param2,_loc13_.GetVertex1()),b2Math.§#!K§(param2,_loc13_.GetVertex2()),param3);
         }
      }
   }
}
