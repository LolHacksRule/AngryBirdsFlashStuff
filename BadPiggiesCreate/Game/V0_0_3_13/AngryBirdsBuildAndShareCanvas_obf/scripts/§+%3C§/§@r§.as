package §+<§
{
   import §%!C§.§'8§;
   
   public class §@r§
   {
      
      public static const §9!z§:uint = 16777215;
      
      public static const §6!y§:uint = 12632256;
      
      public static const §%!=§:uint = 8421504;
      
      public static const §<k§:uint = 0;
      
      public static const RED:uint = 16711680;
      
      public static const §'!G§:uint = 8388608;
      
      public static const §1+§:uint = 16776960;
      
      public static const §9I§:uint = 8421376;
      
      public static const §9a§:uint = 65280;
      
      public static const §=!H§:uint = 32768;
      
      public static const §^l§:uint = 65535;
      
      public static const §-=§:uint = 32896;
      
      public static const §?!I§:uint = 255;
      
      public static const §&!M§:uint = 128;
      
      public static const §=!V§:uint = 16711935;
      
      public static const §+h§:uint = 8388736;
       
      
      public function §@r§()
      {
         super();
         throw new §'8§();
      }
      
      public static function §""$§(param1:uint) : int
      {
         return param1 >> 24 & 255;
      }
      
      public static function §?!E§(param1:uint) : int
      {
         return param1 >> 16 & 255;
      }
      
      public static function §]%§(param1:uint) : int
      {
         return param1 >> 8 & 255;
      }
      
      public static function §&n§(param1:uint) : int
      {
         return param1 & 255;
      }
      
      public static function §7!§(param1:int, param2:int, param3:int) : uint
      {
         return param1 << 16 | param2 << 8 | param3;
      }
      
      public static function §;G§(param1:int, param2:int, param3:int, param4:int) : uint
      {
         return param1 << 24 | param2 << 16 | param3 << 8 | param4;
      }
   }
}
