package §=v§
{
   import §<;§.§,!9§;
   
   public class §?X§
   {
      
      public static const §&z§:uint = 16777215;
      
      public static const §=!>§:uint = 12632256;
      
      public static const §<>§:uint = 8421504;
      
      public static const §4t§:uint = 0;
      
      public static const RED:uint = 16711680;
      
      public static const §8L§:uint = 8388608;
      
      public static const §@!c§:uint = 16776960;
      
      public static const § !`§:uint = 8421376;
      
      public static const §4!'§:uint = 65280;
      
      public static const §#1§:uint = 32768;
      
      public static const §',§:uint = 65535;
      
      public static const §`z§:uint = 32896;
      
      public static const §;C§:uint = 255;
      
      public static const §0![§:uint = 128;
      
      public static const §6@§:uint = 16711935;
      
      public static const §%#§:uint = 8388736;
       
      
      public function §?X§()
      {
         super();
         throw new §,!9§();
      }
      
      public static function §;!s§(param1:uint) : int
      {
         return param1 >> 24 & 255;
      }
      
      public static function §3P§(param1:uint) : int
      {
         return param1 >> 16 & 255;
      }
      
      public static function §2z§(param1:uint) : int
      {
         return param1 >> 8 & 255;
      }
      
      public static function §`!f§(param1:uint) : int
      {
         return param1 & 255;
      }
      
      public static function §<%§(param1:int, param2:int, param3:int) : uint
      {
         return param1 << 16 | param2 << 8 | param3;
      }
      
      public static function §&G§(param1:int, param2:int, param3:int, param4:int) : uint
      {
         return param1 << 24 | param2 << 16 | param3 << 8 | param4;
      }
   }
}
