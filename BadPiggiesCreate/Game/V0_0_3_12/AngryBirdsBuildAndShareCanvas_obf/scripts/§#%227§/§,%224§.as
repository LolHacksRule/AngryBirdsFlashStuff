package §#"7§
{
   import flash.utils.ByteArray;
   import flash.utils.Dictionary;
   import flash.utils.Endian;
   import flash.utils.getTimer;
   
   public class §,"4§
   {
      
      private static var initialized:Boolean = false;
      
      private static const §0!F§:Dictionary = new Dictionary();
      
      private static const §!!,§:Dictionary = new Dictionary();
      
      private static const §#"=§:Dictionary = new Dictionary();
      
      private static const §7x§:int = 4;
      
      private static const §%!v§:int = 256;
      
      private static const FRAGMENT:String = "fragment";
      
      private static const VERTEX:String = "vertex";
      
      private static const §?"1§:uint = 12;
      
      private static const §>6§:uint = 16;
      
      private static const §^"!§:uint = 20;
      
      private static const §#" §:uint = 24;
      
      private static const §8!R§:uint = 28;
      
      private static const §#;§:uint = 1;
      
      private static const §[!I§:uint = 2;
      
      private static const §#!h§:uint = 32;
      
      private static const §,[§:uint = 64;
      
      private static const §%u§:uint = 1;
      
      private static const §[!%§:uint = 2;
      
      private static const §=!G§:uint = 4;
      
      private static const §&"2§:uint = 8;
      
      private static const §+2§:uint = 16;
      
      private static const §,1§:uint = 32;
      
      private static const §2u§:uint = 128;
      
      private static const §^f§:String = "mov";
      
      private static const ADD:String = "add";
      
      private static const §7"§:String = "sub";
      
      private static const §36§:String = "mul";
      
      private static const §6q§:String = "div";
      
      private static const §<"%§:String = "rcp";
      
      private static const §^M§:String = "min";
      
      private static const §1"8§:String = "max";
      
      private static const §9!o§:String = "frc";
      
      private static const §4!?§:String = "sqt";
      
      private static const §"!l§:String = "rsq";
      
      private static const §+v§:String = "pow";
      
      private static const §'"2§:String = "log";
      
      private static const §'y§:String = "exp";
      
      private static const §!q§:String = "nrm";
      
      private static const §15§:String = "sin";
      
      private static const §"c§:String = "cos";
      
      private static const §[e§:String = "crs";
      
      private static const DP3:String = "dp3";
      
      private static const DP4:String = "dp4";
      
      private static const §[g§:String = "abs";
      
      private static const §1p§:String = "neg";
      
      private static const §^!1§:String = "sat";
      
      private static const M33:String = "m33";
      
      private static const M44:String = "m44";
      
      private static const M34:String = "m34";
      
      private static const §"!H§:String = "ifz";
      
      private static const §#=§:String = "inz";
      
      private static const §,!;§:String = "ife";
      
      private static const §!"6§:String = "ine";
      
      private static const §;!B§:String = "ifg";
      
      private static const §]!0§:String = "ifl";
      
      private static const §3d§:String = "ieg";
      
      private static const §6!h§:String = "iel";
      
      private static const §?"!§:String = "els";
      
      private static const §?m§:String = "eif";
      
      private static const §#!Y§:String = "rep";
      
      private static const §4"'§:String = "erp";
      
      private static const § !Q§:String = "brk";
      
      private static const §^a§:String = "kil";
      
      private static const §6"+§:String = "tex";
      
      private static const §=!O§:String = "sge";
      
      private static const §`E§:String = "slt";
      
      private static const §2c§:String = "sgn";
      
      private static const §'!R§:String = "va";
      
      private static const §;c§:String = "vc";
      
      private static const §0!c§:String = "vt";
      
      private static const §0`§:String = "op";
      
      private static const V:String = "v";
      
      private static const §%! §:String = "fc";
      
      private static const §=!p§:String = "ft";
      
      private static const §0=§:String = "fs";
      
      private static const §8">§:String = "oc";
      
      private static const D2:String = "2d";
      
      private static const D3:String = "3d";
      
      private static const §;!M§:String = "cube";
      
      private static const §4M§:String = "mipnearest";
      
      private static const §6o§:String = "miplinear";
      
      private static const §!!@§:String = "mipnone";
      
      private static const §#z§:String = "nomip";
      
      private static const §&"=§:String = "nearest";
      
      private static const § u§:String = "linear";
      
      private static const §&!=§:String = "centroid";
      
      private static const §4c§:String = "single";
      
      private static const §#"1§:String = "depth";
      
      private static const §8j§:String = "repeat";
      
      private static const §;!3§:String = "wrap";
      
      private static const §#8§:String = "clamp";
       
      
      private var §@!I§:ByteArray = null;
      
      private var §"W§:String = "";
      
      private var §#!+§:Boolean = false;
      
      public function §,"4§(param1:Boolean = false)
      {
         super();
         this.§#!+§ = param1;
         if(!initialized)
         {
            init();
         }
      }
      
      private static function init() : void
      {
         initialized = true;
         §0!F§[§^f§] = new OpCode(§^f§,2,0,0);
         §0!F§[ADD] = new OpCode(ADD,3,1,0);
         §0!F§[§7"§] = new OpCode(§7"§,3,2,0);
         §0!F§[§36§] = new OpCode(§36§,3,3,0);
         §0!F§[§6q§] = new OpCode(§6q§,3,4,0);
         §0!F§[§<"%§] = new OpCode(§<"%§,2,5,0);
         §0!F§[§^M§] = new OpCode(§^M§,3,6,0);
         §0!F§[§1"8§] = new OpCode(§1"8§,3,7,0);
         §0!F§[§9!o§] = new OpCode(§9!o§,2,8,0);
         §0!F§[§4!?§] = new OpCode(§4!?§,2,9,0);
         §0!F§[§"!l§] = new OpCode(§"!l§,2,10,0);
         §0!F§[§+v§] = new OpCode(§+v§,3,11,0);
         §0!F§[§'"2§] = new OpCode(§'"2§,2,12,0);
         §0!F§[§'y§] = new OpCode(§'y§,2,13,0);
         §0!F§[§!q§] = new OpCode(§!q§,2,14,0);
         §0!F§[§15§] = new OpCode(§15§,2,15,0);
         §0!F§[§"c§] = new OpCode(§"c§,2,16,0);
         §0!F§[§[e§] = new OpCode(§[e§,3,17,0);
         §0!F§[DP3] = new OpCode(DP3,3,18,0);
         §0!F§[DP4] = new OpCode(DP4,3,19,0);
         §0!F§[§[g§] = new OpCode(§[g§,2,20,0);
         §0!F§[§1p§] = new OpCode(§1p§,2,21,0);
         §0!F§[§^!1§] = new OpCode(§^!1§,2,22,0);
         §0!F§[M33] = new OpCode(M33,3,23,§+2§);
         §0!F§[M44] = new OpCode(M44,3,24,§+2§);
         §0!F§[M34] = new OpCode(M34,3,25,§+2§);
         §0!F§[§"!H§] = new OpCode(§"!H§,1,26,§2u§ | §[!%§ | §%u§);
         §0!F§[§#=§] = new OpCode(§#=§,1,27,§2u§ | §[!%§ | §%u§);
         §0!F§[§,!;§] = new OpCode(§,!;§,2,28,§2u§ | §[!%§ | §%u§);
         §0!F§[§!"6§] = new OpCode(§!"6§,2,29,§2u§ | §[!%§ | §%u§);
         §0!F§[§;!B§] = new OpCode(§;!B§,2,30,§2u§ | §[!%§ | §%u§);
         §0!F§[§]!0§] = new OpCode(§]!0§,2,31,§2u§ | §[!%§ | §%u§);
         §0!F§[§3d§] = new OpCode(§3d§,2,32,§2u§ | §[!%§ | §%u§);
         §0!F§[§6!h§] = new OpCode(§6!h§,2,33,§2u§ | §[!%§ | §%u§);
         §0!F§[§?"!§] = new OpCode(§?"!§,0,34,§2u§ | §[!%§ | §=!G§);
         §0!F§[§?m§] = new OpCode(§?m§,0,35,§2u§ | §=!G§);
         §0!F§[§#!Y§] = new OpCode(§#!Y§,1,36,§2u§ | §[!%§ | §%u§);
         §0!F§[§4"'§] = new OpCode(§4"'§,0,37,§2u§ | §=!G§);
         §0!F§[§ !Q§] = new OpCode(§ !Q§,0,38,§2u§);
         §0!F§[§^a§] = new OpCode(§^a§,1,39,§2u§ | §,1§);
         §0!F§[§6"+§] = new OpCode(§6"+§,3,40,§,1§ | §&"2§);
         §0!F§[§=!O§] = new OpCode(§=!O§,3,41,0);
         §0!F§[§`E§] = new OpCode(§`E§,3,42,0);
         §0!F§[§2c§] = new OpCode(§2c§,2,43,0);
         §!!,§[§'!R§] = new Register(§'!R§,"vertex attribute",0,7,§,[§ | §[!I§);
         §!!,§[§;c§] = new Register(§;c§,"vertex constant",1,127,§,[§ | §[!I§);
         §!!,§[§0!c§] = new Register(§0!c§,"vertex temporary",2,7,§,[§ | §#;§ | §[!I§);
         §!!,§[§0`§] = new Register(§0`§,"vertex output",3,0,§,[§ | §#;§);
         §!!,§[V] = new Register(V,"varying",4,7,§,[§ | §#!h§ | §[!I§ | §#;§);
         §!!,§[§%! §] = new Register(§%! §,"fragment constant",1,27,§#!h§ | §[!I§);
         §!!,§[§=!p§] = new Register(§=!p§,"fragment temporary",2,7,§#!h§ | §#;§ | §[!I§);
         §!!,§[§0=§] = new Register(§0=§,"texture sampler",5,7,§#!h§ | §[!I§);
         §!!,§[§8">§] = new Register(§8">§,"fragment output",3,0,§#!h§ | §#;§);
         §#"=§[D2] = new Sampler(D2,§?"1§,0);
         §#"=§[D3] = new Sampler(D3,§?"1§,2);
         §#"=§[§;!M§] = new Sampler(§;!M§,§?"1§,1);
         §#"=§[§4M§] = new Sampler(§4M§,§#" §,1);
         §#"=§[§6o§] = new Sampler(§6o§,§#" §,2);
         §#"=§[§!!@§] = new Sampler(§!!@§,§#" §,0);
         §#"=§[§#z§] = new Sampler(§#z§,§#" §,0);
         §#"=§[§&"=§] = new Sampler(§&"=§,§8!R§,0);
         §#"=§[§ u§] = new Sampler(§ u§,§8!R§,1);
         §#"=§[§&!=§] = new Sampler(§&!=§,§>6§,1 << 0);
         §#"=§[§4c§] = new Sampler(§4c§,§>6§,1 << 1);
         §#"=§[§#"1§] = new Sampler(§#"1§,§>6§,1 << 2);
         §#"=§[§8j§] = new Sampler(§8j§,§^"!§,1);
         §#"=§[§;!3§] = new Sampler(§;!3§,§^"!§,1);
         §#"=§[§#8§] = new Sampler(§#8§,§^"!§,0);
      }
      
      public function get error() : String
      {
         return this.§"W§;
      }
      
      public function get §5!`§() : ByteArray
      {
         return this.§@!I§;
      }
      
      public function §0!V§(param1:String, param2:String, param3:Boolean = false) : ByteArray
      {
         var _loc9_:int = 0;
         var _loc11_:String = null;
         var _loc12_:int = 0;
         var _loc13_:int = 0;
         var _loc14_:Array = null;
         var _loc15_:Array = null;
         var _loc16_:OpCode = null;
         var _loc17_:Array = null;
         var _loc18_:Boolean = false;
         var _loc19_:uint = 0;
         var _loc20_:uint = 0;
         var _loc21_:int = 0;
         var _loc22_:Boolean = false;
         var _loc23_:Array = null;
         var _loc24_:Array = null;
         var _loc25_:Register = null;
         var _loc26_:Array = null;
         var _loc27_:uint = 0;
         var _loc28_:uint = 0;
         var _loc29_:Array = null;
         var _loc30_:Boolean = false;
         var _loc31_:Boolean = false;
         var _loc32_:uint = 0;
         var _loc33_:uint = 0;
         var _loc34_:int = 0;
         var _loc35_:uint = 0;
         var _loc36_:uint = 0;
         var _loc37_:int = 0;
         var _loc38_:Array = null;
         var _loc39_:Register = null;
         var _loc40_:Array = null;
         var _loc41_:Array = null;
         var _loc42_:uint = 0;
         var _loc43_:uint = 0;
         var _loc44_:Number = NaN;
         var _loc45_:Sampler = null;
         var _loc46_:* = null;
         var _loc47_:uint = 0;
         var _loc48_:uint = 0;
         var _loc49_:String = null;
         var _loc4_:uint = getTimer();
         this.§@!I§ = new ByteArray();
         this.§"W§ = "";
         var _loc5_:Boolean = false;
         if(param1 == FRAGMENT)
         {
            _loc5_ = true;
         }
         else if(param1 != VERTEX)
         {
            this.§"W§ = "ERROR: mode needs to be \"" + FRAGMENT + "\" or \"" + VERTEX + "\" but is \"" + param1 + "\".";
         }
         this.§5!`§.endian = Endian.LITTLE_ENDIAN;
         this.§5!`§.writeByte(160);
         this.§5!`§.writeUnsignedInt(1);
         this.§5!`§.writeByte(161);
         this.§5!`§.writeByte(!!_loc5_ ? 1 : 0);
         var _loc6_:Array = param2.replace(/[\f\n\r\v]+/g,"\n").split("\n");
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         var _loc10_:int = _loc6_.length;
         _loc9_ = 0;
         while(_loc9_ < _loc10_ && this.§"W§ == "")
         {
            if((_loc12_ = (_loc11_ = new String(_loc6_[_loc9_])).search("//")) != -1)
            {
               _loc11_ = _loc11_.slice(0,_loc12_);
            }
            if((_loc13_ = _loc11_.search(/<.*>/g)) != -1)
            {
               _loc14_ = _loc11_.slice(_loc13_).match(/([\w\.\-\+]+)/gi);
               _loc11_ = _loc11_.slice(0,_loc13_);
            }
            _loc15_ = _loc11_.match(/^\w{3}/ig);
            _loc16_ = §0!F§[_loc15_[0]];
            if(!this.§#!+§)
            {
            }
            if(_loc16_ == null)
            {
               if(_loc11_.length >= 3)
               {
               }
            }
            else
            {
               _loc11_ = _loc11_.slice(_loc11_.search(_loc16_.name) + _loc16_.name.length);
               if(_loc16_.flags & §=!G§)
               {
                  if(--_loc7_ < 0)
                  {
                     this.§"W§ = "error: conditional closes without open.";
                     break;
                  }
               }
               if(_loc16_.flags & §[!%§)
               {
                  _loc7_++;
                  if(_loc7_ > §7x§)
                  {
                     this.§"W§ = "error: nesting to deep, maximum allowed is " + §7x§ + ".";
                     break;
                  }
               }
               if(_loc16_.flags & §,1§ && !_loc5_)
               {
                  this.§"W§ = "error: opcode is only allowed in fragment programs.";
                  break;
               }
               if(!param3)
               {
               }
               this.§5!`§.writeUnsignedInt(_loc16_.emitCode);
               _loc8_++;
               if(_loc8_ > §%!v§)
               {
                  this.§"W§ = "error: too many opcodes. maximum is " + §%!v§ + ".";
                  break;
               }
               if((_loc17_ = _loc11_.match(/vc\[([vof][actps]?)(\d*)?(\.[xyzw](\+\d{1,3})?)?\](\.[xyzw]{1,4})?|([vof][actps]?)(\d*)?(\.[xyzw]{1,4})?/gi)).length != _loc16_.numRegister)
               {
                  this.§"W§ = "error: wrong number of operands. found " + _loc17_.length + " but expected " + _loc16_.numRegister + ".";
                  break;
               }
               _loc18_ = false;
               _loc19_ = 64 + 64 + 32;
               _loc20_ = _loc17_.length;
               _loc21_ = 0;
               while(_loc21_ < _loc20_)
               {
                  _loc22_ = false;
                  if((_loc23_ = _loc17_[_loc21_].match(/\[.*\]/ig)).length > 0)
                  {
                     _loc17_[_loc21_] = _loc17_[_loc21_].replace(_loc23_[0],"0");
                     if(!param3)
                     {
                     }
                     _loc22_ = true;
                  }
                  _loc24_ = _loc17_[_loc21_].match(/^\b[A-Za-z]{1,2}/ig);
                  _loc25_ = §!!,§[_loc24_[0]];
                  if(!this.§#!+§)
                  {
                  }
                  if(_loc25_ == null)
                  {
                     this.§"W§ = "error: could not parse operand " + _loc21_ + " (" + _loc17_[_loc21_] + ").";
                     _loc18_ = true;
                     break;
                  }
                  if(_loc5_)
                  {
                     if(!(_loc25_.flags & §#!h§))
                     {
                        this.§"W§ = "error: register operand " + _loc21_ + " (" + _loc17_[_loc21_] + ") only allowed in vertex programs.";
                        _loc18_ = true;
                        break;
                     }
                     if(_loc22_)
                     {
                        this.§"W§ = "error: register operand " + _loc21_ + " (" + _loc17_[_loc21_] + ") relative adressing not allowed in fragment programs.";
                        _loc18_ = true;
                        break;
                     }
                  }
                  else if(!(_loc25_.flags & §,[§))
                  {
                     this.§"W§ = "error: register operand " + _loc21_ + " (" + _loc17_[_loc21_] + ") only allowed in fragment programs.";
                     _loc18_ = true;
                     break;
                  }
                  _loc17_[_loc21_] = _loc17_[_loc21_].slice(_loc17_[_loc21_].search(_loc25_.name) + _loc25_.name.length);
                  _loc26_ = !!_loc22_ ? _loc23_[0].match(/\d+/) : _loc17_[_loc21_].match(/\d+/);
                  _loc27_ = 0;
                  if(_loc26_)
                  {
                     _loc27_ = uint(_loc26_[0]);
                  }
                  if(_loc25_.range < _loc27_)
                  {
                     this.§"W§ = "error: register operand " + _loc21_ + " (" + _loc17_[_loc21_] + ") index exceeds limit of " + (_loc25_.range + 1) + ".";
                     _loc18_ = true;
                     break;
                  }
                  _loc28_ = 0;
                  _loc29_ = _loc17_[_loc21_].match(/(\.[xyzw]{1,4})/);
                  _loc30_ = _loc21_ == 0 && !(_loc16_.flags & §2u§);
                  _loc31_ = _loc21_ == 2 && _loc16_.flags & §&"2§;
                  _loc32_ = 0;
                  _loc33_ = 0;
                  _loc34_ = 0;
                  if(_loc30_ && _loc22_)
                  {
                     this.§"W§ = "error: relative can not be destination";
                     _loc18_ = true;
                     break;
                  }
                  if(_loc29_)
                  {
                     _loc28_ = 0;
                     _loc36_ = _loc29_[0].length;
                     _loc37_ = 1;
                     while(_loc37_ < _loc36_)
                     {
                        if((_loc35_ = _loc29_[0].charCodeAt(_loc37_) - "x".charCodeAt(0)) > 2)
                        {
                           _loc35_ = 3;
                        }
                        if(_loc30_)
                        {
                           _loc28_ |= 1 << _loc35_;
                        }
                        else
                        {
                           _loc28_ |= _loc35_ << (_loc37_ - 1 << 1);
                        }
                        _loc37_++;
                     }
                     if(!_loc30_)
                     {
                        while(_loc37_ <= 4)
                        {
                           _loc28_ |= _loc35_ << (_loc37_ - 1 << 1);
                           _loc37_++;
                        }
                     }
                  }
                  else
                  {
                     _loc28_ = !!_loc30_ ? uint(15) : uint(228);
                  }
                  if(_loc22_)
                  {
                     _loc38_ = _loc23_[0].match(/[A-Za-z]{1,2}/ig);
                     if((_loc39_ = §!!,§[_loc38_[0]]) == null)
                     {
                        this.§"W§ = "error: bad index register";
                        _loc18_ = true;
                        break;
                     }
                     _loc32_ = _loc39_.emitCode;
                     if((_loc40_ = _loc23_[0].match(/(\.[xyzw]{1,1})/)).length == 0)
                     {
                        this.§"W§ = "error: bad index register select";
                        _loc18_ = true;
                        break;
                     }
                     if((_loc33_ = _loc40_[0].charCodeAt(1) - "x".charCodeAt(0)) > 2)
                     {
                        _loc33_ = 3;
                     }
                     if((_loc41_ = _loc23_[0].match(/\+\d{1,3}/ig)).length > 0)
                     {
                        _loc34_ = _loc41_[0];
                     }
                     if(_loc34_ < 0 || _loc34_ > 255)
                     {
                        this.§"W§ = "error: index offset " + _loc34_ + " out of bounds. [0..255]";
                        _loc18_ = true;
                        break;
                     }
                     if(!param3)
                     {
                     }
                  }
                  if(!param3)
                  {
                  }
                  if(_loc30_)
                  {
                     this.§5!`§.writeShort(_loc27_);
                     this.§5!`§.writeByte(_loc28_);
                     this.§5!`§.writeByte(_loc25_.emitCode);
                     _loc19_ -= 32;
                  }
                  else if(_loc31_)
                  {
                     if(!param3)
                     {
                     }
                     _loc42_ = 5;
                     _loc43_ = _loc14_.length;
                     _loc44_ = 0;
                     _loc37_ = 0;
                     while(_loc37_ < _loc43_)
                     {
                        if(!param3)
                        {
                        }
                        if((_loc45_ = §#"=§[_loc14_[_loc37_]]) == null)
                        {
                           _loc44_ = Number(_loc14_[_loc37_]);
                           if(!param3)
                           {
                           }
                        }
                        else
                        {
                           if(_loc45_.flag != §>6§)
                           {
                              _loc42_ &= ~(15 << _loc45_.flag);
                           }
                           _loc42_ |= uint(_loc45_.mask) << uint(_loc45_.flag);
                        }
                        _loc37_++;
                     }
                     this.§5!`§.writeShort(_loc27_);
                     this.§5!`§.writeByte(int(_loc44_ * 8));
                     this.§5!`§.writeByte(0);
                     this.§5!`§.writeUnsignedInt(_loc42_);
                     if(!param3)
                     {
                     }
                     _loc19_ -= 64;
                  }
                  else
                  {
                     if(_loc21_ == 0)
                     {
                        this.§5!`§.writeUnsignedInt(0);
                        _loc19_ -= 32;
                     }
                     this.§5!`§.writeShort(_loc27_);
                     this.§5!`§.writeByte(_loc34_);
                     this.§5!`§.writeByte(_loc28_);
                     this.§5!`§.writeByte(_loc25_.emitCode);
                     this.§5!`§.writeByte(_loc32_);
                     this.§5!`§.writeShort(!!_loc22_ ? _loc33_ | 1 << 15 : 0);
                     _loc19_ -= 64;
                  }
                  _loc21_++;
               }
               _loc21_ = 0;
               while(_loc21_ < _loc19_)
               {
                  this.§5!`§.writeByte(0);
                  _loc21_ += 8;
               }
               if(_loc18_)
               {
                  break;
               }
            }
            _loc9_++;
         }
         if(this.§"W§ != "")
         {
            this.§"W§ += "\n  at line " + _loc9_ + " " + _loc6_[_loc9_];
            this.§5!`§.length = 0;
         }
         if(this.§#!+§)
         {
            _loc46_ = "generated bytecode:";
            _loc47_ = this.§5!`§.length;
            _loc48_ = 0;
            while(_loc48_ < _loc47_)
            {
               if(!(_loc48_ % 16))
               {
                  _loc46_ += "\n";
               }
               if(!(_loc48_ % 4))
               {
                  _loc46_ += " ";
               }
               if((_loc49_ = this.§5!`§[_loc48_].toString(16)).length < 2)
               {
                  _loc49_ = "0" + _loc49_;
               }
               _loc46_ += _loc49_;
               _loc48_++;
            }
         }
         if(!param3)
         {
         }
         return this.§5!`§;
      }
   }
}

class OpCode
{
    
   
   private var _emitCode:uint;
   
   private var _flags:uint;
   
   private var _name:String;
   
   private var _numRegister:uint;
   
   function OpCode(param1:String, param2:uint, param3:uint, param4:uint)
   {
      super();
      this._name = param1;
      this._numRegister = param2;
      this._emitCode = param3;
      this._flags = param4;
   }
   
   public function get emitCode() : uint
   {
      return this._emitCode;
   }
   
   public function get flags() : uint
   {
      return this._flags;
   }
   
   public function get name() : String
   {
      return this._name;
   }
   
   public function get numRegister() : uint
   {
      return this._numRegister;
   }
   
   public function toString() : String
   {
      return "[OpCode name=\"" + this._name + "\", numRegister=" + this._numRegister + ", emitCode=" + this._emitCode + ", flags=" + this._flags + "]";
   }
}

class Register
{
    
   
   private var _emitCode:uint;
   
   private var _name:String;
   
   private var _longName:String;
   
   private var _flags:uint;
   
   private var _range:uint;
   
   function Register(param1:String, param2:String, param3:uint, param4:uint, param5:uint)
   {
      super();
      this._name = param1;
      this._longName = param2;
      this._emitCode = param3;
      this._range = param4;
      this._flags = param5;
   }
   
   public function get emitCode() : uint
   {
      return this._emitCode;
   }
   
   public function get longName() : String
   {
      return this._longName;
   }
   
   public function get name() : String
   {
      return this._name;
   }
   
   public function get flags() : uint
   {
      return this._flags;
   }
   
   public function get range() : uint
   {
      return this._range;
   }
   
   public function toString() : String
   {
      return "[Register name=\"" + this._name + "\", longName=\"" + this._longName + "\", emitCode=" + this._emitCode + ", range=" + this._range + ", flags=" + this._flags + "]";
   }
}

class Sampler
{
    
   
   private var _flag:uint;
   
   private var _mask:uint;
   
   private var _name:String;
   
   function Sampler(param1:String, param2:uint, param3:uint)
   {
      super();
      this._name = param1;
      this._flag = param2;
      this._mask = param3;
   }
   
   public function get flag() : uint
   {
      return this._flag;
   }
   
   public function get mask() : uint
   {
      return this._mask;
   }
   
   public function get name() : String
   {
      return this._name;
   }
   
   public function toString() : String
   {
      return "[Sampler name=\"" + this._name + "\", flag=\"" + this._flag + "\", mask=" + this.mask + "]";
   }
}
