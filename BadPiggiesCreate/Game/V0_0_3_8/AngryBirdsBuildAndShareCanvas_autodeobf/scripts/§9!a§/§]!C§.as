package §9!a§
{
   import §6!$§.§0!q§;
   
   public class §]!C§
   {
      
      public static const §"D§:uint = 16777215;
      
      public static const §>3§:uint = 12632256;
      
      public static const §4!n§:uint = 8421504;
      
      public static const §+J§:uint = 0;
      
      public static const RED:uint = 16711680;
      
      public static const §;!H§:uint = 8388608;
      
      public static const §+<§:uint = 16776960;
      
      public static const §=! §:uint = 8421376;
      
      public static const §%!i§:uint = 65280;
      
      public static const §""3§:uint = 32768;
      
      public static const §1!e§:uint = 65535;
      
      public static const §12§:uint = 32896;
      
      public static const §5!s§:uint = 255;
      
      public static const §'J§:uint = 128;
      
      public static const §#"'§:uint = 16711935;
      
      public static const §[4§:uint = 8388736;
       
      
      public function §]!C§()
      {
         super();
         throw new §0!q§();
      }
      
      public static function §3!p§(param1:uint) : int
      {
         return param1 >> 24 & 255;
      }
      
      public static function §8"§(param1:uint) : int
      {
         return param1 >> 16 & 255;
      }
      
      public static function §+$§(param1:uint) : int
      {
         return param1 >> 8 & 255;
      }
      
      public static function §2!v§(param1:uint) : int
      {
         return param1 & 255;
      }
      
      public static function §+![§(param1:int, param2:int, param3:int) : uint
      {
         return param1 << 16 | param2 << 8 | param3;
      }
      
      public static function §>#§(param1:int, param2:int, param3:int, param4:int) : uint
      {
         return param1 << 24 | param2 << 16 | param3 << 8 | param4;
      }
   }
}
