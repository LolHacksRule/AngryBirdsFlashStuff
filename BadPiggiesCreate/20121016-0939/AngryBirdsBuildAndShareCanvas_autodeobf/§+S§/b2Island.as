package §+S§
{
   import § !V§.*;
   import §2"=§.*;
   import §5!o§.*;
   import §;T§.*;
   import §=o§.*;
   
   use namespace b2internal;
   
   public class b2Island
   {
      
      private static var §3m§:b2ContactImpulse = new b2ContactImpulse();
       
      
      private var §=!6§;
      
      private var §;!!§:b2ContactListener;
      
      private var § !"§:b2ContactSolver;
      
      b2internal var §;!j§:Vector.<b2Body>;
      
      b2internal var §1f§:Vector.<b2Contact>;
      
      b2internal var §4!L§:Vector.<b2Joint>;
      
      b2internal var §&z§:int;
      
      b2internal var §,i§:int;
      
      b2internal var §3!Z§:int;
      
      private var §5H§:int;
      
      b2internal var §]";§:int;
      
      b2internal var §]!n§:int;
      
      public function b2Island()
      {
         super();
         this.§;!j§ = new Vector.<b2Body>();
         this.§1f§ = new Vector.<b2Contact>();
         this.§4!L§ = new Vector.<b2Joint>();
      }
      
      public function §1o§(param1:int, param2:int, param3:int, param4:*, param5:b2ContactListener, param6:b2ContactSolver) : void
      {
         var _loc7_:int = 0;
         this.§5H§ = param1;
         this.§]";§ = param2;
         this.§]!n§ = param3;
         this.§&z§ = 0;
         this.§3!Z§ = 0;
         this.§,i§ = 0;
         this.§=!6§ = param4;
         this.§;!!§ = param5;
         this.§ !"§ = param6;
         _loc7_ = this.§;!j§.length;
         while(_loc7_ < param1)
         {
            this.§;!j§[_loc7_] = null;
            _loc7_++;
         }
         _loc7_ = this.§1f§.length;
         while(_loc7_ < param2)
         {
            this.§1f§[_loc7_] = null;
            _loc7_++;
         }
         _loc7_ = this.§4!L§.length;
         while(_loc7_ < param3)
         {
            this.§4!L§[_loc7_] = null;
            _loc7_++;
         }
      }
      
      public function §^S§() : void
      {
         this.§&z§ = 0;
         this.§3!Z§ = 0;
         this.§,i§ = 0;
      }
      
      public function §=F§(param1:b2TimeStep, param2:b2Vec2, param3:Boolean) : void
      {
         var _loc4_:int = 0;
         var _loc5_:int = 0;
         var _loc6_:b2Body = null;
         var _loc7_:b2Joint = null;
         var _loc9_:Number = NaN;
         var _loc10_:Number = NaN;
         var _loc11_:Number = NaN;
         var _loc12_:Boolean = false;
         var _loc13_:Boolean = false;
         var _loc14_:Boolean = false;
         var _loc15_:Number = NaN;
         var _loc16_:Number = NaN;
         var _loc17_:Number = NaN;
         _loc4_ = 0;
         while(_loc4_ < this.§&z§)
         {
            if((_loc6_ = this.§;!j§[_loc4_]).§+!O§() == b2Body.b2_dynamicBody)
            {
               _loc6_.§,b§.x += param1.§+z§ * (param2.x + _loc6_.§;k§ * _loc6_.§2!d§.x);
               _loc6_.§,b§.y += param1.§+z§ * (param2.y + _loc6_.§;k§ * _loc6_.§2!d§.y);
               _loc6_.m_angularVelocity += param1.§+z§ * _loc6_.§6x§ * _loc6_.§?6§;
               _loc6_.§,b§.§>!t§(b2Math.§&>§(1 - param1.§+z§ * _loc6_.§%!o§,0,1));
               _loc6_.m_angularVelocity *= b2Math.§&>§(1 - param1.§+z§ * _loc6_.§7!_§,0,1);
            }
            _loc4_++;
         }
         this.§ !"§.§1o§(param1,this.§1f§,this.§3!Z§,this.§=!6§);
         var _loc8_:b2ContactSolver;
         (_loc8_ = this.§ !"§).InitVelocityConstraints(param1);
         _loc4_ = 0;
         while(_loc4_ < this.§,i§)
         {
            (_loc7_ = this.§4!L§[_loc4_]).InitVelocityConstraints(param1);
            _loc4_++;
         }
         _loc4_ = 0;
         while(_loc4_ < param1.§?l§)
         {
            _loc5_ = 0;
            while(_loc5_ < this.§,i§)
            {
               (_loc7_ = this.§4!L§[_loc5_]).SolveVelocityConstraints(param1);
               _loc5_++;
            }
            _loc8_.SolveVelocityConstraints();
            _loc4_++;
         }
         _loc4_ = 0;
         while(_loc4_ < this.§,i§)
         {
            (_loc7_ = this.§4!L§[_loc4_]).§8"§();
            _loc4_++;
         }
         _loc8_.§8"§();
         _loc4_ = 0;
         while(_loc4_ < this.§&z§)
         {
            if((_loc6_ = this.§;!j§[_loc4_]).§+!O§() != b2Body.b2_staticBody)
            {
               _loc9_ = param1.§+z§ * _loc6_.§,b§.x;
               _loc10_ = param1.§+z§ * _loc6_.§,b§.y;
               if(_loc9_ * _loc9_ + _loc10_ * _loc10_ > b2Settings.b2_maxTranslationSquared)
               {
                  _loc6_.§,b§.Normalize();
                  _loc6_.§,b§.x *= b2Settings.b2_maxTranslation * param1.§1!4§;
                  _loc6_.§,b§.y *= b2Settings.b2_maxTranslation * param1.§1!4§;
               }
               _loc11_ = param1.§+z§ * _loc6_.m_angularVelocity;
               if(_loc11_ * _loc11_ > b2Settings.b2_maxRotationSquared)
               {
                  if(_loc6_.m_angularVelocity < 0)
                  {
                     _loc6_.m_angularVelocity = -b2Settings.b2_maxRotation * param1.§1!4§;
                  }
                  else
                  {
                     _loc6_.m_angularVelocity = b2Settings.b2_maxRotation * param1.§1!4§;
                  }
               }
               _loc6_.m_sweep.c0.SetV(_loc6_.m_sweep.c);
               _loc6_.m_sweep.a0 = _loc6_.m_sweep.a;
               _loc6_.m_sweep.c.x += param1.§+z§ * _loc6_.§,b§.x;
               _loc6_.m_sweep.c.y += param1.§+z§ * _loc6_.§,b§.y;
               _loc6_.m_sweep.a += param1.§+z§ * _loc6_.m_angularVelocity;
               _loc6_.§#!k§();
            }
            _loc4_++;
         }
         _loc4_ = 0;
         while(_loc4_ < param1.§@!C§)
         {
            _loc12_ = _loc8_.SolvePositionConstraints(b2Settings.b2_contactBaumgarte);
            _loc13_ = true;
            _loc5_ = 0;
            while(_loc5_ < this.§,i§)
            {
               _loc14_ = (_loc7_ = this.§4!L§[_loc5_]).SolvePositionConstraints(b2Settings.b2_contactBaumgarte);
               _loc13_ = _loc13_ && _loc14_;
               _loc5_++;
            }
            if(_loc12_ && _loc13_)
            {
               break;
            }
            _loc4_++;
         }
         this.§;!q§(_loc8_.§#I§);
         if(param3)
         {
            _loc15_ = Number.MAX_VALUE;
            _loc16_ = b2Settings.b2_linearSleepTolerance * b2Settings.b2_linearSleepTolerance;
            _loc17_ = b2Settings.b2_angularSleepTolerance * b2Settings.b2_angularSleepTolerance;
            _loc4_ = 0;
            while(_loc4_ < this.§&z§)
            {
               if((_loc6_ = this.§;!j§[_loc4_]).§+!O§() != b2Body.b2_staticBody)
               {
                  if((_loc6_.§`"8§ & b2Body.§8"8§) == 0)
                  {
                     _loc6_.§[^§ = 0;
                     _loc15_ = 0;
                  }
                  if((_loc6_.§`"8§ & b2Body.§8"8§) == 0 || _loc6_.m_angularVelocity * _loc6_.m_angularVelocity > _loc17_ || b2Math.§@!y§(_loc6_.§,b§,_loc6_.§,b§) > _loc16_)
                  {
                     _loc6_.§[^§ = 0;
                     _loc15_ = 0;
                  }
                  else
                  {
                     _loc6_.§[^§ += param1.§+z§;
                     _loc15_ = b2Math.§8L§(_loc15_,_loc6_.§[^§);
                  }
               }
               _loc4_++;
            }
            if(_loc15_ >= b2Settings.b2_timeToSleep)
            {
               _loc4_ = 0;
               while(_loc4_ < this.§&z§)
               {
                  (_loc6_ = this.§;!j§[_loc4_]).SetAwake(false);
                  _loc4_++;
               }
            }
         }
      }
      
      public function §!!a§(param1:b2TimeStep) : void
      {
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc6_:b2Body = null;
         var _loc7_:Number = NaN;
         var _loc8_:Number = NaN;
         var _loc9_:Number = NaN;
         var _loc10_:Boolean = false;
         var _loc11_:Boolean = false;
         var _loc12_:Boolean = false;
         this.§ !"§.§1o§(param1,this.§1f§,this.§3!Z§,this.§=!6§);
         var _loc4_:b2ContactSolver = this.§ !"§;
         _loc2_ = 0;
         while(_loc2_ < this.§,i§)
         {
            this.§4!L§[_loc2_].InitVelocityConstraints(param1);
            _loc2_++;
         }
         _loc2_ = 0;
         while(_loc2_ < param1.§?l§)
         {
            _loc4_.SolveVelocityConstraints();
            _loc3_ = 0;
            while(_loc3_ < this.§,i§)
            {
               this.§4!L§[_loc3_].SolveVelocityConstraints(param1);
               _loc3_++;
            }
            _loc2_++;
         }
         _loc2_ = 0;
         while(_loc2_ < this.§&z§)
         {
            if((_loc6_ = this.§;!j§[_loc2_]).§+!O§() != b2Body.b2_staticBody)
            {
               _loc7_ = param1.§+z§ * _loc6_.§,b§.x;
               _loc8_ = param1.§+z§ * _loc6_.§,b§.y;
               if(_loc7_ * _loc7_ + _loc8_ * _loc8_ > b2Settings.b2_maxTranslationSquared)
               {
                  _loc6_.§,b§.Normalize();
                  _loc6_.§,b§.x *= b2Settings.b2_maxTranslation * param1.§1!4§;
                  _loc6_.§,b§.y *= b2Settings.b2_maxTranslation * param1.§1!4§;
               }
               _loc9_ = param1.§+z§ * _loc6_.m_angularVelocity;
               if(_loc9_ * _loc9_ > b2Settings.b2_maxRotationSquared)
               {
                  if(_loc6_.m_angularVelocity < 0)
                  {
                     _loc6_.m_angularVelocity = -b2Settings.b2_maxRotation * param1.§1!4§;
                  }
                  else
                  {
                     _loc6_.m_angularVelocity = b2Settings.b2_maxRotation * param1.§1!4§;
                  }
               }
               _loc6_.m_sweep.c0.SetV(_loc6_.m_sweep.c);
               _loc6_.m_sweep.a0 = _loc6_.m_sweep.a;
               _loc6_.m_sweep.c.x += param1.§+z§ * _loc6_.§,b§.x;
               _loc6_.m_sweep.c.y += param1.§+z§ * _loc6_.§,b§.y;
               _loc6_.m_sweep.a += param1.§+z§ * _loc6_.m_angularVelocity;
               _loc6_.§#!k§();
            }
            _loc2_++;
         }
         _loc2_ = 0;
         while(_loc2_ < param1.§@!C§)
         {
            _loc10_ = _loc4_.SolvePositionConstraints(0.75);
            _loc11_ = true;
            _loc3_ = 0;
            while(_loc3_ < this.§,i§)
            {
               _loc12_ = this.§4!L§[_loc3_].SolvePositionConstraints(b2Settings.b2_contactBaumgarte);
               _loc11_ = _loc11_ && _loc12_;
               _loc3_++;
            }
            if(_loc10_ && _loc11_)
            {
               break;
            }
            _loc2_++;
         }
         this.§;!q§(_loc4_.§#I§);
      }
      
      public function §;!q§(param1:Vector.<b2ContactConstraint>) : void
      {
         var _loc3_:b2Contact = null;
         var _loc4_:b2ContactConstraint = null;
         var _loc5_:int = 0;
         if(this.§;!!§ == null)
         {
            return;
         }
         var _loc2_:int = 0;
         while(_loc2_ < this.§3!Z§)
         {
            _loc3_ = this.§1f§[_loc2_];
            _loc4_ = param1[_loc2_];
            _loc5_ = 0;
            while(_loc5_ < _loc4_.§<"5§)
            {
               §3m§.§5!^§[_loc5_] = _loc4_.§>"%§[_loc5_].normalImpulse;
               §3m§.§9!&§[_loc5_] = _loc4_.§>"%§[_loc5_].tangentImpulse;
               _loc5_++;
            }
            this.§;!!§.PostSolve(_loc3_,§3m§);
            _loc2_++;
         }
      }
      
      public function §"[§(param1:b2Body) : void
      {
         param1.§7<§ = this.§&z§;
         var _loc2_:* = this.§&z§++;
         this.§;!j§[_loc2_] = param1;
      }
      
      public function §2"2§(param1:b2Contact) : void
      {
         var _loc2_:* = this.§3!Z§++;
         this.§1f§[_loc2_] = param1;
      }
      
      public function §6E§(param1:b2Joint) : void
      {
         var _loc2_:* = this.§,i§++;
         this.§4!L§[_loc2_] = param1;
      }
   }
}
