   ...
   public class §-k§ extends §,"!§
   {
      ...
      override protected function addDestructionParticles(param1:§7!O§) : void
      {
         ...
         while(_loc5_ < _loc3_)
         {
            ...
            param1.addParticle(§=$4§.§9J§,§5"6§.PARTICLE_GROUP_GAME_EFFECTS,§=$4§.PARTICLE_TYPE_KINETIC_PARTICLE,getBody().GetPosition().x + _loc7_,getBody().GetPosition().y + _loc8_,1500,"",§=$4§.getParticleMaterialFromEngineMaterial(itemName),_loc2_ * Math.cos(_loc6_),-_loc2_ * Math.sin(_loc6_),5,_loc2_ * 20);
            _loc5_++;
         }
      }
