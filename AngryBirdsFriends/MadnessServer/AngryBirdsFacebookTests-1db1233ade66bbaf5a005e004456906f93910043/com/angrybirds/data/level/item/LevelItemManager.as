package com.angrybirds.data.level.item
{
   ...
   public class LevelItemManager
   {
      ...
      public function loadItems(itemData:XML) : void
      {
         ...
         var specialtyAvailableAfterCollision:* = false;
         ...
         for each(item in this.mItemDataTable.Items.Item)
         {
            ...
            specialtyAvailableAfterCollision = String(item.@specialtyAvailableAfterCollision).toLowerCase() == "true";
            this.addItem(id,type,material,sounds,shape,destroyedScoreInc,floatingScoreFont,damageScore,category,strength,scale,front,particleJsonId,particleVariationCount,particleAmount,disableBirdPassThrough,collision,textureType,bubbleDamage,specialtyAvailableAfterCollision);
         }
         this.loadSoundChannels(this.mItemDataTable.SoundChannels);
      }
      
      public function addItem(id:String, type:int, materialName:String, resourcePathsSound:String, shape:String, destroyedScoreInc:int, floatingScoreFont:String, damageScore:int, category:String, strength:int, scale:Number, front:Boolean, particleJSONId:String = "", particleVariationCount:int = 1, particleAmount:int = -1, disableBirdPassThrough:Boolean = false, collision:Boolean = true, textureType:String = null, bubbleDamage:int = 0, specialtyAvailableAfterCollision:Boolean = false) : void
      {
         ...
         this.mItems[id.toLowerCase()] = new LevelItem(id,type,material,soundResource,itemShape,destroyedScoreInc,floatingScoreFont,damageScore,category,strength,scale,front,particleJSONId,particleVariationCount,particleAmount,disableBirdPassThrough,false,collision,textureType,bubbleDamage,specialtyAvailableAfterCollision);
      }
      ...
      public function initAnimations(onlyItems:Array = null) : void
      {
         ...
         for each(levelItem in this.mItems)
         {
            try
            {
               ...
               if(add)
               {
                  if(!AngryBirdsEngine.smLevelMain.animationManager.getAnimation(levelItem.itemName) || overwriteExistingGraphics)
                  {
