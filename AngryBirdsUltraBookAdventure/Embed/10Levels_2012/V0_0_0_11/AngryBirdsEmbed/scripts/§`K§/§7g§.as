package §`K§
{
   import §0!<§.§#y§;
   import com.rovio.Box2D.Dynamics.b2Body;
   
   public class §7g§
   {
      
      public static const §7i§:int = 0;
      
      public static const §0!!§:int = b2Body.b2_staticBody;
      
      public static const §^!=§:int = b2Body.b2_kinematicBody;
      
      public static const §4!8§:int = b2Body.b2_dynamicBody;
      
      public static const §0^§:int = 1;
      
      public static const §5X§:int = 2;
      
      public static const §;!"§:int = 3;
      
      public static const §8!@§:int = 4;
      
      public static const §+n§:int = 5;
      
      public static const §<L§:int = 6;
       
      
      public var mName:String;
      
      public var §-a§:Array;
      
      public var §8X§:Array;
      
      public var §2l§:Array;
      
      public function §7g§(param1:String, param2:String, param3:Number, param4:Number, param5:Number, param6:Number, param7:Number, param8:Number)
      {
         super();
         this.mName = param1;
         this.§-a§ = new Array();
         this.§-a§[§7i§] = this.§?Z§(param2);
         this.§-a§[§0^§] = param3;
         this.§-a§[§5X§] = param4;
         this.§-a§[§;!"§] = param5;
         this.§-a§[§8!@§] = param6;
         this.§-a§[§+n§] = param7;
         this.§-a§[§<L§] = param8;
      }
      
      public function getValue(param1:int) : *
      {
         return this.§-a§[param1];
      }
      
      private function §?Z§(param1:String) : int
      {
         if(param1.toLowerCase() == "kinetic")
         {
            return §^!=§;
         }
         if(param1.toLowerCase() == "static")
         {
            return §0!!§;
         }
         if(param1.toLowerCase() == "dynamic")
         {
            return §4!8§;
         }
         §#y§.log("WARNING LevelItemMaterial has invalid Body Type, name = " + this.mName + " invalid name = " + param1);
         return §0!!§;
      }
      
      public function §=!1§(param1:XML) : void
      {
         var _loc3_:String = null;
         var _loc4_:Number = NaN;
         this.§8X§ = new Array();
         var _loc2_:int = 0;
         while(_loc2_ < param1.attributes().length())
         {
            _loc3_ = param1.attributes()[_loc2_].name();
            _loc4_ = param1.attributes()[_loc2_];
            this.§8X§[_loc3_.toUpperCase()] = _loc4_;
            _loc2_++;
         }
      }
      
      public function §@g§(param1:XML) : void
      {
         var _loc3_:String = null;
         var _loc4_:Number = NaN;
         this.§2l§ = new Array();
         var _loc2_:int = 0;
         while(_loc2_ < param1.attributes().length())
         {
            _loc3_ = param1.attributes()[_loc2_].name();
            _loc4_ = param1.attributes()[_loc2_];
            this.§2l§[_loc3_.toUpperCase()] = _loc4_;
            _loc2_++;
         }
      }
      
      public function §]"§(param1:String) : Number
      {
         if(!this.§8X§)
         {
            return 1;
         }
         param1 = param1.toUpperCase();
         if(this.§8X§[param1])
         {
            return this.§8X§[param1];
         }
         return 1;
      }
      
      public function §@]§(param1:String) : Number
      {
         if(!this.§2l§)
         {
            return 1;
         }
         param1 = param1.toUpperCase();
         if(this.§2l§[param1])
         {
            return this.§2l§[param1];
         }
         return 1;
      }
   }
}
