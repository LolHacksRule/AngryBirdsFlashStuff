package §,!C§
{
   ...
   public class §9b§
   {
      ...
      public static function §%'§(param1:XMLList) : void
      {
         ...
         for each(_loc2_ in param1.Shape)
         {
            ...
            else
            {
               ...
               {
                  §9X§.log("WARNING, LevelItemShapes constructor, bodyType is missing for shape: " + _loc2_.@id);
               }
               if(_loc2_.attribute("width").length() <= 0)
               {
                  §9X§.log("WARNING, LevelItemShapes constructor, density is missing for shape: " + _loc2_.@id);
               }
               if(_loc2_.attribute("height").length() <= 0)
               {
                  §9X§.log("WARNING, LevelItemShapes constructor, friction is missing for shape: " + _loc2_.@id);
               }
      ...
      public static function §,W§(param1:String) : §6m§
      {
         ...
         §9X§.log("WARNING: LevelItemShapes -> getShape request has no return value, this shape does not exist: " + param1);
         return null;
      }
   }
}
