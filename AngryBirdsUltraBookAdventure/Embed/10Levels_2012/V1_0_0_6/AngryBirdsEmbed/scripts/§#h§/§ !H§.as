package §#h§
{
   ...
   public class § !H§
   {
      ...
      private static var §%!G§:EventDispatcher = new EventDispatcher();
      ...
      public static function §+z§(param1:String, param2:int, param3:Number) : void
      {
         ...
         if(§3H§ >= §@+§)
         {
            if(§&!%§)
            {
               §9X§.log("WARNING: SoundEngine:addNewChannelControl() " + param1 + " failed because we already have more than " + §@+§ + " reserved");
            }
            return;
         }
         if(§3H§ + param2 >= §@+§)
      ...
      private static function §+I§(param1:String, param2:String) : §>p§
      {
         ...
         if(!_loc3_)
         {
            if(§&!%§)
            {
               §9X§.log("WARNING: SoundEngine->PlaySound() can not play new sound request " + param2 + " because this channel does not exist " + param1);
            }
            return null;
         }
         if(!_loc3_.§]!%§())
         {
            if(§&!%§)
            {
               §9X§.log("WARNING: SoundEngine->PlaySound() can not play new sound request  " + param2 + " this channel is full " + param1);
            }
            return null;
         }
         return _loc3_;
      ...
      public static function § !%§(param1:String, param2:String = "Default_Channel", param3:int = 0, param4:Number = -1, param5:Number = 0) : §@b§
      {
         ...
         if((_loc7_ = §,V§[param1]) == null)
         {
            ...
            {
               §9X§.log("WARNING: SoundChannelController->PlaySound(), this sound is not in AssetCache: " + param1);
               return null;
            }
            _loc7_ = new _loc8_();
            §,V§[param1] = _loc7_;
      ...
      public static function §]p§(param1:String, param2:String = "Default_Channel") : void
      {
         ...
         if(_loc3_ == 0)
         {
            if(§&!%§)
            {
               §9X§.log("WARNING: SoundEngine->playSoundFromVariation() method received wrong file name, number at the end is missing: " + param1);
            }
            return;
         }
