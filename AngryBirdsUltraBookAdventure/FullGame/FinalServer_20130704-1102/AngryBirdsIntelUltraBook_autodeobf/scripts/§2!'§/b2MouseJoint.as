package §2!'§
{
   import §#I§.b2Body;
   import §#I§.b2TimeStep;
   import §&H§.b2Mat22;
   import §&H§.b2Vec2;
   import §@!3§.b2internal;
   
   use namespace b2internal;
   
   public class b2MouseJoint extends b2Joint
   {
       
      
      private var §+!K§:b2Mat22;
      
      private var K1:b2Mat22;
      
      private var K2:b2Mat22;
      
      private var §1!,§:b2Vec2;
      
      private var §]!A§:b2Vec2;
      
      private var §&""§:b2Vec2;
      
      private var §"X§:b2Mat22;
      
      private var §&!w§:b2Vec2;
      
      private var §>!z§:Number;
      
      private var §=§:Number;
      
      private var §#!d§:Number;
      
      private var §[^§:Number;
      
      private var §4z§:Number;
      
      public function b2MouseJoint(param1:b2MouseJointDef)
      {
         var _loc2_:Number = NaN;
         var _loc4_:b2Mat22 = null;
         this.§+!K§ = new b2Mat22();
         this.K1 = new b2Mat22();
         this.K2 = new b2Mat22();
         this.§1!,§ = new b2Vec2();
         this.§]!A§ = new b2Vec2();
         this.§&""§ = new b2Vec2();
         this.§"X§ = new b2Mat22();
         this.§&!w§ = new b2Vec2();
         super(param1);
         this.§]!A§.SetV(param1.target);
         _loc2_ = this.§]!A§.x - b2internal::4!d.m_xf.position.x;
         var _loc3_:Number = this.§]!A§.y - b2internal::4!d.m_xf.position.y;
         _loc4_ = b2internal::4!d.m_xf.R;
         this.§1!,§.x = _loc2_ * _loc4_.col1.x + _loc3_ * _loc4_.col1.y;
         this.§1!,§.y = _loc2_ * _loc4_.col2.x + _loc3_ * _loc4_.col2.y;
         this.§>!z§ = param1.§6""§;
         this.§&""§.§+J§();
         this.§=§ = param1.§9!@§;
         this.§#!d§ = param1.§6V§;
         this.§[^§ = 0;
         this.§4z§ = 0;
      }
      
      override public function GetAnchorA() : b2Vec2
      {
         return this.§]!A§;
      }
      
      override public function GetAnchorB() : b2Vec2
      {
         return b2internal::4!d.GetWorldPoint(this.§1!,§);
      }
      
      override public function GetReactionForce(param1:Number) : b2Vec2
      {
         return new b2Vec2(param1 * this.§&""§.x,param1 * this.§&""§.y);
      }
      
      override public function GetReactionTorque(param1:Number) : Number
      {
         return 0;
      }
      
      public function §0" §() : b2Vec2
      {
         return this.§]!A§;
      }
      
      public function §^!Z§(param1:b2Vec2) : void
      {
         if(b2internal::4!d.IsAwake() == false)
         {
            b2internal::4!d.SetAwake(true);
         }
         this.§]!A§ = param1;
      }
      
      public function §=!3§() : Number
      {
         return this.§>!z§;
      }
      
      public function §1!1§(param1:Number) : void
      {
         this.§>!z§ = param1;
      }
      
      public function §!K§() : Number
      {
         return this.§=§;
      }
      
      public function §3!D§(param1:Number) : void
      {
         this.§=§ = param1;
      }
      
      public function §?!Y§() : Number
      {
         return this.§#!d§;
      }
      
      public function §7!F§(param1:Number) : void
      {
         this.§#!d§ = param1;
      }
      
      override b2internal function InitVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc7_:b2Mat22 = null;
         var _loc11_:Number = NaN;
         var _loc12_:Number = NaN;
         var _loc2_:b2Body = b2internal::4!d;
         var _loc3_:Number = _loc2_.GetMass();
         var _loc4_:Number = 2 * Math.PI * this.§=§;
         var _loc5_:Number = 2 * _loc3_ * this.§#!d§ * _loc4_;
         var _loc6_:Number = _loc3_ * _loc4_ * _loc4_;
         this.§4z§ = param1.§?!C§ * (_loc5_ + param1.§?!C§ * _loc6_);
         this.§4z§ = this.§4z§ != 0 ? Number(1 / this.§4z§) : Number(0);
         this.§[^§ = param1.§?!C§ * _loc6_ * this.§4z§;
         _loc7_ = _loc2_.m_xf.R;
         var _loc8_:Number = this.§1!,§.x - _loc2_.m_sweep.localCenter.x;
         var _loc9_:Number = this.§1!,§.y - _loc2_.m_sweep.localCenter.y;
         var _loc10_:Number = _loc7_.col1.x * _loc8_ + _loc7_.col2.x * _loc9_;
         _loc9_ = _loc7_.col1.y * _loc8_ + _loc7_.col2.y * _loc9_;
         _loc8_ = _loc10_;
         _loc11_ = _loc2_.§3!q§;
         _loc12_ = _loc2_.§2!1§;
         this.K1.col1.x = _loc11_;
         this.K1.col2.x = 0;
         this.K1.col1.y = 0;
         this.K1.col2.y = _loc11_;
         this.K2.col1.x = _loc12_ * _loc9_ * _loc9_;
         this.K2.col2.x = -_loc12_ * _loc8_ * _loc9_;
         this.K2.col1.y = -_loc12_ * _loc8_ * _loc9_;
         this.K2.col2.y = _loc12_ * _loc8_ * _loc8_;
         this.§+!K§.§97§(this.K1);
         this.§+!K§.§=p§(this.K2);
         this.§+!K§.col1.x += this.§4z§;
         this.§+!K§.col2.y += this.§4z§;
         this.§+!K§.§]i§(this.§"X§);
         this.§&!w§.x = _loc2_.m_sweep.c.x + _loc8_ - this.§]!A§.x;
         this.§&!w§.y = _loc2_.m_sweep.c.y + _loc9_ - this.§]!A§.y;
         _loc2_.m_angularVelocity *= 0.98;
         this.§&""§.x *= param1.§[!4§;
         this.§&""§.y *= param1.§[!4§;
         _loc2_.§%!<§.x += _loc11_ * this.§&""§.x;
         _loc2_.§%!<§.y += _loc11_ * this.§&""§.y;
         _loc2_.m_angularVelocity += _loc12_ * (_loc8_ * this.§&""§.y - _loc9_ * this.§&""§.x);
      }
      
      override b2internal function SolveVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc3_:b2Mat22 = null;
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc2_:b2Body = b2internal::4!d;
         _loc3_ = _loc2_.m_xf.R;
         var _loc6_:Number = this.§1!,§.x - _loc2_.m_sweep.localCenter.x;
         var _loc7_:Number = this.§1!,§.y - _loc2_.m_sweep.localCenter.y;
         _loc4_ = _loc3_.col1.x * _loc6_ + _loc3_.col2.x * _loc7_;
         _loc7_ = _loc3_.col1.y * _loc6_ + _loc3_.col2.y * _loc7_;
         _loc6_ = _loc4_;
         var _loc8_:Number = _loc2_.§%!<§.x + -_loc2_.m_angularVelocity * _loc7_;
         var _loc9_:Number = _loc2_.§%!<§.y + _loc2_.m_angularVelocity * _loc6_;
         _loc3_ = this.§"X§;
         _loc4_ = _loc8_ + this.§[^§ * this.§&!w§.x + this.§4z§ * this.§&""§.x;
         _loc5_ = _loc9_ + this.§[^§ * this.§&!w§.y + this.§4z§ * this.§&""§.y;
         var _loc10_:Number = -(_loc3_.col1.x * _loc4_ + _loc3_.col2.x * _loc5_);
         var _loc11_:Number = -(_loc3_.col1.y * _loc4_ + _loc3_.col2.y * _loc5_);
         var _loc12_:Number = this.§&""§.x;
         var _loc13_:Number = this.§&""§.y;
         this.§&""§.x += _loc10_;
         this.§&""§.y += _loc11_;
         var _loc14_:Number = param1.§?!C§ * this.§>!z§;
         if(this.§&""§.§%!]§() > _loc14_ * _loc14_)
         {
            this.§&""§.§9""§(_loc14_ / this.§&""§.§,!t§());
         }
         _loc10_ = this.§&""§.x - _loc12_;
         _loc11_ = this.§&""§.y - _loc13_;
         _loc2_.§%!<§.x += _loc2_.§3!q§ * _loc10_;
         _loc2_.§%!<§.y += _loc2_.§3!q§ * _loc11_;
         _loc2_.m_angularVelocity += _loc2_.§2!1§ * (_loc6_ * _loc11_ - _loc7_ * _loc10_);
      }
      
      override b2internal function SolvePositionConstraints(param1:Number) : Boolean
      {
         return true;
      }
   }
}
