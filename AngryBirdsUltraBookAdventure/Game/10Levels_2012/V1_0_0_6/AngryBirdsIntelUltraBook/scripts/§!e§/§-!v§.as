package §!e§
{
   ...
   public class §-!v§
   {
      ...
      private function §5S§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§3!O§(param1 as String);
         }
         if(param1 is Number)
         {
         ...
         if(param1 is Array)
         {
            return this.§9A§(param1 as Array);
         }
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§3!O§(v.@name.toString()) + ":" + this.§5S§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
