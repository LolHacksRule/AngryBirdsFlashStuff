package §&!4§
{
   ...
   public class §]!j§
   {
      ...
      private function §]!_§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§3!I§(param1 as String);
         }
         if(param1 is Number)
         {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§3!I§(v.@name.toString()) + ":" + this.§]!_§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
