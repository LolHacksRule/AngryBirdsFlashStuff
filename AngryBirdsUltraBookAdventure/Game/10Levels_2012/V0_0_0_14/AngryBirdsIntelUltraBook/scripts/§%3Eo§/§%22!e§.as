package §>o§
{
   import § D§.b2Body;
   import §6z§.§[g§;
   
   public class §"!e§
   {
      
      public static const §-R§:int = 0;
      
      public static const §[v§:int;
      
      public static const §4`§:int;
      
      public static const §1!R§:int;
      
      public static const §4!T§:int = 1;
      
      public static const §7_§:int = 2;
      
      public static const §@v§:int = 3;
      
      public static const §^!2§:int = 4;
      
      public static const §@U§:int = 5;
      
      public static const §71§:int = 6;
      
      {
         var _loc1_:Boolean = false;
         var _loc2_:Boolean = true;
         if(_loc2_)
         {
            §-R§ = 0;
         }
         while(true)
         {
            §[v§ = b2Body.b2_staticBody;
            while(true)
            {
               §4`§ = b2Body.b2_kinematicBody;
               loop2:
               while(true)
               {
                  §1!R§ = b2Body.b2_dynamicBody;
                  loop3:
                  while(true)
                  {
                     §4!T§ = 1;
                     while(true)
                     {
                        §7_§ = 2;
                        continue loop3;
                        addr76:
                        if(!(_loc1_ && _loc2_))
                        {
                           continue loop2;
                        }
                     }
                  }
               }
            }
         }
      }
      
      public var mName:String;
      
      public var §-!j§:Array;
      
      public var §^1§:Array;
      
      public var §?!f§:Array;
      
      public function §"!e§(param1:String, param2:String, param3:Number, param4:Number, param5:Number, param6:Number, param7:Number, param8:Number)
      {
         var _loc9_:Boolean = true;
         var _loc10_:Boolean = false;
         if(_loc9_ || param2)
         {
            super();
            loop0:
            while(true)
            {
               this.mName = param1;
               loop1:
               while(true)
               {
                  this.§-!j§ = new Array();
                  loop2:
                  while(true)
                  {
                     §§push(this.§-!j§);
                     loop3:
                     while(true)
                     {
                        §§push(§-R§);
                        loop4:
                        while(true)
                        {
                           §§pop()[§§pop()] = this.§2!p§(param2);
                           loop5:
                           while(true)
                           {
                              §§push(this.§-!j§);
                              while(_loc9_)
                              {
                                 §§push(§4!T§);
                                 while(true)
                                 {
                                    §§push(param3);
                                    loop8:
                                    while(true)
                                    {
                                       §§pop()[§§pop()] = §§pop();
                                       loop9:
                                       while(true)
                                       {
                                          §§push(this.§-!j§);
                                          while(true)
                                          {
                                             §§push(§7_§);
                                             addr174:
                                             while(_loc9_ || this)
                                             {
                                                §§push(param4);
                                                while(_loc9_ || param3)
                                                {
                                                   §§pop()[§§pop()] = §§pop();
                                                   continue loop1;
                                                }
                                                continue loop8;
                                             }
                                             addr38:
                                             continue loop4;
                                             if(!(_loc9_ || param1))
                                             {
                                                continue;
                                             }
                                             addr45:
                                             §§push(§71§);
                                             if(!(_loc10_ && this))
                                             {
                                                addr53:
                                                §§push(param8);
                                                if(!(_loc10_ && param2))
                                                {
                                                   if(_loc9_)
                                                   {
                                                      addr63:
                                                      if(_loc9_ || param2)
                                                      {
                                                         if(!_loc10_)
                                                         {
                                                            §§pop()[§§pop()] = §§pop();
                                                            if(!_loc10_)
                                                            {
                                                               if(_loc9_ || this)
                                                               {
                                                                  if(_loc10_ && param3)
                                                                  {
                                                                     while(!_loc10_)
                                                                     {
                                                                        if(_loc10_)
                                                                        {
                                                                           continue loop0;
                                                                        }
                                                                        §§push(this.§-!j§);
                                                                        loop19:
                                                                        while(true)
                                                                        {
                                                                           §§push(§^!2§);
                                                                           addr143:
                                                                           while(true)
                                                                           {
                                                                              §§push(param6);
                                                                              addr144:
                                                                              while(true)
                                                                              {
                                                                                 §§pop()[§§pop()] = §§pop();
                                                                                 addr145:
                                                                                 while(_loc9_)
                                                                                 {
                                                                                    §§push(this.§-!j§);
                                                                                    continue loop19;
                                                                                 }
                                                                                 continue loop1;
                                                                              }
                                                                           }
                                                                        }
                                                                     }
                                                                     continue loop2;
                                                                  }
                                                                  if(_loc9_ || this)
                                                                  {
                                                                     if(_loc9_ || param1)
                                                                     {
                                                                        return;
                                                                     }
                                                                     continue loop5;
                                                                  }
                                                                  continue loop9;
                                                               }
                                                               §§goto(addr145);
                                                            }
                                                            §§goto(addr138);
                                                         }
                                                         §§goto(addr182);
                                                      }
                                                      §§goto(addr164);
                                                   }
                                                   §§goto(addr144);
                                                }
                                                §§goto(addr137);
                                             }
                                             §§goto(addr132);
                                          }
                                       }
                                    }
                                 }
                                 while(!(_loc10_ && param2))
                                 {
                                    §§goto(addr159);
                                    §§push(§@v§);
                                    §§goto(addr124);
                                 }
                              }
                              continue loop3;
                           }
                        }
                     }
                  }
               }
            }
         }
         while(true)
         {
            §§goto(addr151);
         }
      }
      
      public function getValue(param1:int) : *
      {
         return this.§-!j§[param1];
      }
      
      private function §2!p§(param1:String) : int
      {
         var _loc2_:Boolean = false;
         var _loc3_:Boolean = true;
         if(_loc3_)
         {
            §§push(param1);
            loop0:
            while(true)
            {
               if(§§pop().toLowerCase() == "kinetic")
               {
                  if(_loc3_ || _loc2_)
                  {
                     §§push(§4`§);
                     break;
                  }
                  while(true)
                  {
                  }
                  addr144:
               }
               while(true)
               {
                  §§push(param1);
                  loop3:
                  while(§§pop().toLowerCase() != "static")
                  {
                     loop4:
                     while(true)
                     {
                        §§push(param1);
                        if(!(_loc3_ || _loc2_))
                        {
                           break;
                        }
                        if(_loc2_)
                        {
                           continue loop0;
                        }
                        if(§§pop().toLowerCase() == "dynamic")
                        {
                           if(_loc3_)
                           {
                              §§push(§1!R§);
                           }
                           else
                           {
                              loop5:
                              while(!_loc2_)
                              {
                                 if(_loc3_)
                                 {
                                    while(true)
                                    {
                                       §§push(§[g§);
                                       §§push("WARNING LevelItemMaterial has invalid Body Type, name = ");
                                       if(!(_loc2_ && _loc3_))
                                       {
                                          §§push(this.mName);
                                          if(!_loc2_)
                                          {
                                             §§push(§§pop() + §§pop());
                                             if(_loc3_ || param1)
                                             {
                                                addr72:
                                                §§push(§§pop() + " invalid name = ");
                                                if(!(_loc2_ && param1))
                                                {
                                                   §§push(param1);
                                                }
                                                §§pop().log(§§pop());
                                                if(!_loc2_)
                                                {
                                                   break;
                                                }
                                                continue loop5;
                                             }
                                             §§goto(addr72);
                                          }
                                          §§push(§§pop() + §§pop());
                                       }
                                       §§goto(addr72);
                                    }
                                    if(_loc2_ && _loc3_)
                                    {
                                       break loop3;
                                    }
                                    §§push(§[v§);
                                    if(_loc3_)
                                    {
                                       if(_loc3_)
                                       {
                                          return §§pop();
                                       }
                                       break loop0;
                                    }
                                    addr29:
                                 }
                                 else
                                 {
                                    §§goto(addr144);
                                 }
                              }
                              while(true)
                              {
                                 continue loop4;
                              }
                              addr130:
                           }
                           if(!(_loc2_ && _loc2_))
                           {
                              return §§pop();
                           }
                           §§goto(addr129);
                        }
                        §§goto(addr29);
                     }
                  }
                  addr129:
                  return §§pop();
               }
            }
            return §§pop();
         }
         §§goto(addr130);
      }
      
      public function §5!U§(param1:XML) : void
      {
         var _loc5_:Boolean = false;
         var _loc6_:Boolean = true;
         var _loc3_:String = null;
         var _loc4_:Number = NaN;
         if(_loc6_ || this)
         {
            this.§^1§ = new Array();
         }
         var _loc2_:int = 0;
         while(true)
         {
            if(_loc2_ >= param1.attributes().length())
            {
               if(!_loc5_)
               {
                  if(!(_loc5_ && this))
                  {
                     if(_loc6_)
                     {
                        break;
                     }
                     loop1:
                     while(true)
                     {
                        this.§^1§[_loc3_.toUpperCase()] = _loc4_;
                        addr111:
                        while(true)
                        {
                           _loc4_ = param1.attributes()[_loc2_];
                           continue loop1;
                        }
                     }
                  }
                  while(true)
                  {
                     _loc2_++;
                  }
               }
               while(!(_loc6_ || this))
               {
               }
               continue;
            }
            _loc3_ = param1.attributes()[_loc2_].name();
            §§goto(addr111);
         }
      }
      
      public function §'3§(param1:XML) : void
      {
         var _loc5_:Boolean = true;
         var _loc6_:Boolean = false;
         var _loc3_:String = null;
         var _loc4_:Number = NaN;
         if(_loc5_ || this)
         {
            this.§?!f§ = new Array();
         }
         var _loc2_:int = 0;
         while(true)
         {
            if(_loc2_ >= param1.attributes().length())
            {
               if(!(_loc6_ && param1))
               {
                  break;
               }
               while(_loc6_ && this)
               {
                  while(!_loc5_)
                  {
                     while(!_loc5_)
                     {
                     }
                     this.§?!f§[_loc3_.toUpperCase()] = _loc4_;
                  }
                  _loc2_++;
               }
               continue;
            }
            _loc3_ = param1.attributes()[_loc2_].name();
            while(true)
            {
               _loc4_ = param1.attributes()[_loc2_];
               §§goto(addr98);
            }
         }
      }
      
      public function §]!7§(param1:String) : Number
      {
         var _loc2_:Boolean = false;
         var _loc3_:Boolean = true;
         if(_loc3_ || _loc2_)
         {
            §§push(this.§^1§);
            loop0:
            while(§§pop())
            {
               loop1:
               while(true)
               {
                  §§push(param1);
                  if(_loc3_ || _loc2_)
                  {
                     §§push(§§pop().toUpperCase());
                  }
                  param1 = §§pop();
                  while(true)
                  {
                     if(!(_loc3_ || this))
                     {
                        break loop0;
                     }
                     §§push(this.§^1§);
                     if(!(_loc2_ && _loc3_))
                     {
                        §§push(param1);
                        if(_loc3_ || this)
                        {
                           if(§§pop()[§§pop()])
                           {
                              if(!_loc2_)
                              {
                                 if(_loc2_)
                                 {
                                    continue loop1;
                                 }
                                 while(true)
                                 {
                                    §§push(this.§^1§);
                                    addr73:
                                    while(true)
                                    {
                                       if(!_loc2_)
                                       {
                                          return §§pop()[param1];
                                          addr76:
                                       }
                                       continue loop0;
                                    }
                                 }
                              }
                              else if(!_loc3_)
                              {
                                 continue;
                              }
                           }
                           §§push(1);
                           if(_loc3_ || _loc3_)
                           {
                              return §§pop();
                           }
                           break;
                        }
                        §§goto(addr76);
                     }
                     §§goto(addr73);
                  }
                  §§goto(addr115);
               }
            }
            addr115:
            return §§pop();
            §§push(1);
         }
         §§goto(addr71);
      }
      
      public function §`!s§(param1:String) : Number
      {
         var _loc2_:Boolean = true;
         var _loc3_:Boolean = false;
         if(_loc2_ || _loc2_)
         {
            §§push(this.§?!f§);
            loop0:
            for(; §§pop(); if(_loc3_)
            {
               continue;
            },§§push(param1),if(!_loc3_)
            {
               if(§§pop()[§§pop()])
               {
                  if(_loc2_)
                  {
                     if(_loc3_ && _loc2_)
                     {
                        while(true)
                        {
                           §§goto(addr85);
                        }
                        addr100:
                     }
                     §§goto(addr74);
                     §§push(this.§?!f§);
                  }
                  §§goto(addr77);
               }
               §§goto(addr24);
            },§§goto(addr74))
            {
               while(true)
               {
                  §§push(param1);
                  if(!_loc3_)
                  {
                     §§push(§§pop().toUpperCase());
                  }
                  param1 = §§pop();
                  while(true)
                  {
                     if(_loc3_)
                     {
                        break loop0;
                     }
                     §§push(this.§?!f§);
                     if(!_loc2_)
                     {
                        return §§pop()[param1];
                     }
                     addr74:
                     continue loop0;
                     addr77:
                     if(!(_loc3_ && this))
                     {
                        addr24:
                        §§push(1);
                        if(!_loc3_)
                        {
                           return §§pop();
                        }
                        break;
                     }
                  }
                  §§goto(addr99);
               }
            }
            addr99:
            return §§pop();
            §§push(1);
         }
         §§goto(addr100);
      }
   }
}
