package §?!f§
{
   ...
   public class JSONEncoder
   {
      ...
      private function §!!+§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§2!^§(param1 as String);
         }
         if(param1 is Number)
         {
      ...
      private function §'!_§(param1:Array) : String
      {
         var _loc2_:String = "";
         var _loc3_:int = 0;
         ...
         else
         {
            for each(v in classInfo..*.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§2!^§(v.@name.toString()) + ":" + this.§!!+§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
