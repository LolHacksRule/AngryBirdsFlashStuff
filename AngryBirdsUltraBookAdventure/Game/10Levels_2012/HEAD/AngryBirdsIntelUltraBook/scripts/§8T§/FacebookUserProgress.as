   ...
   public class FacebookUserProgress extends UserProgress
   {
      ...
      private function §%!X§(levelId:String) : String
      {
         ...
         if(_loc3_)
         {
            ...
            addr90:
            §§push(episode.name);
            §§push([!(_loc4_ && episode) ? §§pop() : §§pop(),levelId,§4!T§.controller.getScore(),getStarsForLevel(levelId,§4!T§.controller.getScore()),§4!T§.controller.getEagleScore(),this.§1O§()]);
            §§push("|");
            if(!_loc3_)
            {
            }
      ...
      override public function saveLevelProgress(levelId:String, meInUse:Boolean = false, tournamentScore:Boolean = false) : void
      {
         ...
         while(true)
         {
            while(true)
            {
               ...
               {
                  §§goto(addr65);
               },§§push(§-!Q§),§§push(§6![§ + this.§4!j§() + "/"),if(!(_loc10_ && tournamentScore))
               {
                  §§push(§§pop() + tournamentScore.toString());
               },var urlRequest:URLRequest = §§pop().§<!%§(§§pop()),if(_loc10_)
               {
               },urlRequest.method = URLRequestMethod.POST,addr187:,urlRequest.contentType = this.§8!3§(),addr206:,addr197:,if(!_loc10_)
               {
                  if(!(_loc10_ && this))
                  {
                     if(_loc9_)
      ...
      public function setEggUnlocked(eggId:String) : void
      {
         ...
         while(true)
         {
            ...
            var urlRequest:URLRequest = §§pop().§<!%§(§§pop());
            if(_loc4_ || this)
            {
