package §'s§
{
   import §#!"§.*;
   import §&L§.*;
   import §+!,§.*;
   
   use namespace b2internal;
   
   public class b2Distance
   {
      
      private static var b2_gjkCalls:int;
      
      private static var b2_gjkIters:int;
      
      private static var b2_gjkMaxIters:int;
      
      private static var §!6§:b2Simplex = new b2Simplex();
      
      private static var §,9§:Vector.<int> = new Vector.<int>(3);
      
      private static var §9v§:Vector.<int> = new Vector.<int>(3);
       
      
      public function b2Distance()
      {
         super();
      }
      
      public static function §;!U§(param1:b2DistanceOutput, param2:b2SimplexCache, param3:b2DistanceInput) : void
      {
         var _loc17_:int = 0;
         var _loc18_:b2Vec2 = null;
         var _loc20_:b2Vec2 = null;
         var _loc21_:b2SimplexVertex = null;
         var _loc22_:Boolean = false;
         var _loc23_:Number = NaN;
         var _loc24_:Number = NaN;
         var _loc25_:b2Vec2 = null;
         ++b2_gjkCalls;
         var _loc4_:b2DistanceProxy = param3.§5A§;
         var _loc5_:b2DistanceProxy = param3.§"T§;
         var _loc6_:b2Transform = param3.§0!a§;
         var _loc7_:b2Transform = param3.§2!A§;
         var _loc8_:b2Simplex = §!6§;
         _loc8_.§&O§(param2,_loc4_,_loc6_,_loc5_,_loc7_);
         var _loc9_:Vector.<b2SimplexVertex> = _loc8_.§`V§;
         var _loc11_:Vector.<int> = §,9§;
         var _loc12_:Vector.<int> = §9v§;
         var _loc13_:int = 0;
         var _loc14_:b2Vec2 = _loc8_.§3?§();
         var _loc15_:Number = _loc14_.§+!I§();
         var _loc16_:Number = _loc15_;
         var _loc19_:int = 0;
         while(_loc19_ < 20)
         {
            _loc13_ = _loc8_.§[!U§;
            _loc17_ = 0;
            while(_loc17_ < _loc13_)
            {
               _loc11_[_loc17_] = _loc9_[_loc17_].indexA;
               _loc12_[_loc17_] = _loc9_[_loc17_].indexB;
               _loc17_++;
            }
            switch(_loc8_.§[!U§)
            {
               case 1:
                  break;
               case 2:
                  _loc8_.Solve2();
                  break;
               case 3:
                  _loc8_.Solve3();
                  break;
               default:
                  b2Settings.b2Assert(false);
            }
            if(_loc8_.§[!U§ == 3)
            {
               break;
            }
            _loc18_ = _loc8_.§3?§();
            _loc16_ = _loc18_.§+!I§();
            if(_loc16_ > _loc15_)
            {
            }
            _loc15_ = _loc16_;
            _loc20_ = _loc8_.§'2§();
            if(_loc20_.§+!I§() < Number.MIN_VALUE * Number.MIN_VALUE)
            {
               break;
            }
            _loc21_ = _loc9_[_loc8_.§[!U§];
            _loc21_.indexA = _loc4_.§[-§(b2Math.§?a§(_loc6_.R,_loc20_.§[3§()));
            _loc21_.§]Q§ = b2Math.§?!L§(_loc6_,_loc4_.§6!'§(_loc21_.indexA));
            _loc21_.indexB = _loc5_.§[-§(b2Math.§?a§(_loc7_.R,_loc20_));
            _loc21_.§+!?§ = b2Math.§?!L§(_loc7_,_loc5_.§6!'§(_loc21_.indexB));
            _loc21_.§`c§ = b2Math.§;v§(_loc21_.§+!?§,_loc21_.§]Q§);
            _loc19_++;
            ++b2_gjkIters;
            _loc22_ = false;
            _loc17_ = 0;
            while(_loc17_ < _loc13_)
            {
               if(_loc21_.indexA == _loc11_[_loc17_] && _loc21_.indexB == _loc12_[_loc17_])
               {
                  _loc22_ = true;
                  break;
               }
               _loc17_++;
            }
            if(_loc22_)
            {
               break;
            }
            ++_loc8_.§[!U§;
         }
         b2_gjkMaxIters = b2Math.§<p§(b2_gjkMaxIters,_loc19_);
         _loc8_.§?x§(param1.§7P§,param1.§6+§);
         param1.distance = b2Math.§;v§(param1.§7P§,param1.§6+§).§ >§();
         param1.§0!j§ = _loc19_;
         _loc8_.§"!D§(param2);
         if(param3.§'!;§)
         {
            _loc23_ = _loc4_.§5!`§;
            _loc24_ = _loc5_.§5!`§;
            if(param1.distance > _loc23_ + _loc24_ && param1.distance > Number.MIN_VALUE)
            {
               param1.distance -= _loc23_ + _loc24_;
               _loc25_ = b2Math.§;v§(param1.§6+§,param1.§7P§);
               _loc25_.Normalize();
               param1.§7P§.x += _loc23_ * _loc25_.x;
               param1.§7P§.y += _loc23_ * _loc25_.y;
               param1.§6+§.x -= _loc24_ * _loc25_.x;
               param1.§6+§.y -= _loc24_ * _loc25_.y;
            }
            else
            {
               _loc18_ = new b2Vec2();
               _loc18_.x = 0.5 * (param1.§7P§.x + param1.§6+§.x);
               _loc18_.y = 0.5 * (param1.§7P§.y + param1.§6+§.y);
               param1.§7P§.x = param1.§6+§.x = _loc18_.x;
               param1.§7P§.y = param1.§6+§.y = _loc18_.y;
               param1.distance = 0;
            }
         }
      }
   }
}
