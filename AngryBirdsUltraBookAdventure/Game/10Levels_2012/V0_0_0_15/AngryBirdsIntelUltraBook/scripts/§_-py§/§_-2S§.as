package §_-py§
{
   ...
   public class §_-2S§
   {
      ...
      private function §_-mp§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§_-VE§(param1 as String);
         }
         if(param1 is Number)
         {
         ...
         if(param1 is Array)
         {
            return this.§_-04J§(param1 as Array);
         }
         if(param1 is Object && param1 != null)
         {
            return this.§_-yI§(param1);
         }
         return "null";
      }
      ...
      private function §_-04J§(param1:Array) : String
      {
         ...
         while(_loc3_ < param1.length)
         {
            ...
            _loc2_ += this.§_-mp§(param1[_loc3_]);
            _loc3_++;
         }
         return "[" + _loc2_ + "]";
      }
         ...
         if(classInfo.@name.toString() == "Object")
         {
            for(key in o)
            {
               ...
               if(!(value is Function))
               {
                  ...
                  s += this.§_-VE§(key) + ":" + this.§_-mp§(value);
               }
            }
         }
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§_-VE§(v.@name.toString()) + ":" + this.§_-mp§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
