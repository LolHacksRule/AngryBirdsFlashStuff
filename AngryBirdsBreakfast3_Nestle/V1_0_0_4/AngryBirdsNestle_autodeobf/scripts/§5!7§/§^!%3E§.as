package §5!7§
{
   ...
   public class §^!>§
   {
      ...
      {
         this.§?!r§[this.§?!r§.length] = new §7!R§(param1,param2,param3,param4,param5,param6,param7,param8,param9,param10,param11,param12,param13);
      }
      
      public function §;"!§(param1:String) : §7!R§
      {
         var _loc2_:int = 0;
         while(_loc2_ < this.§?!r§.length)
         {
            if(§7!R§(this.§?!r§[_loc2_]).name.toLowerCase() == param1.toLowerCase())
            {
               return §7!R§(this.§?!r§[_loc2_]);
            }
            _loc2_++;
         }
         §=!U§.log("WARNING: LevelItemSoundResourceManager -> getSoundResource request has no return value, this path does not exist: " + param1);
         return null;
      }
   }
}
