package §2J§
{
   public class §>!8§
   {
       
      
      private var §,!d§:Boolean;
      
      private var §!!a§:Object;
      
      private var jsonString:String;
      
      private var §^r§:int;
      
      private var §%!&§:String;
      
      private var §="5§:RegExp;
      
      public function §>!8§(param1:String, param2:Boolean)
      {
         this.§="5§ = /[\x00-\x1F]/;
         super();
         this.jsonString = param1;
         this.§,!d§ = param2;
         this.§^r§ = 0;
         this.§%!O§();
      }
      
      public function §+]§() : §0!j§
      {
         var _loc2_:String = null;
         var _loc3_:String = null;
         var _loc4_:String = null;
         var _loc5_:String = null;
         var _loc1_:§0!j§ = new §0!j§();
         this.§#T§();
         switch(this.§%!&§)
         {
            case "{":
               _loc1_.type = §^!M§.§2!t§;
               _loc1_.value = "{";
               this.§%!O§();
               break;
            case "}":
               _loc1_.type = §^!M§.§'W§;
               _loc1_.value = "}";
               this.§%!O§();
               break;
            case "[":
               _loc1_.type = §^!M§.§4X§;
               _loc1_.value = "[";
               this.§%!O§();
               break;
            case "]":
               _loc1_.type = §^!M§.§6!v§;
               _loc1_.value = "]";
               this.§%!O§();
               break;
            case ",":
               _loc1_.type = §^!M§.§-"3§;
               _loc1_.value = ",";
               this.§%!O§();
               break;
            case ":":
               _loc1_.type = §^!M§.§>"&§;
               _loc1_.value = ":";
               this.§%!O§();
               break;
            case "t":
               _loc2_ = "t" + this.§%!O§() + this.§%!O§() + this.§%!O§();
               if(_loc2_ == "true")
               {
                  _loc1_.type = §^!M§.TRUE;
                  _loc1_.value = true;
                  this.§%!O§();
               }
               else
               {
                  this.parseError("Expecting \'true\' but found " + _loc2_);
               }
               break;
            case "f":
               _loc3_ = "f" + this.§%!O§() + this.§%!O§() + this.§%!O§() + this.§%!O§();
               if(_loc3_ == "false")
               {
                  _loc1_.type = §^!M§.FALSE;
                  _loc1_.value = false;
                  this.§%!O§();
               }
               else
               {
                  this.parseError("Expecting \'false\' but found " + _loc3_);
               }
               break;
            case "n":
               if((_loc4_ = "n" + this.§%!O§() + this.§%!O§() + this.§%!O§()) == "null")
               {
                  _loc1_.type = §^!M§.NULL;
                  _loc1_.value = null;
                  this.§%!O§();
               }
               else
               {
                  this.parseError("Expecting \'null\' but found " + _loc4_);
               }
               break;
            case "N":
               if((_loc5_ = "N" + this.§%!O§() + this.§%!O§()) == "NaN")
               {
                  _loc1_.type = §^!M§.§5!V§;
                  _loc1_.value = NaN;
                  this.§%!O§();
                  break;
               }
               this.parseError("Expecting \'NaN\' but found " + _loc5_);
               break;
            case "\"":
               _loc1_ = this.§!W§();
               break;
            default:
               if(this.§0!8§(this.§%!&§) || this.§%!&§ == "-")
               {
                  _loc1_ = this.§&i§();
                  break;
               }
               if(this.§%!&§ == "")
               {
                  return null;
               }
               this.parseError("Unexpected " + this.§%!&§ + " encountered");
               break;
         }
         return _loc1_;
      }
      
      private function §!W§() : §0!j§
      {
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc1_:int = this.§^r§;
         while(true)
         {
            _loc1_ = this.jsonString.indexOf("\"",_loc1_);
            if(_loc1_ >= 0)
            {
               _loc3_ = 0;
               _loc4_ = _loc1_ - 1;
               while(this.jsonString.charAt(_loc4_) == "\\")
               {
                  _loc3_++;
                  _loc4_--;
               }
               if(_loc3_ % 2 == 0)
               {
                  break;
               }
               _loc1_++;
            }
            else
            {
               this.parseError("Unterminated string literal");
            }
         }
         var _loc2_:§0!j§ = new §0!j§();
         _loc2_.type = §^!M§.§%!D§;
         _loc2_.value = this.§=!=§(this.jsonString.substr(this.§^r§,_loc1_ - this.§^r§));
         this.§^r§ = _loc1_ + 1;
         this.§%!O§();
         return _loc2_;
      }
      
      public function §=!=§(param1:String) : String
      {
         var _loc6_:int = 0;
         var _loc7_:String = null;
         var _loc8_:String = null;
         var _loc9_:int = 0;
         var _loc10_:String = null;
         if(this.§,!d§ && this.§="5§.test(param1))
         {
            this.parseError("String contains unescaped control character (0x00-0x1F)");
         }
         var _loc2_:* = "";
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:int = param1.length;
         do
         {
            _loc3_ = param1.indexOf("\\",_loc4_);
            if(_loc3_ < 0)
            {
               _loc2_ += param1.substr(_loc4_);
               break;
            }
            _loc2_ += param1.substr(_loc4_,_loc3_ - _loc4_);
            _loc4_ = _loc3_ + 2;
            _loc6_ = _loc3_ + 1;
            _loc7_ = param1.charAt(_loc6_);
            switch(_loc7_)
            {
               case "\"":
                  _loc2_ += "\"";
                  break;
               case "\\":
                  _loc2_ += "\\";
                  break;
               case "n":
                  _loc2_ += "\n";
                  break;
               case "r":
                  _loc2_ += "\r";
                  break;
               case "t":
                  _loc2_ += "\t";
                  break;
               case "u":
                  _loc8_ = "";
                  if(_loc4_ + 4 > _loc5_)
                  {
                     this.parseError("Unexpected end of input.  Expecting 4 hex digits after \\u.");
                  }
                  _loc9_ = _loc4_;
                  while(_loc9_ < _loc4_ + 4)
                  {
                     _loc10_ = param1.charAt(_loc9_);
                     if(!this.§4_§(_loc10_))
                     {
                        this.parseError("Excepted a hex digit, but found: " + _loc10_);
                     }
                     _loc8_ += _loc10_;
                     _loc9_++;
                  }
                  _loc2_ += String.fromCharCode(parseInt(_loc8_,16));
                  _loc4_ += 4;
                  break;
               case "f":
                  _loc2_ += "\f";
                  break;
               case "/":
                  _loc2_ += "/";
                  break;
               case "b":
                  _loc2_ += "\b";
                  break;
               default:
                  _loc2_ += "\\" + _loc7_;
                  break;
            }
         }
         while(_loc4_ < _loc5_);
         
         return _loc2_;
      }
      
      private function §&i§() : §0!j§
      {
         var _loc3_:§0!j§ = null;
         var _loc1_:* = "";
         if(this.§%!&§ == "-")
         {
            _loc1_ += "-";
            this.§%!O§();
         }
         if(!this.§0!8§(this.§%!&§))
         {
            this.parseError("Expecting a digit");
         }
         if(this.§%!&§ == "0")
         {
            _loc1_ += this.§%!&§;
            this.§%!O§();
            if(this.§0!8§(this.§%!&§))
            {
               this.parseError("A digit cannot immediately follow 0");
            }
            else if(!this.§,!d§ && this.§%!&§ == "x")
            {
               _loc1_ += this.§%!&§;
               this.§%!O§();
               if(this.§4_§(this.§%!&§))
               {
                  _loc1_ += this.§%!&§;
                  this.§%!O§();
               }
               else
               {
                  this.parseError("Number in hex format require at least one hex digit after \"0x\"");
               }
               while(this.§4_§(this.§%!&§))
               {
                  _loc1_ += this.§%!&§;
                  this.§%!O§();
               }
            }
         }
         else
         {
            while(this.§0!8§(this.§%!&§))
            {
               _loc1_ += this.§%!&§;
               this.§%!O§();
            }
         }
         if(this.§%!&§ == ".")
         {
            _loc1_ += ".";
            this.§%!O§();
            if(!this.§0!8§(this.§%!&§))
            {
               this.parseError("Expecting a digit");
            }
            while(this.§0!8§(this.§%!&§))
            {
               _loc1_ += this.§%!&§;
               this.§%!O§();
            }
         }
         if(this.§%!&§ == "e" || this.§%!&§ == "E")
         {
            _loc1_ += "e";
            this.§%!O§();
            if(this.§%!&§ == "+" || this.§%!&§ == "-")
            {
               _loc1_ += this.§%!&§;
               this.§%!O§();
            }
            if(!this.§0!8§(this.§%!&§))
            {
               this.parseError("Scientific notation number needs exponent value");
            }
            while(this.§0!8§(this.§%!&§))
            {
               _loc1_ += this.§%!&§;
               this.§%!O§();
            }
         }
         var _loc2_:Number = Number(_loc1_);
         if(isFinite(_loc2_) && !isNaN(_loc2_))
         {
            _loc3_ = new §0!j§();
            _loc3_.type = §^!M§.§ 3§;
            _loc3_.value = _loc2_;
            return _loc3_;
         }
         this.parseError("Number " + _loc2_ + " is not valid!");
         return null;
      }
      
      private function §%!O§() : String
      {
         return this.§%!&§ = this.jsonString.charAt(this.§^r§++);
      }
      
      private function §#T§() : void
      {
         var _loc1_:int = 0;
         do
         {
            _loc1_ = this.§^r§;
            this.§8F§();
            this.§?!#§();
         }
         while(_loc1_ != this.§^r§);
         
      }
      
      private function §?!#§() : void
      {
         if(this.§%!&§ == "/")
         {
            this.§%!O§();
            switch(this.§%!&§)
            {
               case "/":
                  do
                  {
                     this.§%!O§();
                  }
                  while(this.§%!&§ != "\n" && this.§%!&§ != "");
                  
                  this.§%!O§();
                  break;
               case "*":
                  this.§%!O§();
                  while(true)
                  {
                     if(this.§%!&§ == "*")
                     {
                        this.§%!O§();
                        if(this.§%!&§ == "/")
                        {
                           break;
                        }
                     }
                     else
                     {
                        this.§%!O§();
                     }
                     if(this.§%!&§ == "")
                     {
                        this.parseError("Multi-line comment not closed");
                     }
                  }
                  this.§%!O§();
                  break;
               default:
                  this.parseError("Unexpected " + this.§%!&§ + " encountered (expecting \'/\' or \'*\' )");
            }
         }
      }
      
      private function §8F§() : void
      {
         while(this.§66§(this.§%!&§))
         {
            this.§%!O§();
         }
      }
      
      private function §66§(param1:String) : Boolean
      {
         if(param1 == " " || param1 == "\t" || param1 == "\n" || param1 == "\r")
         {
            return true;
         }
         if(!this.§,!d§ && param1.charCodeAt(0) == 160)
         {
            return true;
         }
         return false;
      }
      
      private function §0!8§(param1:String) : Boolean
      {
         return param1 >= "0" && param1 <= "9";
      }
      
      private function §4_§(param1:String) : Boolean
      {
         return this.§0!8§(param1) || param1 >= "A" && param1 <= "F" || param1 >= "a" && param1 <= "f";
      }
      
      public function parseError(param1:String) : void
      {
         throw new JSONParseError(param1,this.§^r§,this.jsonString);
      }
   }
}
