package §-$§
{
   import §]"1§.§1"-§;
   
   public class §4!f§
   {
      
      public static const §=!H§:uint = 16777215;
      
      public static const §'5§:uint = 12632256;
      
      public static const §[!j§:uint = 8421504;
      
      public static const §3O§:uint = 0;
      
      public static const §[!§:uint = 16711680;
      
      public static const §2o§:uint = 8388608;
      
      public static const §8!!§:uint = 16776960;
      
      public static const § n§:uint = 8421376;
      
      public static const §&!4§:uint = 65280;
      
      public static const §@G§:uint = 32768;
      
      public static const §0"$§:uint = 65535;
      
      public static const §;!7§:uint = 32896;
      
      public static const §+;§:uint = 255;
      
      public static const §8!z§:uint = 128;
      
      public static const §0+§:uint = 16711935;
      
      public static const §`E§:uint = 8388736;
       
      
      public function §4!f§()
      {
         super();
         throw new §1"-§();
      }
      
      public static function § 3§(param1:uint) : int
      {
         return param1 >> 24 & 255;
      }
      
      public static function §=!S§(param1:uint) : int
      {
         return param1 >> 16 & 255;
      }
      
      public static function §if §(param1:uint) : int
      {
         return param1 >> 8 & 255;
      }
      
      public static function §^j§(param1:uint) : int
      {
         return param1 & 255;
      }
      
      public static function §;1§(param1:int, param2:int, param3:int) : uint
      {
         return param1 << 16 | param2 << 8 | param3;
      }
      
      public static function §9§(param1:int, param2:int, param3:int, param4:int) : uint
      {
         return param1 << 24 | param2 << 16 | param3 << 8 | param4;
      }
   }
}
