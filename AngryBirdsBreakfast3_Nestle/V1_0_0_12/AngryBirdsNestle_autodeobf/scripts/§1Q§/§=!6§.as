package §1Q§
{
   import flash.geom.Matrix3D;
   import flash.geom.Point;
   import flash.geom.Vector3D;
   import flash.utils.ByteArray;
   import flash.utils.Endian;
   
   public class §=!6§
   {
      
      public static const §"g§:int = 4;
      
      public static const §>@§:int = 2;
      
      public static const §''§:int = 3;
      
      public static const §!-§:int = 4;
      
      public static const §in§:int = 0;
      
      public static const §`f§:int = 0;
      
      public static const §3!z§:int = 0;
       
      
      private var § G§:ByteArray;
      
      private var §^"+§:Vector.<Number>;
      
      private var §>1§:Vector.<Number>;
      
      private var §="%§:Boolean;
      
      private var §`>§:int;
      
      private var §@"3§:Vector.<Number>;
      
      public function §=!6§(param1:int, param2:Boolean = false)
      {
         this.§@"3§ = new Vector.<Number>(12,true);
         super();
         this.§ G§ = new ByteArray();
         this.§ G§.endian = Endian.LITTLE_ENDIAN;
         this.§ G§.length = param1 * §>@§ * §!-§;
         var _loc3_:int = 0;
         while(_loc3_ < param1 * §>@§)
         {
            this.§ G§.writeFloat(0);
            _loc3_++;
         }
         this.§^"+§ = new Vector.<Number>(param1 * §"g§,true);
         this.§>1§ = new Vector.<Number>(param1 * §''§,true);
         this.§="%§ = param2;
         this.§`>§ = param1;
      }
      
      public function clone() : §=!6§
      {
         var _loc1_:§=!6§ = new §=!6§(0,this.§="%§);
         _loc1_.§ G§.length = this.§ G§.length;
         _loc1_.§ G§.writeBytes(this.§ G§);
         _loc1_.§^"+§ = this.§^"+§.concat();
         _loc1_.§^"+§.fixed = true;
         _loc1_.§>1§ = this.§>1§.concat();
         _loc1_.§>1§.fixed = true;
         _loc1_.§`>§ = this.§`>§;
         return _loc1_;
      }
      
      public function copyTo(param1:§=!6§, param2:int, param3:Number, param4:Boolean, param5:Matrix3D) : void
      {
         var _loc6_:int = 0;
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         var _loc11_:Vector.<Number> = null;
         var _loc12_:int = 0;
         var _loc13_:Number = NaN;
         var _loc9_:Vector.<Number> = param1.§>1§;
         _loc6_ = this.§`>§ * §''§;
         _loc7_ = param2 * §''§;
         if(param5)
         {
            param5.transformVectors(this.§>1§,this.§@"3§);
            _loc8_ = 0;
            while(_loc8_ < _loc6_)
            {
               var _loc14_:*;
               _loc9_[_loc14_ = _loc7_++] = this.§@"3§[_loc8_];
               _loc8_++;
            }
         }
         else
         {
            _loc8_ = 0;
            while(_loc8_ < _loc6_)
            {
               _loc9_[_loc14_ = _loc7_++] = this.§>1§[_loc8_];
               _loc8_++;
            }
         }
         var _loc10_:ByteArray = param1.§ G§;
         _loc6_ = this.§`>§ * §>@§ * §!-§;
         _loc7_ = param2 * §>@§ * §!-§;
         if(_loc10_.length < _loc7_ + _loc6_)
         {
            _loc10_.length = _loc7_ + _loc6_;
         }
         _loc10_.position = _loc7_;
         _loc10_.writeBytes(this.§ G§);
         if(param4 || param3 != 1)
         {
            _loc11_ = param1.§^"+§;
            _loc6_ = this.§`>§ * §"g§;
            _loc7_ = param2 * §"g§;
            if(param3 == 1)
            {
               _loc8_ = 0;
               while(_loc8_ < _loc6_)
               {
                  _loc11_[_loc14_ = _loc7_++] = this.§^"+§[_loc8_];
                  _loc8_++;
               }
            }
            else
            {
               _loc12_ = 0;
               _loc8_ = 0;
               while(_loc8_ < this.§`>§)
               {
                  if(this.§="%§)
                  {
                     _loc13_ = this.§^"+§[_loc12_ + 3];
                     _loc13_ = param3 / _loc13_;
                     _loc11_[_loc14_ = _loc7_++] = this.§^"+§[_loc12_++] * _loc13_;
                     var _loc15_:*;
                     _loc11_[_loc15_ = _loc7_++] = this.§^"+§[_loc12_++] * _loc13_;
                     var _loc16_:*;
                     _loc11_[_loc16_ = _loc7_++] = this.§^"+§[_loc12_++] * _loc13_;
                     var _loc17_:*;
                     _loc11_[_loc17_ = _loc7_++] = param3;
                  }
                  else
                  {
                     _loc11_[_loc14_ = _loc7_++] = this.§^"+§[_loc12_++];
                     _loc11_[_loc15_ = _loc7_++] = this.§^"+§[_loc12_++];
                     _loc11_[_loc16_ = _loc7_++] = this.§^"+§[_loc12_++];
                     _loc11_[_loc17_ = _loc7_++] = param3;
                  }
                  _loc12_++;
                  _loc8_++;
               }
            }
         }
      }
      
      public function append(param1:§=!6§) : void
      {
         var _loc2_:int = this.§ G§.length;
         this.§ G§.length += param1.§ G§.length;
         this.§ G§.position = _loc2_;
         this.§ G§.writeBytes(param1.§ G§);
         this.§^"+§.fixed = false;
         var _loc3_:Vector.<Number> = param1.§^"+§;
         var _loc4_:int = _loc3_.length;
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            this.§^"+§.push(_loc3_[_loc5_]);
            _loc5_++;
         }
         this.§^"+§.fixed = true;
         this.§>1§.fixed = false;
         var _loc6_:Vector.<Number>;
         var _loc7_:int = (_loc6_ = param1.§>1§).length;
         var _loc8_:int = 0;
         while(_loc8_ < _loc7_)
         {
            this.§>1§.push(_loc6_[_loc8_]);
            _loc8_++;
         }
         this.§>1§.fixed = true;
         this.§`>§ += param1.§<!e§;
      }
      
      public function setPosition(param1:int, param2:Number, param3:Number, param4:Number = 0.0) : void
      {
         var _loc5_:int = param1 * §''§;
         var _loc6_:*;
         this.§>1§[_loc6_ = _loc5_++] = param2;
         var _loc7_:*;
         this.§>1§[_loc7_ = _loc5_++] = param3;
         this.§>1§[_loc5_] = param4;
      }
      
      public function §,!Z§(param1:int, param2:Number, param3:Number) : void
      {
         var _loc4_:int = param1 * §''§;
         var _loc5_:*;
         this.§>1§[_loc5_ = _loc4_++] = param2;
         this.§>1§[_loc4_] = param3;
      }
      
      public function §4s§(param1:int, param2:Vector3D) : void
      {
         var _loc3_:int = param1 * §''§;
         param2.x = this.§>1§[_loc3_++];
         param2.y = this.§>1§[_loc3_++];
         param2.z = this.§>1§[_loc3_];
      }
      
      public function §70§(param1:int, param2:uint, param3:Number = 1.0) : void
      {
         var _loc4_:Number = (_loc4_ = !!this.§="%§ ? Number(param3) : Number(1)) / 255;
         var _loc5_:Number = §-"-§.§&D§(param2) * _loc4_;
         var _loc6_:Number = §-"-§.§7"5§(param2) * _loc4_;
         var _loc7_:Number = §-"-§.§]!_§(param2) * _loc4_;
         this.§77§(param1,1,_loc5_,_loc6_,_loc7_,param3);
      }
      
      private function §77§(param1:int, param2:int, param3:Number, param4:Number, param5:Number, param6:Number) : void
      {
         var _loc7_:int = param1 * §"g§;
         var _loc8_:int = param2 - 1;
         while(_loc8_ >= 0)
         {
            var _loc9_:*;
            this.§^"+§[_loc9_ = _loc7_++] = param3;
            var _loc10_:*;
            this.§^"+§[_loc10_ = _loc7_++] = param4;
            var _loc11_:*;
            this.§^"+§[_loc11_ = _loc7_++] = param5;
            var _loc12_:*;
            this.§^"+§[_loc12_ = _loc7_++] = param6;
            _loc8_--;
         }
      }
      
      private function §-U§(param1:int, param2:Number) : void
      {
         var _loc3_:int = param1 * §"g§;
         var _loc4_:*;
         this.§^"+§[_loc4_ = _loc3_++] = this.§^"+§[_loc4_] * param2;
         var _loc5_:*;
         this.§^"+§[_loc5_ = _loc3_++] = this.§^"+§[_loc5_] * param2;
         var _loc6_:*;
         this.§^"+§[_loc6_ = _loc3_++] = this.§^"+§[_loc6_] * param2;
         this.§^"+§[_loc3_] *= param2;
      }
      
      public function §0"4§(param1:int) : uint
      {
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc2_:int = param1 * §"g§;
         var _loc3_:Number = !!this.§="%§ ? Number(this.§^"+§[_loc2_ + 3]) : Number(1);
         if(_loc3_ == 0)
         {
            return 0;
         }
         _loc3_ = 255 / _loc3_;
         _loc4_ = this.§^"+§[_loc2_] * _loc3_;
         _loc5_ = this.§^"+§[_loc2_ + 1] * _loc3_;
         _loc6_ = this.§^"+§[_loc2_ + 2] * _loc3_;
         return §-"-§.§7!r§(_loc4_,_loc5_,_loc6_);
      }
      
      public function §&'§(param1:int, param2:Number) : void
      {
         var _loc3_:int = 0;
         if(param2 < 0.001)
         {
            param2 = 0.001;
         }
         if(this.§="%§)
         {
            this.§70§(param1,this.§0"4§(param1),param2);
         }
         else
         {
            _loc3_ = param1 * §"g§ + 3;
            this.§^"+§[_loc3_] = param2;
         }
      }
      
      public function §26§(param1:int) : Number
      {
         var _loc2_:int = param1 * §"g§ + 3;
         return this.§^"+§[_loc2_];
      }
      
      public function setTexCoords(param1:int, param2:Number, param3:Number) : void
      {
         var _loc4_:int = param1 * §>@§;
         this.§ G§.position = _loc4_ * §!-§;
         this.§ G§.writeFloat(param2);
         this.§ G§.writeFloat(param3);
      }
      
      public function getTexCoords(param1:int, param2:Point) : void
      {
         var _loc3_:int = param1 * §>@§;
         this.§ G§.position = _loc3_ * §!-§;
         param2.x = this.§ G§.readFloat();
         param2.y = this.§ G§.readFloat();
      }
      
      public function §^!!§(param1:int, param2:Number, param3:Number, param4:Number = 0.0) : void
      {
         var _loc5_:int = param1 * §''§;
         var _loc6_:*;
         this.§>1§[_loc6_ = _loc5_++] = this.§>1§[_loc6_] + param2;
         var _loc7_:*;
         this.§>1§[_loc7_ = _loc5_++] = this.§>1§[_loc7_] + param3;
         this.§>1§[_loc5_] += param4;
      }
      
      public function §]!p§(param1:int, param2:Matrix3D) : void
      {
         var _loc3_:int = param1 * §''§;
         this.§@"3§[0] = this.§>1§[_loc3_];
         this.§@"3§[1] = this.§>1§[_loc3_ + 1];
         this.§@"3§[2] = this.§>1§[_loc3_ + 2];
         param2.transformVectors(this.§@"3§,this.§@"3§);
         this.§>1§[_loc3_] = this.§@"3§[0];
         this.§>1§[_loc3_ + 1] = this.§@"3§[1];
         this.§>1§[_loc3_ + 2] = this.§@"3§[2];
      }
      
      public function §"6§(param1:uint, param2:Number = 1.0) : void
      {
         if(param2 < 0.001)
         {
            param2 = 0.001;
         }
         var _loc3_:Number = !!this.§="%§ ? Number(param2) : Number(1);
         _loc3_ /= 255;
         var _loc4_:Number = §-"-§.§&D§(param1) * _loc3_;
         var _loc5_:Number = §-"-§.§7"5§(param1) * _loc3_;
         var _loc6_:Number = §-"-§.§]!_§(param1) * _loc3_;
         this.§77§(0,this.§`>§,_loc4_,_loc5_,_loc6_,param2);
      }
      
      public function §&7§(param1:int, param2:int, param3:Number, param4:Number, param5:Number, param6:Number = 1.0) : void
      {
         if(param6 < 0.001)
         {
            param6 = 0.001;
         }
         var _loc7_:int = param1 * §"g§;
         var _loc8_:int = 0;
         while(_loc8_ < param2)
         {
            var _loc9_:*;
            this.§^"+§[_loc9_ = _loc7_++] = param3;
            var _loc10_:*;
            this.§^"+§[_loc10_ = _loc7_++] = param4;
            var _loc11_:*;
            this.§^"+§[_loc11_ = _loc7_++] = param5;
            var _loc12_:*;
            this.§^"+§[_loc12_ = _loc7_++] = param6;
            _loc8_++;
         }
      }
      
      public function §!!K§(param1:int, param2:Number) : void
      {
         var _loc4_:Number = NaN;
         if(param2 < 0.001)
         {
            param2 = 0.001;
         }
         if(param2 == 1)
         {
            return;
         }
         var _loc3_:int = param1 * §"g§;
         if(this.§="%§)
         {
            _loc4_ = this.§^"+§[_loc3_ + 3];
            _loc4_ = param2 / _loc4_;
            this.§^"+§[_loc3_] *= _loc4_;
            this.§^"+§[_loc3_ + 1] *= _loc4_;
            this.§^"+§[_loc3_ + 2] *= _loc4_;
            this.§^"+§[_loc3_ + 3] = param2;
         }
         else
         {
            this.§^"+§[_loc3_ + 3] *= param2;
         }
      }
      
      public function §9"+§(param1:Boolean, param2:Boolean = true) : void
      {
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc7_:Number = NaN;
         if(param1 == this.§="%§)
         {
            return;
         }
         if(param2)
         {
            _loc3_ = this.§`>§ * §"g§;
            _loc4_ = 0;
            while(_loc4_ < _loc3_)
            {
               _loc5_ = this.§26§(0);
               _loc6_ = !!this.§="%§ ? Number(_loc5_) : Number(1);
               _loc7_ = !!param1 ? Number(_loc5_) : Number(1);
               if(_loc6_ != 0)
               {
                  _loc7_ /= _loc6_;
                  this.§-U§(_loc4_,_loc7_);
               }
               _loc4_++;
            }
         }
         this.§="%§ = param1;
      }
      
      public function get premultipliedAlpha() : Boolean
      {
         return this.§="%§;
      }
      
      public function get §<!e§() : int
      {
         return this.§`>§;
      }
      
      public function set §<!e§(param1:int) : void
      {
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         _loc2_ = param1 * §>@§ * §!-§ - this.§ G§.length;
         _loc4_ = this.§ G§.length;
         this.§ G§.length += _loc2_;
         if(_loc2_ > 0)
         {
            this.§ G§.position = _loc4_;
            _loc3_ = 0;
            while(_loc3_ < _loc2_ / §!-§)
            {
               this.§ G§.writeFloat(0);
               _loc3_++;
            }
         }
         this.§^"+§.fixed = false;
         _loc2_ = param1 * §"g§ - this.§^"+§.length;
         _loc3_ = 0;
         while(_loc3_ < _loc2_)
         {
            this.§^"+§.push(0);
            _loc3_++;
         }
         _loc3_ = _loc2_;
         while(_loc3_ < 0)
         {
            this.§^"+§.pop();
            _loc3_++;
         }
         this.§^"+§.fixed = true;
         this.§>1§.fixed = false;
         _loc2_ = param1 * §''§ - this.§>1§.length;
         _loc3_ = 0;
         while(_loc3_ < _loc2_)
         {
            this.§>1§.push(0);
            _loc3_++;
         }
         _loc3_ = _loc2_;
         while(_loc3_ < 0)
         {
            this.§>1§.pop();
            _loc3_++;
         }
         this.§>1§.fixed = true;
         this.§`>§ = param1;
      }
      
      public function get §?!S§() : ByteArray
      {
         this.§ G§.position = 0;
         return this.§ G§;
      }
      
      public function get §'!W§() : Vector.<Number>
      {
         return this.§^"+§;
      }
      
      public function get §=!u§() : Vector.<Number>
      {
         return this.§>1§;
      }
   }
}
