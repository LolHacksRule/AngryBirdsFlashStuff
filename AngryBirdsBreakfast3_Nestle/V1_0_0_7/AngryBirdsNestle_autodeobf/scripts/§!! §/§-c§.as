package §!! §
{
   ...
   public class §-c§
   {
      ...
      private function §?f§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§]!D§(param1 as String);
         }
         if(param1 is Number)
         {
      ...
      private function §]!S§(param1:Object) : String
      {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§]!D§(v.@name.toString()) + ":" + this.§?f§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
