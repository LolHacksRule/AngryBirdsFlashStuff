package §<!?§
{
   import §!r§.b2Settings;
   import §!r§.b2internal;
   import §-!2§.b2Mat22;
   import §-!2§.b2Mat33;
   import §-!2§.b2Math;
   import §-!2§.b2Vec2;
   import §-!2§.b2Vec3;
   import §0!j§.b2Body;
   import §0!j§.b2TimeStep;
   
   use namespace b2internal;
   
   public class b2RevoluteJoint extends b2Joint
   {
      
      private static var §8X§:b2Vec2 = new b2Vec2();
       
      
      private var K:b2Mat22;
      
      private var K1:b2Mat22;
      
      private var K2:b2Mat22;
      
      private var K3:b2Mat22;
      
      private var impulse3:b2Vec3;
      
      private var impulse2:b2Vec2;
      
      private var §,!w§:b2Vec2;
      
      b2internal var m_localAnchor1:b2Vec2;
      
      b2internal var m_localAnchor2:b2Vec2;
      
      private var §1E§:b2Vec3;
      
      private var §7l§:Number;
      
      private var §+s§:b2Mat33;
      
      private var §"P§:Number;
      
      private var §^!m§:Boolean;
      
      private var §,!e§:Number;
      
      private var §4-§:Number;
      
      private var §9!y§:Boolean;
      
      private var §`!`§:Number;
      
      private var §#! §:Number;
      
      private var §-!z§:Number;
      
      private var §@x§:int;
      
      public function b2RevoluteJoint(param1:b2RevoluteJointDef)
      {
         this.K = new b2Mat22();
         this.K1 = new b2Mat22();
         this.K2 = new b2Mat22();
         this.K3 = new b2Mat22();
         this.impulse3 = new b2Vec3();
         this.impulse2 = new b2Vec2();
         this.§,!w§ = new b2Vec2();
         this.m_localAnchor1 = new b2Vec2();
         this.m_localAnchor2 = new b2Vec2();
         this.§1E§ = new b2Vec3();
         this.§+s§ = new b2Mat33();
         super(param1);
         this.m_localAnchor1.SetV(param1.§'!]§);
         this.m_localAnchor2.SetV(param1.§@!p§);
         this.§`!`§ = param1.§]-§;
         this.§1E§.§`5§();
         this.§7l§ = 0;
         this.§#! § = param1.§^!0§;
         this.§-!z§ = param1.§#-§;
         this.§,!e§ = param1.§75§;
         this.§4-§ = param1.motorSpeed;
         this.§9!y§ = param1.§?!t§;
         this.§^!m§ = param1.§>!e§;
         this.§@x§ = b2internal::4E;
      }
      
      override public function GetAnchorA() : b2Vec2
      {
         return b2internal::"n.GetWorldPoint(this.m_localAnchor1);
      }
      
      override public function GetAnchorB() : b2Vec2
      {
         return b2internal::]!0.GetWorldPoint(this.m_localAnchor2);
      }
      
      override public function GetReactionForce(param1:Number) : b2Vec2
      {
         return new b2Vec2(param1 * this.§1E§.x,param1 * this.§1E§.y);
      }
      
      override public function GetReactionTorque(param1:Number) : Number
      {
         return param1 * this.§1E§.z;
      }
      
      public function §+! §() : Number
      {
         return b2internal::]!0.m_sweep.a - b2internal::"n.m_sweep.a - this.§`!`§;
      }
      
      public function §9J§() : Number
      {
         return b2internal::]!0.m_angularVelocity - b2internal::"n.m_angularVelocity;
      }
      
      public function §<f§() : Boolean
      {
         return this.§9!y§;
      }
      
      public function §]!1§(param1:Boolean) : void
      {
         this.§9!y§ = param1;
      }
      
      public function §'",§() : Number
      {
         return this.§#! §;
      }
      
      public function §?d§() : Number
      {
         return this.§-!z§;
      }
      
      public function §]!5§(param1:Number, param2:Number) : void
      {
         this.§#! § = param1;
         this.§-!z§ = param2;
      }
      
      override public function IsMotorEnabled() : Boolean
      {
         return this.§^!m§;
      }
      
      override public function EnableMotor(param1:Boolean) : void
      {
         b2internal::"n.SetAwake(true);
         b2internal::]!0.SetAwake(true);
         this.§^!m§ = param1;
      }
      
      public function §4u§(param1:Number) : void
      {
         b2internal::"n.SetAwake(true);
         b2internal::]!0.SetAwake(true);
         this.§4-§ = param1;
      }
      
      public function §6_§() : Number
      {
         return this.§4-§;
      }
      
      public function §<[§(param1:Number) : void
      {
         this.§,!e§ = param1;
      }
      
      public function §8!q§() : Number
      {
         return this.§,!e§;
      }
      
      override b2internal function InitVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc2_:b2Body = null;
         var _loc3_:b2Body = null;
         var _loc4_:b2Mat22 = null;
         var _loc5_:Number = NaN;
         var _loc7_:Number = NaN;
         var _loc15_:Number = NaN;
         var _loc16_:Number = NaN;
         var _loc17_:Number = NaN;
         _loc2_ = b2internal::"n;
         _loc3_ = b2internal::]!0;
         if(this.§^!m§ || this.§9!y§)
         {
         }
         _loc4_ = _loc2_.m_xf.R;
         var _loc6_:Number = this.m_localAnchor1.x - _loc2_.m_sweep.localCenter.x;
         _loc7_ = this.m_localAnchor1.y - _loc2_.m_sweep.localCenter.y;
         _loc5_ = _loc4_.col1.x * _loc6_ + _loc4_.col2.x * _loc7_;
         _loc7_ = _loc4_.col1.y * _loc6_ + _loc4_.col2.y * _loc7_;
         _loc6_ = _loc5_;
         _loc4_ = _loc3_.m_xf.R;
         var _loc8_:Number = this.m_localAnchor2.x - _loc3_.m_sweep.localCenter.x;
         var _loc9_:Number = this.m_localAnchor2.y - _loc3_.m_sweep.localCenter.y;
         _loc5_ = _loc4_.col1.x * _loc8_ + _loc4_.col2.x * _loc9_;
         _loc9_ = _loc4_.col1.y * _loc8_ + _loc4_.col2.y * _loc9_;
         _loc8_ = _loc5_;
         var _loc10_:Number = _loc2_.§2![§;
         var _loc11_:Number = _loc3_.§2![§;
         var _loc12_:Number = _loc2_.§2s§;
         var _loc13_:Number = _loc3_.§2s§;
         var _loc14_:* = _loc12_ + _loc13_ == 0;
         this.§+s§.col1.x = _loc10_ + _loc11_ + _loc7_ * _loc7_ * _loc12_ + _loc9_ * _loc9_ * _loc13_;
         this.§+s§.col2.x = -_loc7_ * _loc6_ * _loc12_ - _loc9_ * _loc8_ * _loc13_;
         this.§+s§.col3.x = -_loc7_ * _loc12_ - _loc9_ * _loc13_;
         this.§+s§.col1.y = this.§+s§.col2.x;
         this.§+s§.col2.y = _loc10_ + _loc11_ + _loc6_ * _loc6_ * _loc12_ + _loc8_ * _loc8_ * _loc13_;
         this.§+s§.col3.y = _loc6_ * _loc12_ + _loc8_ * _loc13_;
         this.§+s§.col1.z = this.§+s§.col3.x;
         this.§+s§.col2.z = this.§+s§.col3.y;
         this.§+s§.col3.z = _loc12_ + _loc13_;
         this.§"P§ = 1 / (_loc12_ + _loc13_);
         if(this.§^!m§ == false || _loc14_)
         {
            this.§7l§ = 0;
         }
         if(this.§9!y§ && !_loc14_)
         {
            _loc15_ = _loc3_.m_sweep.a - _loc2_.m_sweep.a - this.§`!`§;
            if(b2Math.§!!<§(this.§-!z§ - this.§#! §) < 2 * b2Settings.b2_angularSlop)
            {
               this.§@x§ = b2internal::7!;;
            }
            else if(_loc15_ <= this.§#! §)
            {
               if(this.§@x§ != b2internal::<!L)
               {
                  this.§1E§.z = 0;
               }
               this.§@x§ = b2internal::<!L;
            }
            else if(_loc15_ >= this.§-!z§)
            {
               if(this.§@x§ != b2internal::9!`)
               {
                  this.§1E§.z = 0;
               }
               this.§@x§ = b2internal::9!`;
            }
            else
            {
               this.§@x§ = b2internal::4E;
               this.§1E§.z = 0;
            }
         }
         else
         {
            this.§@x§ = b2internal::4E;
         }
         if(param1.§'!t§)
         {
            this.§1E§.x *= param1.§1!m§;
            this.§1E§.y *= param1.§1!m§;
            this.§7l§ *= param1.§1!m§;
            _loc16_ = this.§1E§.x;
            _loc17_ = this.§1E§.y;
            _loc2_.§3N§.x -= _loc10_ * _loc16_;
            _loc2_.§3N§.y -= _loc10_ * _loc17_;
            _loc2_.m_angularVelocity -= _loc12_ * (_loc6_ * _loc17_ - _loc7_ * _loc16_ + this.§7l§ + this.§1E§.z);
            _loc3_.§3N§.x += _loc11_ * _loc16_;
            _loc3_.§3N§.y += _loc11_ * _loc17_;
            _loc3_.m_angularVelocity += _loc13_ * (_loc8_ * _loc17_ - _loc9_ * _loc16_ + this.§7l§ + this.§1E§.z);
         }
         else
         {
            this.§1E§.§`5§();
            this.§7l§ = 0;
         }
      }
      
      override b2internal function SolveVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc4_:b2Mat22 = null;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc7_:Number = NaN;
         var _loc8_:Number = NaN;
         var _loc9_:Number = NaN;
         var _loc10_:Number = NaN;
         var _loc20_:Number = NaN;
         var _loc21_:Number = NaN;
         var _loc22_:Number = NaN;
         var _loc23_:Number = NaN;
         var _loc24_:Number = NaN;
         var _loc25_:Number = NaN;
         var _loc26_:Number = NaN;
         var _loc27_:Number = NaN;
         var _loc28_:Number = NaN;
         var _loc2_:b2Body = b2internal::"n;
         var _loc3_:b2Body = b2internal::]!0;
         var _loc11_:b2Vec2 = _loc2_.§3N§;
         var _loc12_:Number = _loc2_.m_angularVelocity;
         var _loc13_:b2Vec2 = _loc3_.§3N§;
         var _loc14_:Number = _loc3_.m_angularVelocity;
         var _loc15_:Number = _loc2_.§2![§;
         var _loc16_:Number = _loc3_.§2![§;
         var _loc17_:Number = _loc2_.§2s§;
         var _loc18_:Number = _loc3_.§2s§;
         var _loc19_:* = _loc17_ + _loc18_ == 0;
         if(this.§^!m§ && this.§@x§ != b2internal::7!; && !_loc19_)
         {
            _loc20_ = _loc14_ - _loc12_ - this.§4-§;
            _loc21_ = this.§"P§ * -_loc20_;
            _loc22_ = this.§7l§;
            _loc23_ = param1.§&c§ * this.§,!e§;
            this.§7l§ = b2Math.§+u§(this.§7l§ + _loc21_,-_loc23_,_loc23_);
            _loc21_ = this.§7l§ - _loc22_;
            _loc12_ -= _loc17_ * _loc21_;
            _loc14_ += _loc18_ * _loc21_;
         }
         if(this.§9!y§ && this.§@x§ != b2internal::4E && !_loc19_)
         {
            _loc4_ = _loc2_.m_xf.R;
            _loc7_ = this.m_localAnchor1.x - _loc2_.m_sweep.localCenter.x;
            _loc8_ = this.m_localAnchor1.y - _loc2_.m_sweep.localCenter.y;
            _loc5_ = _loc4_.col1.x * _loc7_ + _loc4_.col2.x * _loc8_;
            _loc8_ = _loc4_.col1.y * _loc7_ + _loc4_.col2.y * _loc8_;
            _loc7_ = _loc5_;
            _loc4_ = _loc3_.m_xf.R;
            _loc9_ = this.m_localAnchor2.x - _loc3_.m_sweep.localCenter.x;
            _loc10_ = this.m_localAnchor2.y - _loc3_.m_sweep.localCenter.y;
            _loc5_ = _loc4_.col1.x * _loc9_ + _loc4_.col2.x * _loc10_;
            _loc10_ = _loc4_.col1.y * _loc9_ + _loc4_.col2.y * _loc10_;
            _loc9_ = _loc5_;
            _loc24_ = _loc13_.x + -_loc14_ * _loc10_ - _loc11_.x - -_loc12_ * _loc8_;
            _loc25_ = _loc13_.y + _loc14_ * _loc9_ - _loc11_.y - _loc12_ * _loc7_;
            _loc26_ = _loc14_ - _loc12_;
            this.§+s§.Solve33(this.impulse3,-_loc24_,-_loc25_,-_loc26_);
            if(this.§@x§ == b2internal::7!;)
            {
               this.§1E§.§3!j§(this.impulse3);
            }
            else if(this.§@x§ == b2internal::<!L)
            {
               if((_loc6_ = this.§1E§.z + this.impulse3.z) < 0)
               {
                  this.§+s§.Solve22(this.§,!w§,-_loc24_ + this.§1E§.z * this.§+s§.col3.x,-_loc25_ + this.§1E§.z * this.§+s§.col3.y);
                  this.impulse3.x = this.§,!w§.x;
                  this.impulse3.y = this.§,!w§.y;
                  this.impulse3.z = -this.§1E§.z;
                  this.§1E§.x += this.§,!w§.x;
                  this.§1E§.y += this.§,!w§.y;
                  this.§1E§.z = 0;
               }
               else
               {
                  this.§1E§.x += this.impulse3.x;
                  this.§1E§.y += this.impulse3.y;
                  this.§1E§.z += this.impulse3.z;
               }
            }
            else if(this.§@x§ == b2internal::9!`)
            {
               if((_loc6_ = this.§1E§.z + this.impulse3.z) > 0)
               {
                  this.§+s§.Solve22(this.§,!w§,-_loc24_ + this.§1E§.z * this.§+s§.col3.x,-_loc25_ + this.§1E§.z * this.§+s§.col3.y);
                  this.impulse3.x = this.§,!w§.x;
                  this.impulse3.y = this.§,!w§.y;
                  this.impulse3.z = -this.§1E§.z;
                  this.§1E§.x += this.§,!w§.x;
                  this.§1E§.y += this.§,!w§.y;
                  this.§1E§.z = 0;
               }
               else
               {
                  this.§1E§.x += this.impulse3.x;
                  this.§1E§.y += this.impulse3.y;
                  this.§1E§.z += this.impulse3.z;
               }
            }
            _loc11_.x -= _loc15_ * this.impulse3.x;
            _loc11_.y -= _loc15_ * this.impulse3.y;
            _loc12_ -= _loc17_ * (_loc7_ * this.impulse3.y - _loc8_ * this.impulse3.x + this.impulse3.z);
            _loc13_.x += _loc16_ * this.impulse3.x;
            _loc13_.y += _loc16_ * this.impulse3.y;
            _loc14_ += _loc18_ * (_loc9_ * this.impulse3.y - _loc10_ * this.impulse3.x + this.impulse3.z);
         }
         else
         {
            _loc4_ = _loc2_.m_xf.R;
            _loc7_ = this.m_localAnchor1.x - _loc2_.m_sweep.localCenter.x;
            _loc8_ = this.m_localAnchor1.y - _loc2_.m_sweep.localCenter.y;
            _loc5_ = _loc4_.col1.x * _loc7_ + _loc4_.col2.x * _loc8_;
            _loc8_ = _loc4_.col1.y * _loc7_ + _loc4_.col2.y * _loc8_;
            _loc7_ = _loc5_;
            _loc4_ = _loc3_.m_xf.R;
            _loc9_ = this.m_localAnchor2.x - _loc3_.m_sweep.localCenter.x;
            _loc10_ = this.m_localAnchor2.y - _loc3_.m_sweep.localCenter.y;
            _loc5_ = _loc4_.col1.x * _loc9_ + _loc4_.col2.x * _loc10_;
            _loc10_ = _loc4_.col1.y * _loc9_ + _loc4_.col2.y * _loc10_;
            _loc9_ = _loc5_;
            _loc27_ = _loc13_.x + -_loc14_ * _loc10_ - _loc11_.x - -_loc12_ * _loc8_;
            _loc28_ = _loc13_.y + _loc14_ * _loc9_ - _loc11_.y - _loc12_ * _loc7_;
            this.§+s§.Solve22(this.impulse2,-_loc27_,-_loc28_);
            this.§1E§.x += this.impulse2.x;
            this.§1E§.y += this.impulse2.y;
            _loc11_.x -= _loc15_ * this.impulse2.x;
            _loc11_.y -= _loc15_ * this.impulse2.y;
            _loc12_ -= _loc17_ * (_loc7_ * this.impulse2.y - _loc8_ * this.impulse2.x);
            _loc13_.x += _loc16_ * this.impulse2.x;
            _loc13_.y += _loc16_ * this.impulse2.y;
            _loc14_ += _loc18_ * (_loc9_ * this.impulse2.y - _loc10_ * this.impulse2.x);
         }
         _loc2_.§3N§.SetV(_loc11_);
         _loc2_.m_angularVelocity = _loc12_;
         _loc3_.§3N§.SetV(_loc13_);
         _loc3_.m_angularVelocity = _loc14_;
      }
      
      override b2internal function SolvePositionConstraints(param1:Number) : Boolean
      {
         var _loc2_:Number = NaN;
         var _loc3_:Number = NaN;
         var _loc4_:b2Mat22 = null;
         var _loc10_:Number = NaN;
         var _loc11_:Number = NaN;
         var _loc12_:Number = NaN;
         var _loc26_:Number = NaN;
         var _loc27_:Number = NaN;
         var _loc28_:Number = NaN;
         var _loc29_:Number = NaN;
         var _loc30_:Number = NaN;
         var _loc31_:Number = NaN;
         var _loc32_:Number = NaN;
         var _loc5_:b2Body = b2internal::"n;
         var _loc6_:b2Body = b2internal::]!0;
         var _loc7_:* = b2internal::"!9 + b2internal::8!L == 0;
         var _loc8_:Number = 0;
         var _loc9_:Number = 0;
         if(this.§9!y§ && this.§@x§ != b2internal::4E && !_loc7_)
         {
            _loc26_ = _loc6_.m_sweep.a - _loc5_.m_sweep.a - this.§`!`§;
            _loc27_ = 0;
            if(this.§@x§ == b2internal::7!;)
            {
               _loc3_ = b2Math.§+u§(_loc26_ - this.§#! §,-b2Settings.b2_maxAngularCorrection,b2Settings.b2_maxAngularCorrection);
               _loc27_ = -this.§"P§ * _loc3_;
               _loc8_ = b2Math.§!!<§(_loc3_);
            }
            else if(this.§@x§ == b2internal::<!L)
            {
               _loc3_ = _loc26_ - this.§#! §;
               _loc8_ = -_loc3_;
               _loc3_ = b2Math.§+u§(_loc3_ + b2Settings.b2_angularSlop,-b2Settings.b2_maxAngularCorrection,0);
               _loc27_ = -this.§"P§ * _loc3_;
            }
            else if(this.§@x§ == b2internal::9!`)
            {
               _loc3_ = _loc26_ - this.§-!z§;
               _loc8_ = _loc3_;
               _loc3_ = b2Math.§+u§(_loc3_ - b2Settings.b2_angularSlop,0,b2Settings.b2_maxAngularCorrection);
               _loc27_ = -this.§"P§ * _loc3_;
            }
            _loc5_.m_sweep.a -= _loc5_.§2s§ * _loc27_;
            _loc6_.m_sweep.a += _loc6_.§2s§ * _loc27_;
            _loc5_.§+!8§();
            _loc6_.§+!8§();
         }
         _loc4_ = _loc5_.m_xf.R;
         var _loc13_:Number = this.m_localAnchor1.x - _loc5_.m_sweep.localCenter.x;
         var _loc14_:Number = this.m_localAnchor1.y - _loc5_.m_sweep.localCenter.y;
         _loc10_ = _loc4_.col1.x * _loc13_ + _loc4_.col2.x * _loc14_;
         _loc14_ = _loc4_.col1.y * _loc13_ + _loc4_.col2.y * _loc14_;
         _loc13_ = _loc10_;
         _loc4_ = _loc6_.m_xf.R;
         var _loc15_:Number = this.m_localAnchor2.x - _loc6_.m_sweep.localCenter.x;
         var _loc16_:Number = this.m_localAnchor2.y - _loc6_.m_sweep.localCenter.y;
         _loc10_ = _loc4_.col1.x * _loc15_ + _loc4_.col2.x * _loc16_;
         _loc16_ = _loc4_.col1.y * _loc15_ + _loc4_.col2.y * _loc16_;
         _loc15_ = _loc10_;
         var _loc17_:Number = _loc6_.m_sweep.c.x + _loc15_ - _loc5_.m_sweep.c.x - _loc13_;
         var _loc18_:Number = _loc6_.m_sweep.c.y + _loc16_ - _loc5_.m_sweep.c.y - _loc14_;
         var _loc19_:Number = _loc17_ * _loc17_ + _loc18_ * _loc18_;
         var _loc20_:Number;
         _loc9_ = _loc20_ = Math.sqrt(_loc19_);
         var _loc21_:Number = _loc5_.§2![§;
         var _loc22_:Number = _loc6_.§2![§;
         var _loc23_:Number = _loc5_.§2s§;
         var _loc24_:Number = _loc6_.§2s§;
         var _loc25_:Number = 10 * b2Settings.b2_linearSlop * 4;
         if(_loc19_ > _loc25_ * _loc25_)
         {
            _loc28_ = _loc17_ / _loc20_;
            _loc29_ = _loc18_ / _loc20_;
            _loc30_ = _loc21_ + _loc22_;
            _loc11_ = (_loc31_ = 1 / _loc30_) * -_loc17_;
            _loc12_ = _loc31_ * -_loc18_;
            _loc32_ = 0.5;
            _loc5_.m_sweep.c.x -= _loc32_ * _loc21_ * _loc11_;
            _loc5_.m_sweep.c.y -= _loc32_ * _loc21_ * _loc12_;
            _loc6_.m_sweep.c.x += _loc32_ * _loc22_ * _loc11_;
            _loc6_.m_sweep.c.y += _loc32_ * _loc22_ * _loc12_;
            _loc17_ = _loc6_.m_sweep.c.x + _loc15_ - _loc5_.m_sweep.c.x - _loc13_;
            _loc18_ = _loc6_.m_sweep.c.y + _loc16_ - _loc5_.m_sweep.c.y - _loc14_;
         }
         this.K1.col1.x = _loc21_ + _loc22_;
         this.K1.col2.x = 0;
         this.K1.col1.y = 0;
         this.K1.col2.y = _loc21_ + _loc22_;
         this.K2.col1.x = _loc23_ * _loc14_ * _loc14_;
         this.K2.col2.x = -_loc23_ * _loc13_ * _loc14_;
         this.K2.col1.y = -_loc23_ * _loc13_ * _loc14_;
         this.K2.col2.y = _loc23_ * _loc13_ * _loc13_;
         this.K3.col1.x = _loc24_ * _loc16_ * _loc16_;
         this.K3.col2.x = -_loc24_ * _loc15_ * _loc16_;
         this.K3.col1.y = -_loc24_ * _loc15_ * _loc16_;
         this.K3.col2.y = _loc24_ * _loc15_ * _loc15_;
         this.K.§3K§(this.K1);
         this.K.§[w§(this.K2);
         this.K.§[w§(this.K3);
         this.K.§9!§(§8X§,-_loc17_,-_loc18_);
         _loc11_ = §8X§.x;
         _loc12_ = §8X§.y;
         _loc5_.m_sweep.c.x -= _loc5_.§2![§ * _loc11_;
         _loc5_.m_sweep.c.y -= _loc5_.§2![§ * _loc12_;
         _loc5_.m_sweep.a -= _loc5_.§2s§ * (_loc13_ * _loc12_ - _loc14_ * _loc11_);
         _loc6_.m_sweep.c.x += _loc6_.§2![§ * _loc11_;
         _loc6_.m_sweep.c.y += _loc6_.§2![§ * _loc12_;
         _loc6_.m_sweep.a += _loc6_.§2s§ * (_loc15_ * _loc12_ - _loc16_ * _loc11_);
         _loc5_.§+!8§();
         _loc6_.§+!8§();
         return _loc9_ <= b2Settings.b2_linearSlop * 4 && _loc8_ <= b2Settings.b2_angularSlop;
      }
   }
}
