   ...
   public class LevelObject extends Sprite
   {
      ...
      public function applyDamage(damage:Number, addScore:Boolean = true, damagedByActiveObject:Boolean = false) : Number
      {
         ...
         else
         {
            ...
            {
               count = 8 * damage / this.mHealthMax;
               speed = damage / 10;
               angle = 90;
               for(b = 0; b < count; b++)
               {
      ...
      public function isFastEnoughToDamage() : Boolean
      {
         ...
         return this.getBody().IsAwake() && (this.isBird() && this.mHealth == this.mHealthMax || Math.abs(this.getBody().GetLinearVelocity().x) > b2Settings.b2_linearSleepTolerance * LIMIT_MULTIPLIER || Math.abs(this.getBody().GetLinearVelocity().y) > b2Settings.b2_linearSleepTolerance * LIMIT_MULTIPLIER || Math.abs(this.getBody().GetAngularVelocity()) > b2Settings.b2_angularSleepTolerance * LIMIT_MULTIPLIER);
      }
      ...
      public function considerSleeping() : Boolean
      {
         ...
         var LIMIT_MULTIPLIER:int = 30;
         if(Math.abs(this.getBody().GetLinearVelocity().x) < b2Settings.b2_linearSleepTolerance * LIMIT_MULTIPLIER && Math.abs(this.getBody().GetLinearVelocity().y) < b2Settings.b2_linearSleepTolerance * LIMIT_MULTIPLIER && Math.abs(this.getBody().GetAngularVelocity()) < b2Settings.b2_angularSleepTolerance * LIMIT_MULTIPLIER)
         {
            return true;
         }
         return false;
