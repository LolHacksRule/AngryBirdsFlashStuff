package com.AngryBirds.AvatarCreator.ui
{
   ...
   public class ViewAvatarCreator extends UIView
   {
      ...
      public function getRepeaterItemsDataXML() : Array
      {
         ...
         var _loc1_:Array = new Array();
         var _loc5_:Class = AssetCache.getAssetFromCache("Repeater_Button_Editor_Items");
         var _loc6_:Number = 0;
         while(_loc6_ < AvatarCreator.smItems.mCategories.length)
         {
            _loc7_ = AvatarCreator.smItems.mCategories[_loc6_] as String;
            _loc8_ = AvatarCreator.smItems.getItemsInCategory(_loc7_);
            _loc1_[_loc6_] = new Array();
            _loc3_ = new (_loc4_ = AssetCache.getAssetFromCache("Item_Inventory_Deselect"))();
            _loc3_.stop();
            (_loc9_ = new _loc5_()).Image_Lock.visible = false;
            _loc9_.TextField_Lock.visible = false;
            _loc9_.TextField_Cost.visible = false;
            _loc2_ = <Button/>;
            _loc2_.@name = _loc7_;
            _loc2_.@MouseUp = "REMOVE_" + _loc7_;
            (_loc1_[_loc6_] as Array).push(new Array(_loc2_,_loc3_,_loc9_));
            _loc10_ = 0;
            while(_loc10_ < _loc8_.length)
            {
               _loc11_ = _loc8_[_loc10_];
               _loc3_ = new (_loc4_ = AssetCache.getAssetFromCache("Item_Inventory_" + _loc11_.mId))();
               _loc3_.stop();
               _loc2_ = <Button/>;
               _loc2_.@name = _loc11_.mId;
               _loc2_.@MouseUp = _loc11_.mId;
               (_loc9_ = new _loc5_()).TextField_Cost.text = _loc11_.mCost;
               if(AvatarCreator.smLevel < _loc11_.mLevel)
               {
                  _loc2_.@enabled = false;
                  _loc9_.Image_Lock.visible = true;
                  _loc9_.TextField_Lock.text = _loc11_.mLevel;
                  _loc9_.TextField_Lock.visible = true;
                  _loc9_.TextField_Cost.visible = false;
               }
               else if(_loc11_.mCost > 0 && AvatarCreator.smPaidItems.indexOf(_loc11_.mId) == -1)
               {
                  _loc9_.Image_Lock.visible = false;
                  _loc9_.TextField_Lock.visible = false;
                  _loc9_.TextField_Cost.visible = true;
                  _loc9_.TextField_Cost.text = _loc11_.mCost;
               }
               else
               {
                  _loc9_.Image_Lock.visible = false;
