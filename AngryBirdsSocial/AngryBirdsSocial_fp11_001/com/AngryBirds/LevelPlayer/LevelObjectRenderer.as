package com.AngryBirds.LevelPlayer
{
   ...
   public class LevelObjectRenderer extends Sprite
   {
      ...
      private static var bitmapCache:Array = null;
      ...
      public var mBufferedBitmap:BitmapData = null;
      
      public var mBitmap:Bitmap = null;
      
      public var mUseBufferedBitmapRendering:Boolean = false;
      ...
      public function LevelObjectRenderer(param1:LevelObject)
      {
         ...
         this.mSparkles = null;
         if(!this.mLevelObject.isPig() && !this.mLevelObject.isBird())
         {
            this.mUseBufferedBitmapRendering = true;
         }
      }
      ...
      public function loadAnimation() : Boolean
      {
         ...
         if(this.mAnimations)
         {
            if(this.mUseBufferedBitmapRendering)
            {
               this.mBufferedBitmap = new BitmapData(this.mAnimations.width,this.mAnimations.height,true,0);
               this.mBitmap = new Bitmap(this.mBufferedBitmap,"auto",true);
      ...
      public function setVisualState() : void
      {
         ...
         else if(this.mVisibleDisplayObject != null)
         {
            this.mVisibleDisplayObject.visible = true;
         }
         if(this.mUseBufferedBitmapRendering)
         {
            _loc4_ = this.mAnimations;
            if(this.mVisibleDisplayObject != null)
            {
               _loc4_ = this.mVisibleDisplayObject;
            }
            _loc5_ = this.mAnimationsName;
            if(_loc4_.name.indexOf("instance") == -1)
            {
               _loc5_ += ":" + _loc4_.name;
            }
            if(bitmapCache == null)
            {
               bitmapCache = new Array();
            }
            _loc6_ = bitmapCache[_loc5_];
            _loc8_ = (_loc7_ = _loc4_.getRect(_loc4_)).x;
            _loc9_ = _loc7_.y;
            if(_loc6_ == null)
            {
               _loc6_ = new BitmapData(_loc4_.width,_loc4_.height,true,0);
               (_loc10_ = new Matrix()).translate(-_loc8_,-_loc9_);
               _loc6_.draw(_loc4_,_loc10_);
               bitmapCache[_loc5_] = _loc6_;
            }
            this.mBitmap.bitmapData = _loc6_;
            this.mBitmap.x = _loc8_;
            this.mBitmap.y = _loc9_;
      ...
      public function calculateDamageFrameCount() : void
      {
         ...
         var _loc1_:String = "Damaged";
         var _loc2_:Boolean = true;
         var _loc3_:int = 1;
         this.mDamagedFrameCount = 0;
         while(_loc2_)
         {
            if(this.mAnimations.getChildByName(_loc1_ + _loc3_))
            {
               _loc3_++;
      ...
      public function createSparkles() : void
      {
         ...
         var _loc1_:int = 0;
         while(_loc1_ < Math.min(8,Math.max(this.mLevelObject.getVolume(false),1)))
         {
