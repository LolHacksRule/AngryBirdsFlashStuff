package §4N§
{
   ...
   public class §switch§
   {
      ...
      public static function §>!,§(param1:XMLList) : void
      {
         ...
         for each(_loc2_ in param1.Shape)
         {
            ...
            else
            {
               ...
               {
                  §3f§.log("WARNING, LevelItemShapes constructor, bodyType is missing for shape: " + _loc2_.@id);
               }
               if(_loc2_.attribute("width").length() <= 0)
               {
                  §3f§.log("WARNING, LevelItemShapes constructor, density is missing for shape: " + _loc2_.@id);
               }
               if(_loc2_.attribute("height").length() <= 0)
               {
                  §3f§.log("WARNING, LevelItemShapes constructor, friction is missing for shape: " + _loc2_.@id);
               }
      ...
      public static function §0V§(param1:String) : §2D§
      {
         ...
         §3f§.log("WARNING: LevelItemShapes -> getShape request has no return value, this shape does not exist: " + param1);
         return null;
      }
   }
}
