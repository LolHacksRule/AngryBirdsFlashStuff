package §'" §
{
   ...
   public class §%![§
   {
      ...
      private function §2"@§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§1F§(param1 as String);
         }
         if(param1 is Number)
         {
      ...
      private function §1F§(param1:String) : String
      {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§1F§(v.@name.toString()) + ":" + this.§2"@§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
