package §!!U§
{
   import §&#H§.§%"H§;
   import §=]§.§%"G§;
   import §=]§.transformCoords;
   import flash.display.BitmapData;
   import flash.geom.Matrix;
   import flash.geom.Matrix3D;
   import flash.geom.Point;
   import flash.geom.Rectangle;
   import flash.geom.Vector3D;
   
   public class §2"D§ extends DisplayObject
   {
      
      private static var §?"L§:Vector3D = new Vector3D();
      
      private static var §1!k§:Point = new Point();
      
      private static var §#!O§:Matrix = new Matrix();
       
      
      protected var §;q§:§%"G§;
      
      private var §!#,§:Number = 0.0;
      
      private var §#!r§:Number = 0.0;
      
      private var §]H§:uint = 16777215;
      
      public function §2"D§(param1:Number, param2:Number, param3:uint = 16777215, param4:Boolean = true)
      {
         super();
         this.§;q§ = new §%"G§(4,param4);
         this.updateVertexData(param1,param2,param3,param4);
         this.§]H§ = param3;
      }
      
      public function get highQuality() : Boolean
      {
         return false;
      }
      
      protected function updateVertexData(param1:Number, param2:Number, param3:uint, param4:Boolean) : void
      {
         this.§;q§.§[!p§(param4);
         this.§;q§.setPosition(0,0,0);
         this.§;q§.setPosition(1,param1,0);
         this.§;q§.setPosition(2,0,param2);
         this.§;q§.setPosition(3,param1,param2);
         this.§;q§.§7E§(param3);
         this.§!#,§ = param1;
         this.§#!r§ = param2;
      }
      
      override public function getBounds(param1:DisplayObject, param2:Rectangle = null) : Rectangle
      {
         var _loc7_:int = 0;
         if(param2 == null)
         {
            param2 = new Rectangle();
         }
         var _loc3_:Number = Number.MAX_VALUE;
         var _loc4_:Number = -Number.MAX_VALUE;
         var _loc5_:Number = Number.MAX_VALUE;
         var _loc6_:Number = -Number.MAX_VALUE;
         if(param1 == this)
         {
            _loc7_ = 0;
            while(_loc7_ < 4)
            {
               this.§;q§.getPosition(_loc7_,§?"L§);
               _loc3_ = _loc3_ < §?"L§.x ? Number(_loc3_) : Number(§?"L§.x);
               _loc4_ = _loc4_ > §?"L§.x ? Number(_loc4_) : Number(§?"L§.x);
               _loc5_ = _loc5_ < §?"L§.y ? Number(_loc5_) : Number(§?"L§.y);
               _loc6_ = _loc6_ > §?"L§.y ? Number(_loc6_) : Number(§?"L§.y);
               _loc7_++;
            }
         }
         else
         {
            §^">§(param1,§#!O§);
            _loc7_ = 0;
            while(_loc7_ < 4)
            {
               this.§;q§.getPosition(_loc7_,§?"L§);
               transformCoords(§#!O§,§?"L§.x,§?"L§.y,§1!k§);
               _loc3_ = _loc3_ < §1!k§.x ? Number(_loc3_) : Number(§1!k§.x);
               _loc4_ = _loc4_ > §1!k§.x ? Number(_loc4_) : Number(§1!k§.x);
               _loc5_ = _loc5_ < §1!k§.y ? Number(_loc5_) : Number(§1!k§.y);
               _loc6_ = _loc6_ > §1!k§.y ? Number(_loc6_) : Number(§1!k§.y);
               _loc7_++;
            }
         }
         param2.x = _loc3_;
         param2.y = _loc5_;
         param2.width = _loc4_ - _loc3_;
         param2.height = _loc6_ - _loc5_;
         return param2;
      }
      
      public function §1d§(param1:int) : uint
      {
         return this.§;q§.§&!H§(param1);
      }
      
      public function setVertexColor(param1:int, param2:uint) : void
      {
         this.§;q§.§""S§(param1,param2);
      }
      
      public function §0#?§(param1:int) : Number
      {
         return this.§;q§.§="-§(param1);
      }
      
      public function setVertexAlpha(param1:int, param2:Number) : void
      {
         this.§;q§.§+h§(param1,param2);
      }
      
      public function get color() : uint
      {
         return this.§]H§;
      }
      
      override public function set color(param1:uint) : void
      {
         if(this.§]H§ != param1)
         {
            this.§]H§ = param1;
            this.§;q§.§7E§(param1,1);
         }
      }
      
      public function copyVertexDataTo(param1:§%"G§, param2:int = 0, param3:Number = 1.0, param4:Matrix3D = null) : void
      {
         this.§;q§.copyTo(param1,param2,param3,true,param4);
      }
      
      override public function render(param1:§%"H§, param2:Number) : void
      {
         param1.batchQuad(this,param2);
      }
      
      public function get useColor() : Boolean
      {
         return true;
      }
      
      public function get clippedBitmapData() : BitmapData
      {
         return null;
      }
      
      public function get §9"#§() : Number
      {
         return this.§!#,§;
      }
      
      public function get §2"U§() : Number
      {
         return this.§#!r§;
      }
   }
}
