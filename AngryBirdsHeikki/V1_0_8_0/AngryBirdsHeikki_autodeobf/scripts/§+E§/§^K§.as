package §+E§
{
   ...
   public class §^K§
   {
      ...
      private function § !X§(param1:*) : String
      {
         ...
         if(param1 is String)
         {
            return this.§8o§(param1 as String);
         }
         if(param1 is Number)
         {
      ...
      private function §8o§(param1:String) : String
      {
      ...
      private function §=x§(param1:Array) : String
      {
         var _loc2_:* = "";
         var _loc3_:int = 0;
         while(_loc3_ < param1.length)
         {
            ...
            _loc2_ += this.§ !X§(param1[_loc3_]);
            _loc3_++;
         }
         return "[" + _loc2_ + "]";
      }
      
      private function §else §(param1:Object) : String
      {
         ...
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§8o§(v.@name.toString()) + ":" + this.§ !X§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
