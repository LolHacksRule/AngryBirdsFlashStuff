package §8<§
{
   import §0!=§.b2Mat22;
   import §0!=§.b2Vec2;
   import §0!?§.b2Body;
   import §0!?§.b2TimeStep;
   import §0^§.b2internal;
   
   use namespace b2internal;
   
   public class b2MouseJoint extends b2Joint
   {
       
      
      private var §6_§:b2Mat22;
      
      private var K1:b2Mat22;
      
      private var K2:b2Mat22;
      
      private var § !8§:b2Vec2;
      
      private var §9!e§:b2Vec2;
      
      private var §0$§:b2Vec2;
      
      private var §[J§:b2Mat22;
      
      private var § !V§:b2Vec2;
      
      private var §"!V§:Number;
      
      private var §>!0§:Number;
      
      private var §"!M§:Number;
      
      private var §=?§:Number;
      
      private var §>! §:Number;
      
      public function b2MouseJoint(param1:b2MouseJointDef)
      {
         var _loc2_:Number = NaN;
         var _loc4_:b2Mat22 = null;
         this.§6_§ = new b2Mat22();
         this.K1 = new b2Mat22();
         this.K2 = new b2Mat22();
         this.§ !8§ = new b2Vec2();
         this.§9!e§ = new b2Vec2();
         this.§0$§ = new b2Vec2();
         this.§[J§ = new b2Mat22();
         this.§ !V§ = new b2Vec2();
         super(param1);
         this.§9!e§.SetV(param1.target);
         _loc2_ = this.§9!e§.x - b2internal::!N.m_xf.position.x;
         var _loc3_:Number = this.§9!e§.y - b2internal::!N.m_xf.position.y;
         _loc4_ = b2internal::!N.m_xf.R;
         this.§ !8§.x = _loc2_ * _loc4_.col1.x + _loc3_ * _loc4_.col1.y;
         this.§ !8§.y = _loc2_ * _loc4_.col2.x + _loc3_ * _loc4_.col2.y;
         this.§"!V§ = param1.§07§;
         this.§0$§.§0'§();
         this.§>!0§ = param1.§else§;
         this.§"!M§ = param1.§`3§;
         this.§=?§ = 0;
         this.§>! § = 0;
      }
      
      override public function GetAnchorA() : b2Vec2
      {
         return this.§9!e§;
      }
      
      override public function GetAnchorB() : b2Vec2
      {
         return b2internal::!N.GetWorldPoint(this.§ !8§);
      }
      
      override public function GetReactionForce(param1:Number) : b2Vec2
      {
         return new b2Vec2(param1 * this.§0$§.x,param1 * this.§0$§.y);
      }
      
      override public function GetReactionTorque(param1:Number) : Number
      {
         return 0;
      }
      
      public function §`!=§() : b2Vec2
      {
         return this.§9!e§;
      }
      
      public function §"X§(param1:b2Vec2) : void
      {
         if(b2internal::!N.IsAwake() == false)
         {
            b2internal::!N.SetAwake(true);
         }
         this.§9!e§ = param1;
      }
      
      public function §%d§() : Number
      {
         return this.§"!V§;
      }
      
      public function §3!R§(param1:Number) : void
      {
         this.§"!V§ = param1;
      }
      
      public function §0"§() : Number
      {
         return this.§>!0§;
      }
      
      public function §^!I§(param1:Number) : void
      {
         this.§>!0§ = param1;
      }
      
      public function §"!X§() : Number
      {
         return this.§"!M§;
      }
      
      public function §-!N§(param1:Number) : void
      {
         this.§"!M§ = param1;
      }
      
      override b2internal function InitVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc7_:b2Mat22 = null;
         var _loc11_:Number = NaN;
         var _loc12_:Number = NaN;
         var _loc2_:b2Body = b2internal::!N;
         var _loc3_:Number = _loc2_.GetMass();
         var _loc4_:Number = 2 * Math.PI * this.§>!0§;
         var _loc5_:Number = 2 * _loc3_ * this.§"!M§ * _loc4_;
         var _loc6_:Number = _loc3_ * _loc4_ * _loc4_;
         this.§>! § = param1.§<G§ * (_loc5_ + param1.§<G§ * _loc6_);
         this.§>! § = this.§>! § != 0 ? Number(1 / this.§>! §) : Number(0);
         this.§=?§ = param1.§<G§ * _loc6_ * this.§>! §;
         _loc7_ = _loc2_.m_xf.R;
         var _loc8_:Number = this.§ !8§.x - _loc2_.m_sweep.localCenter.x;
         var _loc9_:Number = this.§ !8§.y - _loc2_.m_sweep.localCenter.y;
         var _loc10_:Number = _loc7_.col1.x * _loc8_ + _loc7_.col2.x * _loc9_;
         _loc9_ = _loc7_.col1.y * _loc8_ + _loc7_.col2.y * _loc9_;
         _loc8_ = _loc10_;
         _loc11_ = _loc2_.§!q§;
         _loc12_ = _loc2_.§&!Y§;
         this.K1.col1.x = _loc11_;
         this.K1.col2.x = 0;
         this.K1.col1.y = 0;
         this.K1.col2.y = _loc11_;
         this.K2.col1.x = _loc12_ * _loc9_ * _loc9_;
         this.K2.col2.x = -_loc12_ * _loc8_ * _loc9_;
         this.K2.col1.y = -_loc12_ * _loc8_ * _loc9_;
         this.K2.col2.y = _loc12_ * _loc8_ * _loc8_;
         this.§6_§.§8G§(this.K1);
         this.§6_§.§03§(this.K2);
         this.§6_§.col1.x += this.§>! §;
         this.§6_§.col2.y += this.§>! §;
         this.§6_§.§;+§(this.§[J§);
         this.§ !V§.x = _loc2_.m_sweep.c.x + _loc8_ - this.§9!e§.x;
         this.§ !V§.y = _loc2_.m_sweep.c.y + _loc9_ - this.§9!e§.y;
         _loc2_.m_angularVelocity *= 0.98;
         this.§0$§.x *= param1.§1J§;
         this.§0$§.y *= param1.§1J§;
         _loc2_.§#P§.x += _loc11_ * this.§0$§.x;
         _loc2_.§#P§.y += _loc11_ * this.§0$§.y;
         _loc2_.m_angularVelocity += _loc12_ * (_loc8_ * this.§0$§.y - _loc9_ * this.§0$§.x);
      }
      
      override b2internal function SolveVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc3_:b2Mat22 = null;
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc2_:b2Body = b2internal::!N;
         _loc3_ = _loc2_.m_xf.R;
         var _loc6_:Number = this.§ !8§.x - _loc2_.m_sweep.localCenter.x;
         var _loc7_:Number = this.§ !8§.y - _loc2_.m_sweep.localCenter.y;
         _loc4_ = _loc3_.col1.x * _loc6_ + _loc3_.col2.x * _loc7_;
         _loc7_ = _loc3_.col1.y * _loc6_ + _loc3_.col2.y * _loc7_;
         _loc6_ = _loc4_;
         var _loc8_:Number = _loc2_.§#P§.x + -_loc2_.m_angularVelocity * _loc7_;
         var _loc9_:Number = _loc2_.§#P§.y + _loc2_.m_angularVelocity * _loc6_;
         _loc3_ = this.§[J§;
         _loc4_ = _loc8_ + this.§=?§ * this.§ !V§.x + this.§>! § * this.§0$§.x;
         _loc5_ = _loc9_ + this.§=?§ * this.§ !V§.y + this.§>! § * this.§0$§.y;
         var _loc10_:Number = -(_loc3_.col1.x * _loc4_ + _loc3_.col2.x * _loc5_);
         var _loc11_:Number = -(_loc3_.col1.y * _loc4_ + _loc3_.col2.y * _loc5_);
         var _loc12_:Number = this.§0$§.x;
         var _loc13_:Number = this.§0$§.y;
         this.§0$§.x += _loc10_;
         this.§0$§.y += _loc11_;
         var _loc14_:Number = param1.§<G§ * this.§"!V§;
         if(this.§0$§.§&!T§() > _loc14_ * _loc14_)
         {
            this.§0$§.§>!W§(_loc14_ / this.§0$§.§6<§());
         }
         _loc10_ = this.§0$§.x - _loc12_;
         _loc11_ = this.§0$§.y - _loc13_;
         _loc2_.§#P§.x += _loc2_.§!q§ * _loc10_;
         _loc2_.§#P§.y += _loc2_.§!q§ * _loc11_;
         _loc2_.m_angularVelocity += _loc2_.§&!Y§ * (_loc6_ * _loc11_ - _loc7_ * _loc10_);
      }
      
      override b2internal function SolvePositionConstraints(param1:Number) : Boolean
      {
         return true;
      }
   }
}
