package §@!]§
{
   import §,!3§.b2Mat22;
   import §,!3§.b2Vec2;
   import §3'§.b2internal;
   import §?!F§.b2Body;
   import §?!F§.b2TimeStep;
   
   use namespace b2internal;
   
   public class b2MouseJoint extends b2Joint
   {
       
      
      private var §>f§:b2Mat22;
      
      private var K1:b2Mat22;
      
      private var K2:b2Mat22;
      
      private var §`]§:b2Vec2;
      
      private var §;0§:b2Vec2;
      
      private var §9y§:b2Vec2;
      
      private var §8!#§:b2Mat22;
      
      private var §"!4§:b2Vec2;
      
      private var §=!;§:Number;
      
      private var §@r§:Number;
      
      private var §%!I§:Number;
      
      private var §3!W§:Number;
      
      private var §1!$§:Number;
      
      public function b2MouseJoint(param1:b2MouseJointDef)
      {
         var _loc2_:Number = NaN;
         var _loc4_:b2Mat22 = null;
         this.§>f§ = new b2Mat22();
         this.K1 = new b2Mat22();
         this.K2 = new b2Mat22();
         this.§`]§ = new b2Vec2();
         this.§;0§ = new b2Vec2();
         this.§9y§ = new b2Vec2();
         this.§8!#§ = new b2Mat22();
         this.§"!4§ = new b2Vec2();
         super(param1);
         this.§;0§.SetV(param1.target);
         _loc2_ = this.§;0§.x - b2internal::2!=.m_xf.position.x;
         var _loc3_:Number = this.§;0§.y - b2internal::2!=.m_xf.position.y;
         _loc4_ = b2internal::2!=.m_xf.R;
         this.§`]§.x = _loc2_ * _loc4_.col1.x + _loc3_ * _loc4_.col1.y;
         this.§`]§.y = _loc2_ * _loc4_.col2.x + _loc3_ * _loc4_.col2.y;
         this.§=!;§ = param1.§5$§;
         this.§9y§.§#!d§();
         this.§@r§ = param1.§?1§;
         this.§%!I§ = param1.§%m§;
         this.§3!W§ = 0;
         this.§1!$§ = 0;
      }
      
      override public function GetAnchorA() : b2Vec2
      {
         return this.§;0§;
      }
      
      override public function GetAnchorB() : b2Vec2
      {
         return b2internal::2!=.GetWorldPoint(this.§`]§);
      }
      
      override public function GetReactionForce(param1:Number) : b2Vec2
      {
         return new b2Vec2(param1 * this.§9y§.x,param1 * this.§9y§.y);
      }
      
      override public function GetReactionTorque(param1:Number) : Number
      {
         return 0;
      }
      
      public function §!!K§() : b2Vec2
      {
         return this.§;0§;
      }
      
      public function §!! §(param1:b2Vec2) : void
      {
         if(b2internal::2!=.IsAwake() == false)
         {
            b2internal::2!=.SetAwake(true);
         }
         this.§;0§ = param1;
      }
      
      public function §!!G§() : Number
      {
         return this.§=!;§;
      }
      
      public function §#9§(param1:Number) : void
      {
         this.§=!;§ = param1;
      }
      
      public function §&!`§() : Number
      {
         return this.§@r§;
      }
      
      public function §>m§(param1:Number) : void
      {
         this.§@r§ = param1;
      }
      
      public function §38§() : Number
      {
         return this.§%!I§;
      }
      
      public function §?c§(param1:Number) : void
      {
         this.§%!I§ = param1;
      }
      
      override b2internal function InitVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc7_:b2Mat22 = null;
         var _loc11_:Number = NaN;
         var _loc12_:Number = NaN;
         var _loc2_:b2Body = b2internal::2!=;
         var _loc3_:Number = _loc2_.GetMass();
         var _loc4_:Number = 2 * Math.PI * this.§@r§;
         var _loc5_:Number = 2 * _loc3_ * this.§%!I§ * _loc4_;
         var _loc6_:Number = _loc3_ * _loc4_ * _loc4_;
         this.§1!$§ = param1.§3v§ * (_loc5_ + param1.§3v§ * _loc6_);
         this.§1!$§ = this.§1!$§ != 0 ? Number(1 / this.§1!$§) : Number(0);
         this.§3!W§ = param1.§3v§ * _loc6_ * this.§1!$§;
         _loc7_ = _loc2_.m_xf.R;
         var _loc8_:Number = this.§`]§.x - _loc2_.m_sweep.localCenter.x;
         var _loc9_:Number = this.§`]§.y - _loc2_.m_sweep.localCenter.y;
         var _loc10_:Number = _loc7_.col1.x * _loc8_ + _loc7_.col2.x * _loc9_;
         _loc9_ = _loc7_.col1.y * _loc8_ + _loc7_.col2.y * _loc9_;
         _loc8_ = _loc10_;
         _loc11_ = _loc2_.§;!E§;
         _loc12_ = _loc2_.§8z§;
         this.K1.col1.x = _loc11_;
         this.K1.col2.x = 0;
         this.K1.col1.y = 0;
         this.K1.col2.y = _loc11_;
         this.K2.col1.x = _loc12_ * _loc9_ * _loc9_;
         this.K2.col2.x = -_loc12_ * _loc8_ * _loc9_;
         this.K2.col1.y = -_loc12_ * _loc8_ * _loc9_;
         this.K2.col2.y = _loc12_ * _loc8_ * _loc8_;
         this.§>f§.§]!@§(this.K1);
         this.§>f§.§`'§(this.K2);
         this.§>f§.col1.x += this.§1!$§;
         this.§>f§.col2.y += this.§1!$§;
         this.§>f§.§`1§(this.§8!#§);
         this.§"!4§.x = _loc2_.m_sweep.c.x + _loc8_ - this.§;0§.x;
         this.§"!4§.y = _loc2_.m_sweep.c.y + _loc9_ - this.§;0§.y;
         _loc2_.m_angularVelocity *= 0.98;
         this.§9y§.x *= param1.§%"§;
         this.§9y§.y *= param1.§%"§;
         _loc2_.§#A§.x += _loc11_ * this.§9y§.x;
         _loc2_.§#A§.y += _loc11_ * this.§9y§.y;
         _loc2_.m_angularVelocity += _loc12_ * (_loc8_ * this.§9y§.y - _loc9_ * this.§9y§.x);
      }
      
      override b2internal function SolveVelocityConstraints(param1:b2TimeStep) : void
      {
         var _loc3_:b2Mat22 = null;
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc2_:b2Body = b2internal::2!=;
         _loc3_ = _loc2_.m_xf.R;
         var _loc6_:Number = this.§`]§.x - _loc2_.m_sweep.localCenter.x;
         var _loc7_:Number = this.§`]§.y - _loc2_.m_sweep.localCenter.y;
         _loc4_ = _loc3_.col1.x * _loc6_ + _loc3_.col2.x * _loc7_;
         _loc7_ = _loc3_.col1.y * _loc6_ + _loc3_.col2.y * _loc7_;
         _loc6_ = _loc4_;
         var _loc8_:Number = _loc2_.§#A§.x + -_loc2_.m_angularVelocity * _loc7_;
         var _loc9_:Number = _loc2_.§#A§.y + _loc2_.m_angularVelocity * _loc6_;
         _loc3_ = this.§8!#§;
         _loc4_ = _loc8_ + this.§3!W§ * this.§"!4§.x + this.§1!$§ * this.§9y§.x;
         _loc5_ = _loc9_ + this.§3!W§ * this.§"!4§.y + this.§1!$§ * this.§9y§.y;
         var _loc10_:Number = -(_loc3_.col1.x * _loc4_ + _loc3_.col2.x * _loc5_);
         var _loc11_:Number = -(_loc3_.col1.y * _loc4_ + _loc3_.col2.y * _loc5_);
         var _loc12_:Number = this.§9y§.x;
         var _loc13_:Number = this.§9y§.y;
         this.§9y§.x += _loc10_;
         this.§9y§.y += _loc11_;
         var _loc14_:Number = param1.§3v§ * this.§=!;§;
         if(this.§9y§.§5§() > _loc14_ * _loc14_)
         {
            this.§9y§.§6!%§(_loc14_ / this.§9y§.§;@§());
         }
         _loc10_ = this.§9y§.x - _loc12_;
         _loc11_ = this.§9y§.y - _loc13_;
         _loc2_.§#A§.x += _loc2_.§;!E§ * _loc10_;
         _loc2_.§#A§.y += _loc2_.§;!E§ * _loc11_;
         _loc2_.m_angularVelocity += _loc2_.§8z§ * (_loc6_ * _loc11_ - _loc7_ * _loc10_);
      }
      
      override b2internal function SolvePositionConstraints(param1:Number) : Boolean
      {
         return true;
      }
   }
}
