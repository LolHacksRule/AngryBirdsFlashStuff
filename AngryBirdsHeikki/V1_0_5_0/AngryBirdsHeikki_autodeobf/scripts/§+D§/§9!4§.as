package §+D§
{
   import §9!T§.§;!"§;
   
   public final class §9!4§
   {
      
      public static const MIGHTY_EAGLE_USE_SHADE:Boolean = true;
      
      public static const MIGHTY_EAGLE_FLYING_ANGLE:Number = -22.9;
      
      public static const MIGHTY_EAGLE_FLYING_SPEED:Number = 0.128;
      
      public static const MIGHTY_EAGLE_ROTATION_SPEED:Number = 8;
      
      public static const MIGHTY_EAGLE_WAIT_TIME:Number = 2400;
      
      public static const MIGHTY_EAGLE_SOUND_DELAY:Number = 1000;
      
      public static const MIGHTY_EAGLE_SHADING_DELAY:Number = 2900;
      
      public static const MIGHTY_EAGLE_STARTING_DISTANCE_X:Number = 140;
      
      public static const MIGHTY_EAGLE_STARTING_DISTANCE_Y:Number = 140;
      
      public static const MIGHTY_EAGLE_Y_CHANGE:Number = Math.sin(§;!"§.§@Z§(MIGHTY_EAGLE_FLYING_ANGLE));
      
      public static const MIGHTY_EAGLE_X_DIRECTION:Number = 1;
      
      public static const §>C§:Number = 1500;
      
      public static const §[X§:Number = 0.7;
      
      public static const §6!Z§:Number = 0.2;
      
      public static const §4n§:Number = 8.8;
      
      public static const §0!F§:Number = 2400;
      
      public static const §&!_§:Number = 1;
      
      public static const §58§:Number = 0.15;
      
      public static const §&!=§:Number = 4;
      
      public static const §-!b§:Number = 60;
      
      public static const §5!7§:Number = 16;
      
      public static const §3e§:Number = 6 * §5!7§;
      
      public static const SARDINE_CAN_INITIAL_ROTATION_SPEED:Number = 0.07;
      
      public static const SARDINE_CAN_ROTATION_ACCELERATION:Number = 0.0002;
      
      public static const SARDINE_CAN_MAX_ROTATION_SPEED:Number = SARDINE_CAN_INITIAL_ROTATION_SPEED * 4.4;
      
      public static const SARDINE_CAN_DELAY_AFTER_HIT:int = 3000;
      
      public static const §!G§:Number = 5;
      
      public static const §0#§:int = 2;
      
      public static const §]!2§:Number = 600;
      
      public static const §!!Q§:Number = 1.3;
      
      public static const §"'§:Number = 1.3;
      
      public static const §8N§:Number = 1.3;
      
      public static const §%L§:Number = 1.3;
      
      public static const §'7§:Number = 1.3;
      
      public static const § n§:Number = 1.3;
      
      public static const §?'§:Number = 1.5;
      
      public static const §8!>§:int = 60;
      
      public static const §=f§:Number = 9;
      
      public static const §4!`§:Number = 5;
      
      public static const §?^§:Number = 3000;
      
      public static const §!!0§:Number = 60;
      
      public static const §"e§:Number = 6;
      
      public static const §2! §:Number = 3;
      
      public static const §,3§:Number = 1 / 30;
      
      public static const §^#§:int = 100;
      
      public static const §[9§:int = 3;
      
      public static const §,M§:Number = 70;
      
      public static const §-!_§:Number = 10;
      
      public static const §^!I§:Number = 1000;
      
      public static const §-!Z§:Number = 1.3;
      
      public static const §9i§:Number = 7.5;
      
      public static const §;y§:Number = 300;
      
      public static const §0$§:Number = 0.5;
      
      public static const §'q§:Number = 6;
      
      public static const §&!N§:Number = 9;
      
      public static const §!^§:Number = 0.15;
      
      public static const §?@§:Number = 10;
       
      
      public function §9!4§()
      {
         super();
      }
   }
}
