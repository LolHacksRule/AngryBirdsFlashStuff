package § !9§
{
   import §>Q§.§5!5§;
   import flash.display.BitmapData;
   import flash.display3D.Context3D;
   import flash.display3D.Context3DTextureFormat;
   import flash.display3D.textures.Texture;
   import flash.display3D.textures.TextureBase;
   import flash.utils.ByteArray;
   import starling.events.Event;
   
   public class §&!J§ extends § !9§.Texture
   {
       
      
      private var §6!6§:TextureBase;
      
      private var §,d§:int;
      
      private var §"q§:int;
      
      private var §4!G§:Boolean;
      
      private var §&!;§:Boolean;
      
      private var §0w§:Boolean;
      
      private var §"=§:Object;
      
      private var §1!U§:Boolean = false;
      
      public function §&!J§(param1:TextureBase, param2:int, param3:int, param4:Boolean, param5:Boolean, param6:Boolean = false)
      {
         super();
         this.§6!6§ = param1;
         this.§,d§ = param2;
         this.§"q§ = param3;
         this.§4!G§ = param4;
         this.§&!;§ = param5;
         this.§0w§ = param6;
      }
      
      override public function dispose() : void
      {
         if(this.§6!6§)
         {
            this.§6!6§.dispose();
         }
         this.§2!F§(null);
         super.dispose();
      }
      
      public function §2!F§(param1:Object) : void
      {
         if(this.§"=§ == null && param1 != null)
         {
            §5!5§.§5F§.addEventListener(Event.CONTEXT3D_CREATE,this.§?!D§);
         }
         if(param1 == null)
         {
            §5!5§.§5F§.removeEventListener(Event.CONTEXT3D_CREATE,this.§?!D§);
         }
         this.§"=§ = param1;
      }
      
      private function §?!D§(param1:Event) : void
      {
         this.requestBaseTextureUpdate(null);
      }
      
      override public function requestBaseTextureUpdate(param1:Object) : void
      {
         this.§1!U§ = true;
         if(param1 != null)
         {
            this.§2!F§(param1);
         }
      }
      
      public function get §<B§() : Boolean
      {
         return this.§0w§;
      }
      
      override public function getBase(param1:Context3D) : TextureBase
      {
         this.§2d§(param1);
         return this.§6!6§;
      }
      
      override public function get root() : § !9§.Texture
      {
         return this;
      }
      
      private function §2d§(param1:Context3D) : void
      {
         var _loc4_:flash.display3D.textures.Texture = null;
         var _loc5_:String = null;
         if(!this.§1!U§)
         {
            return;
         }
         var _loc2_:BitmapData = this.§"=§ as BitmapData;
         var _loc3_:ByteArray = this.§"=§ as ByteArray;
         if(_loc2_)
         {
            _loc4_ = param1.createTexture(this.§,d§,this.§"q§,Context3DTextureFormat.BGRA,this.§0w§);
            § !9§.Texture.uploadBitmapData(_loc4_,_loc2_,this.§4!G§);
         }
         else if(_loc3_)
         {
            _loc5_ = _loc3_[6] == 2 ? Context3DTextureFormat.COMPRESSED : Context3DTextureFormat.BGRA;
            _loc4_ = param1.createTexture(this.§,d§,this.§"q§,_loc5_,this.§0w§);
            § !9§.Texture.uploadAtfData(_loc4_,_loc3_);
         }
         this.§6!6§ = _loc4_;
         this.§1!U§ = false;
      }
      
      override public function get width() : Number
      {
         return this.§,d§;
      }
      
      override public function get height() : Number
      {
         return this.§"q§;
      }
      
      override public function get mipMapping() : Boolean
      {
         return this.§4!G§;
      }
      
      override public function get premultipliedAlpha() : Boolean
      {
         return this.§&!;§;
      }
      
      override public function get bitmapData() : BitmapData
      {
         return this.§"=§ as BitmapData;
      }
      
      override public function get clippedBitmapData() : BitmapData
      {
         return this.bitmapData;
      }
   }
}
