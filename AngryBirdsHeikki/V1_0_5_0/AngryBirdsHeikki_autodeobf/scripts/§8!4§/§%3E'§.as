package §8!4§
{
   import flash.geom.Matrix3D;
   import flash.geom.Point;
   import flash.geom.Vector3D;
   import flash.utils.ByteArray;
   import flash.utils.Endian;
   
   public class §>'§
   {
      
      public static const §!!a§:int = 4;
      
      public static const §=F§:int = 2;
      
      public static const §]!L§:int = 3;
      
      public static const §!%§:int = 4;
      
      public static const §+d§:int = 0;
      
      public static const §0P§:int = 0;
      
      public static const §%!<§:int = 0;
       
      
      private var §&'§:ByteArray;
      
      private var §=Y§:Vector.<Number>;
      
      private var §@L§:Vector.<Number>;
      
      private var §&!;§:Boolean;
      
      private var §9Z§:int;
      
      private var §<!b§:Vector.<Number>;
      
      public function §>'§(param1:int, param2:Boolean = false)
      {
         this.§<!b§ = new Vector.<Number>(12,true);
         super();
         this.§&'§ = new ByteArray();
         this.§&'§.endian = Endian.LITTLE_ENDIAN;
         this.§&'§.length = param1 * §=F§ * §!%§;
         var _loc3_:int = 0;
         while(_loc3_ < param1 * §=F§)
         {
            this.§&'§.writeFloat(0);
            _loc3_++;
         }
         this.§=Y§ = new Vector.<Number>(param1 * §!!a§,true);
         this.§@L§ = new Vector.<Number>(param1 * §]!L§,true);
         this.§&!;§ = param2;
         this.§9Z§ = param1;
      }
      
      public function clone() : §>'§
      {
         var _loc1_:§>'§ = new §>'§(0,this.§&!;§);
         _loc1_.§&'§.length = this.§&'§.length;
         _loc1_.§&'§.writeBytes(this.§&'§);
         _loc1_.§=Y§ = this.§=Y§.concat();
         _loc1_.§=Y§.fixed = true;
         _loc1_.§@L§ = this.§@L§.concat();
         _loc1_.§@L§.fixed = true;
         _loc1_.§9Z§ = this.§9Z§;
         return _loc1_;
      }
      
      public function copyTo(param1:§>'§, param2:int, param3:Number, param4:Boolean, param5:Matrix3D) : void
      {
         var _loc6_:int = 0;
         var _loc7_:int = 0;
         var _loc8_:int = 0;
         var _loc11_:Vector.<Number> = null;
         var _loc12_:int = 0;
         var _loc13_:Number = NaN;
         var _loc9_:Vector.<Number> = param1.§@L§;
         _loc6_ = this.§9Z§ * §]!L§;
         _loc7_ = param2 * §]!L§;
         if(param5)
         {
            param5.transformVectors(this.§@L§,this.§<!b§);
            _loc8_ = 0;
            while(_loc8_ < _loc6_)
            {
               var _loc14_:*;
               _loc9_[_loc14_ = _loc7_++] = this.§<!b§[_loc8_];
               _loc8_++;
            }
         }
         else
         {
            _loc8_ = 0;
            while(_loc8_ < _loc6_)
            {
               _loc9_[_loc14_ = _loc7_++] = this.§@L§[_loc8_];
               _loc8_++;
            }
         }
         var _loc10_:ByteArray = param1.§&'§;
         _loc6_ = this.§9Z§ * §=F§ * §!%§;
         _loc7_ = param2 * §=F§ * §!%§;
         if(_loc10_.length < _loc7_ + _loc6_)
         {
            _loc10_.length = _loc7_ + _loc6_;
         }
         _loc10_.position = _loc7_;
         _loc10_.writeBytes(this.§&'§);
         if(param4 || param3 != 1)
         {
            _loc11_ = param1.§=Y§;
            _loc6_ = this.§9Z§ * §!!a§;
            _loc7_ = param2 * §!!a§;
            if(param3 == 1)
            {
               _loc8_ = 0;
               while(_loc8_ < _loc6_)
               {
                  _loc11_[_loc14_ = _loc7_++] = this.§=Y§[_loc8_];
                  _loc8_++;
               }
            }
            else
            {
               _loc12_ = 0;
               _loc8_ = 0;
               while(_loc8_ < this.§9Z§)
               {
                  if(this.§&!;§)
                  {
                     _loc13_ = this.§=Y§[_loc12_ + 3];
                     _loc13_ = param3 / _loc13_;
                     _loc11_[_loc14_ = _loc7_++] = this.§=Y§[_loc12_++] * _loc13_;
                     var _loc15_:*;
                     _loc11_[_loc15_ = _loc7_++] = this.§=Y§[_loc12_++] * _loc13_;
                     var _loc16_:*;
                     _loc11_[_loc16_ = _loc7_++] = this.§=Y§[_loc12_++] * _loc13_;
                     var _loc17_:*;
                     _loc11_[_loc17_ = _loc7_++] = param3;
                  }
                  else
                  {
                     _loc11_[_loc14_ = _loc7_++] = this.§=Y§[_loc12_++];
                     _loc11_[_loc15_ = _loc7_++] = this.§=Y§[_loc12_++];
                     _loc11_[_loc16_ = _loc7_++] = this.§=Y§[_loc12_++];
                     _loc11_[_loc17_ = _loc7_++] = param3;
                  }
                  _loc12_++;
                  _loc8_++;
               }
            }
         }
      }
      
      public function append(param1:§>'§) : void
      {
         var _loc2_:int = this.§&'§.length;
         this.§&'§.length += param1.§&'§.length;
         this.§&'§.position = _loc2_;
         this.§&'§.writeBytes(param1.§&'§);
         this.§=Y§.fixed = false;
         var _loc3_:Vector.<Number> = param1.§=Y§;
         var _loc4_:int = _loc3_.length;
         var _loc5_:int = 0;
         while(_loc5_ < _loc4_)
         {
            this.§=Y§.push(_loc3_[_loc5_]);
            _loc5_++;
         }
         this.§=Y§.fixed = true;
         this.§@L§.fixed = false;
         var _loc6_:Vector.<Number>;
         var _loc7_:int = (_loc6_ = param1.§@L§).length;
         var _loc8_:int = 0;
         while(_loc8_ < _loc7_)
         {
            this.§@L§.push(_loc6_[_loc8_]);
            _loc8_++;
         }
         this.§@L§.fixed = true;
         this.§9Z§ += param1.§<Z§;
      }
      
      public function setPosition(param1:int, param2:Number, param3:Number, param4:Number = 0.0) : void
      {
         var _loc5_:int = param1 * §]!L§;
         var _loc6_:*;
         this.§@L§[_loc6_ = _loc5_++] = param2;
         var _loc7_:*;
         this.§@L§[_loc7_ = _loc5_++] = param3;
         this.§@L§[_loc5_] = param4;
      }
      
      public function §7!T§(param1:int, param2:Number, param3:Number) : void
      {
         var _loc4_:int = param1 * §]!L§;
         var _loc5_:*;
         this.§@L§[_loc5_ = _loc4_++] = param2;
         this.§@L§[_loc4_] = param3;
      }
      
      public function §,]§(param1:int, param2:Vector3D) : void
      {
         var _loc3_:int = param1 * §]!L§;
         param2.x = this.§@L§[_loc3_++];
         param2.y = this.§@L§[_loc3_++];
         param2.z = this.§@L§[_loc3_];
      }
      
      public function §9q§(param1:int, param2:uint, param3:Number = 1.0) : void
      {
         var _loc4_:Number = (_loc4_ = !!this.§&!;§ ? Number(param3) : Number(1)) / 255;
         var _loc5_:Number = §'! §.§;_§(param2) * _loc4_;
         var _loc6_:Number = §'! §.§8^§(param2) * _loc4_;
         var _loc7_:Number = §'! §.§'c§(param2) * _loc4_;
         this.§1!-§(param1,1,_loc5_,_loc6_,_loc7_,param3);
      }
      
      private function §1!-§(param1:int, param2:int, param3:Number, param4:Number, param5:Number, param6:Number) : void
      {
         var _loc7_:int = param1 * §!!a§;
         var _loc8_:int = param2 - 1;
         while(_loc8_ >= 0)
         {
            var _loc9_:*;
            this.§=Y§[_loc9_ = _loc7_++] = param3;
            var _loc10_:*;
            this.§=Y§[_loc10_ = _loc7_++] = param4;
            var _loc11_:*;
            this.§=Y§[_loc11_ = _loc7_++] = param5;
            var _loc12_:*;
            this.§=Y§[_loc12_ = _loc7_++] = param6;
            _loc8_--;
         }
      }
      
      private function §"!^§(param1:int, param2:Number) : void
      {
         var _loc3_:int = param1 * §!!a§;
         var _loc4_:*;
         this.§=Y§[_loc4_ = _loc3_++] = this.§=Y§[_loc4_] * param2;
         var _loc5_:*;
         this.§=Y§[_loc5_ = _loc3_++] = this.§=Y§[_loc5_] * param2;
         var _loc6_:*;
         this.§=Y§[_loc6_ = _loc3_++] = this.§=Y§[_loc6_] * param2;
         this.§=Y§[_loc3_] *= param2;
      }
      
      public function §+@§(param1:int) : uint
      {
         var _loc4_:Number = NaN;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc2_:int = param1 * §!!a§;
         var _loc3_:Number = !!this.§&!;§ ? Number(this.§=Y§[_loc2_ + 3]) : Number(1);
         if(_loc3_ == 0)
         {
            return 0;
         }
         _loc3_ = 255 / _loc3_;
         _loc4_ = this.§=Y§[_loc2_] * _loc3_;
         _loc5_ = this.§=Y§[_loc2_ + 1] * _loc3_;
         _loc6_ = this.§=Y§[_loc2_ + 2] * _loc3_;
         return §'! §.§?,§(_loc4_,_loc5_,_loc6_);
      }
      
      public function §?!#§(param1:int, param2:Number) : void
      {
         var _loc3_:int = 0;
         if(param2 < 0.001)
         {
            param2 = 0.001;
         }
         if(this.§&!;§)
         {
            this.§9q§(param1,this.§+@§(param1),param2);
         }
         else
         {
            _loc3_ = param1 * §!!a§ + 3;
            this.§=Y§[_loc3_] = param2;
         }
      }
      
      public function §8>§(param1:int) : Number
      {
         var _loc2_:int = param1 * §!!a§ + 3;
         return this.§=Y§[_loc2_];
      }
      
      public function setTexCoords(param1:int, param2:Number, param3:Number) : void
      {
         var _loc4_:int = param1 * §=F§;
         this.§&'§.position = _loc4_ * §!%§;
         this.§&'§.writeFloat(param2);
         this.§&'§.writeFloat(param3);
      }
      
      public function getTexCoords(param1:int, param2:Point) : void
      {
         var _loc3_:int = param1 * §=F§;
         this.§&'§.position = _loc3_ * §!%§;
         param2.x = this.§&'§.readFloat();
         param2.y = this.§&'§.readFloat();
      }
      
      public function §`<§(param1:int, param2:Number, param3:Number, param4:Number = 0.0) : void
      {
         var _loc5_:int = param1 * §]!L§;
         var _loc6_:*;
         this.§@L§[_loc6_ = _loc5_++] = this.§@L§[_loc6_] + param2;
         var _loc7_:*;
         this.§@L§[_loc7_ = _loc5_++] = this.§@L§[_loc7_] + param3;
         this.§@L§[_loc5_] += param4;
      }
      
      public function §1!'§(param1:int, param2:Matrix3D) : void
      {
         var _loc3_:int = param1 * §]!L§;
         this.§<!b§[0] = this.§@L§[_loc3_];
         this.§<!b§[1] = this.§@L§[_loc3_ + 1];
         this.§<!b§[2] = this.§@L§[_loc3_ + 2];
         param2.transformVectors(this.§<!b§,this.§<!b§);
         this.§@L§[_loc3_] = this.§<!b§[0];
         this.§@L§[_loc3_ + 1] = this.§<!b§[1];
         this.§@L§[_loc3_ + 2] = this.§<!b§[2];
      }
      
      public function §=!_§(param1:uint, param2:Number = 1.0) : void
      {
         if(param2 < 0.001)
         {
            param2 = 0.001;
         }
         var _loc3_:Number = !!this.§&!;§ ? Number(param2) : Number(1);
         _loc3_ /= 255;
         var _loc4_:Number = §'! §.§;_§(param1) * _loc3_;
         var _loc5_:Number = §'! §.§8^§(param1) * _loc3_;
         var _loc6_:Number = §'! §.§'c§(param1) * _loc3_;
         this.§1!-§(0,this.§9Z§,_loc4_,_loc5_,_loc6_,param2);
      }
      
      public function §3!J§(param1:int, param2:int, param3:Number, param4:Number, param5:Number, param6:Number = 1.0) : void
      {
         if(param6 < 0.001)
         {
            param6 = 0.001;
         }
         var _loc7_:int = param1 * §!!a§;
         var _loc8_:int = 0;
         while(_loc8_ < param2)
         {
            var _loc9_:*;
            this.§=Y§[_loc9_ = _loc7_++] = param3;
            var _loc10_:*;
            this.§=Y§[_loc10_ = _loc7_++] = param4;
            var _loc11_:*;
            this.§=Y§[_loc11_ = _loc7_++] = param5;
            var _loc12_:*;
            this.§=Y§[_loc12_ = _loc7_++] = param6;
            _loc8_++;
         }
      }
      
      public function §'!!§(param1:int, param2:Number) : void
      {
         var _loc4_:Number = NaN;
         if(param2 < 0.001)
         {
            param2 = 0.001;
         }
         if(param2 == 1)
         {
            return;
         }
         var _loc3_:int = param1 * §!!a§;
         if(this.§&!;§)
         {
            _loc4_ = this.§=Y§[_loc3_ + 3];
            _loc4_ = param2 / _loc4_;
            this.§=Y§[_loc3_] *= _loc4_;
            this.§=Y§[_loc3_ + 1] *= _loc4_;
            this.§=Y§[_loc3_ + 2] *= _loc4_;
            this.§=Y§[_loc3_ + 3] = param2;
         }
         else
         {
            this.§=Y§[_loc3_ + 3] *= param2;
         }
      }
      
      public function setPremultipliedAlpha(param1:Boolean, param2:Boolean = true) : void
      {
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         var _loc5_:Number = NaN;
         var _loc6_:Number = NaN;
         var _loc7_:Number = NaN;
         if(param1 == this.§&!;§)
         {
            return;
         }
         if(param2)
         {
            _loc3_ = this.§9Z§ * §!!a§;
            _loc4_ = 0;
            while(_loc4_ < _loc3_)
            {
               _loc5_ = this.§8>§(0);
               _loc6_ = !!this.§&!;§ ? Number(_loc5_) : Number(1);
               _loc7_ = !!param1 ? Number(_loc5_) : Number(1);
               if(_loc6_ != 0)
               {
                  _loc7_ /= _loc6_;
                  this.§"!^§(_loc4_,_loc7_);
               }
               _loc4_++;
            }
         }
         this.§&!;§ = param1;
      }
      
      public function get premultipliedAlpha() : Boolean
      {
         return this.§&!;§;
      }
      
      public function get §<Z§() : int
      {
         return this.§9Z§;
      }
      
      public function set §<Z§(param1:int) : void
      {
         var _loc2_:int = 0;
         var _loc3_:int = 0;
         var _loc4_:int = 0;
         _loc2_ = param1 * §=F§ * §!%§ - this.§&'§.length;
         _loc4_ = this.§&'§.length;
         this.§&'§.length += _loc2_;
         if(_loc2_ > 0)
         {
            this.§&'§.position = _loc4_;
            _loc3_ = 0;
            while(_loc3_ < _loc2_ / §!%§)
            {
               this.§&'§.writeFloat(0);
               _loc3_++;
            }
         }
         this.§=Y§.fixed = false;
         _loc2_ = param1 * §!!a§ - this.§=Y§.length;
         _loc3_ = 0;
         while(_loc3_ < _loc2_)
         {
            this.§=Y§.push(0);
            _loc3_++;
         }
         _loc3_ = _loc2_;
         while(_loc3_ < 0)
         {
            this.§=Y§.pop();
            _loc3_++;
         }
         this.§=Y§.fixed = true;
         this.§@L§.fixed = false;
         _loc2_ = param1 * §]!L§ - this.§@L§.length;
         _loc3_ = 0;
         while(_loc3_ < _loc2_)
         {
            this.§@L§.push(0);
            _loc3_++;
         }
         _loc3_ = _loc2_;
         while(_loc3_ < 0)
         {
            this.§@L§.pop();
            _loc3_++;
         }
         this.§@L§.fixed = true;
         this.§9Z§ = param1;
      }
      
      public function get §[! §() : ByteArray
      {
         this.§&'§.position = 0;
         return this.§&'§;
      }
      
      public function get §2t§() : Vector.<Number>
      {
         return this.§=Y§;
      }
      
      public function get §>!W§() : Vector.<Number>
      {
         return this.§@L§;
      }
   }
}
