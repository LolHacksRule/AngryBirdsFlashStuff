package com.angrybirds.states
{
   ...
   public class StateLevelSelectionCustom extends StateBaseLevel
   {
      ...
      private function updateLevelButtonStates() : void
      {
         ...
         var _loc3_:int = 0;
         var _loc1_:int = 1;
         while(_loc1_ <= mouseOver.LEVEL_ID_LIST.length)
         {
            _loc2_ = mUIView.getItemByName("Button_Level" + _loc1_).mClip;
            _loc2_.TextField_LevelNum.text.mouseEnabled = false;
            if(UserManager.isLevelUnlocked(mouseOver.LEVEL_ID_LIST[_loc1_ - 1]))
            {
               ...
               _loc2_.TextField_LevelNum.text.text = mouseOver.LEVEL_NAMES[_loc1_ - 1];
               _loc3_ = AngryBirdsBase.singleton.dataModel.userProgress.getStarsForLevel(mouseOver.LEVEL_ID_LIST[_loc1_ - 1]);
