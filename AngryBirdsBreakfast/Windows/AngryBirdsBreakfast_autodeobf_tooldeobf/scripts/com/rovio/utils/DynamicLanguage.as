package com.rovio.utils
{
   ...
   public class DynamicLanguage
   {
      ...
      public function setSourceXML(param1:XML) : void
      {
         ...
         for each(_loc2_ in this.mXML.language)
         {
            if(_loc2_.@id != "")
            {
      ...
      public function updateTextFields(param1:UIContainerRovio, param2:String) : void
      {
         ...
         group = this.mXML.group.(@name == groupId);
         if(group.automatic != undefined)
         {
            fontName = this.mXML.font.language.(@id == mLanguage);
      ...
      public function getManualText(param1:String, param2:String) : String
      {
         ...
         xml = this.mXML.group.(@name == groupId).manual.text.(@id == textId).language.(@id == mLanguage).toString();
         if(xml != null)
         {
            return xml.toString();
         }
      ...
      public function setManualTextToTextField(param1:UIContainerRovio, param2:String, param3:String, param4:String) : void
      {
         ...
         fontName = this.mXML.font.language.(@id == mLanguage);
         xml = this.mXML.group.(@name == groupId).manual.text.(@id == textId).language.(@id == mLanguage);
         if(xml.length() > 0)
         {
      ...
      public function usedCharacters(param1:String) : String
      {
         ...
         var chars:String = "";
         for each(text in this.mXML.group.*.*.language.(@id == mLanguage))
         {
            i = 0;
            while(i < text.length)
            {
