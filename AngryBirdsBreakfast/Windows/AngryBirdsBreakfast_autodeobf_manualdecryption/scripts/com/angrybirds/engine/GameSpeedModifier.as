package com.angrybirds.engine
{
   public class GameSpeedModifier
   {
      ...
      public function update(param1:Number) : Boolean
      {
         ...
         else if(this.mOffsetMilliSeconds < this.mFadeInMilliSeconds + this.mZoom)
         {
            this.mCurrentSpeed = this.mSpeed;
         }
         else
         {
            if(this.mOffsetMilliSeconds >= this.mFadeInMilliSeconds + this.mZoom + this.mFadeOutMilliSeconds)
            {
               this.mCurrentSpeed = DEFAULT_SPEED;
               return false;
            }
            _loc2_ = this.mOffsetMilliSeconds - (this.mFadeInMilliSeconds + this.mZoom);
