package §_-hq§
{
   ...
   public class §_-uK§
   {
      ...
      private function §_-bC§(param1:*) : String
      {
         if(param1 is String)
         {
            return this.§_-P1§(param1 as String);
         }
         if(param1 is Number)
         {
         ...
         if(param1 is Array)
         {
            return this.§_-aH§(param1 as Array);
         }
      ...
      private function §_-aH§(param1:Array) : String
      {
         ...
         while(_loc3_ < param1.length)
         {
            ...
            _loc2_ += this.§_-bC§(param1[_loc3_]);
            _loc3_++;
         }
         return "[" + _loc2_ + "]";
      }
         ...
         if(classInfo.@name.toString() == "Object")
         {
            for(key in o)
            {
               ...
               if(!(value is Function))
               {
                  ...
                  s += this.§_-P1§(key) + ":" + this.§_-bC§(value);
               }
            }
         }
         else
         {
            for each(v in classInfo..§*§.(name() == "variable" || name() == "accessor" && attribute("access").charAt(0) == "r"))
            {
               if(!(v.metadata && v.metadata.(@name == "Transient").length() > 0))
               {
                  ...
                  s += this.§_-P1§(v.@name.toString()) + ":" + this.§_-bC§(o[v.@name]);
               }
            }
         }
         return "{" + s + "}";
