package §"![§
{
   import §6l§.§79§;
   import flash.display.BitmapData;
   import flash.display3D.Context3D;
   import flash.display3D.Context3DTextureFormat;
   import flash.display3D.textures.Texture;
   import flash.display3D.textures.TextureBase;
   import flash.utils.ByteArray;
   import starling.events.Event;
   
   public class §'!8§ extends §"![§.Texture
   {
       
      
      private var §,i§:TextureBase;
      
      private var §3§:int;
      
      private var §=!=§:int;
      
      private var §!!A§:Boolean;
      
      private var §[=§:Boolean;
      
      private var §3&§:Boolean;
      
      private var §[N§:Object;
      
      private var §>!Q§:Boolean = false;
      
      public function §'!8§(param1:TextureBase, param2:int, param3:int, param4:Boolean, param5:Boolean, param6:Boolean = false)
      {
         super();
         this.§,i§ = param1;
         this.§3§ = param2;
         this.§=!=§ = param3;
         this.§!!A§ = param4;
         this.§[=§ = param5;
         this.§3&§ = param6;
      }
      
      override public function dispose() : void
      {
         if(this.§,i§)
         {
            this.§,i§.dispose();
         }
         this.§6!3§(null);
         super.dispose();
      }
      
      public function §6!3§(param1:Object) : void
      {
         if(this.§[N§ == null && param1 != null)
         {
            §79§.§#[§.addEventListener(Event.CONTEXT3D_CREATE,this.§!6§);
         }
         if(param1 == null)
         {
            §79§.§#[§.removeEventListener(Event.CONTEXT3D_CREATE,this.§!6§);
         }
         this.§[N§ = param1;
      }
      
      private function §!6§(param1:Event) : void
      {
         this.requestBaseTextureUpdate(null);
      }
      
      override public function requestBaseTextureUpdate(param1:Object) : void
      {
         this.§>!Q§ = true;
         if(param1 != null)
         {
            this.§6!3§(param1);
         }
      }
      
      public function get §0!d§() : Boolean
      {
         return this.§3&§;
      }
      
      override public function getBase(param1:Context3D) : TextureBase
      {
         this.§'R§(param1);
         return this.§,i§;
      }
      
      override public function get root() : §"![§.Texture
      {
         return this;
      }
      
      private function §'R§(param1:Context3D) : void
      {
         var _loc4_:flash.display3D.textures.Texture = null;
         var _loc5_:String = null;
         if(!this.§>!Q§)
         {
            return;
         }
         var _loc2_:BitmapData = this.§[N§ as BitmapData;
         var _loc3_:ByteArray = this.§[N§ as ByteArray;
         if(_loc2_)
         {
            _loc4_ = param1.createTexture(this.§3§,this.§=!=§,Context3DTextureFormat.BGRA,this.§3&§);
            §"![§.Texture.uploadBitmapData(_loc4_,_loc2_,this.§!!A§);
         }
         else if(_loc3_)
         {
            _loc5_ = _loc3_[6] == 2 ? Context3DTextureFormat.COMPRESSED : Context3DTextureFormat.BGRA;
            _loc4_ = param1.createTexture(this.§3§,this.§=!=§,_loc5_,this.§3&§);
            §"![§.Texture.uploadAtfData(_loc4_,_loc3_);
         }
         this.§,i§ = _loc4_;
         this.§>!Q§ = false;
      }
      
      override public function get width() : Number
      {
         return this.§3§;
      }
      
      override public function get height() : Number
      {
         return this.§=!=§;
      }
      
      override public function get mipMapping() : Boolean
      {
         return this.§!!A§;
      }
      
      override public function get premultipliedAlpha() : Boolean
      {
         return this.§[=§;
      }
      
      override public function get bitmapData() : BitmapData
      {
         return this.§[N§ as BitmapData;
      }
      
      override public function get clippedBitmapData() : BitmapData
      {
         return this.bitmapData;
      }
   }
}
