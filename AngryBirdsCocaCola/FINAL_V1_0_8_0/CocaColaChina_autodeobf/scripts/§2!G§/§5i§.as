package §2!G§
{
   import §%!a§.§ !§;
   import §&!9§.Sprite;
   import §&F§.§>!2§;
   import §+x§.§-6§;
   import §9y§.§ @§;
   import §<!L§.b2World;
   import §?=§.§&j§;
   import §?=§.§8S§;
   import §^!$§.§;!D§;
   
   public class §5i§ extends §while§
   {
       
      
      private var §&l§:Number = 0;
      
      public function §5i§(param1:§;!D§, param2:Sprite, param3:b2World, param4:§>!2§, param5:int, param6:String, param7:Number, param8:Number, param9:Number, param10:Number, param11:Boolean = false)
      {
         super(param1,param2,param3,param4,param5,param6,param7,param8,param9,param10,param11);
      }
      
      override public function addDestructionParticles(param1:§&j§) : void
      {
         this.§'A§(param1);
         this.§>!%§(param1,1);
         this.§>!%§(param1,2);
         this.§>!%§(param1,3);
         this.§>!%§(param1,4);
         §;W§.createExplosion(§;W§.§ u§,x,y);
      }
      
      override public function applyDamage(param1:Number, param2:Boolean = true, param3:Boolean = false, param4:Boolean = false, param5:Boolean = false, param6:Boolean = false) : Number
      {
         this.§&l§ += param1;
         var _loc7_:Number = §'7§.material.mValues[§ !§.§&!2§];
         if(this.§&l§ >= _loc7_)
         {
            this.§&l§ = 0;
            param1 = §>!`§ / 4 + 1 + _loc7_ / 4;
            if(§-!#§.§4,§.§3!&§() == 4)
            {
               param6 = true;
            }
            var _loc8_:Number = super.applyDamage(param1,param2,param3,param4,param5,param6);
            if(§+m§ < §>!`§)
            {
               if(§-!#§.§4,§.§3!&§() == 0)
               {
                  §-!#§.§4,§.§75§(this);
                  §-6§.§6!#§("Drum_1_1");
               }
               if(§+m§ <= §>!`§ * 0.75)
               {
                  if(§-!#§.§4,§.§3!&§() == 1)
                  {
                     §-!#§.§4,§.§75§(this);
                     §-6§.§6!#§("Drum_2_1");
                  }
                  if(§+m§ <= §>!`§ * 0.5)
                  {
                     if(§-!#§.§4,§.§3!&§() == 2)
                     {
                        §-!#§.§4,§.§75§(this);
                        §-6§.§6!#§("Drum_3_1");
                     }
                     if(§+m§ <= §>!`§ * 0.25)
                     {
                        if(§-!#§.§4,§.§3!&§() == 3)
                        {
                           §-!#§.§4,§.§75§(this);
                           §-6§.§6!#§("Golden_Drum_1","ChannelMisc",1,1);
                           §+m§ = 0;
                        }
                     }
                  }
               }
            }
            return _loc8_;
         }
         return 0;
      }
      
      override public function addDamageParticles(param1:§&j§, param2:int) : void
      {
         if(§+m§ < §>!`§)
         {
            if(§-!#§.§4,§.§3!&§() <= 0)
            {
               this.§>!%§(param1,1);
            }
            if(§+m§ <= §>!`§ * 0.75)
            {
               if(§-!#§.§4,§.§3!&§() == 1)
               {
                  this.§>!%§(param1,2);
               }
               if(§+m§ <= §>!`§ * 0.5)
               {
                  if(§-!#§.§4,§.§3!&§() == 2)
                  {
                     this.§>!%§(param1,3);
                  }
                  if(§+m§ <= §>!`§ * 0.25)
                  {
                     if(§-!#§.§4,§.§3!&§() == 3)
                     {
                        this.§>!%§(param1,4);
                     }
                  }
               }
            }
         }
      }
      
      private function §'A§(param1:§&j§) : void
      {
         var _loc8_:Number = NaN;
         var _loc9_:Number = NaN;
         var _loc10_:Number = NaN;
         var _loc3_:int = 70;
         var _loc4_:Number = 90;
         _loc3_ *= 1;
         var _loc7_:int = 0;
         while(_loc7_ < _loc3_)
         {
            _loc8_ = (_loc4_ += Math.random() * (720 / _loc3_)) / (180 / Math.PI);
            _loc9_ = 30 * Math.cos(_loc8_) * Math.random();
            _loc10_ = -30 * Math.sin(_loc8_) * Math.random();
            param1.§<§("PARTICLE_COLLECT_GOLDEN_DRUM_STAR",§&j§.§`W§,§8S§.§[!C§,§#!!§().GetPosition().x,§#!!§().GetPosition().y,400 + 700 * Math.random(),"",§8S§.§"0§(§=^§),_loc9_,_loc10_,10,_loc10_ * 20,0.5 + Math.random() * 1);
            _loc7_++;
         }
      }
      
      private function §>!%§(param1:§&j§, param2:Number) : void
      {
         var _loc3_:§8S§ = param1.§<§("PARTICLE_SOUNDWAVE_COKE",§&j§.§"!5§,§ @§.§?`§,§#!!§().GetPosition().x,§#!!§().GetPosition().y,600,"",§8S§.§"0§(§=^§),0,0,0,0,param2 * 0.5);
      }
   }
}
