package §'!^§
{
   import flash.utils.getTimer;
   
   public class §!!H§ extends §+4§
   {
      
      public static const §#"§:String = "level-selection";
      
      public static const §3p§:String = "registration-success";
      
      public static const §%!'§:String = "registration-failure";
      
      public static const §%!Y§:String = "registration-attempt";
      
      public static const §[_§:String = "login-success";
      
      public static const §,W§:String = "login-failure";
      
      private static const §+A§:Boolean = §6T§.§2H§;
      
      private static const §do§:String = "framerate";
      
      private static const §%!]§:String = "warning";
      
      private static const §^!W§:String = "user-statistics";
      
      private static const §'!!§:String = "error";
      
      private static const §[q§:String = "download-failed";
      
      private static const §"v§:String = "invalid-level";
      
      private static const §-!c§:String = "flash-var-missing";
      
      private static const SAMPLE_1_PERCENT_FLASH_ACTIONS:Array = [§63§,§#m§];
      
      private static const SAMPLE_100_PERCENT_FLASH_ACTIONS:Array = [ACTION_APPLICATION_CRASH,§6c§,§]!0§];
      
      private static const SAMPLE_1_PERCENT_CATEGORIES:Array = [];
      
      private static const SAMPLE_100_PERCENT_CATEGORIES:Array = [§'!!§,§1n§,§do§,§^!W§,§%!]§];
      
      private static const FULL_SCREEN:String = "-full-screen";
      
      private static const §8c§:String = ACTION_CPU_FPS_REPORT + FULL_SCREEN;
      
      private static const §8W§:String = ACTION_GPU_FPS_REPORT + FULL_SCREEN;
      
      private static var sSample10Percent:Boolean = false;
      
      private static var sSample1Percent:Boolean = false;
      
      private static var §`!X§:Array = [];
       
      
      public function §!!H§()
      {
         super();
      }
      
      public static function §>q§() : void
      {
         if(Math.random() <= 0.1)
         {
            sSample10Percent = true;
         }
         if(Math.random() <= 0.01)
         {
            sSample1Percent = true;
         }
      }
      
      public static function §%!4§(param1:int, param2:int) : void
      {
         §?!O§(§+4§.ACTION_APPLICATION_CRASH,param1.toString(),param2);
      }
      
      public static function §?!O§(param1:String, param2:String = null, param3:int = 0) : void
      {
         var _loc4_:String = §3!"§(param1,SAMPLE_100_PERCENT_FLASH_ACTIONS,SAMPLE_1_PERCENT_FLASH_ACTIONS);
         §+4§.trackSampledEvent(_loc4_,§1n§,param1,param2,param3);
      }
      
      public static function §8!2§(param1:int, param2:String, param3:Boolean, param4:Boolean) : void
      {
         var _loc6_:String = null;
         if(§`!X§.indexOf(param2) >= 0)
         {
            return;
         }
         var _loc5_:* = §`!X§.length == 0;
         §`!X§.push(param2);
         if(param3)
         {
            _loc6_ = §+4§.ACTION_CPU_FPS_REPORT;
            if(param4)
            {
               _loc6_ = §!!H§.§8c§;
            }
         }
         else
         {
            _loc6_ = §+4§.ACTION_GPU_FPS_REPORT;
            if(param4)
            {
               _loc6_ = §!!H§.§8W§;
            }
         }
         if(_loc5_)
         {
            §?!O§(_loc6_,param1.toString(),param1);
         }
         §3A§(§do§,_loc6_,param2,param1);
      }
      
      public static function §"U§(param1:String) : void
      {
         §3A§(§%!]§,§[q§,param1,0);
      }
      
      public static function §8§(param1:String) : void
      {
         if(param1 == null)
         {
            param1 = "[null]";
         }
         else if(param1.length == 0)
         {
            param1 = "[empty]";
         }
         §3A§(§%!]§,§"v§,param1,0);
      }
      
      public static function §&!M§(param1:String) : void
      {
         §3A§(§%!]§,§-!c§,param1,0);
      }
      
      public static function §,!?§(param1:String) : void
      {
         §3A§(§'!!§,param1,"",0,false);
      }
      
      private static function §3A§(param1:String, param2:String, param3:String, param4:int, param5:Boolean = true) : void
      {
         var _loc6_:String = null;
         if(enabled)
         {
            _loc6_ = §?!K§;
            if(param5)
            {
               _loc6_ = §3!"§(param1,SAMPLE_100_PERCENT_CATEGORIES,SAMPLE_1_PERCENT_CATEGORIES);
            }
            trackSampledEvent(_loc6_,param1,param2,param3,param4);
         }
      }
      
      private static function §3!"§(param1:String, param2:Array, param3:Array) : String
      {
         if(param3.indexOf(param1) >= 0)
         {
            return TRACKING_FUNCTION_1_PERCENT;
         }
         if(param2.indexOf(param1) < 0)
         {
            return TRACKING_FUNCTION_10_PERCENT;
         }
         return §?!K§;
      }
      
      private static function §1!U§(param1:Array) : String
      {
         var _loc4_:String = null;
         if(param1 == null || param1.length == 0)
         {
            return null;
         }
         var _loc2_:String = "";
         var _loc3_:Array = param1.concat();
         _loc3_.sort();
         for each(_loc4_ in _loc3_)
         {
            if(_loc2_.length > 0)
            {
               _loc2_ += "-" + _loc4_;
            }
            else
            {
               _loc2_ = _loc4_;
            }
         }
         return _loc2_;
      }
      
      private static function §<p§(param1:int = 0) : int
      {
         return getTimer() / 1000 - param1;
      }
   }
}
